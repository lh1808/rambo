...
# ── R-Score Helpers ──
# EconML's est.score() liefert rohen MSE (R-Loss bzw. DR-MSE). Dieser Wert
# bestraft Heterogenität überproportional, weil ein konstantes Effektmodell
# (Intercept-Only) niedrigeren OOS-MSE hat als ein heterogenes Modell mit
# Varianz. Der R-Score normalisiert gegen die Intercept-Baseline:
#   R-Score = 1 − MSE(heterogen) / MSE(konstant)
# Ein Intercept-Only-Modell bekommt R-Score ≈ 0, echte Heterogenität > 0.
# Ranking-Erhaltung: R-Score ist monotone Transformation von MSE (base_mse
# pro Fold konstant) → identische Trial-Reihenfolge wie roher MSE.
# Kongeniality (Schuler et al., 2018): Jeder Learner nutzt seine native
# est.score()-Metrik → kein Train-Eval-Mismatch bei Hyperparameter-Tuning.
...
EconML est.score()-Methoden bewertet, normalisiert als R-Score:

- **NonParamDML:** est.score() = R-Loss (Nie & Wager, 2021): E[(Y_res − τ·T_res)²].
- **DRLearner:** est.score() = DR-MSE: doubly-robust Pseudo-Outcome-MSE.

R-Score-Normalisierung (Schuler et al., 2018; Nie & Wager, 2021):
  R-Score = 1 − MSE(heterogen) / MSE(konstant)
Der rohe MSE aus est.score() bestraft Heterogenität überproportional, weil
ein Intercept-Only-Modell (τ(X) = const) niedrigeren OOS-MSE hat als ein
heterogenes Modell mit Varianz. Der R-Score normalisiert dagegen:
Intercept-Only → R-Score ≈ 0, echte Heterogenität → R-Score > 0.
Die Base-MSE (Intercept-Only) wird einmalig pro Fold VOR dem Trial-Loop
via DummyRegressor + refit_final() berechnet.
...
- Pro Trial wird NUR model_final als neues Objekt erstellt (est.model_final = ...)
  und via est.refit_final() gefittet.
...
wird erst am Ende auf stabile K-Fold-Mittelwerte angewendet.
...
in allen Cross-Prediction-Folds wiederverwendet."""
    ...
    def __init__(self, cfg: AnalysisConfig) -> None:
        ...
        self.best_scores: Dict[str, float] = {}
        self.skill_scores: Dict[str, float] = {}  # FMT: R-Score (1 − trial_MSE / base_MSE)
    ...
    def tune_final_model(
        ...
        if name == "nonparamdml":
            ...
            # ── Base-MSE pro Fold vorberechnen (Intercept-Only-Baseline für R-Score) ──
            _fold_base_mse = []
            _fold_train_base_mse = []
            for _est_b, _tr_b, _va_b in _cached_folds:
                _bm_val = _compute_base_mse_via_dummy(_est_b, Y_tune[_va_b], T_tune[_va_b], X_tune.iloc[_va_b])
                _fold_base_mse.append(_bm_val)
                if _op > 0:
                    _bm_tr = _compute_base_mse_via_dummy(_est_b, Y_tune[_tr_b], T_tune[_tr_b], X_tune.iloc[_tr_b])
                    _fold_train_base_mse.append(_bm_tr)
                # refit_final() mit DummyRegressor hat model_final überschrieben → Restore
                _est_b.model_final = build_base_learner(base_type, {**fmt_fixed_params}, seed=self.seed, task="regressor", parallel_jobs=-1)
                _est_b.refit_final()
            _tlog.info(
                "FMT '%s': Base-MSE (Intercept-Only) pro Fold berechnet: %s. Scoring: R-Score.",
                model_name, [f"{b:.6g}" for b in _fold_base_mse],
            )
            ...
            def objective(trial):
                ...
                for fold_i, (est, tr, va) in enumerate(_cached_folds):
                    ...
                    est.refit_final()
                    _trial_mse = float(est.score(Y_tune[va], T_tune[va], X=X_tune.iloc[va]))
                    val_score = _to_r_score(_trial_mse, _fold_base_mse[fold_i])
                    fold_scores_raw.append(val_score)
                    if _op > 0:
                        _tr_mse = float(est.score(Y_tune[tr], T_tune[tr], X=X_tune.iloc[tr]))
                        fold_train_scores.append(_to_r_score(_tr_mse, _fold_train_base_mse[fold_i]))
            ...
            for _est_b, _tr_b, _va_b in _cached_folds:
                ...
                if _op > 0:
                    _bm_tr = _compute_base_mse_via_dummy(_est_b, Y_tune[_tr_b], T_tune[_tr_b], X_tune.iloc[_tr_b])
                    _fold_train_base_mse.append(_bm_tr)
                # Restore model_final nach DummyRegressor
                _est_b.model_final = build_base_learner(base_type, {**fmt_fixed_params}, seed=self.seed, task="regressor", parallel_jobs=-1)
                _est_b.refit_final()
            _tlog.info(
                "FMT '%s': Base-MSE (Intercept-Only) pro Fold berechnet: %s. Scoring: R-Score.",
...
def tune_causal_forest(
    ...
    - CausalForestDML: Nuisance wird einmalig gecacht (cache_values=True),
      ...
      ACHTUNG: CausalForestDML hat KEIN set_params() (kein sklearn BaseEstimator).
    - CausalForest (GRF): Pro Trial neuer Estimator mit Params im Konstruktor.
      Nuisance-Residuen werden vorab manuell berechnet, R-Score als Metrik.

    n_estimators wird auf Vielfache von subforest_size=4 beschränkt (step=4
    in suggest_int), da EconML sonst ValueError wirft.

    Bei RCT (study_type="rct") wird model_t als DummyClassifier(strategy="prior")
    im Nuisance-Caching verwendet → konstante Propensity P(T|X) = mean(T).
    ...
    # R-Score braucht E[Y|X] und E[T|X]. Diese ändern sich nicht zwischen Trials
    ...
    if not is_cfdml:
        ...
        _tlog.info(
            "GRF R-Score: Nuisance-Residuen vorberechnet (%d Folds, %s Base-Learner).%s",
    ...
    _cft_fold_base_mse = []
    if is_cfdml:
        for _est_b, _tr_b, _va_b in _cfdml_cached_folds:
            _Y_res_b, _T_res_b = _nuisance_residuals_for_val(_est_b, Y_np[_va_b], T_np[_va_b], X_np[_va_b])
            _cft_fold_base_mse.append(_compute_base_mse_from_residuals(_Y_res_b, _T_res_b))
    else:
        for _tr_b, _va_b, _Y_res_b, _T_res_b in _grf_fold_data:
            _cft_fold_base_mse.append(_compute_base_mse_from_residuals(_Y_res_b, _T_res_b))
    _tlog.info(
        "CFT '%s': Base-MSE (Intercept-Only) pro Fold: %s. Scoring: R-Score.",
        model_type, [f"{b:.6g}" for b in _cft_fold_base_mse],
    )
        ...
        if is_cfdml:
            ...
            for fold_i, (est, tr, va) in enumerate(_cfdml_cached_folds):
                try:
                    ...
                    for k, v in _forest_params.items():
                        setattr(est, k, v)
                    est.refit_final()
                    _trial_mse = float(est.score(Y_np[va], T_np[va], X=X_np[va]))
                    score = _to_r_score(_trial_mse, _cft_fold_base_mse[fold_i])
            ...
            for fold_i, (tr, va, Y_res_va, T_res_va) in enumerate(_grf_fold_data):
                try:
                    ...
                    # R-Score: 1 − MSE(heterogen) / MSE(konstant)
                    _trial_mse = float(np.mean((Y_res_va - cate_va * T_res_va) ** 2))
                    score = _to_r_score(_trial_mse, _cft_fold_base_mse[fold_i])
                    fold_scores.append(score)
                    del est
                    import gc; gc.collect()
    ...
    _cft_mode = "cache_values + refit_final" if is_cfdml else "vorberechnete Residuen"
    _tlog.info(

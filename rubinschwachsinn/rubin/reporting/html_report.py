...
def generate_html_report(collector: ReportCollector, output_path: str) -> str:
    ...
    if cs.get("final_tuning_enabled"):
        ...
        h = '<p class="expl">Optimiert das CATE-Effektmodell (model_final) via Optuna mit cache_values: Nuisance-Modelle werden einmalig pro äußerem Fold gecacht, Trials fitten nur noch model_final via refit_final(). Bewertung per R-Score (1 − MSE/Base-MSE) auf Out-of-Fold-Val. Intercept-Only → R-Score ≈ 0, echte Heterogenität → &gt; 0.</p>'
        ...
        if fmt.get("best_scores"):
            ...
            h += '<p class="expl">R-Score: 1 − MSE(heterogen) / MSE(konstant). Normalisiert gegen Intercept-Only-Baseline. R-Score &gt; 0 = echte Heterogenität, ≈ 0 = Intercept-Only, &lt; 0 = Overfitting.</p>'
    ...
    if cft:
        ...
        h += f'<p class="expl">Optuna TPE über 8–9 Wald-Parameter (n_estimators, max_depth, min_samples_leaf, etc.). '
        h += f'Modus: <strong>{escape(_cft_mode)}</strong>. '
        h += f'Bewertung per R-Score (1 − MSE/Base-MSE) auf Tuning-CV-Folds. '
        h += f'Intercept-Only → R-Score ≈ 0, echte Heterogenität → &gt; 0.</p>'

        h += '<div class="card-grid">'
        h += f'<div class="cd"><div class="cd-l">Modell</div><div class="cd-v">{escape(_cft_model)}</div></div>'
        h += f'<div class="cd"><div class="cd-l">Trials</div><div class="cd-v">{_cft_nc} / {_cft_nt}</div></div>'
        h += f'<div class="cd"><div class="cd-l">Modus</div><div class="cd-v">{escape(_cft_mode)}</div></div>'
        ...
        if _cft_score is not None:
            ...
            h += '<p class="expl">R-Score: 1 − MSE(heterogen) / MSE(konstant). Normalisiert gegen Intercept-Only-Baseline.</p>'
        ...
        if collector.cft_best_params:
            ...
            for model, params in sorted(collector.cft_best_params.items()):

...
- `search_space`: Low/High Overrides pro Parameter. Beispiel:
  `{max_depth: {low: 5, high: 20}}`. Nicht überschriebene Parameter nutzen Defaults.

**Wichtig:**

- `n_estimators` muss durch `subforest_size=4` teilbar sein (EconML-Constraint).
  CFT erzwingt `step=4` bei der Optuna-Suggestion, UI-Slider ebenfalls.

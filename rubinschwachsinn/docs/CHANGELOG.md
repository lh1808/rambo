...
- **`n_estimators` muss durch `subforest_size=4` teilbar sein:** Optuna-Suggestion mit `step=4` in `suggest_int()` (nicht Post-Rounding, da `study.best_trial.params` Original-Werte enthält). UI-Slider: `min=100, max=500, step=4`. Safety-Net: `_sanitize_forest_params()` in model_registry rundet als letzte Absicherung.
...
- **CausalForestDML `max_samples` Crash:** CFT-Suchraum auf `[0.15, 0.5]` begrenzt (EconML: `max_samples ≤ 0.5` bei `inference=True`). UI-Slider und Fallback-Ranges synchronisiert.
...
- **CausalForest-Tuning (CFT):** Optuna TPE über 8-9 Wald-Parameter (n_estimators, max_depth, min_samples_leaf, etc.). CausalForestDML via R-Score (normalisiert gegen Intercept-Baseline), CausalForest via vorberechnete Residuen. Ersetzt EconML tune() Grid-Search.

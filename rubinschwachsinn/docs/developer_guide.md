...
Beide FMT-Modelle (NonParamDML, DRLearner) nutzen äußere OOF-CV mit `est.score()`, normalisiert als R-Score: model_final wird auf Train gefittet und auf komplett Out-of-Fold-Val bewertet. Dies verhindert optimistische Score-Schätzungen. Der R-Score (1 − MSE(heterogen) / MSE(konstant)) normalisiert gegen eine Intercept-Only-Baseline und verhindert, dass Optuna zu konservativen Konfigurationen konvergiert. Overfit-Penalty ist skalen-sicher (relative Tolerance).

### CausalForest-Tuning (CFT)

CausalForestDML und CausalForest werden via Optuna TPE über 8–9 Wald-Parameter getunt. CausalForestDML: Nuisance wird einmalig gecacht (`cache_values=True`), Trials ändern Forest-Parameter via `setattr()` + `refit_final()` (CausalForestDML hat kein `set_params()`). CausalForest: Pro Trial frischer Estimator mit Params im Konstruktor. `n_estimators` wird auf Vielfache von `subforest_size=4` beschränkt. Bei RCT: DummyClassifier für model_t im Cache. Siehe `docs/tuning_optuna.md → CausalForest-Tuning (Optuna)` für Details.

...
- Zusätzlich wird für jeden BLT-Task ein **Skill Score** berechnet und geloggt, der die Interpretierbarkeit verbessert:
  ...
  - **FMT/CFT:** Der R-Score (1 − MSE(heterogen) / MSE(konstant)) normalisiert den rohen MSE aus `est.score()` gegen eine Intercept-Only-Baseline. R-Score ≈ 0 = nicht besser als konstantes Effektmodell, R-Score > 0 = echte Heterogenität, R-Score < 0 = Overfitting. Die Base-MSE wird einmalig pro Fold vorberechnet.
...
**Score-Konvention:** Beide FMT-Modelle werden über den **R-Score** bewertet — EconMLs `est.score()` normalisiert gegen eine Intercept-Only-Baseline:

- **NonParamDML:** `est.score()` = R-Loss (Nie & Wager): `E[(Y_res − τ·T_res)²]` → R-Score = 1 − R-Loss / Base-R-Loss
- **DRLearner:** `est.score()` = DR-MSE: doubly-robust Pseudo-Outcome-MSE → R-Score = 1 − DR-MSE / Var(Γ) (Standard-R²)
...
Die Defaults orientieren sich an EconML, leicht konservativer für Stabilität. CFT kann diese weiter optimieren:

| Parameter | Default | EconML | Begründung |
|-----------|---------|--------|------------|
| `max_depth` | `None` | `None` | Unbegrenzt — CFT findet die optimale Tiefe |
| `min_samples_leaf` | `10` | `5` | Leicht konservativer |
| `min_samples_split` | `20` | `10` | Leicht konservativer |
...
> **Wichtig:** `max_samples` darf bei `inference=True` (Default) maximal 0.5 betragen. Der CFT-Suchraum ist auf `[0.15, 0.5]` begrenzt, um diesen EconML-Constraint einzuhalten.
...
| FMT | NonParamDML | R-Score (via est.score()) | higher=better | direkt | R-Score ✓ | Relativ ✓ |
| FMT | DRLearner | R-Score (via est.score()) | higher=better | direkt | R-Score ✓ | Relativ ✓ |
| GRF | CausalForestDML | R-Score (via est.score()) | higher=better | direkt | R-Score ✓ | Relativ ✓ |
| GRF | CausalForest | R-Score (via Residuals) | higher=better | direkt | R-Score ✓ | Relativ ✓ |

**R-Score-Normalisierung (Nie & Wager, 2021; Schuler et al., 2018):** FMT und CFT verwenden den R-Score = 1 − MSE(heterogen) / MSE(konstant). Der rohe MSE aus `est.score()` bestraft Heterogenität überproportional, da ein Intercept-Only-Modell niedrigeren OOS-MSE hat. Der R-Score normalisiert dagegen: Intercept-Only → ≈ 0, echte Heterogenität → > 0, Overfitting → < 0. Die Base-MSE wird einmalig pro Fold via DummyRegressor (FMT) bzw. analytisch (CFT GRF) vorberechnet.

### Warum R-Score statt rohem MSE?

- **Problem:** Der rohe MSE aus `est.score()` bestraft Heterogenität überproportional.
...
- **NonParamDML/CausalForestDML:** Base-MSE = min_θ E[(Y_res − θ·T_res)²] (OLS-Lösung).
  Bei FMT via DummyRegressor + refit_final() vorberechnet, bei CFT via Nuisance-Residuals.
- **DRLearner:** Base-MSE = Var(Γ) der DR-Pseudo-Outcomes (= Standard-R²).
  Via DummyRegressor + refit_final() vorberechnet.
- **CausalForest (GRF):** Base-MSE analytisch aus vorberechneten Nuisance-Residuals.
...
Schuler et al. (2018) beobachten, dass Metriken wie R-Loss und DR-Loss dazu tendieren, den "kongenial" zugehörigen Learner zu bevorzugen (R-Loss → R-Learner, DR-Loss → DR-Learner). Dies ist ein Problem beim **Modellvergleich zwischen verschiedenen Lernern**, nicht aber beim **Hyperparameter-Tuning innerhalb eines Lerners** (rubins Anwendungsfall). Für FMT/CFT nutzt jeder Learner seine native Metrik (`est.score()`), was korrekt ist — es gibt keinen Train-Eval-Mismatch.

### Base-MSE-Approximation (FMT NonParamDML)

Für FMT NonParamDML wird die Base-MSE via `DummyRegressor("mean") + refit_final()` berechnet. Da EconML model_final intern mit dem Reweighting-Trick wrappet, produziert der DummyRegressor eine leicht andere konstante Vorhersage als das theoretische OLS-Optimum θ* = Σ(Y_res·T_res)/Σ(T_res²). Die Abweichung in der Base-MSE beträgt ~0.015% und ist für das Ranking irrelevant (monotone Transformation). Für CFT (GRF + CFDML) und FMT DRLearner ist die Base-MSE exakt.
...
### CausalForestDML
CausalForestDML wird via Optuna über 8 Wald-Parameter getunt (n_estimators, max_depth, min_samples_leaf, min_samples_split, max_features, max_samples, min_var_fraction_leaf, min_impurity_decrease). Die Nuisance-Modelle (model_y, model_t) werden einmalig pro äußerem Fold gecacht (`cache_values=True`). Bei RCT wird model_t als `DummyClassifier(strategy="prior")` im Cache verwendet (konstante Propensity).
...
Bewertung via R-Score (est.score() normalisiert gegen Intercept-Only-Baseline, Nie & Wager 2021) auf äußeren CV-Folds (tuning_cv_seed, StratifiedKFold T×Y). `n_estimators` wird auf Vielfache von `subforest_size=4` beschränkt (`step=4` in `suggest_int`).

### CausalForest (GRF)
Der reine CausalForest (econml.grf) hat keine eigene `score()`-Methode. Stattdessen wird der R-Score manuell berechnet: Nuisance-Modelle (BLT-getunte CatBoost/LightGBM) werden einmalig pro Fold vorberechnet, Residuals und Base-MSE (Intercept-Only) vorberechnet. Pro Trial wird ein frischer `CausalForest(**trial_params)` erstellt, gefittet und via R-Score = 1 − MSE(CATE) / Base-MSE bewertet. 9 Wald-Parameter (inkl. criterion: mse/het). `n_estimators` ebenfalls `step=4`.
...
- **n_estimators ÷ 4:** EconML-Constraint (subforest_size=4 für BLB-Inferenz)
...
- **CFT-Objective:** `trial.suggest_int("n_estimators", 100, 500, step=4)` — Optuna schlägt NUR Vielfache von 4 vor
- **UI-Slider:** `step=4`, Range `[100, 500]` — nur gültige Werte auswählbar
- **Defaults:** `_CFDML_DEFAULTS` und `_CF_DEFAULTS` verwenden `n_estimators=200` (÷4=50)

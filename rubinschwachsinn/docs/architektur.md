...
**CausalForest-Tuning (CFT):** Optimiert Wald-Parameter (max_depth, min_samples_leaf, max_features, n_estimators etc.) für CausalForestDML und CausalForest via Optuna TPE. `n_estimators` muss durch `subforest_size=4` teilbar sein (`step=4`). Trials laufen sequentiell (n_jobs=1), Forest intern nutzt alle Kerne.

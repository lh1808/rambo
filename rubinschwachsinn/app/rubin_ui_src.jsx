...
const FinalTuningPlanPreview = ({models, fmtEnabled, fmtModels, fmtSingleFold, fmtTrials, fmtCv, outerCv: outerCvProp, mcIters, isBoth}) => {
  ...
  eligible.forEach(m => {
    ...
    rows.push({
      ...
      note: `Pre-fit: ${outerFolds} Fold(s) × Nuisance (${nuisanceDesc}, cache_values). Pro Trial: ${outerFolds}× refit_final() + R-Score(est.score(val)).`
    });
  });
  ...
  return (
    <div>
      ...
      <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
        Nuisance-Modelle werden <strong>einmalig</strong> pro äußerem Fold mit cache_values gecacht. Trials fitten nur model_final via refit_final() auf gecachten Residuals (sequentiell, n_jobs=1). OOF-Evaluation via R-Score (est.score normalisiert gegen Intercept-Baseline).
              ...
              return (<>
                ...
                <div style={{border:"1px solid #ede6e7",borderRadius:8,overflow:"hidden"}}>
                  ...
                  {hasCF && <div style={{borderBottom:"1px solid #f0f0f0"}}>
                    ...
                    <div style={{fontSize:10.5,color:"#999",padding:"0 14px 6px",lineHeight:1.4}}>Pre-fit: {evalFolds}F × Nuisance ({dc}×model_y + {dc}×model_t + 1×Forest, cache_values). Pro Trial: {evalFolds}× refit_final() + R-Score(est.score(val)). 8 Wald-Params.</div>
                  </div>}
                  {hasGRF && <div style={{background:hasCF?"#fdfbfb":"#fff"}}>
                    ...
                    <div style={{fontSize:10.5,color:"#999",padding:"0 14px 6px",lineHeight:1.4}}>1 Forest-Fit pro Fold. Scoring via manuellem R-Score: {evalFolds}×2 BLT-getunte Nuisance-Modelle einmalig vorberechnet. 9 Wald-Params (+ criterion).</div>
                  </div>}
                </div>
              </>);

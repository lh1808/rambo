...
class DataPrepPipeline:
    ...
    def __init__(self, cfg: AnalysisConfig, dataprep_cfg: DataPrepConfig) -> None:
        ...
        # Sanity-Checks früh (bei der Konstruktion), damit Fehler nicht erst
        # nach teuren I/O-Schritten auffallen. Hinweis: Diese Checks standen
        # historisch unerreichbar hinter dem return in _target_cols (toter Code).
    ...
    @staticmethod
    def _expand_replacement_mapping(mapping: Optional[Dict[str, Any]]) -> Optional[Dict[Any, Any]]:
        """Erweitert Replacement-Keys um Typ-Varianten (str/int/float).

        Hintergrund: Die UI bzw. YAML-Config liefert Mapping-Keys IMMER als
        Strings (settings.py typisiert ``Dict[str, Any]``; Pydantic lehnt
        Int-Keys sogar ab). Die Treatment-/Target-Spalten aus SAS7bdat/CSV
        sind aber häufig numerisch (float64/int64). ``Series.replace()``
        matcht Dict-Keys typgenau — ``{"2": 1}`` greift auf einer
        float64-Spalte mit ``2.0`` daher NICHT (stiller No-op, siehe Bug
        „T bleibt [0 1 2] trotz Mapping").

        Deshalb wird hier für jeden numerisch interpretierbaren Key
        zusätzlich eine float- und int-Variante erzeugt:
        ``{"2": 1}`` → ``{"2": 1, 2.0: 1, 2: 1}``.
        Nicht-numerische Keys (z. B. ``"J"``/``"N"``) bleiben unverändert.
        """
        if not mapping:
            return None
        ...
        for k, v in mapping.items():
            expanded[k] = v
            ks = str(k).strip()
            expanded.setdefault(ks, v)
            try:
                kf = float(ks)
            except (TypeError, ValueError):
                continue  # nicht-numerischer Key (z. B. "J") — keine Varianten nötig
            expanded.setdefault(kf, v)          # float-Variante (SAS liefert float64)
            if kf.is_integer():
                expanded.setdefault(int(kf), v)  # int-Variante (CSV/Parquet: int64)
        return expanded

    def _verify_replacement_applied(self, series: pd.Series, mapping: Dict[str, Any],
                                    col_name: str, file_path: str) -> None:
        """Prüft nach ``replace()``, ob die konfigurierten Rohwerte wirklich ersetzt wurden.

        Fail-fast-Schutz: Ein stilles Nicht-Greifen des Mappings (z. B. durch
        Typ-Mismatch) führte bisher erst spät in der Analyse zu Folgefehlern
        (EconML „binary treatment only", gescheiterte Optuna-Trials). Hier wird
        stattdessen direkt beim Einlesen ein klarer Fehler geworfen.

        Identitäts-Mappings (z. B. ``"1" → 1``) werden übersprungen — dort ist
        das Verbleiben des Werts korrekt und kein Fehlerindiz. Ebenso werden
        Keys übersprungen, die zugleich ZIELWERT eines anderen Mapping-Eintrags
        sind (z. B. Swap-Mapping ``"0" → 1, "1" → 0``): Der verbleibende Wert
        kann dort das korrekte Ergebnis des anderen Eintrags sein — ein Rest
        ist prinzipiell nicht vom Mapping-Ergebnis unterscheidbar, und ein
        False-Positive-Abbruch eines korrekten Laufs wiegt schwerer als eine
        entgangene Erkennung in diesem Sonderfall.
        """
        # Zielwerte des Mappings normalisieren (numerisch + als String) —
        # Keys, die selbst Zielwert sind, sind nicht verifizierbar (s. o.).
        target_strs = set()
        target_floats = set()
        for v in mapping.values():
            target_strs.add(str(v).strip())
            try:
                target_floats.add(float(v))
            except (TypeError, ValueError):
                pass

        remaining = pd.unique(series.dropna())
        leftovers: List[str] = []
        for k, v in mapping.items():
            ks = str(k).strip()
            try:
                kf: Optional[float] = float(ks)
            except (TypeError, ValueError):
                kf = None
            # Identitäts-Mapping? (Key == Value, numerisch oder als String)
            try:
                if kf is not None and float(v) == kf:
                    continue
            except (TypeError, ValueError):
                pass
            if ks == str(v).strip():
                continue
            # Key ist Zielwert eines anderen Eintrags? → nicht verifizierbar, überspringen.
            if ks in target_strs or (kf is not None and kf in target_floats):
                continue
            # Kommt der Rohwert (numerisch oder als String) noch in der Spalte vor?
            for u in remaining:
                _match = str(u).strip() == ks
                if not _match and kf is not None:
                    try:
                        _match = float(u) == kf
                    except (TypeError, ValueError):
                        _match = False
                if _match:
                    leftovers.append(str(k))
                    break
        if leftovers:
            raise ValueError(
                f"Replacement-Mapping für Spalte '{col_name}' hat nicht gegriffen: "
                f"Die Rohwerte {leftovers} sind nach dem Mapping weiterhin in "
                f"'{file_path}' vorhanden (Unique-Werte: {sorted(map(str, remaining))[:10]}). "
                f"Typischer Grund: Typ-Mismatch zwischen Mapping-Keys (Strings aus "
                f"YAML/UI) und Spalten-Dtype ({series.dtype}). Bitte Mapping und "
                f"Spalteninhalt prüfen."
            )
    ...
    def _read_files(self, file_paths: Optional[List[str]] = None, merge_only: bool = False) -> pd.DataFrame:
        ...
        # Replacement-Mappings EINMALIG typ-robust erweitern (str → zusätzlich
        # int/float-Varianten), damit sie auch auf numerischen Spalten greifen.
        _treat_repl = self._expand_replacement_mapping(dp.treatment_replacement)
        _target_repl = self._expand_replacement_mapping(dp.target_replacement)
        ...
        for file_path in paths:
            ...
            # Replacement-Mappings (z. B. 'J'/'N' -> 0/1 oder '2' -> 1).
            # WICHTIG: _treat_repl/_target_repl sind die typ-robust erweiterten
            # Varianten (siehe _expand_replacement_mapping) — die rohen Config-
            # Mappings mit reinen String-Keys greifen auf numerischen Spalten nicht.
            if _treat_repl:
                first[treat_col] = first[treat_col].replace(_treat_repl)
            if _target_repl:
                for _tc in target_cols:
                    first[_tc] = first[_tc].replace(_target_repl)
            ...
            for chunk in iterator:
                ...
                if _treat_repl:
                    chunk[treat_col] = chunk[treat_col].replace(_treat_repl)
                if _target_repl:
                    for _tc in target_cols:
                        chunk[_tc] = chunk[_tc].replace(_target_repl)
            ...
            _df_file = pd.concat(chunk_list, ignore_index=True)

            # Fail-fast-Validierung: Haben die Mappings tatsächlich gegriffen?
            # (Schützt vor stillen No-ops durch Typ-Mismatch, die sonst erst in
            # der Analyse als EconML-/Optuna-Fehler auffallen würden.)
            if dp.treatment_replacement:
                self._verify_replacement_applied(_df_file[treat_col], dp.treatment_replacement, treat_col, p)
                self._logger.info(
                    "Treatment-Replacement angewendet ('%s', Datei '%s'): Unique-Werte nach Mapping: %s",
                    treat_col, p, sorted(pd.unique(_df_file[treat_col].dropna()).tolist())[:10],
                )
            if dp.target_replacement:
                for _tc in target_cols:
                    self._verify_replacement_applied(_df_file[_tc], dp.target_replacement, _tc, p)

            df_list.append(_df_file)

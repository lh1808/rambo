...
from rubin.evaluation.drtester_core import filter_tester_for_mask, fit_drtester_nuisance
from rubin.evaluation.evaluation_plots import (
    evaluate_cate_with_plots,
    generate_ate_barplot,
    generate_cate_distribution_plot,
    generate_uplift_plots,
)
from rubin.evaluation.score_plots import plot_custom_qini_curve, policy_value_comparison_plots
...
def _log_ram(logger, label: str):
    """Loggt aktuelle RAM-Auslastung des Prozesses."""
    try:
        import psutil
        proc = psutil.Process()
        rss = proc.memory_info().rss / (1024 ** 3)
        total = psutil.virtual_memory().total / (1024 ** 3)
        pct = proc.memory_info().rss / psutil.virtual_memory().total * 100
        logger.info("RAM [%s]: %.1f GB RSS (%.0f%% von %.1f GB)", label, rss, pct, total)
        if pct > 80:
            logger.warning("RAM-Warnung [%s]: %.0f%% Auslastung — OOM-Risiko!", label, pct)
    except ImportError:
        pass  # psutil nicht verfügbar — still ignorieren
    except Exception:
        pass
...
class AnalysisPipeline:
    ...
    def _load_inputs(self) -> tuple[pd.DataFrame, np.ndarray, np.ndarray, Optional[np.ndarray], Optional[np.ndarray]]:
        ...
        # ── Eval-Maske laden (Train Many, Evaluate Some) ──
        # Wird VOR df_frac geladen, damit sie mit-subsampled wird.
        eval_mask: Optional[np.ndarray] = None
        _mask_cfg = self.cfg.data_files.eval_mask_file
        if _mask_cfg:
            ...
            _mask_files = [_mask_cfg] if isinstance(_mask_cfg, str) else list(_mask_cfg)
            _mask_files = [f for f in _mask_files if f and f.strip()]
            if _mask_files:
                try:
                    combined = np.zeros(len(X), dtype=bool)
                    _loaded = 0
                    for _mf in _mask_files:
                        _raw = np.load(_mf).astype(bool)
                        if len(_raw) == len(X):
                            combined |= _raw
                            _loaded += 1
                        else:
                            self._logger.warning(
                                "eval_mask '%s': Länge (%d) passt nicht zu X (%d) — übersprungen.",
                                _mf, len(_raw), len(X),
                            )
                    if _loaded > 0:
                        eval_mask = combined
                        self._logger.info(
                            "TMES: %d Masken geladen (%d/%d Zeilen für Evaluation).",
                            _loaded, int(combined.sum()), len(X),
                        )
                except Exception:
        ...
        if self.cfg.data_files.dtypes_file:
            ...
            # Auto-Erkennung: dtypes.json neben X-Datei suchen (DataPrep-Output)
            from pathlib import Path
            _x_path = Path(self.cfg.data_files.x_file)
            _auto_dtypes = _x_path.parent / "dtypes.json"
            if _auto_dtypes.exists():
                try:
                    with open(_auto_dtypes, "r", encoding="utf-8") as f:
                        dtypes = json.load(f)
                    _n_restored = 0
                    _n_cat = 0
                    for c, dt in dtypes.items():
                        if c in X.columns and str(X[c].dtype) != dt:
                            try:
                                X[c] = X[c].astype(dt)
                                _n_restored += 1
                                if dt == "category":
        ...
        # Fail-fast: Treatment-Arme gegen treatment.type validieren.
        # Ohne diesen Check fällt z. B. ein nicht gegriffenes treatment_replacement
        # (T=[0,1,2] statt [0,1]) erst tief in Tuning/Training als EconML-Fehler
        # bzw. Serie gescheiterter Optuna-Trials auf.
        self._validate_treatment_arms(T, context="Trainingsdaten (t_file)")
        ...
        if eval_mask is not None:
            self._logger.info(
                "Eval-Maske: %d von %d Zeilen für Evaluation (Train Many, Evaluate Some).",
    ...
    def _validate_treatment_arms(self, T: np.ndarray, context: str = "Trainingsdaten") -> None:
        ...
        Wirft einen klaren ``ValueError`` direkt nach dem Datenladen, statt
        die Inkonsistenz erst tief in Tuning/Training auffallen zu lassen
        (EconML: „binary categorical treatment only"; uplift_metrics:
        „t muss binär (0/1) sein"; reihenweise gescheiterte Optuna-Trials).

        Regeln:
        - binary: exakt die Werte {0, 1} (EconML + Qini/AUUC setzen 0/1 voraus).
        - multi:  Referenzgruppe 0 muss vorhanden sein (reference_group=0 ist
          aktuell die einzige unterstützte Konfiguration, siehe settings.py);
          nicht-konsekutive Arme (z. B. [0, 1, 3]) werden nur gewarnt.
        """
        uniques = sorted(np.unique(np.asarray(T)).tolist())
        t_type = (self.cfg.treatment.type or "binary").lower()

        if t_type == "binary":
            if uniques != [0, 1]:
                raise ValueError(
                    f"treatment.type='binary', aber {context} enthalten die "
                    f"Treatment-Werte {uniques} (erwartet: [0, 1]). "
                    f"Häufigste Ursache: das treatment_replacement-Mapping in der "
                    f"Datenvorbereitung hat nicht (vollständig) gegriffen oder ist "
                    f"nicht konfiguriert. Alternativ treatment.type='multi' setzen, "
                    f"falls bewusst mehrere Treatment-Arme analysiert werden sollen."
                )
        else:  # "multi"
            if 0 not in uniques:
                raise ValueError(
                    f"treatment.type='multi', aber {context} enthalten keine "
                    f"Referenzgruppe 0 (Control). Gefundene Werte: {uniques}. "
                    f"Aktuell wird nur reference_group=0 unterstützt — bitte das "
                    f"treatment_replacement-Mapping so wählen, dass Control auf 0 abbildet."
                )
            expected = list(range(len(uniques)))
            if uniques != expected:
                self._logger.warning(
                    "Multi-Treatment: Arme %s sind nicht konsekutiv (erwartet %s). "
                    "Per-Arm-Metriken (qini_T1, ...) und Reports nummerieren nach "
                    "Arm-Wert — Lücken können zu verwirrenden Bezeichnungen führen.",
                    uniques, expected,
                )
    ...
    def _run_feature_selection(self, cfg, X, T, Y, mlflow):
        ...
        if fs.enabled and effective_methods:
            ...
            if importances:
                ...
                mlflow.log_param("feature_selection_max_features", fs.max_features)
        ...
        # Finale ausgewählte Feature-Menge wegloggen (Namen + Datentyp nach abgeschlossener
        # Reduktion), sobald überhaupt ein Reduktionsschritt lief (Importance-Selektion oder
        # Korrelationsfilter).
        if fs.enabled and (importances or n_corr):
            selected = [(str(c), str(X[c].dtype)) for c in X.columns]
            self._logger.info(
                "Feature-Selektion: %d ausgewählte Features nach Reduktion: %s",
                len(selected), ", ".join(f"{name} ({dt})" for name, dt in selected),
            )

            def _write_selected(p):
                with open(p, "w", encoding="utf-8") as fh:
                    json.dump(
                        [{"feature": name, "dtype": dt} for name, dt in selected],
                        fh, ensure_ascii=False, indent=2,
                    )
            _log_temp_artifact(mlflow, _write_selected, "selected_features.json")
    ...
    def _run_tuning(self, cfg, X, T, Y, mlflow, _progress_cb=None):
        ...
        # Referenzen auf Methoden-Ebene, damit das Params-JSON am ENDE der Methode
        # (nach BLT UND FMT) geschrieben werden kann — sonst fehlen die FMT-Params
        # für model_final (inkl. _learner_type im "both"-Modus) im Artefakt.
        _blt_tuner = None
        _fmt_tuner = None
        ...
        if cfg.tuning.enabled:
            ...
            # Hinweis: Das Params-JSON (tuned_baselearner_params.json) wird bewusst
            # erst am ENDE von _run_tuning geschrieben (nach dem FMT-Block), damit
            # auch die model_final-Params — inkl. des im "both"-Modus gewonnenen
            # _learner_type — im Artefakt landen. Historisch wurde es hier
            # geschrieben, wodurch FMT-Ergebnisse im JSON fehlten.

            # Für HTML-Report sammeln
            if hasattr(self, '_report'):
                self._report.tuning_scores.update(tuner.best_scores)
                self._report.tuning_skill_scores.update(tuner.skill_scores)
                # Tuning-Plan (Task-Sharing-Tabelle)
                try:
                    # Nur die tatsächlich getunten Modelle (= active_models in tune_all):
                    # tuning.models filtert, None = alle. Ohne diesen Filter würde der
                    # Report Tasks für Modelle zeigen, die gar nicht BLT-getunt werden.
                    _blt_sel = cfg.tuning.models
                    _blt_active = ([m for m in cfg.models.models_to_train if m in _blt_sel]
                                   if _blt_sel is not None else list(cfg.models.models_to_train))
                    plan = tuner._build_plan(_blt_active)
                    plan_list = []
        ...
        if getattr(cfg, "final_model_tuning", None) is not None and cfg.final_model_tuning.enabled:
            ...
            for mname in cfg.models.models_to_train:
                ...
                try:
                    ...
                    self._logger.error(
                        "FMT '%s' fehlgeschlagen — bisherige Ergebnisse bleiben erhalten. "
                        "Modell wird mit Default-/BLT-Parametern trainiert.", mname, exc_info=True)
                # Zwischen FMT-Modellen: EconML + CatBoost C++-Speicher freigeben
                gc.collect()
                _log_ram(self._logger, f"nach FMT {mname}")
            ...
            # Logger: Zusammenfassung (konsistent mit BLT)
            n_fmt = len([k for k in final_tuner.best_scores if "__adjusted" not in k])
            self._logger.info("Final-Model-Tuning: %d Modelle abgeschlossen.", n_fmt)
            if n_fmt > 0:
                    ...
                    if pen_val is not None:
                        ...
                        if delta > 1e-10:
                            self._logger.info(
                                ...
                                model_short, score_label, raw_val, pen_val, raw_val - pen_val)
                        else:
                            self._logger.info(
                                "FMT '%s': %s → %.6g (Penalty aktiv, Gap innerhalb Toleranz → kein Effekt)",
                                model_short, score_label, raw_val)
                    else:
                        self._logger.info("FMT '%s': %s → %.6g", model_short, score_label, raw_val)
            if hasattr(self, '_report'):
                ...
                try:
                    ...
                    fmt_internal_cv = getattr(cfg.final_model_tuning, "cv_splits", 5)
                    _mc = cfg.data_processing.mc_iters or 1
                    _fits_per_dml = _mc * fmt_internal_cv * 2 + 1  # Nuisance + model_final
                    _fmt_scorer = getattr(cfg.final_model_tuning, "scorer", "rscore")
                    _scorer_fits_per_fold = 0 if _fmt_scorer == "qini" else 4  # RScorer: cv=2 × (model_y + model_t)
                    _scorer_note = "OOF-Qini" if _fmt_scorer == "qini" else "RScorer.score"
                    fmt_plan_list = []
                    # Nur die tatsächlich FMT-getunten Modelle listen (= dieselbe Auswahl
                    # wie die echte FMT-Schleife oben): final_model_tuning.models filtert,
                    # None = alle FMT-fähigen. Ohne diesen Filter würde der Report-Plan
                    # FMT-fähige Modelle zeigen, die gar nicht getunt wurden.
                    _fmt_sel = cfg.final_model_tuning.models
                    for mname in cfg.models.models_to_train:
                        if _fmt_sel is not None and mname not in _fmt_sel:
                            continue
                        ...
                        if name_lower in ("nonparamdml", "drlearner"):
                            ...
                            fmt_plan_list.append({
                                ...
                                "note": f"{_outer} Fold(s) × ({_fits_per_dml}+{_scorer_fits_per_fold}) Pre-fit + {fmt_n_trials}T × {_outer} refit_final ({_scorer_note}).",
                            })
                    if fmt_plan_list:
                    ...
                    self._report.add_fmt_info({
                        ...
                        "method": "oof_cv",
                        "scorer": getattr(cfg.final_model_tuning, "scorer", "rscore"),
    ...
    def _run_training(self, cfg, X, T, Y, tuned_params_by_model, holdout_data, mlflow, _progress_cb=None):
        ...
        # verarbeiten, wenn CausalForest-Tuning aktiv ist. Models-to-train sind voneinander
        # unabhängig, deshalb ist Umsortierung sicher.
        _grf_names = ("CausalForestDML", "CausalForest")
        _has_grf_tune = any(n in (cfg.models.models_to_train or []) for n in _grf_names) and getattr(cfg.causal_forest, "tune_enabled", False)
        ...
        for name in _models_order:
            ...
            if _progress_cb is not None:
                ...
                _label = "CausalForest-Tuning" if (_is_grf and _has_grf_tune) else "Training & Predictions"
            ...
            # Vollständige Architektur-Dokumentation: docs/architektur.md
            ...
            if name.lower() in {"nonparamdml", "drlearner"} and not cfg.final_model_tuning.enabled:
                ...
                if not has_explicit_final:
                    self._logger.warning(
                        ...
                        "um model_final über Scorer-Optimierung (Qini/R-Score) zu tunen.",
                        name,
                    )

            # CausalForest / CausalForestDML: Optuna-basiertes Tuning der Forest-Parameter
            if name in ("CausalForestDML", "CausalForest") and getattr(cfg.causal_forest, "tune_enabled", False) and (not getattr(cfg.causal_forest, "tune_models", []) or name in getattr(cfg.causal_forest, "tune_models", [])):
                try:
                    ...
                    if best_p:
                        # Getunte Forest-Params speichern
                        _forest_key = "forest" if name == "CausalForestDML" else "grf"
                        tuned_params_by_model.setdefault(name, {})[_forest_key] = best_p
                        # Modell mit getunten Params neu konstruieren — den BEREITS oben
                        # korrekt gebauten ctx wiederverwenden (enthält is_rct, den
                        # forest_fixed_params-Merge und parallel_jobs=-1) und nur die
                        # getunten Forest-Params überlagern. Ein frischer ModelContext
                        # an dieser Stelle verlor is_rct/forest_defaults/parallel_jobs →
                        # im RCT-Modus wurde fälschlich geschätzte statt konstanter
                        # Propensity verwendet (inkonsistent mit dem CFT-Tuning und dem
                        # RCT-Log oben).
                        ctx.tuned_params = dict(ctx.tuned_params or {})
                        ctx.tuned_params[_forest_key] = {
                            **(ctx.tuned_params.get(_forest_key) or {}),
                            **best_p,
                        }
                        model = self.registry.create(name, ctx)
                        models[name] = model
                    ...
                    self._logger.info(
                        "CFT '%s': %s → %.6g (%d Trials abgeschlossen).",
                        name, _cft_score_label, best_score if best_score is not None else float("nan"),
                        result.get("n_trials_completed", 0),
                    )
                    # Report: CFT-Ergebnisse analog zu FMT
                    if hasattr(self, '_report'):
                        _cft_mode = "cache_values" if name == "CausalForestDML" else "CausalForestAdapter"
                        self._report.add_cft_info({
                            ...
                            "mode": _cft_mode,
                            "scorer": getattr(cfg.causal_forest, "scorer", "rscore"),
                            "single_fold": bool(getattr(cfg.causal_forest, "single_fold", False)),
                        })
                        if best_p:
            ...
            # Konsistenz mit dem model_final-Log (NonParamDML/DRLearner): für die
            # Forest-Modelle die effektiven Forest-Parameter loggen — deren "model_final"
            # IST der Causal Forest selbst. Nach dem CFT-Block, damit getunte Werte
            # reflektiert sind. Gezeigt werden forest_fixed_params (Config) + CFT-getunte
            # Params; die EconML-Forest-Defaults wirken zusätzlich darunter (analog zu
            # base_fixed_params beim model_final-Log).
            if name in ("CausalForestDML", "CausalForest"):
                _fkey = "forest" if name == "CausalForestDML" else "grf"
                _cft_best = dict((tuned_params_by_model.get(name) or {}).get(_fkey) or {})
                _forest_eff = dict(getattr(cfg.causal_forest, "forest_fixed_params", {}) or {})
                _forest_eff.update(_cft_best)
                self._logger.info(
                    "%s model_final (Forest) effektive Params: %s (cft_tuned=%s, forest_fixed=%s)",
                    name, {k: v for k, v in _forest_eff.items() if k in (
                        "n_estimators", "max_depth", "min_samples_leaf", "min_samples_split",
                        "min_weight_fraction_leaf", "min_var_fraction_leaf", "criterion",
                        "max_samples", "max_features", "min_impurity_decrease",
                    )},
                    "ja" if _cft_best else "nein",
                    bool(getattr(cfg.causal_forest, "forest_fixed_params", {}) or {}),
                )
            ...
            if holdout_data is None:
                ...
                self._logger.info("  %s: Starte %d-Fold Cross-Prediction (Seed=%d).", name, cfg.data_processing.cross_validation_splits, cfg.constants.random_seed)
                ...
                self._logger.info("  %s: Single-Fit auf %d Training-Zeilen, Eval auf %d Holdout-Zeilen.", name, len(X), len(X_test))
                model.fit(Y, T, X=X)
                current_model = model
    ...
    def _run_evaluation(self, cfg, X, T, Y, S, holdout_data, preds, models, tuned_params_by_model, mlflow, eval_mask=None):
        ...
        # Beste Classifier-Params finden: DML model_y/model_t bevorzugt, dann
        # DRLearner model_regression/model_propensity, dann base_learner.fixed_params.
        # model_regression ist bei DRLearner der Outcome-CLASSIFIER (discrete_outcome
        # =True, BLT tunt ihn auf log_loss wie model_y) — ohne diesen Zweig liefe
        # der DRTester bei DRLearner-only-Läufen mit Default-Params (und warnte im
        # "both"-Modus fälschlich über fehlendes _learner_type).
        ...
        for mname in ["NonParamDML", "ParamDML", "CausalForestDML", "DRLearner"]:
            ...
            _y_params = roles.get("model_y") or roles.get("model_regression")
            if _y_params:
                _best_clf_params_y = {**_best_clf_params_y, **_y_params}
                break
        for mname in ["NonParamDML", "ParamDML", "CausalForestDML", "DRLearner"]:
            roles = tuned_params_by_model.get(mname, {})
        ...
        # ── Eval-Maske: Die eigentliche Filterung von X/T/Y/dfp passiert in
        # _evaluate_bt/_evaluate_mt (via eval_mask-Parameter). Hier nur Logging.
        if eval_mask is not None and holdout_data is None:
            self._logger.info(
                "Eval-Maske aktiv: Metriken auf %d von %d Zeilen.",
    ...
    def _evaluate_bt(self, cfg, X, T, Y, holdout_data, mname, dfp, y, t, tuned_params_by_model, eval_summary, policy_values_dict, mlflow, fitted_tester=None, eval_mask=None):
        ...
        try:
            ...
            # Bewusst aus dem VOLLEN dfp (nicht dfp_eval): evaluate_cal nutzt
            # cate_preds_train_ ausschließlich für die Quantil-Cuts
            # (np.quantile) — die Gruppen-Zuordnung läuft über cate_preds_val_.
            # Bei aktiver Eval-Maske (TMES) repräsentiert nur die volle Spalte
            # die Train-Verteilung; eine auf Eval-Zeilen gefilterte Teilmenge
            # würde die Cuts zur Eval-Subpopulation hin verzerren und die
            # Stichprobe grundlos verkleinern. Passt zudem zur Länge von
            # X_train/T_train/Y_train (= volle Daten) unten.
            if train_col in dfp.columns and not np.all(np.isnan(dfp[train_col].to_numpy(dtype=float))):
        ...
        try:
            if base_type == "lgbm":
                ...
                if tree_info:
                    ...
                    # LGBM dump_model kennt keinen 'max_depth'-Key (weder pro Baum noch
                    # top-level) → Tiefe aus der tree_structure ableiten: Anzahl der
                    # Split-Ebenen bis zum tiefsten Blatt (Blatt = kein 'split_index').
                    def _lgbm_depth(node):
                        if not isinstance(node, dict) or "split_index" not in node:
                            return 0
                        return 1 + max(_lgbm_depth(node.get("left_child", {})),
                                       _lgbm_depth(node.get("right_child", {})))
                    depth = _lgbm_depth(tree_info[0].get("tree_structure", {}))
            elif base_type == "catboost":
    ...
    def _train_and_evaluate_surrogate(self, cfg, X, T, Y, teacher_name, preds, holdout_data, models, eval_summary, mlflow, surrogate_name=None, run_drtester=False, fitted_tester=None, eval_mask=None, S=None):
        ...
        if holdout_data is None:
            ...
            # Auch das Ensemble wird direkt prädiziert: ShapeSafeEnsembleCate
            # kapselt Mitglieder + Gewichte selbst (const_marginal_effect,
            # shape-normalisiert) — die Vorhersage nutzt damit garantiert die
            # evaluierte Ensemble-Zusammensetzung.
    ...
    @staticmethod
    def _collect_base_learner_info(cfg, tuned_params_by_model):
        """Base-Learner-Sektion für die Bundle-metadata.json.

        Für type='lgbm'/'catboost' genügt der konfigurierte Typ. Für
        type='both' wird zusätzlich der von Optuna pro Modell und Rolle
        gewählte Learner (_learner_type aus den getunten Params) festgehalten —
        sonst wäre aus dem Bundle nicht rekonstruierbar, welcher Learner
        „gewonnen" hat. Rollen ohne getunte Params fallen zur Laufzeit auf
        CatBoost zurück (siehe build_base_learner in tuning/common.py);
        das dokumentiert 'fallback_without_tuned_params'.
        """
        info = {"type": cfg.base_learner.type}
        if (cfg.base_learner.type or "").lower() != "both":
            return info
        info["fallback_without_tuned_params"] = "catboost"
        chosen = {}
        for mname, roles in (tuned_params_by_model or {}).items():
            role_map = {}
            for role, params in (roles or {}).items():
                # "default" ist ein interner Fallback-Lookup-Key (Kopie der ersten
                # getunten Rolle) — wie im Params-JSON wird er hier gefiltert.
                if role == "default" or not isinstance(params, dict):
                    continue
                lt = params.get("_learner_type")
                if lt:
                    role_map[role] = lt
            if role_map:
                chosen[mname] = role_map
        info["chosen_per_role"] = chosen
        return info
    ...
    def _run_bundle_export(self, cfg, models, eval_summary, X, T, Y, X_full, T_full, Y_full, selected_feature_columns, holdout_data, export_bundle, bundle_dir, bundle_id, mlflow,
                           tuned_params_by_model=None):
        """Synchroner Bundle-Export am Ende des Analyselaufs.

        tuned_params_by_model: Optionales Tuning-Ergebnis (model → role → params).
        Wird genutzt, um die Base-Learner-Wahl (insb. den im "both"-Modus von
        Optuna gewählten _learner_type pro Rolle) in die metadata.json zu schreiben."""
        ...
        # Bundle-Preprocessor: Production bekommt ROHDATEN, daher muss das Bundle die
        # gelernten Encoding-Maps + NA-Fill des DataPrep-FittedPreprocessor tragen.
        # Der SimpleCSVPreprocessor (nur Schema/Dtype) würde Kategoriale per
        # .astype("category") batch-abhängig fehl-enkodieren. Wir laden daher den
        # DataPrep-Preprocessor (liegt neben x_file) und schränken ihn auf die
        # selektierten Features ein. Fallback (mit Warnung) auf Schema-only, wenn er
        # fehlt oder selektierte Features ihm unbekannt sind.
        import pickle as _pickle
        from pathlib import Path as _Path
        preproc = None
        try:
            _dp_pp = _Path(cfg.data_files.x_file).resolve().parent / "preprocessor.pkl"
            if _dp_pp.is_file():
                with open(_dp_pp, "rb") as _f:
                    _dp_preproc = _pickle.load(_f)
                if isinstance(_dp_preproc, FittedPreprocessor):
                    _selected = list(X.columns)
                    _unknown = [c for c in _selected if c not in set(_dp_preproc.feature_columns)]
                    if _unknown:
                        self._logger.warning(
                            "Bundle-Preprocessor: %d selektierte Features sind dem DataPrep-"
                            "Preprocessor unbekannt (%s%s) — aus Rohdaten nicht enkodierbar. "
                            "Fallback auf Schema-only-Preprocessor.",
                            len(_unknown), _unknown[:5], "..." if len(_unknown) > 5 else "",
                        )
                    else:
                        preproc = restrict_fitted_preprocessor(_dp_preproc, _selected)
                        self._logger.info(
                            "Bundle-Preprocessor: DataPrep-FittedPreprocessor übernommen "
                            "(rohdaten-fähig: Encoding-Maps + NA-Fill für %d Features).",
                            len(_selected),
                        )
        except Exception:
            self._logger.warning(
                "Bundle-Preprocessor: DataPrep-Preprocessor konnte nicht geladen werden.",
                exc_info=True,
            )
        if preproc is None:
            preproc = build_simple_preprocessor_from_dataframe(X)
            self._logger.warning(
                "Bundle-Preprocessor: Schema-only (SimpleCSVPreprocessor). Production-Input "
                "MUSS bereits DataPrep-enkodiert sein — Rohdaten werden NICHT korrekt enkodiert."
            )
        ...
        # ── Export-Set bestimmen ───────────────────────────────────────────────
        # ALLE trainierten Modelle werden gebündelt und (außerhalb des Holdout-
        # Modus) auf vollen Daten refittet, damit in Production frei gewählt
        # werden kann, welches Modell eingesetzt wird. Es gibt bewusst keine
        # Optionen für Champion-only-Bundles oder Export ohne Refit.
        _ensemble_name = "Ensemble"
        _ensemble_is_champion = champion == _ensemble_name
        _all_cate_models = {n: m for n, m in models.items() if n not in _surr_names}

        models_to_export = dict(_all_cate_models)
        ...
        _refit_rows = int(len(X_refit))
        # Full-Data-Refit nur AUSSERHALB des Holdout-Modus: im Holdout sind die Modelle
        # bereits auf Train gefittet, und ein Refit auf "voll" würde die Holdout-Zeilen
        # ins Produktionsmodell ziehen und die Evaluation entwerten.
        _do_full_refit = not holdout_data

        def _refit_one(_name, _src):
            """Liefert eine fit-fähige, möglichst auf vollen Daten refittete Kopie von _src."""
            try:
                _obj = copy.deepcopy(_src)
            except Exception:
                self._logger.warning("deepcopy von %s fehlgeschlagen, verwende Original.", _name, exc_info=True)
                _obj = _src
            try:
                if _do_full_refit:
                    _obj.fit(Y_full, T_full, X=X_refit)
                    self._logger.info("Full-Data-Refit: %s auf %d Zeilen refittet.", _name, _refit_rows)
                elif not holdout_data:
                    # Refit-All deaktiviert, CV-Modus: auf Trainingsdaten fitten.
                    _obj.fit(Y, T, X=X.loc[:, selected_feature_columns])
                else:
                    # Holdout-Modus: kein Refit — _src ist bereits auf Train gefittet.
                    _obj = _src
            except Exception:
                self._logger.warning("Refit von %s fehlgeschlagen, verwende ursprüngliche Instanz.", _name, exc_info=True)
                _obj = _src
            return _obj

        # ── Schritt 1: Alle Einzelmodelle refitten und schreiben ────────────────
        # Das Ensemble hat kein fit() und wird in Schritt 2 aus den refitteten
        # Mitgliedern neu zusammengesetzt.
        refit_members = {}
        for mname, mobj in list(models_to_export.items()):
            if mname == _ensemble_name:
                continue
            obj_to_write = _refit_one(mname, mobj)
            models_to_export[mname] = obj_to_write
            refit_members[mname] = obj_to_write
            bundler.write_model(paths, mname, obj_to_write)
            if mname == champion:
                champion_fitted_obj = obj_to_write
                champion_refit_on_full_data = _do_full_refit
                champion_refit_rows = _refit_rows if _do_full_refit else None

        # ── Schritt 2: Ensemble self-contained aus refitteten Mitgliedern bauen ──
        # ShapeSafeEnsembleCate serialisiert seine Mitglieder in die eigene Pickle-
        # Datei. Das Ensemble funktioniert in Production damit unabhängig davon, welche
        # Einzelmodelle als Standalone-Datei im Bundle liegen — und vor allem mit
        # GARANTIERT gefitteten Mitgliedern (behebt den fehlenden _d_t_in-Fehler).
        if _ensemble_name in models_to_export:
            try:
                from rubin.training import ShapeSafeEnsembleCate
                # Mitgliedschaft: Single Source of Truth ist das EVALUIERTE Ensemble
                # (member_names, Pflichtattribut von ShapeSafeEnsembleCate). Nur so
                # ist garantiert, dass das exportierte Bundle-Ensemble exakt dem
                # Objekt entspricht, dessen Metriken im Report stehen.
                _member_src = models[_ensemble_name].member_names
                _member_iter = [n for n in _member_src if n in _all_cate_models]
                _missing = [n for n in _member_src if n not in _all_cate_models]
                if _missing:
                    self._logger.warning(
                        "Ensemble-Rebuild: evaluierte Mitglieder %s nicht mehr in models — "
                        "Export-Ensemble weicht vom evaluierten ab.", _missing,
                    )
                _members, _used = [], []
                for n in _member_iter:
                    m = refit_members.get(n)
                    if m is None:
                        # Mitglied war nicht im Export-Set → jetzt fürs Ensemble refitten.
                        m = _refit_one(n, models[n])
                    if hasattr(m, "const_marginal_effect") or hasattr(m, "effect"):
                        _members.append(m)
                        _used.append(n)
                    else:
                        self._logger.warning("Ensemble: Mitglied %s ist nicht fit-fähig und wird ausgelassen.", n)
                if len(_members) >= 2:
                    _w = np.ones(len(_members)) / len(_members)
                    ensemble_refitted = ShapeSafeEnsembleCate(
                        cate_models=_members, weights=_w, member_names=list(_used),
                    )
                    models_to_export[_ensemble_name] = ensemble_refitted
                    bundler.write_model(paths, _ensemble_name, ensemble_refitted)
                    self._logger.info(
                        "Ensemble neu gebaut: %d refittete Mitglieder gleichgewichtet (%s), self-contained.",
                        len(_members), ", ".join(_used),
                    )
                    if _ensemble_is_champion:
                        champion_fitted_obj = ensemble_refitted
                        champion_refit_on_full_data = _do_full_refit
                        champion_refit_rows = _refit_rows if _do_full_refit else champion_refit_rows
                else:
                    self._logger.warning(
                        "Ensemble-Rebuild übersprungen: nur %d fit-fähige Mitglieder (<2).", len(_members),
                    )
                    models_to_export.pop(_ensemble_name, None)
                    registry_entries = [e for e in registry_entries if e.name != _ensemble_name]
            except Exception:
                self._logger.warning("Ensemble-Rebuild fehlgeschlagen.", exc_info=True)
                models_to_export.pop(_ensemble_name, None)
                registry_entries = [e for e in registry_entries if e.name != _ensemble_name]
        ...
        if cfg.surrogate_tree.enabled and champion_fitted_obj is not None:
            try:
                ...
                # champion_fitted_obj ist für das Ensemble jetzt der self-contained,
                # refittete ShapeSafeEnsembleCate → const_marginal_effect liefert exakt
        ...
        bundler.write_metadata(paths, {
            ...
            "champion_refit_rows": champion_refit_rows, "refit_all_on_full_data": _do_full_refit,
            "selected_feature_columns": selected_feature_columns,
            "surrogate_tree_enabled": surrogate_exported,
            # Base-Learner-Wahl: bei type='both' inkl. des von Optuna pro
            # Modell/Rolle gewählten Learners (siehe _collect_base_learner_info).
            "base_learner": self._collect_base_learner_info(cfg, tuned_params_by_model),
        })
    ...
    def _run_explainability(self, cfg, X, T, Y, models, eval_summary, mlflow, report, holdout_data=None, fold_models=None):
        ...
        # Das Ensemble (ShapeSafeEnsembleCate) hat kein natives SHAP — KernelSHAP wäre langsam und
        ...
        try:
            ...
            try:
                ...
                if hasattr(model, "shap_values"):
                    # Modell unterstützt den nativen EconML-SHAP-Pfad grundsätzlich → echter Fehler (mit Traceback).
                    self._logger.info("SHAP-Plot-Satz fehlgeschlagen, Fallback auf generische SHAP-Werte.", exc_info=True)
                else:
                    # Erwartet: Modelle ohne native 'shap_values' (z.B. CausalForest-Adapter) nutzen direkt den generischen Pfad.
                    self._logger.info("Champion '%s' stellt keine native 'shap_values'-Methode bereit → generischer SHAP-Pfad.", champion_name)
    ...
    def run(self, export_bundle: bool | None = None, bundle_dir: str | None = None, bundle_id: str | None = None) -> AnalysisResult:
        ...
        _grf_tune_active = (
            ...
            and getattr(cfg.causal_forest, "tune_enabled", False)
        )
        if _grf_tune_active:
            total += 1  # grf
        ...
        if not os.environ.get("MLFLOW_TRACKING_URI"):
            ...
            # verlangt dafuer das Opt-in — bedingungslos gesetzt, s. u.).
        ...
        with mlflow.start_run(run_name=_run_name):
            ...
            # Internes Cross-Fitting Info (gilt für DML UND DRLearner)
            _dml_cv = cfg.data_processing.dml_crossfit_folds
            _mc = cfg.data_processing.mc_iters
            if _dml_cv != 2 or _mc:
                _mc_str = f", mc_iters={_mc}" if _mc else ""
                self._logger.info("Nuisance Cross-Fitting (DML + DR): %d interne Folds%s", _dml_cv, _mc_str)
            ...
            if getattr(cfg.causal_forest, "tune_enabled", False):
                _cf_trials = getattr(cfg.causal_forest, "n_trials", 50)
                _tuning_parts.append(f"CFT ({_cf_trials} Trials)")
            if getattr(cfg.models, "ensemble", False):
                _tuning_parts.append("Ensemble")
            if _tuning_parts:
                self._logger.info("Aktiv: %s", " | ".join(_tuning_parts))

            # Validierungsmodus explizit loggen
            _val_mode = str(getattr(cfg.data_processing, "validate_on", "cross")).lower()
            _has_mask = getattr(cfg.data_files, "eval_mask_file", None)
            if _val_mode == "external":
                self._logger.info("Validierungsmodus: External Eval (separate Eval-Daten)")
            elif _has_mask:
                self._logger.info("Validierungsmodus: Train Many, Evaluate Some (eval_mask)")
            else:
                self._logger.info("Validierungsmodus: Cross-Validation (%d Folds, Seed=%d, Tuning-Seed=%d)",
                    cfg.data_processing.cross_validation_splits, cfg.constants.random_seed, cfg.constants.tuning_seed)
            ...
            # ── TMES (Train Many, Evaluate Some): Mask für Post-Training-Evaluation ──
            # TMES trainiert und cross-predictet auf ALLEN Daten (maximale statistische Power).
            # Die eval_mask wird erst bei der Evaluation angewendet: Uplift-Metriken,
            # DRTester-Plots und Policy Values werden nur auf dem Mask-Subset berechnet.
            # Kein Holdout-Split — alle Daten fließen ins Training und Cross-Prediction.
            if eval_mask is not None and not use_external:
                import numpy as _np
                _mask = _np.asarray(eval_mask).astype(bool)
                if _mask.any() and not _mask.all():
                    _n_eval = int(_mask.sum())
                    self._logger.info(
                        "TMES aktiviert: Training + Cross-Prediction auf allen %d Zeilen. "
                        "Evaluation (Metriken, Plots) nur auf %d Mask-Zeilen (%.1f%%).",
                        len(_mask), _n_eval, _n_eval / len(_mask) * 100,
                    )
                    mlflow.log_param("tmes_total_n", len(_mask))
                    mlflow.log_param("tmes_eval_n", _n_eval)
                else:
                    self._logger.warning(
                        "eval_mask ist leer oder umfasst alle Zeilen — TMES wird ignoriert."
                    )
                    eval_mask = None
            ...
            if use_external and X_eval is not None:
                ...
                if cfg.data_files.eval_s_file:
                    try:
                        ...
                        S_eval = None
                # Dtype-Konsistenz: Eval-Daten an Train-Dtypes angleichen,
                # damit es bei der Evaluation keine Dtype-Mismatches gibt.
                # Übernimmt ALLE Dtypes (int8, float32, category, ...) — nicht nur category.
                _n_dtype_aligned = 0
                for c in X_eval.columns:
                    if c in X.columns and str(X_eval[c].dtype) != str(X[c].dtype):
                        try:
                            X_eval[c] = X_eval[c].astype(X[c].dtype)
                            _n_dtype_aligned += 1
                        except Exception:
                            pass
                if _n_dtype_aligned:
                    self._logger.info(
                        "Eval-Daten: %d Spalten-Dtypes an Trainings-Dtypes angeglichen.",
                        _n_dtype_aligned,
                    )
                # Fail-fast: Auch die externen Eval-Daten müssen zur konfigurierten
                # Treatment-Struktur passen (gleiche Prüfung wie für Trainingsdaten).
                self._validate_treatment_arms(T_eval, context="externe Eval-Daten (eval_t_file)")
            ...
            try:
                ate_fig = generate_ate_barplot(T, Y)
                if ate_fig is not None:
                    import matplotlib.pyplot as plt
                    from rubin.reporting.html_report import fig_to_base64
                    report.ate_barplot = fig_to_base64(ate_fig)
                    _log_figure_fast(mlflow, ate_fig, "barplot_Y_against_T.png")
                    plt.close(ate_fig)
            except Exception:
            ...
            # lgbm_importance IMMER LGBMRegressor nutzt (unabhängig vom Base-Learner)
            # und das Tuning danach den konfigurierten Base-Learner-Typ verwendet.
            ...
            with patch_categorical_features(X, base_learner_type=cfg.base_learner.type) as cat_indices:
                ...
                _log_ram(self._logger, "vor Tuning")
                ...
                _log_ram(self._logger, "nach BLT-Tuning")

                # _run_training emittiert Progress-Labels "CausalForest-Tuning" bzw.
                ...
                gc.collect()  # Cross-Prediction: Fold-Modelle, deepcopy-Reste, OOF-Arrays freigeben
                _log_ram(self._logger, "nach Training")
                ...
                if getattr(cfg.models, "ensemble", False) and len(models) >= 2:
                    try:
                        ...
                        # Index-Rahmen an den Prediction-Frames ausrichten — NICHT an len(X):
                        # im CV-Modus sind das len(X) OOF-Zeilen (identisch), im External-/
                        # Holdout-Modus aber die len(Eval) Holdout-Zeilen. Ein len(X)-Rahmen
                        # würde dort mit "Length of values (n_eval) does not match
                        # length of index (n_train)" crashen → Ensemble fehlte still.
                        ensemble_df = pd.DataFrame(index=pd.RangeIndex(len(first_df)))
                        if "Y" in first_df.columns:
                            ensemble_df["Y"] = first_df["Y"].values
                        if "T" in first_df.columns:
                            ensemble_df["T"] = first_df["T"].values

                        # ── Mitgliedschaft: Evaluation und Objekt MÜSSEN identisch sein ──
                        # Nur Modelle, die vollständige Predictions liefern, werden Mitglied:
                        # BT: Predictions_{n} vorhanden; MT: ALLE Arm-Spalten vorhanden.
                        if is_mt:
                            n_arms = int(T.max()) if hasattr(T, 'max') else int(np.max(T))
                            contributing = [
                                n for n in model_names
                                if all(f"Predictions_{n}_T{k}" in preds[n].columns for k in range(1, n_arms + 1))
                            ]
                        else:
                            contributing = [n for n in model_names if f"Predictions_{n}" in preds[n].columns]
                        _excluded = [n for n in model_names if n not in contributing]
                        if _excluded:
                            self._logger.warning(
                                "Ensemble: %s ohne (vollständige) Prediction-Spalten — von Evaluation "
                                "UND Objekt ausgeschlossen (Konsistenz evaluiert == exportiert).",
                                _excluded,
                            )
                        model_names = contributing
                        if len(model_names) < 2:
                            raise ValueError(
                                f"Ensemble übersprungen: nur {len(model_names)} Modell(e) mit "
                                f"vollständigen Predictions (mindestens 2 erforderlich)."
                            )

                        if is_mt:
                            for k in range(1, n_arms + 1):
                                arm_cols = [preds[n][f"Predictions_{n}_T{k}"] for n in model_names]
                                if arm_cols:
                            ...
                            pred_cols = [preds[n][f"Predictions_{n}"] for n in model_names]
                            if pred_cols:
                                ensemble_df[f"Predictions_{ENSEMBLE_NAME}"] = np.mean(pred_cols, axis=0)
                                ensemble_df[f"Train_{ENSEMBLE_NAME}"] = np.nan

                        # ShapeSafeEnsembleCate: Gleichgewichtung, Mitglieder-Outputs shape-normalisiert.
                        # member_names persistiert die Zusammensetzung für den Export-Rebuild.
                        model_list = [models[n] for n in model_names]
                        weights = np.ones(len(model_list)) / len(model_list)
                        ensemble_model = ShapeSafeEnsembleCate(
                        ...
                        self._logger.info(
                            "Ensemble erstellt (ShapeSafeEnsembleCate, shape-normalisiert): %d Modelle gleichgewichtet (%s).",
                ...
                if eval_summary:
                    ...
                    if _champ:
                        ...
                        _val_mode = "external" if use_external else ("TMES" if eval_mask is not None else "cross")
                ...
                gc.collect()  # Evaluation: DRTester-Nuisance, Matplotlib-Figures, Metriken-DataFrames freigeben
                _log_ram(self._logger, "nach Evaluation")
                ...
                _log_ram(self._logger, "nach Surrogate")
                ...
                gc.collect()  # SHAP-Values (n_samples × n_features) + TreeExplainer freigeben
                _log_ram(self._logger, "nach Explainability")
            # ── Ende categorical patch — originale Methoden wiederhergestellt ──

...
in allen Cross-Prediction-Folds wiederverwendet."""
    ...
    def _apply_overfit_penalty(self, val_score: float, train_score: float,
                               ...
                               max_penalized_gap: float = None) -> float:
        ...
        tolerance ist ein relativer Anteil (z.B. 0.10 = 10% Gap wird toleriert).
        ...
        if penalty <= 0:
            return val_score
        if max_penalized_gap is None:
            max_penalized_gap = getattr(self.cfg.final_model_tuning, "overfit_max_penalized_gap", 1.0)
        ...
        # Cap: saturiert den Abzug, damit ein bei kleinem |val_score| explodierender
        # relativer Gap das Vorzeichen nicht kippt und die Selektion nicht dominiert.
    ...
    def _tuning_n_jobs(self) -> int:
        ...
        # _parallel_jobs_per_fit (≈ 8 Kerne) — und DAS betrifft nur die
        # NUISANCE-Builder (einmaliger Cache-Aufbau). Das model_final der
        # Trials wird explizit mit parallel_jobs=-1 gebaut: jeder
        # refit_final()-Fit nutzt ALLE Kerne (siehe Trial-Objective).
    ...
    def _create_study(self, study_key: str, use_pruner: bool = True):
        ...
        # Direkte Konstruktion ohne Versions-Fallback: optuna>=3.5 ist gepinnt
        # (pyproject/pixi), alle Parameter existieren dort. Eine try/except-
        # Leiter würde echte TypeErrors (z.B. Parameter-Tippfehler) verschlucken.
    ...
    @staticmethod
    def _fmt_params_fingerprint(*dicts) -> str:
        """Deterministischer Fingerprint einer Param-Konfiguration für den Scorer-Cache-Key."""
        import json
        return json.dumps(
    ...
    def _run_fmt_study(self, *, model_name, base_type, role, cached_folds,
                       X_tune, Y_tune, T_tune, base_fixed_params, fmt_fixed_params,
                       outcome_params, propensity_params, is_rct, op, ot, sf, outer_splits):
        """Gemeinsame FMT-Study-Logik für NonParamDML und DRLearner.

        Wählt den Scorer (Qini/RScore), definiert das per-Trial-Objective
        (`refit_final()` auf den einmalig gecachten Nuisances), optimiert und
        extrahiert die besten `model_final`-Parameter. Beide Modelle teilen
        exakt diese Logik; sie unterscheiden sich nur (a) im Bau von
        `cached_folds` und (b) in der Herkunft der Nuisance-Params, die hier
        bereits als `outcome_params`/`propensity_params` aufgelöst hereinkommen.
        """
        ...
        if _scorer_type in ("qini", "qini_argmax"):
            ...
            # Cache-Key inkl. Nuisance-Fingerprint: verhindert, dass ein zweites
            # FMT-Modell (z.B. DRLearner nach NonParamDML) Scorer wiederverwendet,
            # die mit ANDEREN Nuisance-Params gebaut wurden. Identische Configs
            # teilen sich weiterhin den Cache (Speed-Vorteil bleibt erhalten).
            _cache_key = (
                outer_splits, self.tuning_cv_seed, sf, len(Y_tune), op > 0,
                base_type, bool(is_rct),
                self._fmt_params_fingerprint(outcome_params, propensity_params, base_fixed_params),
            )
            if self._scorer_cache_key == _cache_key and self._scorer_cache_val:
                ...
                _fold_scorers = []
                _fold_train_scorers = []
                for _est_b, _tr_b, _va_b in cached_folds:
                    _fold_scorers.append(_create_fold_scorer(
                        base_type, dict(base_fixed_params), outcome_params, propensity_params,
                        Y_tune[_va_b], T_tune[_va_b], X_tune.iloc[_va_b],
                        seed=self.seed, is_rct=is_rct, build_base_learner_fn=build_base_learner,
                    ))
                    if op > 0:
                        _fold_train_scorers.append(_create_fold_scorer(
                            base_type, dict(base_fixed_params), outcome_params, propensity_params,
            ...
            def objective(trial):
                ...
                for fold_i, (est, tr, va) in enumerate(cached_folds):
                    est.model_final = build_base_learner(base_type, {**fmt_fixed_params, **cand_params}, seed=self.seed, task="regressor", parallel_jobs=-1)
                    est.refit_final()
                    fold_scores_raw.append(float(_fold_scorers[fold_i].score(est)))
                    if op > 0:
                ...
                if op > 0 and fold_train_scores:
                    return self._apply_overfit_penalty(mean_raw, float(np.mean(fold_train_scores)), penalty=op, tolerance=ot)
                return mean_raw
        ...
        study.optimize(objective, n_trials=_fmt_nt, timeout=_fmt_timeout, n_jobs=1, catch=(Exception,))
        ...
        try:
            ...
            if op > 0 and raw != study.best_value:
        ...
        try:
            ...
            result = {}
        # Gecachte Estimatoren + Study freigeben (Scorers bleiben in self._scorer_cache_*)
        cached_folds.clear()
        del study
        gc.collect()
    ...
    def tune_final_model(
        ...
        # Platzhalter-Params für model_final beim Nuisance-Caching (est.fit unten).
        # Der Platzhalter wird pro Trial via refit_final() durch das eigentliche
        # Kandidaten-Modell ersetzt — sein Learner-Typ ist für das Ergebnis
        # irrelevant. Im "both"-Modus wird _learner_type hier explizit gesetzt,
        # damit build_base_learner() nicht pro äußerem Fold die (in diesem
        # Kontext irreführende) Warnung „kein _learner_type — Fallback auf
        # catboost" loggt. 'catboost' entspricht dem dokumentierten Fallback.
        _placeholder_final_params = dict(fmt_fixed_params)
        if base_type == "both":
            _placeholder_final_params.setdefault("_learner_type", "catboost")

        if name == "nonparamdml":
            from econml.dml import NonParamDML

            _op = float(getattr(self.cfg.final_model_tuning, "overfit_penalty", 0.0))
            _ot = float(getattr(self.cfg.final_model_tuning, "overfit_tolerance", 0.10))
            ...
            for fold_i, (tr, va) in enumerate(split_iter):
                ...
                est = NonParamDML(
                    ...
                    model_final=build_base_learner(base_type, dict(_placeholder_final_params), seed=self.seed, task="regressor", parallel_jobs=-1),
            ...
            return self._run_fmt_study(
                model_name=model_name, base_type=base_type, role=role,
                cached_folds=_cached_folds,
                X_tune=X_tune, Y_tune=Y_tune, T_tune=T_tune,
                base_fixed_params=base_fixed_params, fmt_fixed_params=fmt_fixed_params,
                outcome_params=tuned_roles.get("model_y", tuned_roles.get("model_regression", {})),
                propensity_params=tuned_roles.get("model_t", tuned_roles.get("model_propensity", {})),
                is_rct=_fmt_is_rct, op=_op, ot=_ot, sf=_sf, outer_splits=_outer_splits,
            )
            ...
            return self._run_fmt_study(
                ...
                X_tune=X_tune, Y_tune=Y_tune, T_tune=T_tune,
                base_fixed_params=base_fixed_params, fmt_fixed_params=fmt_fixed_params,
                outcome_params=tuned_roles.get("model_regression", tuned_roles.get("model_y", {})),
                propensity_params=tuned_roles.get("model_propensity", tuned_roles.get("model_t", {})),
                is_rct=_fmt_is_rct, op=_op, ot=_ot, sf=_sf, outer_splits=_outer_splits,
            )

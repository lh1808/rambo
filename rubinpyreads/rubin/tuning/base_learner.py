...
class BaseLearnerTuner:
    ...
    def _downsample_indices(self, model_name: str, T: np.ndarray, n: int) -> np.ndarray:
        ...
        Alle Modelle nutzen 100% der Daten — die Methode existiert als
        Erweiterungspunkt für modellspezifische Subsample-Anteile."""
    ...
    def _objective_all_classification(self, params: Dict[str, Any], X_mat: np.ndarray, target: np.ndarray, strata: np.ndarray, train_ratio: float = 1.0, trial=None, allow_penalty: bool = True) -> float:
        val_scores: List[float] = []
        train_scores: List[float] = []
        is_multiclass = len(np.unique(target)) > 2
        _op_active = allow_penalty and getattr(self.cfg.tuning, "overfit_penalty", 0.0) > 0
    ...
    def _penalty_applies(self, task: "TuningTask") -> bool:
        """Overfit-Penalty gilt im BLT ausschließlich für Meta-Learner-Tasks.

        DML/DR-Nuisances laufen innerhalb des internen Cross-Fittings
        (``sample_scope == "all"``) und gehen in einen Neyman-orthogonalen Score
        ein — Cross-Fitting + Orthogonalität puffern Nuisance-Overfitting bereits
        ab (Chernozhukov et al. 2018, Bach et al. 2024). Sie werden daher NIE
        bestraft, unabhängig von der Konfiguration (``overfit_penalty`` wird für
        sie ignoriert).

        Meta-Learner-Modelle (S-/T-/X-Learner; ``sample_scope`` ist
        ``"all_direct"`` oder ``"group_specific_shared_params"``) werden direkt
        trainiert und bilden den CATE ohne diesen Puffer. Für sie ist die
        Overfit-Penalty der primäre Overfitting-Hebel beim Tuning und bleibt
        aktivierbar.
        """
        return task.sample_scope != "all"

    def _apply_overfit_penalty(self, val_score: float, train_score: float,
                               penalty: float = None, tolerance: float = None,
        ...
        tolerance ist ein relativer Anteil (z.B. 0.20 = 20% Gap wird toleriert).
        max_penalized_gap deckelt den bestraften Exzess-Gap (Saturierung): Bei
        kleinen |val_score| würde der relative Gap sonst explodieren und der Abzug
        das Vorzeichen kippen / die Selektion dominieren. <=0 deaktiviert den Cap.
        """
        if penalty is None:
            penalty = getattr(self.cfg.tuning, "overfit_penalty", 0.0)
        if tolerance is None:
            tolerance = getattr(self.cfg.tuning, "overfit_tolerance", 0.20)
        if max_penalized_gap is None:
            max_penalized_gap = getattr(self.cfg.tuning, "overfit_max_penalized_gap", 1.0)
        if penalty <= 0:
            return val_score
        gap = train_score - val_score  # positiv = Overfitting
        # Epsilon-Floor: Verhindert Division-by-Zero bei val_score ≈ 0.
        # Kein metrischer Floor — die Penalty ist damit
    ...
    def _objective_all_regression(self, params: Dict[str, Any], X_mat: np.ndarray, target: np.ndarray, strata: np.ndarray, train_ratio: float = 1.0, trial=None, allow_penalty: bool = True) -> float:
    ...
    def _objective_grouped_regression(self, params: Dict[str, Any], X_mat: np.ndarray, Y: np.ndarray, T: np.ndarray, trial=None, allow_penalty: bool = True) -> float:
    ...
    def _objective_xlearner_cate(self, params: Dict[str, Any], X_mat: np.ndarray, Y: np.ndarray, T: np.ndarray, pseudo: np.ndarray, trial=None, allow_penalty: bool = True) -> float:
    ...
    def _tune_task(self, task: TuningTask, X: pd.DataFrame, Y: np.ndarray, T: np.ndarray, shared_params: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # der cate_models. Ein Neuaufbau pro Trial wäre redundant (n_trials×
        # 2K identische Nuisance-Fits); die Vorberechnung liefert dasselbe
        ...
        def objective(trial):
            ...
            # Overfit-Penalty nur für Meta-Learner-Tasks; DML/DR-Nuisances (scope "all")
            # werden nie bestraft (siehe _penalty_applies).
            _allow_pen = self._penalty_applies(task)
            ...
            if task.objective_family == "outcome_regression":
                return self._objective_all_regression(params, X_mat=X_mat, target=target.astype(float), strata=_strata, train_ratio=_train_ratio, trial=trial, allow_penalty=_allow_pen)
            if task.objective_family == "grouped_outcome_regression":
                return self._objective_grouped_regression(params, X_mat=X_mat, Y=target.astype(float), T=T_task.astype(int), trial=trial, allow_penalty=_allow_pen)
    ...
    def tune_all(self, model_names: List[str], X: pd.DataFrame, Y: np.ndarray, T: np.ndarray) -> Dict[str, Dict[str, Dict[str, Any]]]:
        ...
        if any((m or "").lower() == "causalforestdml" for m in active_models):
            logging.getLogger("rubin.tuning").debug(
                ...
                "Forest-Parameter werden separat im CFT (CausalForest-Tuning) via Optuna TPE optimiert."
            )

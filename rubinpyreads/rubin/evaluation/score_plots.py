...
from rubin.utils.plot_theme import (
    apply_rubin_theme, COLOR_MODEL, COLOR_REFERENCE, COLOR_BASELINE, COLOR_DIFFERENCE,
)
apply_rubin_theme()
...
def plot_score_redistribution(
    ...
    # ── Dezil-Zuordnung: RANGBASIERT (equal-frequency) ──
    # Position in der Sortierung statt Wert-Perzentile → jedes Dezil enthält
    # exakt ~N/n Samples, auch bei Bindungen (diskrete/gerundete/geclumpte
    # Scores, viele Nullen). Konsistent zum oben verwendeten Spearman-ρ und zur
    # Diagonal-Interpretation des Plots. Bindungen werden per stabiler Sortierung
    # (Array-Position) gleichmäßig auf benachbarte Dezile aufgeteilt; wertbasierte
    # Perzentil-Kanten würden alle gleichen Werte in ein Dezil werfen und so
    # einzelne Dezile überladen, andere leeren.

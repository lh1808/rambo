...
if TYPE_CHECKING:
    from rubin.utils.schema_utils import Schema, SchemaValidationResult
...
Dieses Artefakt ist zentral für reproduzierbares Scoring in Production."""
    ...
    num_cols = X.select_dtypes(exclude=["category"] + object_like_selectors()).columns.tolist()
...
def restrict_fitted_preprocessor(
    fp: FittedPreprocessor, keep_columns: Sequence[str]
) -> FittedPreprocessor:
    """Schränkt einen ``FittedPreprocessor`` auf eine Feature-Teilmenge ein.

    Nach der Feature-Selektion nutzt das Modell nur noch eine Teilmenge der
    ursprünglichen DataPrep-Features. Dieser Helper erzeugt einen
    FittedPreprocessor, der genau diese Spalten erwartet, aber die gelernten
    Encoding-Maps, Fill-Werte und Dtypes der behaltenen Spalten beibehält.
    Damit kann das Bundle Rohdaten korrekt enkodieren (string→int via Maps,
    NA-Fill) UND in einem Schritt auf die Modell-Features reduzieren —
    ``transform`` ergänzt fehlende Spalten als NaN und subsetted auf
    ``feature_columns``.

    Spalten in ``keep_columns``, die dem Preprocessor unbekannt sind, werden
    übersprungen (der Aufrufer sollte diesen Fall vorab prüfen/warnen).
    """
    known = set(fp.feature_columns)
    keep = [c for c in keep_columns if c in known]
    keep_set = set(keep)
    return FittedPreprocessor(
        feature_columns=keep,
        categorical_columns=[c for c in fp.categorical_columns if c in keep_set],
        encoding_maps={c: m for c, m in fp.encoding_maps.items() if c in keep_set},
        fillna_values={c: v for c, v in fp.fillna_values.items() if c in keep_set},
        dtypes_after={c: d for c, d in fp.dtypes_after.items() if c in keep_set},
    )
...
nachvollziehbar."""
    ...
    categorical_columns = X.select_dtypes(include=["category"] + object_like_selectors()).columns.tolist()

...
Modellierung. CatBoost crasht sogar, weil es float-Werte bei cat_features ablehnt.

Lösung
------
Context-Manager ``patch_categorical_features`` patcht die relevanten Methoden:

- **LightGBM**: Nur ``fit()`` — ``categorical_feature`` wird injiziert.
  predict() akzeptiert float-Kategorien nativ (rundet zu int).
- **CatBoost**: ``fit()``, ``predict()``, ``predict_proba()``, ``score()`` —
  ``cat_features`` wird injiziert und X von numpy-float zu DataFrame mit
  int32-Kategorien konvertiert. ``score()`` muss separat gepatcht werden,
  da CatBoost intern ``_predict()`` (privat) aufruft und damit den
  öffentlichen ``predict()``-Wrapper umgeht.
...
from rubin.utils.data_utils import object_like_selectors
...
def _detect_cat_indices(X: pd.DataFrame) -> List[int]:
    ...
    cat_cols = X.select_dtypes(include=["category"] + object_like_selectors()).columns
...
def patch_categorical_features(X: pd.DataFrame, base_learner_type: str = "lgbm"):
    ...
    Spaltenindizes und patcht die Methoden der Base Learner:

    - **LightGBM**: Nur ``fit()`` — ``categorical_feature`` wird injiziert.
      predict() braucht kein Patching (LGBM akzeptiert float-Kategorien nativ).
    - **CatBoost**: ``fit()``, ``predict()``, ``predict_proba()``, ``score()`` —
      ``cat_features`` wird injiziert und X von numpy-float zu DataFrame mit
      int32-Kategorien konvertiert. ``score()`` muss separat gepatcht werden,
      da CatBoost intern ``_predict()`` (privat) aufruft und damit den
    ...
    Nach Verlassen des Kontexts werden alle originalen Methoden
    ...
    base_learner_type : str
        "lgbm", "catboost" oder "both". Bestimmt, welche Klassen gepatcht werden.
    """
    ...
    _logger.info(
        ...
        "Patche %s Methoden (fit/predict/predict_proba/score) für EconML-Kompatibilität.",
    ...
    try:
        if patch_lgbm:
            ...
            # Originale sichern — aus __dict__, NICHT via Attributzugriff:
            # partialmethod ist ein Descriptor; Klassenattribut-Zugriff liefert
            # unter bereits aktivem Patch (verschachtelte Kontexte) ein
            # aufgelöstes partial-Objekt ohne Descriptor-Protokoll. Als
            # "Original" gesichert und restauriert, würde model.fit(X, y)
            # das self-Binding verlieren.
            originals["LGBMClassifier.fit"] = lgbm.LGBMClassifier.__dict__["fit"]
            originals["LGBMRegressor.fit"] = lgbm.LGBMRegressor.__dict__["fit"]

            # Patchen: categorical_feature wird bei JEDEM .fit()-Aufruf übergeben,
            # auch wenn EconML intern nur model.fit(X_numpy, y) aufruft.
            # Basis ist das gesicherte Raw-Objekt (partialmethod komponiert
            # auch über einem äußeren partialmethod korrekt — nesting-sicher).
            lgbm.LGBMClassifier.fit = partialmethod(
                originals["LGBMClassifier.fit"], categorical_feature=cat_indices
            )
            lgbm.LGBMRegressor.fit = partialmethod(
                originals["LGBMRegressor.fit"], categorical_feature=cat_indices
            )
        ...
        if patch_catboost:
            ...
            originals["CatBoostClassifier.score"] = CatBoostClassifier.score
            originals["CatBoostRegressor.score"] = CatBoostRegressor.score
            ...
            def _convert_float_to_df(X, ci, np_mod):
                """Konvertiert numpy float-Array → pandas DataFrame mit int-Kategorien.

                CatBoost akzeptiert DataFrames mit gemischten Typen und cat_features-Indices.
                Optimierung: Kein float32-Cast (vermeidet Kopie des gesamten Arrays).
                Nur kategorische Spalten werden von float→int32 konvertiert.

                Hinweis: pandas.Categorical wurde evaluiert, aber verworfen —
                CatBoost GitHub Issues #814 und #757 zeigen, dass category-dtype
                schlechtere Ergebnisse und langsamere Pool-Erstellung liefern kann.
                int32 ist der sicherste und schnellste Weg.
                """
                if not ci or not isinstance(X, np_mod.ndarray) or X.dtype.kind != "f":
                    return X
                import pandas as _pd
                # Zero-copy DataFrame aus numpy (kein astype, keine Kopie)
                df = _pd.DataFrame(X, copy=False)
                # Kategorische Spalten: float → int32
                for idx in ci:
                    if idx < df.shape[1]:
                        df[df.columns[idx]] = np_mod.nan_to_num(X[:, idx], nan=-1).astype(np_mod.int32)
                return df
            ...
            def _make_score_wrapper(original_score):
                """Wrapper für CatBoost .score() — ruft intern self._predict() auf,
                das die öffentliche predict()-Methode UMGEHT. Deshalb muss score()
                separat gepatcht werden, um X zu konvertieren."""
                def _wrapped_score(self, X, y=None, *args, **kwargs):
                    X_conv = _convert_float_to_df(X, cat_indices, _np)
                    return original_score(self, X_conv, y, *args, **kwargs)
                return _wrapped_score
            ...
            CatBoostClassifier.score = _make_score_wrapper(originals["CatBoostClassifier.score"])
            CatBoostRegressor.score = _make_score_wrapper(originals["CatBoostRegressor.score"])
        ...
        if "CatBoostClassifier.fit" in originals:
            ...
            CatBoostClassifier.score = originals["CatBoostClassifier.score"]
            CatBoostRegressor.score = originals["CatBoostRegressor.score"]
            _logger.debug("CatBoost .fit()/.predict()/.predict_proba()/.score() wiederhergestellt.")

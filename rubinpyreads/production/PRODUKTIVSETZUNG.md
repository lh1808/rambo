...
| `GIT_REF` | `master` | **Für Prod pinnen** — annotiertes Tag empfohlen (Shallow-Clone, schnell); Commit-SHAs funktionieren über den automatischen Fallback (voller Clone + Checkout, langsamer). Pinnt Code **und** `pixi.lock` als Einheit, `--frozen` installiert exakt diesen Stand |
...
| `job_<uc>.conf: line N: <pfad>: No such file or directory` (Exit 127, kein Start-Log) | Restinhalt hinter einer Zuweisung in der Job-Datei (z. B. Anführungszeichen beim Kürzen von `SCORING_CONFIGS` an falscher Stelle geschlossen — bash führt den Rest als Kommando aus). Aktuelle Skript-Stände fangen das bereits im Validierungs-Guard mit klarer Meldung ab | Betroffene Zeile auf exakt `KEY="wert"` bringen; nichts hinter dem schließenden Anführungszeichen außer optionalem `# Kommentar` |
| `could not convert string to float: '…'` bzw. CatBoost `Bad value … Cannot convert '…' to float` (mitten im Scoren) | Der Gesamtbestand enthält Sonderwerte (z. B. SAS-`'V'`) in Spalten, die im Trainings-Ausschnitt rein numerisch waren — die Spalte kommt als Text an | Seit diesem Stand automatisch behandelt (`coerce_numeric_strings`, Default an): Nicht-Parsebares → NaN → **gelernte** Imputation der Pipeline; Raten pro Spalte als Warnung im Log und unter `preprocessing.numeric_coerce_rate_per_feature` im Monitoring — hohe Raten fachlich prüfen (Drift-Signal). Frühere Handarbeit pro Use-Case (fixe Ersatzwerte, `to_numeric`+`fillna` im Vorskript) entfällt |
| CatBoost `'data' is numpy array of floating point numerical type … but 'cat_features' …` (mitten im Scoren, Metalearner-Bundles) | econml konvertiert X intern zu float-Arrays; die CatBoost-Basismodelle des Bundles erwarten aber ihre kategorischen Spalten. In der Analyse fängt das der categorical_patch ab — ältere Produktions-Stände installierten ihn nicht | Seit diesem Stand automatisch: `score_dataframe` wickelt alle Predicts in denselben Patch-Kontext wie die Analyse (No-Op bei rein numerischen Bundles). Repo-Stand des Jobs aktualisieren (`GIT_REF`) |
| `ml_package_versions fehlt in metadata.json (…)` | Das Bundle wurde mit einem rubin-Stand **vor** Einführung des Versions-Stempels exportiert (ältere Bundles, z. B. aus dem Frühjahr) | Sauberster Weg: Bundle mit aktuellem Stand neu exportieren. Soll exakt das alte Modell produktiv gehen: Bundle an die Prod-Ablage **kopieren** und in der Kopie `ml_package_versions` nachtragen (Versionen aus dem damaligen `pixi.lock` via `git show <commit>:pixi.lock` bzw. dem mlflow-Run); bei Abweichungen zur Laufzeit warnt der Check nur, er bricht nicht ab |

...
Vor dem Training können Features automatisch selektiert werden. Drei Methoden stehen zur Verfügung und werden per Union kombiniert:

- **catboost_importance** (Default): CatBoost Gain-Importance (PredictionValuesChange). Vorteil: native kategorische Feature-Unterstützung, Ordered Boosting für stabilere Rankings.
- **lgbm_importance**: LightGBM Gain-Importance pro Feature.
- **causal_forest**: GRF CausalForest Feature-Importances — misst kausale Relevanz (Treatment-Effekt-Heterogenität).

Pro Methode werden die Top-N% Features behalten (default 15%), anschließend per Union aller Methoden zusammengeführt. Korrelationsfilter entfernt redundante Features.
...
Wenn das Ensemble Champion wird, kann die SHAP-Analyse nicht direkt auf dem Ensemble-Objekt (`ShapeSafeEnsembleCate`) durchgeführt werden (kein natives SHAP, KernelSHAP wäre langsam und verrauscht). Stattdessen wird automatisch das **beste Einzelmodell** im Ensemble für die Explainability herangezogen — das Modell mit dem höchsten Score auf der Selektionsmetrik. Der HTML-Report zeigt an, welches Modell verwendet wurde.
...
- **CV-Modus:** Nutzt das letzte CV-Fold-Modell (bereits auf K-1 Folds trainiert, kein zusätzliches Training nötig) und sampelt aus dessen Out-of-Fold-Indices. Alle CV-Splits verwenden T×Y-Stratifizierung (Treatment × Outcome) für balancierte Folds.

...
data_files:
  ...
  # Optional: Boolean-Maske für "Train Many, Evaluate Some".
...
- `eval_mask_file` (optional): Boolean-Maske (.npy) für „Train Many, Evaluate Some". Training und Cross-Prediction laufen auf allen Daten. Evaluation (Metriken, Plots) wird nur auf den markierten Zeilen berechnet. Wird automatisch von DataPrep erzeugt, wenn `eval_file_index` gesetzt ist. MLflow loggt `tmes_total_n` und `tmes_eval_n`.
...
data_prep:
  ...
  # Mapping-Keys MÜSSEN Strings sein (YAML-Zahlen als Key werden von der
  # Config-Validierung abgelehnt). Numerische Spalten sind trotzdem kein
  # Problem: rubin matcht die Keys typ-robust — "2" greift auch auf einer
  # float64-/int64-Spalte mit 2.0 bzw. 2. Greift ein konfiguriertes Mapping
  # nicht (Rohwert bleibt nach dem Replace erhalten), bricht DataPrep sofort
  # mit einer klaren Fehlermeldung ab statt still weiterzulaufen.
  ...
  balance_treatments: false               # Treatment-Raten angleichen (Auswahl nach effektiver Stichprobe N·p·(1−p))
  balance_min_arm_abs: 0                   # optional: absolute Mindestanzahl je Arm (0 = aus)
  balance_min_arm_frac: 0.0               # optional: Mindestanteil je Arm rel. zum kleineren Ausgangs-Arm (0 = aus)
  balance_max_loss_frac: 1.0              # optional: max. erlaubter Zeilenverlust-Anteil (1.0 = aus)
  balance_target_grid_step: 0.0           # optional: Gitter-Schrittweite für Zielraten Richtung 0.5 (0 = nur min/max)
...
- `parallel_level`: steuert, wie aggressiv parallelisiert wird (Default: 3). **Reproduzierbarkeit:** Bei Level 3/4 laufen Optuna-Trials und Base-Learner-Fits parallel; weil parallele Trials timing-abhängig zurückmelden und multi-threaded LightGBM/CatBoost eine thread-zahl-abhängige Floating-Point-Reihenfolge haben, können Läufe mit gleicher Config und gleichen Daten leicht unterschiedliche Ergebnisse liefern — abhängig von der Anzahl der dem Prozess zugeteilten CPU-Kerne. Für vollständig reproduzierbare Läufe `parallel_level: 1` setzen (langsamer). Details: `docs/architektur.md → CPU-Parallelisierung → Reproduzierbarkeit`.
...
Bei Level 3–4 werden die CPU-Kerne proportional aufgeteilt — sowohl für parallele Optuna-Trials im Tuning als auch für parallele CV-Folds im Training. Die parallelen Trials sind auf max. 8 (reines LightGBM) bzw. 6 (CatBoost sowie „CatBoost & LGBM") gecappt, damit der TPE-Sampler genug sequentielle Wellen zum Lernen hat; die Tuning-Seite der Web-UI rechnet mit denselben Caps (Wellen-Presets und Anzeige zeigen die gecappte Parallelität, z. B. „250 Kerne / 4 = 62, gecappt auf 8"). Die **Wellen-Presets skalieren zusätzlich moderat mit der Maschinengröße** (Kern-Boost ×1–×4 über √(Kerne/16), gedeckelt auf 32 Wellen; Basis-Wellen Schnell 3 / Standard 5 / Gründlich 12): Mehr Kerne machen jede Welle schneller, also setzen die Presets dort mehr Wellen — z. B. Level 3 mit 250 Kernen: „Standard" = 20 Wellen = 80 Trials, „Gründlich" = 32 Wellen = 128 Trials; auf 16 Kernen bleiben die Basiswerte. Die Parallelität selbst bleibt dabei immer gecappt — mehr Kerne bedeuten mehr Wellen und schnellere Fits, nie mehr gleichzeitige Trials. Übrige Kerne gehen an die einzelnen Fits. Level 3 ist auf max. 4 parallele Trials begrenzt. Die Feature-Selektion läuft immer sequentiell, nutzt aber ab Level 2 alle Kerne pro Methode (`n_jobs=-1`). Bei großen Datensätzen (>100k Zeilen) subsampelt CausalForest automatisch stratifiziert.

**Evaluation:** Schnelle Metriken (Qini, AUUC, Policy Value) und CATE-Verteilungs-Histogramme werden immer für alle Modelle berechnet — unabhängig vom Level. Die DRTester-Diagnostik-Plots (Calibration, Qini/TOC mit Bootstrap-CIs) werden bei Level 3 nur für Champion + besten Challenger erzeugt, bei Level 4 nur für den Champion. Uplift-Plots (Qini-Kurve, Uplift-by-Percentile, Treatment-Balance) laufen ebenfalls immer für alle Modelle. Die DRTester-Nuisance-Modelle nutzen leichtere Varianten (n_estimators≤100, cv=5) für schnelleres Fitting.

**Trade-offs bei Level 3–4:**

*Tuning — Speed vs. Hyperparameter-Qualität:*  
Optuna (TPE) lernt aus vorherigen Trial-Ergebnissen, welche Hyperparameter-Regionen vielversprechend sind. Bei parallelen Trials fehlen diese Ergebnisse teilweise. Beispiel mit 50 Trials auf 64 Kernen: Level 2 läuft komplett sequentiell (jeder Trial voll informiert), Level 3 hat ~12 Wellen (je 4 parallele Trials), Level 4 hat ~6 Wellen (je 8 Trials bei LightGBM, ~8 Wellen je 6 bei CatBoost). Auf noch größeren Maschinen (z. B. 250 Kerne) greifen dieselben Caps — mehr Kerne erhöhen die Kerne pro Fit, nie die parallelen Trials. Optuna kompensiert mit der „Constant Liar"-Strategie, aber die Exploration ist bei Level 4 weniger gezielt. Die n_startup_trials (zufällige Trials vor TPE-Exploration) skalieren mit der Anzahl paralleler Jobs: `max(parallel_jobs, min(10, n_trials//5))`. Dadurch hat TPE nach der ersten Welle genug Datenpunkte zum Lernen.
...
- `validate_on`:
  - `"cross"`: Cross‑Predictions (robust, Standard). Kombinierbar mit einer Eval-Maske (`data_files.eval_mask_file`) für TMES — in diesem Fall wird die Maske vor dem Training angewendet, Mask-Rows werden als Holdout zurückgehalten, und die Pipeline läuft wie External Eval (kein äußeres CV, Training nur auf ~mask-Rows).
...
- `type`: Steuert, ob die Pipeline für binäres Treatment (T in {0,1}) oder Multi-Treatment (T in {0,1,...,K-1}) laeuft.
  Bei `"multi"` werden SLearner, TLearner, XLearner, CausalForest und NonParamDML automatisch blockiert, da diese nur Binary Treatment unterstützen (NonParamDML: econml beschränkt das nichtparametrische Final-Model auf Binary Treatment).
...
feature_selection:
  ...
  max_features: 77                                # Exakte Ziel-Anzahl Features nach Selektion
...
- `methods`: Liste der Importance-Methoden. Mehrere können kombiniert werden – die Ergebnisse werden per Union zusammengeführt.
  - `"catboost_importance"`: CatBoost-Regressor auf Outcome (Y), Gain-Importance. Nutzt native kategorische Splits und Ordered Boosting — stabilere Importance als LightGBM bei vielen kategorialen Features.
...
- `max_features`: **Exakte Ziel-Anzahl** Features nach Selektion (Default: 77). Bei mehreren Methoden liefert zunächst jede Methode ihre Top-`ceil(max_features / n_methods)` Features (garantierte Repräsentation), die per Union zusammengeführt werden. Anschließend wird das Ergebnis auf exakt `max_features` gebracht: ist die Union **größer**, wird per Konsens-Rang (Summe der Importance-Positionen über alle Methoden) gekappt; ist sie **kleiner** (weil sich die Methoden überschneiden), wird mit den global bestbewerteten verbleibenden Features aufgefüllt. Es kommen also genau `max_features` Features heraus — Ausnahme: der Feature-Pool nach dem Korrelationsfilter ist kleiner als `max_features`, dann werden alle verbleibenden behalten.
...
4. Auf das exakte Feature-Budget bringen: Top-N pro Methode per Union, dann per Konsens-Rang kappen (bei Überschreitung) bzw. mit den global bestbewerteten Features auffüllen (bei Unterschreitung durch Überschneidungen)
...
Reiner Causal Forest (`econml.grf.CausalForest`) ohne DML-Residualisierung. Schätzt den Treatment-Effekt direkt mit Honest Estimation. Keine Nuisance-Modelle nötig, kein Base-Learner-Tuning. Nur Binary Treatment, keine NaN-Toleranz. Optionale Forest-Parameter-Optimierung über CausalForest-Tuning (CFT) per Optuna (`causal_forest.tune_enabled: true`).
...
Der `CausalForestAdapter` erbt von `BaseCateEstimator` — CausalForest nimmt damit vollwertig am Ensemble (`ShapeSafeEnsembleCate`) teil.

**Hinweis zu `ensemble`:**  
Nutzt rubins `ShapeSafeEnsembleCate` mit gleichgewichteten (`1/N`) Vorhersagen aller trainierten Modelle (rubin-eigene Alternative zu EconMLs `EnsembleCateEstimator`; Mitglieder-Outputs werden shape-normalisiert, bevor gemittelt wird). Alle Modelle liefern Out-of-Fold Cross-Predictions aus externer K-Fold CV. Das Ensemble nimmt an der Champion-Selektion teil. Beim Bundle-Export werden alle Modelle auf vollen Daten refittet und das Ensemble self-contained aus den refitteten Mitgliedern neu aufgebaut — unabhängig davon, ob es Champion ist.
...
der GRF-Forest nutzt X als Split-Features und kann keine NaN-Werte verarbeiten.
...
**Kategorische Features:** EconML konvertiert X intern zu numpy-Arrays, wodurch pandas `category`-Dtypes verloren gehen. rubin löst dieses Problem automatisch: Vor dem Training werden die `fit()`, `predict()`, `predict_proba()` und `score()`-Methoden von CatBoost so gepatcht, dass `cat_features` bei jedem internen Aufruf mitgegeben und X von numpy-float zu einem DataFrame mit int32-Kategorien konvertiert wird. Für LightGBM wird nur `fit()` gepatcht (`categorical_feature`), da LightGBM float-Kategorien bei `predict()` nativ akzeptiert. Zusätzlich setzt rubin `max_ctr_complexity=1` (keine automatischen Feature-Kombinationen — bei vielen kat. Features extrem langsam) und `one_hot_max_size=2` (CatBoost-Default). Ordered Boosting und Ordered Target Statistics bleiben vollständig erhalten.

**Externe Cross-Validation und internes Cross-Fitting:**

Alle Modelle — Meta-Learner (SLearner, TLearner, XLearner), DML-Familie (NonParamDML, ParamDML, CausalForestDML), DRLearner und CausalForest — durchlaufen im Cross-Modus (`validate_on: cross`) externe K-Fold Cross-Validation. So erhält jede Beobachtung eine CATE-Prediction aus einem Modell, das sie nicht gesehen hat (echte Out-of-Fold-Garantie). Im External-Modus (`validate_on: external`) entfällt die externe CV: Single-Fit auf den Trainingsdaten, Evaluation auf den separaten Eval-Dateien — die Out-of-Sample-Garantie kommt dort aus der Datentrennung selbst.
...
causal_forest:
  ...
  n_trials: 50
  single_fold: false
  scorer: auto            # auto → qini bei RCT, rscore bei observational; bei Multi-Treatment immer rscore. Weitere Option: qini_argmax (MT-Ranking)
...
2) **Forest‑Parameter** der finalen Forest‑Stufe – diese werden über `causal_forest` gesteuert.

Felder:

- `forest_fixed_params`: Feste Parameter, die immer an die Forest‑Stufe übergeben werden
  (z. B. `honest`, `n_jobs`, `min_samples_leaf`).
- `tune_enabled`: Wenn `true`, wird ein **Optuna-basiertes CausalForest-Tuning (CFT)** über
  4 kausale Parameter (max_depth, min_weight_fraction_leaf, min_var_fraction_leaf, criterion) durchgeführt. Alle anderen Forest-Parameter werden auf EconML-Defaults fixiert.
...
- `scorer`: Scoring-Modus für CFT. `auto` (Default) → `qini` bei RCT, `rscore` bei Beobachtungsdaten; bei Multi-Treatment immer `rscore` (das binäre `qini` wird von der Config-Validierung abgelehnt). Zusätzlich `qini_argmax` (Opt-in): Multi-Treatment-Ranking über alle Arme — jede Person wird nach ihrem besten geschätzten Effekt s(x)=max_k tau_k(x) gerankt, bewertet wird die Politik „gib jedem seinen empfohlenen Arm“ (Controls + passend behandelte Personen als Evidenz, Horvitz-Thompson-gewichtet über die Arm-Priors). Geeignet für Personen-Priorisierungs-Anwendungen; verrauschteres Signal als rscore, da nur ein Teil der behandelten Zeilen als Evidenz zählt. Bei K=2 numerisch exakt identisch zu `qini`. **Wichtig — wann qini_argmax kein Signal hat:** Ist der dominante Arm (nahezu) homogen wirksam (jeder profitiert ähnlich stark), trägt das Argmax-Ranking keine Ordnung — der Scorer meldet dann ehrlich Werte ≈ 0 und das Tuning hat kein Optimierungssignal (Diagnose: per-Arm-Qini des dominanten Arms liegt auf der Random-Linie bei gleichzeitig hohem `policy_value_T<k>`). In diesem Regime `rscore` verwenden — er optimiert die Genauigkeit der Effektfläche und funktioniert unabhängig von Ranking-Heterogenität. `qini` optimiert direkt die Ranking-Qualität (aggregierter OOF-Qini, kein Pruning). `rscore` nutzt den EconML RScorer (unabhängige Nuisance, Pruning möglich). Wird synchron mit FMT gesetzt.
...
- **BLT** optimiert die Base Learner (model_y/model_t), **CFT** die Forest-Parameter — getrennt.
...
tuning:
  ...
  cv_splits: 5
  single_fold: false
  # metric und metric_regression sind fest: log_loss (Klassifikation) und
  # neg_mse (Regression). Beide messen Kalibrierung — Voraussetzung für
  # unverzerrte CATE-Schätzungen bei DML-Residualisierung.
  models: null
...
`metric` und `metric_regression` sind **fest**: `log_loss` (Klassifikation) und `neg_mse` (Regression). Log-Loss misst die Kalibrierung der Nuisance-Vorhersagen P(T|X) und P(Y|X) — Voraussetzung für unverzerrte CATE-Schätzungen bei DML-Residualisierung (Bach et al., 2024). MSE auf binären Targets entspricht dem Brier Score. Andere Metriken (AUC, Accuracy) messen nur Diskrimination, nicht Kalibrierung, und sind daher für DML-Nuisance-Tuning ungeeignet.
...
Zusätzlich besitzt es eine Forest‑Stufe mit eigenen Forest‑Parametern.

- Das Optuna‑Tuning (`tuning`) betrifft weiterhin `model_y`/`model_t`.
- Die Forest‑Parameter können optional über **CausalForest-Tuning (CFT)** per Optuna optimiert werden
...
final_model_tuning:
  ...
  models: null
  single_fold: false
  scorer: auto               # auto | qini | rscore | qini_argmax. auto = Qini bei RCT, R-Score bei Beobachtungsdaten; bei Multi-Treatment immer R-Score. qini_argmax = MT-Ranking über alle Arme
  overfit_penalty: 0.0
...
- Scorer-Auswahl: **Qini** (Default bei RCT) optimiert Ranking-Qualität (aggregierter OOF-Qini), **R-Score** (Default bei Beobachtungsdaten) optimiert pointwise CATE-Genauigkeit via EconML RScorer.
...
- `overfit_penalty`: Train-Val-Gap-Penalty für model_final (0=deaktiviert). Bestraft Konfigurationen, deren Score auf Train deutlich besser ist als auf Val (OOF). Tolerance ist relativ: BLT Default 0.20 (20% Gap; die BLT-Penalty wirkt nur auf Meta-Learner, DML/DR werden nie bestraft), FMT Default 0.10 (10% Gap — CATE-Signal schwächer). Formel: `adjusted = val_score - penalty × scale × min(max(0, relative_gap - tolerance), max_penalized_gap)` mit `scale = |val_score|`, `relative_gap = gap / scale`. Der `overfit_max_penalized_gap`-Cap (Default 1.0) saturiert den Abzug.
...
### Overfit-Penalty (BLT: nur Meta-Learner)

Die BLT-Overfit-Penalty bestraft Hyperparameter-Konfigurationen mit großem Train-Val-Gap — **ausschließlich für Meta-Learner-Tasks** (S-/T-/X-Learner). Diese trainieren ihre Base-Models direkt und bilden den CATE ohne internes Cross-Fitting, weshalb deren Overfitting unmittelbar in die CATE durchschlägt. **DML/DR-Nuisances (`sample_scope == "all"`) werden nie bestraft**, unabhängig von der Konfiguration: Cross-Fitting + `mc_iters` + Neyman-Orthogonalität fangen ihr Overfitting bereits ab (Chernozhukov et al., 2018; Bach et al. 2024). Die Gating-Logik steckt in `BaseLearnerTuner._penalty_applies`.

Konfiguration:
- `tuning.overfit_penalty`: Stärke der BLT-Penalty. Default 0 (aktivierbar, z.B. 0.2). Wirkt nur auf Meta-Learner-Base-Models; für DML/DR-Tasks wird der Wert ignoriert.
- `tuning.overfit_tolerance`: Relativer Gap-Schwellwert, unter dem keine Penalty greift. Default: 0.20 (20%).
- `tuning.overfit_max_penalized_gap`: Cap auf den bestraften relativen Gap (Saturierung). Default: 1.0. Verhindert Sign-Flip/Dominanz bei kleinen Scores; `<=0` deaktiviert den Cap.
...
**Wann aktivieren:** Bei Meta-Learnern (S/T/X) gegen Base-Model-Overfitting (gute In-Sample-, aber schwache OOF-Werte) sowie auf der **Finalstufe** (FMT/CFT) bei Verdacht auf CATE-Overfitting. Für DML/DR-Nuisances hat die Einstellung keinen Effekt — deren Overfitting fängt das Cross-Fitting ab. Schwache Qini bei guten DML/DR-BLT-Scores deutet meist nicht auf Nuisance-Overfitting hin, sondern auf zu starke Nuisance-Regularisierung, instabile Propensities (nahe 0/1) oder ein Magnitude-vs-Ranking-Problem.

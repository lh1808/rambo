...
data_processing:
  ...
  dml_crossfit_folds: 2       # Internes Nuisance-Cross-Fitting (EconML-Default=2)
...
- `dml_crossfit_folds`: **Internes Nuisance-Cross-Fitting** für DML/DR-Modelle in Produktion. Steuert wie viele Folds für die Residualisierung `Y − E[Y|X]` verwendet werden. EconML-Default: 2. Getrennt von `cross_validation_splits` und von den BLT/FMT-internen CVs (`tuning.cv_splits`, `final_model_tuning.cv_splits`).
...
data_processing:
  cross_validation_splits: 5   # Äußere CV für Out-of-Fold CATE-Predictions
  dml_crossfit_folds: 2        # Internes Nuisance-Cross-Fitting (EconML-Default)
...
**CV-Folds (`cv_splits`):**  
Innere Cross-Validation pro Tuning-Trial. Default: 2 (EconML-Default). Bewusst getrennt von `cross_validation_splits` (äußere CV) und `dml_crossfit_folds` (Produktion). Höhere Werte stabilisieren die Trial-Bewertung, kosten aber linear mehr Rechenzeit pro Trial.

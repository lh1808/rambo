...
> **Hinweis LAN:** Für die PyPI-Installation im Firmennetz muss `PIXI_TLS_ROOT_CERTS="all"` gesetzt sein. Die `[pypi-options]` in `pixi.toml` verweisen auf den internen Nexus-Mirror.
...
## 13b) LightGBM Determinismus

LightGBMs `deterministic=True` erzwingt Single-Thread-Training — unabhängig von `n_jobs`. Das garantiert bitgenaue Reproduzierbarkeit, kostet aber 3–5× Performance. In rubin wird `deterministic=True` deshalb **nur bei `parallel_jobs=1` (Level 1)** gesetzt. Bei Level 2+ sorgt `random_state=seed` für Reproduzierbarkeit der Splits und Initialisierung; die minimale Floating-Point-Varianz durch parallele Histogramm-Berechnung ist in der Praxis vernachlässigbar.

Betrifft: `_build_lgbm_classifier()`, `_build_lgbm_regressor()` in `tuning_optuna.py` sowie `_lgbm_gain_importance()`, `_lgbm_permutation_importance()` in `feature_selection.py`.
...
## 18) Ensemble (EnsembleCateEstimator)

Das Ensemble wird über `models.ensemble: true` aktiviert und erstellt ein gleichgewichtetes `EnsembleCateEstimator` (EconML) aus allen trainierten Modellen.

**Wichtige Implementierungsdetails:**

- **Ensemble-Predictions:** Mittelwert der individuellen Out-of-Fold-Predictions aller Modelle.
- **Kein eigenes `fit()`:** `EnsembleCateEstimator` aggregiert nur — es hält Referenzen auf die Einzelmodelle. Refit-Logik muss daher die Einzelmodelle refitten und das Ensemble neu erstellen.
- **Refit bei Ensemble-Champion:** Wenn das Ensemble Champion wird und `refit_champion_on_full_data: true`, werden **alle** Einzelmodelle auf vollen Daten refittet. Anschließend wird ein neues `EnsembleCateEstimator` mit den refitteten Modellen und den gleichen Gewichten (`1/N`) erstellt und ins Bundle geschrieben.
- **Explainability-Fallback:** `EnsembleCateEstimator` hat kein natives SHAP. Bei einem Ensemble-Champion wird automatisch das beste Einzelmodell (nach Selektionsmetrik) für die SHAP-Analyse herangezogen. Der HTML-Report dokumentiert, welches Modell verwendet wurde.
- **CausalForest-Kompatibilität:** Der `CausalForestAdapter` erbt von `BaseCateEstimator` und implementiert `effect()`, `const_marginal_effect()` etc. — dadurch ist CausalForest ensemble-fähig ohne Sonderbehandlung.
- **Evaluation:** Das Ensemble wird wie jedes andere Modell evaluiert (Phase 1–3). Es wird bei der Champion-Selektion berücksichtigt, aber von der Surrogate-Erzeugung und DRTester-Level-Filterung ausgeschlossen.

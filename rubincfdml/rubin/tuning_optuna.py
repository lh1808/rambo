...
def tune_causal_forest(
    ...
    def objective(trial):
        ...
        # Ohne dies scheitern ~75% der Trials mit ValueError (still gefangen).
        if "n_estimators" in params:
            _sf_size = 4  # EconML Default subforest_size
            params["n_estimators"] = max(_sf_size, (params["n_estimators"] // _sf_size) * _sf_size)
        ...
        if is_cfdml:
            ...
            for fold_i, (est, tr, va) in enumerate(_cfdml_cached_folds):
                try:
                    ...
                    # Diagnose: Bei erstem Trial + erstem Fold prüfen ob Parameter ankamen
                    if trial.number == 0 and fold_i == 0:
                        _actual = {p: getattr(est, p, "?") for p in _forest_params}
                        _tlog.info("CFT Diagnose (Trial 0, Fold 0): set=%s, actual=%s, score=%.6g", _forest_params, _actual, score)
                except Exception:

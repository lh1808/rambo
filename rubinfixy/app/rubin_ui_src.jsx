...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Tuning-Regularisierung" accent="#7c3aed">
    ...
    {(()=>{
      const REG_PRESETS = [
        {key:"moderate", label:"Moderat", desc:"Empfohlen – 15% Gap toleriert", penalty:0.2, tolerance:0.15, c:"#d97706"},
        {key:"strong", label:"Stark", desc:"Konservativ – 8% Gap toleriert", penalty:0.4, tolerance:0.08, c:"#dc2626"},
      ];
      ...
      const matched = REG_PRESETS.find(r => r.penalty===bp && r.tolerance===bt);
      const activeKey = matched ? matched.key : (bp===0 ? null : "custom");
      const toggleReg = (r) => {
        if(activeKey===r.key) set({...cfg, overfitPenalty:0, overfitTolerance:0.15});
        else set({...cfg, overfitPenalty:r.penalty, overfitTolerance:r.tolerance});
      };
      return (<>
        <div style={{display:"flex",gap:10,marginTop:4}}>
          {REG_PRESETS.map(r => {
...
<FixedParamsEditor params={(cfg.fmtFixed||{}).lgbm||{}} defaults={LGBM_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:{...(cfg.fmtFixed||{}),lgbm:v}})}/>
              ...
              return (<>
                ...
                <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
                  RScorer-basierter Grid-Search über Wald-Parameter. CausalForestDML benötigt vorab Nuisance-Fits (cv={dc}×2{mc>1?" × mc="+mc:""}). CausalForest arbeitet direkt auf den Daten (kein Nuisance).
                </div>
                <div style={{border:"1px solid #ede6e7",borderRadius:8,overflow:"hidden"}}>
                  ...
                  {hasGRF && <div style={{background:hasCF?"#fdfbfb":"#fff"}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 80px",padding:"8px 14px",fontSize:12}}>
                      ...
                      <span style={{fontSize:11,color:"#2d6a4f"}}>{grk} Forest-Grid (kein Nuisance)</span>
                      <span style={{fontFamily:"monospace",fontWeight:700,color:"#2d6a4f"}}>{grfTotal}</span>
                    </div>
                    <div style={{fontSize:10.5,color:"#999",padding:"0 14px 6px",lineHeight:1.4}}>RScorer Grid-Search ({grk} Kombis{cfg.cfTuneIntensive?" inkl. criterion":""}).</div>
                  </div>}
                </div>
              </>);
...
function App() {
  ...
  const estimateFits = () => {
    ...
    if (cfg.cfTune) {
      ...
      if ((cfg.cfTuneModels||[]).includes("CausalForest") && m.includes("CausalForest")) f += _grk;
    }

...
Die Penalty ist **skalen-sicher** durch relative Tolerance. BLT und FMT verwenden
bewusst unterschiedliche Default-Toleranzen:

| Stufe | Preset Moderat | Preset Stark | Begründung |
|---|---|---|---|
| **BLT** | penalty=0.2, tol=0.10 | penalty=0.4, tol=0.05 | Nuisance darf moderat overfitten — DML-Orthogonalität kompensiert |
| **FMT** | penalty=0.3, tol=0.05 | penalty=0.6, tol=0.03 | CATE-Signal ist schwächer — stärkere Regularisierung nötig |

**Warum unterschiedlich?** BLT-Nuisance-Modelle (model_y, model_t) sind klassische ML-Aufgaben.
Ein 10-15% Train-Val-Gap ist bei Gradient Boosting normal und kein Problem, weil DML's
Orthogonalität (Chernozhukov et al., 2016) die CATE-Schätzung robust gegenüber moderatem
Nuisance-Overfitting macht. FMT model_final hingegen schätzt die CATE-Funktion direkt —
hier deutet ein >5% Gap auf falsche Heterogenität hin (Rauschen statt echtem Signal).
...
`CausalForest` nutzt einen eigenen Grid-Search (kein Optuna): Jede Parameterkombination wird

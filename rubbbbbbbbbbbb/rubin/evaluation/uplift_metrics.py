...
def uplift_curve(y: np.ndarray, t: np.ndarray, score: np.ndarray) -> UpliftCurve:
    ...
    # nicht — sie hängt von der Datenreihenfolge ab. Random Jitter verhindert,
    # dass Tie-Gruppen artifizielle Treatment-Balance-Schwankungen erzeugen.
    # Seed=42 für Reproduzierbarkeit.
    _rng = np.random.default_rng(42)
    order = np.argsort(-(score + _rng.uniform(-1e-10, 1e-10, n)))
    y = y[order]
    t = t[order]
...
def qini_coefficient(curve: UpliftCurve) -> float:
    ...
    x = curve.fraction
    baseline = x * y_qini[-1]
    return float(_trapz(y_qini - baseline, x))
...
def uplift_curve_mt_per_arm(
    ...
    order = np.argsort(-(s_sub + _rng_mt.uniform(-1e-10, 1e-10, len(s_sub))))
    y_sub = y_sub[order]
    t_sub = t_sub[order]

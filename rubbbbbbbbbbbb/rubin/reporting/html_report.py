...
class ReportCollector:
    ...
    def add_cft_info(self, info: Dict[str, Any]) -> None:
        ...
        aufgerufen und sammelt alle Ergebnisse in einer Liste, damit der Report
        — wie FMT — beide Modelle zeigt. ``cft_info`` bleibt als zuletzt
        gesetztes Dict für Abwärtskompatibilität erhalten.
        """
        self.cft_info = info
...
def _assessment_card(data, metrics_list, title_override=None):
    ...
    card = f'<div style="margin:14px 28px;background:#fff;border-radius:var(--r);box-shadow:var(--sh);overflow:hidden;display:flex">'
    ...
    card += f'<div style="display:flex;align-items:center;gap:14px;margin-bottom:14px">'
    ...
    if metrics_list:
        ...
        for m_lbl, m_val in metrics_list:
            card += f'<div style="background:var(--ruby-bg);border:1px solid var(--border);border-radius:8px;padding:8px 14px;min-width:80px">'
...
def _render_tuning(collector, cs) -> str:
    ...
    # Config info
    h += '<div class="cg">'
    h += f'<div class="cd"><div class="cd-l">Metrik (Clf)</div><div class="cd-v">log_loss (fest) + Skill Score</div></div>'
    h += f'<div class="cd"><div class="cd-l">Metrik (Reg)</div><div class="cd-v">neg_mse (fest) + R²</div></div>'
...
def _render_cft(collector, cs) -> str:
    ...
    """
    results = list(getattr(collector, "cft_results", []) or [])
    if not results and collector.cft_info:
        results = [collector.cft_info]  # Backward-Compat (z.B. Golden-Fixture setzt cft_info direkt)
    if not results:
        return ""
...
def _render_explainability(collector) -> str:
    ...
    if collector.champion_name == "Ensemble" and expl_model and expl_model != "Ensemble":
        ensemble_hint = f' <em>(Ensemble ist Champion — Explainability auf bestem Einzelmodell.)</em>'

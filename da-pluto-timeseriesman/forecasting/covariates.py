...
def build_weekly_covariates(
    ...
    """
    ts = build_covariates_all_states(
        df=df,
        freq=cfg.data.freq,
    ...
    # Kovariaten-Selektion: nur die konfigurierten Spalten durchreichen.
    # Reduziert die Feature-Dimensionalität (16 Einzelländer-Flags sind
    # bei wenigen Trainingssamples reines Rauschen).
    selected = cfg.data.selected_future_covariates
    if selected:
        available = [c for c in selected if c in ts.components]
        if available:
            ts = ts[available]

    # Skalierung: die Ferien-Features haben unterschiedliche Wertebereiche
    # (vac_DE_count: 0–16, holidays_workdays_DE_count: 0–80, binäre Flags:
    # 0–1). StandardScaler normalisiert auf einheitliche Skala, was die
    # Konvergenz des TFT verbessert. Da die Features kalenderbasiert und
    # deterministisch sind (keine Abhängigkeit von der Zielvariable),
    # ist globale Skalierung unbedenklich (kein Temporal Leakage).
    #
    # Guard: wenn alle Werte 0 sind (z. B. keine ICS-Dateien vorhanden),
    # überspringen — StandardScaler würde Division durch 0 produzieren.

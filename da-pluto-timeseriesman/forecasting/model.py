...
# Tensor Cores (A100, H100, …) arbeiten effizienter mit reduzierter
# Float32-Matmul-Precision. "high" bietet den besten Kompromiss
# zwischen Geschwindigkeit und numerischer Genauigkeit.
...
def build_transformers() -> TransformArtifacts:
    ...
    1. :class:`RollingWinsorizer` mit ``[0.01, 0.99]`` über ein Fenster
       von 26 Wochen. Die Obergrenze von 0.99 begrenzt nur echte
       Ausreißer; sie kappt — anders als ein scharfes 0.90 — keine
       saisonalen Spitzen weg, deren Unterschätzung in der
       Kapazitätsplanung die teure Fehlerrichtung wäre.
...
def build_tft(
    ...
    - Ist ``tft_cfg.early_stopping_patience > 0``, wird ein
      ``EarlyStopping``-Callback auf den Training-Loss gesetzt.
    - ``extra_callbacks`` ermöglicht das Einfügen zusätzlicher
      Callbacks (z. B. Optuna-Pruning) ohne die Basis-Callbacks zu
      überschreiben.
    ...
    tft_cfg
        TFT-Hyperparameter.
    extra_callbacks
        Zusätzliche PyTorch-Lightning-Callbacks (z. B. Optuna-Pruning).
    """
    ...
    if tft_cfg.gradient_clip_val > 0:
        pl_trainer_kwargs["gradient_clip_val"] = tft_cfg.gradient_clip_val

    # Optimizer-Kwargs: Lernrate + Weight-Decay (L2-Regularisierung)
    opt_kwargs = {"lr": tft_cfg.learning_rate}
    if tft_cfg.weight_decay > 0:
        opt_kwargs["weight_decay"] = tft_cfg.weight_decay

    # Zeitfeatures: Monat, Kalenderwoche, zyklische Encodierungen,
    # relative Position. Ersetzt add_relative_index. ``year`` wird
    # bewusst NICHT als Encoder genutzt: monoton steigend, würde über
    # den MinMax-Transformer auf den Trainingsbereich skaliert und im
    # Prognosezeitraum außerhalb [0, 1] liegen (schlechte Extrapolation).
    # Das Saisonsignal tragen month/weekofyear zyklisch bereits.
    if tft_cfg.use_time_encoders:
        add_encoders = {
            "datetime_attribute": {
                "future": ["month", "weekofyear"],
            },
            "cyclic": {
                "future": ["month", "weekofyear"],
            },
    ...
    return TFTModel(
        ...
        feed_forward=tft_cfg.feed_forward,

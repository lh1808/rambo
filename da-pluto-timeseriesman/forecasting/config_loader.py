...
def _deep_merge_into_dataclass(instance: Any, overrides: Mapping[str, Any], path: str = "") -> None:
    ...
    for key, value in overrides.items():
        ...
        # Typ-Coercion: YAML parst z. B. '5e-4' als String statt Float.
        # Wir casten auf den Typ des dataclass-Felds, wenn möglich.
        expected_type = field_map[key].type
        value = _coerce_primitive(value, expected_type)
        setattr(instance, key, value)


def _coerce_primitive(value: Any, type_hint: Any) -> Any:
    """
    Castet ``value`` auf den erwarteten Primitiv-Typ, falls nötig.

    Löst Fälle wie YAML ``5e-4`` → str statt float, oder
    YAML ``true`` → bool statt int.
    """
    if value is None:
        return value

    # type_hint kann ein String sein (forward ref) oder ein echter Typ
    if isinstance(type_hint, str):
        type_hint = {"int": int, "float": float, "bool": bool, "str": str}.get(type_hint)
        if type_hint is None:
            return value

    if type_hint is float and isinstance(value, (str, int)):
        try:
            return float(value)
        except (ValueError, TypeError):
            return value

    if type_hint is int and isinstance(value, (str, float)):
        try:
            return int(value)
        except (ValueError, TypeError):
            return value

    if type_hint is bool and isinstance(value, (str, int)):

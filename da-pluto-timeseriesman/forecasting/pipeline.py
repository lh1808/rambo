...
def _align_components(ts: TimeSeries, components: list, freq) -> TimeSeries:
    """
    Bringt eine Zeitreihe auf eine feste Komponentenmenge und -reihenfolge.

    Fehlende Komponenten werden mit 0 ergänzt, überzählige verworfen. Das
    schützt die Inferenz davor, dass eine veränderte Datenlage (z. B. eine
    Zelle, die den ``min_component_total``-Schwellwert neu über- oder
    unterschreitet) die Reihenbreite gegenüber dem Trainingsstand
    verschiebt – ``model.predict`` verlangt exakt dieselbe Komponentenmenge
    wie beim Training.
    """
    df = ts.to_dataframe()
    for c in components:
        if c not in df.columns:
            df[c] = 0.0
    df = df[list(components)]
    return TimeSeries.from_dataframe(df, freq=freq)
...
class ModelArtifacts:
    ...
    tft_cfg
        ...
        überschrieben).
    target_components
        Reihenfolge der Zielkomponenten, auf denen das finale Modell
        trainiert wurde (nach min_component_total-Drop). Wird in der
        Inferenz zum Angleichen der aktuellen Daten verwendet, damit die
        Reihenbreite zum Modell passt.
    past_cov_components
        Reihenfolge der Past-Kovariaten-Kanäle des finalen Modells.
        Analog zum Angleichen in der Inferenz.
    """
...
def _build_tuned_tft_cfg_from_trial(
    ...
    if tuning_cfg.num_attention_heads_choices:
        heads = trial.suggest_categorical(
            "num_attention_heads",
            tuning_cfg.num_attention_heads_choices,
        )
        # MultiheadAttention verlangt hidden_size teilbar durch heads.
        if cfg.hidden_size % heads != 0:
            raise optuna.TrialPruned(
                f"hidden_size={cfg.hidden_size} nicht teilbar durch "
...
def _optuna_objective_for_horizon(
    ...
    """
    # input_chunk_length (Kontextfenster) optional tunen. Da es ein
    # HorizonConfig-Feld ist, wird für den Trial eine angepasste Kopie
    # verwendet; alle weiteren hcfg-Zugriffe im Objective nutzen sie.
    if tuning_cfg.input_chunk_length_choices:
        sampled_icl = trial.suggest_categorical(
            "input_chunk_length",
            tuning_cfg.input_chunk_length_choices,
        )
        hcfg = replace(hcfg, input_chunk_length=sampled_icl)
    ...
    if tuning_cfg.use_pruning:
        try:
            ...
            logging.getLogger(__name__).warning(
                "use_pruning=True, aber PyTorchLightningPruningCallback ist "
                "nicht importierbar (Paket 'optuna-integration' fehlt?) – das "
                "Pruning ist damit faktisch INAKTIV, alle Trials laufen voll "
                "durch."
            )
...
def _compute_tuning_score(
    ...
    if metric_name in ("combined", "combined_pooled"):
        alpha = tuning_cfg.tuning_metric_alpha
        if "wape" not in metrics_df.columns or "smape" not in metrics_df.columns:
            raise ValueError(
                "metric='combined'/'combined_pooled' benötigt die Spalten "
                "'wape' und 'smape' im Metriken-DataFrame."
            )

        if metric_name == "combined_pooled":
            for col in ("abs_error_sum", "abs_actual_sum"):
                if col not in metrics_df.columns:
                    raise ValueError(
                        f"metric='combined_pooled' benötigt die Spalte "
                        f"'{col}'. evaluate_multivariate liefert sie seit "
                        "Batch 6 – ggf. Version prüfen."
                    )
            # Gepoolte (volumengewichtete) WAPE: Σ|Fehler| / Σ|Ist|.
            denom = float(metrics_df["abs_actual_sum"].sum())
            if denom <= 0:
                return float("inf")
            wape_score = float(metrics_df["abs_error_sum"].sum() / denom * 100.0)
            # Volumengewichtete sMAPE (Gewicht = Komponenten-Volumen).
            weights = metrics_df["abs_actual_sum"].to_numpy(dtype=float)
            smape_vals = metrics_df["smape"].to_numpy(dtype=float)
            mask = ~np.isnan(smape_vals)
            if mask.any() and weights[mask].sum() > 0:
                smape_score = float(np.average(smape_vals[mask], weights=weights[mask]))
            else:
                smape_score = float(np.nanmean(smape_vals))
        else:
            wape_score = float(metrics_df["wape"].mean())
            smape_score = float(metrics_df["smape"].mean())

        return alpha * wape_score + (1 - alpha) * smape_score
...
def train_and_evaluate_for_horizon(
    ...
    effective_input_chunk_length = hcfg.input_chunk_length
    effective_tft_cfg = deepcopy(cfg.tft)

    if tuning_cfg.enabled:
        # Sampler: multivariater TPE modelliert Wechselwirkungen zwischen
        # den Hyperparametern; Seed sorgt für Reproduzierbarkeit.
        seed = (
            tuning_cfg.sampler_seed
            if tuning_cfg.sampler_seed is not None
            else cfg.tft.random_state
        )
        sampler = optuna.samplers.TPESampler(
            multivariate=tuning_cfg.sampler_multivariate,
            group=tuning_cfg.sampler_multivariate,
            constant_liar=True,
            seed=seed,
        )
        ...
        # Hinweis: train_loss ist kein Held-out-Signal; über use_pruning
        # abschaltbar (dann NopPruner → volle Trials).
        if tuning_cfg.use_pruning:
        ...
        if "input_chunk_length" in best_params:
            effective_input_chunk_length = best_params["input_chunk_length"]
        effective_train_length_weeks = max(
            best_params["train_length_weeks"],
            effective_input_chunk_length + 1,
            hcfg.output_chunk_length + 1,
        )
        ...
        if cfg.tuned_params_dir:
            save_tuned_params(
                ...
                best_params=best_params,
                best_score=study.best_value,
                input_chunk_length=effective_input_chunk_length,
            )
        ...
        if saved is not None:
            ...
            effective_input_chunk_length = saved.get(
                "input_chunk_length", hcfg.input_chunk_length
            )
            tft_dict = saved.get("tft", {})
            # Nur die tatsächlich GETUNETEN Hyperparameter übernehmen –
            # symmetrisch zu _build_tuned_tft_cfg_from_trial. Operative /
            # architektonische Felder (n_epochs, early_stopping_patience,
            # optimizer, loss, gradient_clip_val, …) bleiben bei den Werten
            # der AKTUELLEN Config, statt vom Tuning-Lauf überschrieben zu
            # werden. train_length_weeks und input_chunk_length werden
            # separat (oben) behandelt. Der gespeicherte Schlüssel heißt
            # ``optuna_best_params`` (siehe save_tuned_params).
            best_params = saved.get("optuna_best_params", {})
            tuned_keys = [
                k for k in best_params
                if k not in ("train_length_weeks", "input_chunk_length")
            ]
            if tuned_keys:
                for key in tuned_keys:
                    if hasattr(effective_tft_cfg, key) and key in tft_dict:
                        setattr(effective_tft_cfg, key, tft_dict[key])
            else:
                # Älteres Format ohne optuna_best_params → altes Verhalten
                # (vollständiges tft-Dict), mit Hinweis.
                logging.getLogger(__name__).warning(
                    "Getunete Parameter ohne 'optuna_best_params' geladen – "
                    "wende das vollständige tft-Dict an (kann Operativ-"
                    "Parameter der aktuellen Config überschreiben)."
                )
    ...
    # Effektive Horizont-Config: identisch zu hcfg, aber mit dem ggf.
    # getuneten/geladenen input_chunk_length. Ab hier wird durchgängig
    # effective_hcfg verwendet (Eval-Backtest, finales Training, Artefakte),
    # damit Kontextfenster überall konsistent ist.
    effective_hcfg = replace(hcfg, input_chunk_length=effective_input_chunk_length)
    ...
    if len(ts_y_static) <= effective_hcfg.validation_weeks:
        raise RuntimeError("Zu wenige Beobachtungen für das gewählte Validierungsfenster.")

    test_start = ts_y_static.time_index[-effective_hcfg.validation_weeks]
    ...
    res = rolling_block_forecast(
        ...
        horizon=effective_hcfg.horizon_weeks,
        stride=effective_hcfg.stride_weeks,
        fit_kwargs={"verbose": False},
        predict_kwargs={},
        merge="keep_last",
        required_input_chunk_length=effective_hcfg.input_chunk_length + effective_hcfg.output_chunk_length,
        verbose=True,
        transforms_builder=build_transformers,
        window_mode=hcfg.window_mode,
    )
    ...
    min_train_length = effective_hcfg.input_chunk_length + effective_hcfg.output_chunk_length
    if len(y_train) < min_train_length:
        raise RuntimeError(
            f"Trainingserie für finales {effective_hcfg.name}-Modell zu kurz: "
            f"{len(y_train)} Wochen verfügbar, aber mindestens "
            f"{min_train_length} benötigt (input_chunk_length="
            f"{effective_hcfg.input_chunk_length} + output_chunk_length="
            f"{effective_hcfg.output_chunk_length}). "
    ...
    final_model = build_tft(effective_hcfg, effective_tft_cfg)
    final_model.fit(
    ...
    artifacts = ModelArtifacts(
        horizon_weeks=effective_hcfg.horizon_weeks,
        hcfg=effective_hcfg,
        model=final_model,
        ...
        train_length_weeks=effective_train_length_weeks,
        tft_cfg=effective_tft_cfg,
        target_components=list(ts_y_static.components),
        past_cov_components=list(ts_X.components),
    )
...
def forecast_with_artifacts(
    ...
    # An den Trainingsstand angleichen (Breite + Reihenfolge), damit
    # predict nicht an einer abweichenden Komponentenmenge scheitert.
    ts_y = _align_components(ts_y, artifacts.target_components, cfg.data.freq)
    ts_X = _align_components(ts_X, artifacts.past_cov_components, cfg.data.freq)
    ...
    static_df = pd.DataFrame(
        ...
        columns=["Kennzahl", "Sparte", "Status", "Orga"],

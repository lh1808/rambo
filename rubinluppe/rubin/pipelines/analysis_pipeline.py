    ...
    def _load_inputs(self) -> tuple[pd.DataFrame, np.ndarray, np.ndarray, Optional[np.ndarray], Optional[np.ndarray]]:
        ...
        if self.cfg.data_files.dtypes_file:
            try:
                ...
                if _n_applied:
                    print(
                        f"[rubin] dtypes.json geladen: {_n_applied} Spalten-Dtypes angewendet"
                        f" ({_n_cat} kategorial).",
                        flush=True,
                    )
            except FileNotFoundError:
            ...
            if _auto_dtypes.exists():
                try:
                    ...
                    if _n_restored:
                        ...
                        print(
                            f"[rubin] dtypes.json auto-erkannt: {_n_restored} Spalten-Dtypes wiederhergestellt"
                            f" ({_n_cat} kategorial).",
                            flush=True,
                        )
                except Exception:
        ...
        for name in _models_order:
            ...
            if name in ("CausalForestDML", "CausalForest") and getattr(cfg.causal_forest, "use_econml_tune", False) and (not getattr(cfg.causal_forest, "tune_models", []) or name in getattr(cfg.causal_forest, "tune_models", [])):
                try:
                    ...
                    self._logger.warning("CausalForest-Tuning (%s) fehlgeschlagen.", name, exc_info=True)
    ...
    def run(self, export_bundle: bool | None = None, bundle_dir: str | None = None, bundle_id: str | None = None) -> AnalysisResult:
        ...
        with mlflow.start_run(run_name=_run_name):
            ...
            if getattr(cfg.causal_forest, "use_econml_tune", False):
                _tuning_parts.append("CausalForest-Tuning")
            ...
            if _val_mode == "external":
                ...
                self._logger.info("Validierungsmodus: Train-Many-Evaluate-One (eval_mask)")
            else:

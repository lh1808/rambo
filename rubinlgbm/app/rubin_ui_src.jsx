...
const LGBM_DEFAULTS={boosting_type:"dart",n_estimators:500,learning_rate:0.05,num_leaves:63,max_depth:6,min_child_samples:20,min_child_weight:1.0,subsample:0.8,colsample_bytree:0.6,min_split_gain:0.0,reg_alpha:0.1,reg_lambda:1.0,path_smooth:0.0,drop_rate:0.1,skip_drop:0.5,max_drop:50};
...
const LGBM_FINAL_DEFAULTS={boosting_type:"dart",n_estimators:150,learning_rate:0.03,num_leaves:15,max_depth:4,min_child_samples:200,min_child_weight:10.0,subsample:0.6,colsample_bytree:0.3,min_split_gain:1.0,reg_alpha:5.0,reg_lambda:30.0,path_smooth:10.0,drop_rate:0.15,skip_drop:0.5};
...
const STEPS=[{id:"load",label:"Daten laden & Preprocessing"},{id:"fs",label:"Feature-Selektion"},{id:"tune",label:"Base-Learner-Tuning"},{id:"fmt",label:"Final-Model-Tuning"},{id:"train",label:"Training & Cross-Predictions"},{id:"eval",label:"Evaluation & Metriken"},{id:"surrogate",label:"Surrogate-Tree"},{id:"bundle",label:"Bundle-Export"},{id:"explain",label:"Explainability"},{id:"report",label:"HTML-Report"}];
...
const ADDON_PRESETS = [
  ...
  // ── Validierung ──
  {key:"benchmark",label:"Benchmark",desc:"Vergleich gegen historischen Score",group:"validation",
    cfg:{}},
...
const parseYamlToCfg = (yamlText) => {
  ...
  // Parse nested fixed_params for base_learner (beim "both"-Modus):
  //   base_learner:
  //     type: both
  //     fixed_params:
  //       lgbm: { ... }
  //       catboost: { ... }
  // Gleiche Logik für final_model_tuning.fixed_params.
...
const PData = ({cfg,set,setCfg,activeBase,setActiveBase,activeAddons,setActiveAddons,view}) => {
  ...
  return (
    <>
      {view==="files"&&<Sec title="Analysedaten">
        ...
        {cfg.validateOn==="external" && (<>
          <Divider/>
          <div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:6}}>Evaluationsdaten (separater Datensatz)</div>
      ...
      {view==="template"&&<Sec title="2. Studientyp">
        <Info>Bestimmt, welche Modelle empfohlen und welche Evaluationen berechnet werden. Bei Beobachtungsdaten werden nur Modelle mit Confounding-Korrektur als Default gesetzt.</Info>
        <div style={{display:"flex",gap:10,marginTop:10}}>
          {[{k:"rct",l:"RCT (randomisiert)",d:"Alle 8 Modelle + alle Evaluationen"},{k:"observational",l:"Beobachtungsdaten",d:"DR/DML-Modelle empfohlen, kein naiver Policy Value"}].map(o => {
        ...
        {(cfg.studyType||"rct") !== "rct" && <div style={{marginTop:8}}>
          <Info>Confounding-Korrektur essentiell. SLearner, TLearner und CausalForest sind als Baseline auswählbar, aber nicht gegen Confounding geschützt.</Info>
        </div>}
      </Sec>}
      ...
      {view==="template"&&<Sec title="4. Add-ons">
        ...
        {(() => {
          ...
          const R3 = [
            ...
            {group:"exploration",label:"Exploration",c:"#e67e22",bg:"#fff8f0",bd:"#f5cba7",abg:"#fdebd0"},
          ];
          const R4 = [
            {group:"validation",label:"Validierung",c:"#6366f1",bg:"#f6f7ff",bd:"#c7d2fe",abg:"#e0e7ff"},
...
const PConfig = ({cfg,set}) => {
  ...
  useEffect(() => {
    fetch("./api/work-dir").then(r=>r.json()).then(d=>setWorkDirInfo(d)).catch(()=>{});
  }, []);
  return (<>
  <Sec title="Allgemein">
    ...
    <div style={{fontSize:11,color:C.textSec,lineHeight:1.5,marginBottom:10}}>Alle erzeugten Artefakte (MLflow, Report, Uploads, Cache) landen hier — nicht im Repository. Konfigurierbar über die Umgebungsvariable <code style={{fontSize:10,background:C.rose,padding:"1px 6px",borderRadius:4}}>RUBIN_WORK_DIR</code> oder <code style={{fontSize:10,background:C.rose,padding:"1px 6px",borderRadius:4}}>constants.work_dir</code> in der Config.</div>
    <div style={{background:"#f6f8fa",border:"1px solid "+C.border,borderRadius:8,padding:"10px 14px",fontSize:11,fontFamily:"'JetBrains Mono',monospace",lineHeight:1.7}}>
      ...
      <div style={{color:"#57606a",fontSize:10}}>Report/Cache → {workDirInfo.cache_dir}</div>
    </div>
    <Inp label="work_dir überschreiben (optional)" type="text" value={cfg.workDir||""} onChange={v=>set({...cfg,workDir:v||null})} help="Überschreibt den Default. Leer = Env-Variable oder ./runs. Wird in der Config als constants.work_dir geschrieben."/>
    </>}
  </Sec>
  ...
  <Sec title="Validierungsstrategie">
    ...
    {cfg.validateOn==="external" && <Info>Evaluationsdaten werden auf der Seite <strong>Datenpfade</strong> angegeben.{cfg.eval_x_file ? ` Aktuell: ${cfg.eval_x_file.split("/").pop()}` : " Noch keine Eval-Dateien konfiguriert."}</Info>}
    {cfg.eval_mask_file && cfg.validateOn!=="external" && <Info type="warn"><strong>Train Many, Evaluate One</strong> aktiv: Metriken werden nur auf dem Eval-Set berechnet. Maske: <span style={{fontFamily:"monospace",fontSize:11}}>{cfg.eval_mask_file.split("/").pop()}</span></Info>}
    ...
    <Info>Alle Modelle durchlaufen externe {cfg.cvSplits||5}-Fold Cross-Validation für echte Out-of-Fold CATE-Predictions. DML/DR-Modelle nutzen zusätzlich internes Cross-Fitting (cv={cfg.cvSplits||5}) für die Nuisance-Residualisierung innerhalb jedes äußeren Folds.</Info>
...
const TuningPlanPreview = ({models, perRole, perLearner, trials, cv, isBoth}) => {
  const plan = computeTuningPlan(models, perRole, perLearner);
  // "Beide"-Modus: Optuna verdoppelt Trials intern, um beiden Learner-Familien Budget zu geben
  const bothMultiplier = isBoth ? 2 : 1;
  const nTrials = (trials || 100) * bothMultiplier;
  const nCv = cv || 3;
  ...
  const totalFits = plan.reduce((a,t) => a + nTrials * fitsForTask(t.sig), 0);
  ...
  const fitsDetail = (sig) => {
    const fam = sig.split("__")[0];
    if (fam==="grouped_outcome"||fam==="grouped_outcome_regression") return nTrials+"T × "+nCv+"F × "+K+"G";
    if (fam==="pseudo_effect") return nTrials+"T × ("+(nCv*K)+"obj + "+(nCv*2+2)+"nuis)";
    return nTrials+"T × "+nCv+"F";
  };
  ...
  return (
    <div>
      ...
      <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
        ...
        {isBoth && <span style={{display:"block",marginTop:6,color:"#7a5a00",background:"#fffbeb",padding:"4px 8px",borderRadius:6,border:"1px solid #e8d49c",fontSize:10.5}}>
          <strong>CatBoost &amp; LGBM aktiv:</strong> Trials sind verdoppelt ({(trials||100)}T × 2 = {nTrials}T), damit beide Learner-Familien ausreichend Budget bekommen.
        </span>}
      </div>
      <div style={{border:"1px solid #ede6e7",borderRadius:8,overflow:"hidden"}}>
        ...
        {plan.map((t,i) => {
          ...
          const rowFits = nTrials * fitsForTask(t.sig);
          return (
...
const FinalTuningPlanPreview = ({models, fmtEnabled, fmtModels, fmtSingleFold, fmtTrials, fmtCv, mcIters, isBoth}) => {
  // "Beide"-Modus: FMT-Trials werden auch intern verdoppelt
  const bothMultiplier = isBoth ? 2 : 1;
  ...
  return (
    <div>
      ...
      <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
        ...
        {isBoth && <span style={{display:"block",marginTop:6,color:"#7a5a00",background:"#fffbeb",padding:"4px 8px",borderRadius:6,border:"1px solid #e8d49c",fontSize:10.5}}>
          <strong>CatBoost &amp; LGBM aktiv:</strong> FMT-Trials sind verdoppelt ({(fmtTrials||50)}T × 2 = {nTrials}T), damit beide Learner-Familien ausreichend Budget bekommen.
        </span>}
      </div>
      <div style={{border:"1px solid #ede6e7",borderRadius:8,overflow:"hidden"}}>
...
const PModels = ({cfg,set,sp,setSp,spFmt,setSpFmt}) => {
  ...
  return (<><Sec title="Learner"><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
          {[
            {key:"catboost",logos:[CB_LOGO],name:"CatBoost",sub:"Schneller bei kategorialen Features"},
            {key:"lgbm",logos:[LGBM_LOGO],name:"LightGBM",sub:"Schneller, DART-Boosting als Default"},
            {key:"both",logos:[CB_LOGO,LGBM_LOGO],name:"CatBoost & LGBM",sub:"Optuna wählt pro Task"},
          ].map(bl => {
            ...
            return (
              <button key={bl.key} onClick={()=>set({...cfg,baseLearner:bl.key,blFixed:{},fmtFixed:{}})}
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:bl.logos.length>1?10:0,height:40}}>
                  {bl.logos.map((lg,i)=>(
                    <img key={i} src={lg} alt={bl.name} style={{height:bl.logos.length>1?32:40,objectFit:"contain",filter:active?"none":"grayscale(40%) opacity(0.6)"}}/>
                  ))}
                </div>
        ...
        </div>{_isBoth && <div style={{background:"#fff8e7",border:"1px solid #e8d49c",borderRadius:10,padding:"12px 16px",marginTop:12,fontSize:12,color:C.textSec,lineHeight:1.55}}>
          <strong style={{color:"#7a5a00"}}>So funktioniert &quot;CatBoost &amp; LGBM&quot;:</strong> Optuna wählt pro Tuning-Task (model_y, model_t, model_final, ...) kategorisch zwischen LightGBM und CatBoost. TPE lernt anhand der Trial-Ergebnisse, welcher Learner für diesen Task besser abschneidet, und konzentriert die Suche entsprechend. Am Ende kann jede Rolle einen anderen Learner nutzen — z.B. CatBoost für model_y, LightGBM für model_t. Die Trial-Zahl wird automatisch verdoppelt, damit pro Learner-Familie die gleiche Tuning-Qualität erreicht wird.
        </div>}{!cfg.tuningEnabled && <><Divider/><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12,flexWrap:"wrap"}}><span style={{fontSize:13,fontWeight:600,color:C.dark}}>Nuisance-Hyperparameter</span><span style={{fontSize:10,background:"#fffbeb",color:"#7a5a00",padding:"3px 12px",borderRadius:12,border:"1px solid #e8d49c",fontWeight:600,letterSpacing:0.2}}>Aktiv – Tuning ist deaktiviert</span></div><Info>Tuning ist deaktiviert. Diese Parameter werden direkt für alle Nuisance-Modelle (Outcome, Propensity) verwendet.</Info>{_isBoth ? (
...
const PExplain = ({cfg,set}) => (<>
  <Sec title="Methode" accent="#D4A853">
    ...
    {cfg.explEnabled&&<>
      <Divider/>
      <Info>Dreistufiger Fallback: ① EconML SHAP-Plots → ② generischer SHAP → ③ Permutation-Importance. Die Pipeline wählt automatisch die beste verfügbare Methode. Für gezielte Methodenwahl steht der CLI-Runner <code style={{background:"rgba(255,255,255,0.5)",padding:"1px 5px",borderRadius:3}}>run_explain.py --method shap|permutation</code> zur Verfügung.</Info>
    </>}
  </Sec>
...
const buildYaml = (cfg, sp, spFmt) => {
  ...
  if(bl === "both") {
    // Nested: {lgbm: {...}, catboost: {...}}
    const lgbmF = blF.lgbm||{}, cbF = blF.catboost||{};
    if(Object.keys(lgbmF).length > 0 || Object.keys(cbF).length > 0) {
      a("  fixed_params:");
...
export default function App() {
  ...
  const estimateFits = () => {
    ...
    if (cfg.tuningEnabled) {
      ...
      const tr = cfg.tuningTrials||100, tc = cfg.tuningSingleFold ? 1 : cv;
    ...
    // 5. Training — ALLE Modelle durchlaufen externe K-Fold CV
    m.forEach(x => {
    ...
    // 6. DRTester Nuisance (cv=5, 2 Modelle, train+val)
    f += 4 * 5;
    ...
    // Ohne Bundle: Champion-Refit
      ...
      // Champion braucht Refit (ein Modell)
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",fontFamily:"'DM Sans',system-ui,-apple-system,sans-serif",fontSize:15,color:"#24292f",background:"#faf7f8"}}>
      ...
      <main style={{flex:1,maxWidth:960,padding:"36px 52px 80px",margin:"0 auto"}}>
        ...
        {pg==="datafiles" && <PData cfg={cfg} set={set} setCfg={set} activeBase={activeBase} setActiveBase={setActiveBase} activeAddons={activeAddons} setActiveAddons={setActiveAddons} view="files"/>}
        {pg==="template" && <PData cfg={cfg} set={set} setCfg={set} activeBase={activeBase} setActiveBase={setActiveBase} activeAddons={activeAddons} setActiveAddons={setActiveAddons} view="template"/>}

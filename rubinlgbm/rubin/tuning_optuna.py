    ...
    if base_type == "both":
        ...
        if chosen not in ("lgbm", "catboost"):
            ...
            logging.getLogger("rubin.tuning").warning(
                "base_type='both' aber kein '_learner_type' in params — Fallback auf 'lgbm'. "
                "Das deutet auf ein fehlendes Tuning oder eine Rolle ohne getunte Params hin."
            )
            chosen = "lgbm"
        base_type = chosen
...
class BaseLearnerTuner:
    ...
    def _tune_task(self, task: TuningTask, X: pd.DataFrame, Y: np.ndarray, T: np.ndarray, shared_params: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if (self.cfg.base_learner.type or "").lower() == "both":
            _n_trials *= 2

        study.optimize(objective, n_trials=_n_trials, timeout=self.cfg.tuning.timeout_seconds,

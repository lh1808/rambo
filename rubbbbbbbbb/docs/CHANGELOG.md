...
- **Dokumentation an die neue Penalty-Methodik angeglichen (Konsistenz-Pass).** Nach der Preset-Korrektur (Nuisance-Penalty aus, Regularisierung auf der Finalstufe) widersprachen mehrere Dok-Stellen noch der neuen Linie: BLT-Penalty wurde weiterhin mit „0.2–0.5"/„0.3 empfohlen" beworben (`konfiguration.md`, `config_reference_all_options.yml`, `rubin_overview.html`), Preset-Tabellen zeigten die alten Werte (BLT p=0.2/0.4, FMT-Stark p=0.6/t=0.03 statt p=0.5/t=0.05) in `tuning_optuna.md` (2 Tabellen) und `rubin_overview.html`, die „Wann aktivieren"-Empfehlung in `konfiguration.md` riet sogar, die *Nuisance*-Penalty bei schwacher Qini einzuschalten (das Gegenteil der Analyse), und die BLT-Tuning-UI (`11_page_tuning.jsx`) erkannte noch alte Preset-Werte (0.2/0.4) und framte die Nuisance-Penalty als empfohlen. Alle Stellen geradegerückt: BLT-Penalty wird durchgängig als „Default/empfohlen 0" beschrieben (Cross-Fitting + `mc_iters` fangen Overfitting ab; Bach et al. 2024), die Regularisierung auf die CATE-Finalstufe verortet, alle Preset-Tabellen auf die neuen Werte (BLT aus, FMT/CFT 0.3/0.5 @ 5% mit Cap) aktualisiert, die „Wann aktivieren"-Empfehlung korrigiert und die UI-Help-Texte + die stale Preset-Erkennung in `11_page_tuning.jsx` angepasst (Bundle neu gebaut). `settings.py`-Kommentar der BLT-Config ergänzt. Die Finalstufen-Empfehlungen (CFT/`model_final`: „0.3"/„0.2–0.5") bleiben korrekt. Reiner Doku-/UI-Text — kein Produktivcode, Tests unberührt.
- **Regularisierungs-Presets methodisch geradegerückt (Verhaltensänderung der UI-Presets).** Die Presets „Moderat"/„Stark" aktivierten bisher eine Train-Val-Gap-Penalty auf die **Nuisance**-Stufe (BLT: `penalty` 0.2 bzw. 0.4) und verschärften bei „Stark" zusätzlich deren Toleranz (0.15 → 0.08) — also mehr Underfitting-Druck genau dort, wo Cross-Fitting + `mc_iters` Overfitting bereits abfangen und eine Nuisance-Penalty redundant ist (Bach et al. 2024: Unterfitting der Nuisances verursacht Bias trotz Orthogonalität). Neu: Beide Presets setzen die **BLT-Penalty auf 0** (Nuisance-Overfitting wird dem Cross-Fitting überlassen; eine Nuisance-Penalty bleibt über die YAML konfigurierbar), und die Regularisierungsstärke wirkt nur noch auf die **CATE-Finalstufe** (FMT/CFT) — „Moderat" 0.3, „Stark" 0.5, beide bei 5% Toleranz (keine Toleranz-Verschärfung mehr). Betrifft `03_presets.jsx` (reg_moderate/reg_strong) und `09_page_config.jsx` (REG_PRESETS + erklärender Info-Text); Bundle via `build_ui.py` neu gebaut. Reiner UI-Default — kein Produktivcode, Tests unberührt (162). Die ausgelieferten Config-Defaults (`overfit_penalty: 0.0` in allen Stufen) waren bereits korrekt; korrigiert wird, welche Werte die Presets *setzen*.

...
causal_forest:
  ...
  overfit_penalty: 0.0    # Train-Val-Gap-Penalty (empfohlen: 0.3)
  overfit_tolerance: 0.05 # Relativer Gap-Toleranz (5%)
  tune_models: []
  tune_max_rows: null
  search_space: {}
```
...
- `overfit_penalty`: Train-Val-Gap-Penalty (0 = deaktiviert, empfohlen: 0.3). Bestraft Konfigurationen mit hohem In-Sample vs. OOF-Score-Gap.
- `overfit_tolerance`: Relativer Gap-Toleranzwert (Default 0.05 = 5%).
...
- `overfit_penalty`: Train-Val-Gap-Penalty für model_final (0=deaktiviert). Bestraft Konfigurationen, deren Score auf Train deutlich besser ist als auf Val (OOF). Tolerance ist relativ: BLT Default 0.15 (15% Gap — DML kompensiert Nuisance-Overfitting), FMT Default 0.05 (5% Gap — CATE-Signal schwächer). Formel: `adjusted = val_score - penalty × scale × min(max(0, relative_gap - tolerance), max_penalized_gap)` mit `scale = |val_score|`, `relative_gap = gap / scale`. Der `overfit_max_penalized_gap`-Cap (Default 1.0) saturiert den Abzug.
...
Konfiguration:
- `tuning.overfit_penalty`: Stärke der Penalty auf die **Nuisance**-Stufe. Default 0 — und so empfohlen: Cross-Fitting + `mc_iters` fangen Nuisance-Overfitting bereits ab, eine Penalty hier ist redundant und drückt Richtung Unterfitting (Bach et al. 2024). Nur experimentell aktivieren; die Regularisierung gehört auf die CATE-Finalstufe (FMT/CFT).
...
**Wann aktivieren:** Auf der **Finalstufe** (FMT/CFT) bei Verdacht auf CATE-Overfitting (gute In-Sample-, aber schwache OOF-Qini). Auf der **Nuisance**-Stufe (BLT) wird die Penalty *nicht* empfohlen: Schwache Qini bei guten BLT-Scores ist meist kein Nuisance-Overfitting (das fängt Cross-Fitting ab), sondern deutet auf zu starke Nuisance-Regularisierung, instabile Propensities (nahe 0/1) oder ein Magnitude-vs-Ranking-Problem — eine BLT-Penalty würde das eher verschärfen.

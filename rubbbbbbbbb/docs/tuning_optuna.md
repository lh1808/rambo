...
| **Moderat** | p=0 (aus) | p=0.3, t=0.05 | Nuisance-Penalty aus (Cross-Fitting genügt); CATE-Stufe moderat reguliert |
| **Stark** | p=0 (aus) | p=0.5, t=0.05 | Nuisance-Penalty aus; stärkere CATE-Regularisierung (vom Cap geschützt) |

Die Presets bestrafen bewusst nur die CATE-Finalstufe (FMT/CFT): Nuisance-Overfitting wird bereits durch Cross-Fitting + `mc_iters` abgefangen, sodass eine BLT-Penalty redundant ist und nur Richtung Unterfitting drückt (Bach et al. 2024). „Stark" erhöht daher die Final-Penalty (0.3 → 0.5), nicht die Toleranz. Eine BLT-Penalty bleibt über die YAML konfigurierbar, ist aber nicht der empfohlene Default.
...
|---|---|---|---|---|
| **BLT** | p=0 (aus) | p=0 (aus) | 0.15 (15%) | Cross-Fitting + mc_iters fangen Nuisance-Overfitting ab — Penalty redundant; via YAML aktivierbar |
| **FMT/CFT** | p=0.3, t=0.05 | p=0.5, t=0.05 | 0.05 (5%) | CATE-Stufe: milde Penalty gegen echtes Overfitting, vom `overfit_max_penalized_gap`-Cap gegen Entgleisen geschützt |
...
**Hintergrund:** Bei Double/Debiased ML (Chernozhukov et al., 2016) dienen die BLT-Nuisance-Modelle dazu, Residuals Ỹ = Y - E[Y|X] und T̃ = T - E[T|X] zu berechnen. Wenn diese Nuisance-Modelle overfitten, absorbieren sie kausales Signal — die Residuals werden systematisch zu klein, und model_final (CATE) findet weniger Heterogenität. **Wichtig:** Cross-Fitting (OOF-Vorhersagen) und `mc_iters` (Mittelung über mehrere Fold-Partitionen) fangen genau dieses Nuisance-Overfitting bereits ab. Eine zusätzliche Penalty auf die *Nuisance*-Stufe ist daher weitgehend redundant und kann Richtung Unterfitting drücken (Bach et al. 2024: unterangepasste Nuisances verursachen Bias trotz Orthogonalität). Die Penalty entfaltet ihren Nutzen vor allem auf der **CATE-Finalstufe** (FMT/CFT) gegen echtes Heterogenitäts-Overfitting — daher setzen die Presets sie nur dort ein und lassen die BLT-Stufe unbestraft.
...
```yaml
tuning:
  overfit_penalty: 0.0    # Nuisance-Penalty: Default 0 (Cross-Fitting genügt); nur experimentell aktivieren
...
| **Moderat** | 0.0 | 0.15 | Nuisance-Penalty aus (Cross-Fitting + mc_iters genügen) |
| **Stark** | 0.0 | 0.15 | Nuisance-Penalty aus — Regularisierung wirkt auf die Finalstufe |

Die FMT/CFT-Presets (CATE-Regularisierung) tragen die Regularisierungslast: Moderat p=0.3/t=0.05, Stark p=0.5/t=0.05 — beide via `overfit_max_penalized_gap` (Default 1.0) gegen Sign-Flip gedeckelt. Siehe Abschnitt „BLT vs. FMT Defaults" für die vollständige Tabelle.

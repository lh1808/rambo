...
const DEFAULT_CFG = {studyType:"rct",expName:"rubin",seed:42,tuningSeed:18,models:["NonParamDML","DRLearner","SLearner","TLearner","XLearner","ParamDML","CausalForestDML","CausalForest"],baseLearner:"catboost",ensembleEnabled:true,tuningEnabled:false,tuningTrials:50,tuningSingleFold:false,tuningModels:[],fmtEnabled:false,fmtModels:[],fmtSingleFold:false,fmtTrials:50,fmtMaxRows:0,fmtOverfitPenalty:0.0,fmtOverfitTolerance:0.05,fmtOverfitMaxGap:1.0,fmtScorer:"auto",cfTune:false,cfTrials:50,cfSingleFold:false,cfScorer:"auto",cfTuneModels:[],cfOverfitPenalty:0.0,cfOverfitTolerance:0.05,cfOverfitMaxGap:1.0,bundleEnabled:false,bundleDir:"runs/bundles",bundleChallengers:true,bundleMlflow:true,surrEnabled:false,surrMinLeaf:50,surrLeaves:31,surrDepth:0,fsEnabled:false,fsMethods:["catboost_importance"],fsCorrThresh:0.9,fsMaxFeatures:77,downsample:false,dfFrac:0.1,reduceMem:true,validateOn:"cross",cvSplits:5,treatmentType:"binary",refGroup:0,selMetric:"qini",higherBetter:true,refitChamp:true,manualChamp:null,hasNaN:false,nanCols:[],explEnabled:false,explSampleSize:10000,explTopN:20,shapModels:[],shapBins:10,histScoreName:"historical_score",histScoreCol:"S",histScoreHigher:true,outputDir:"",blFixed:{},fmtFixed:{},cfFixed:{},tuningTimeout:0,tuningMaxRows:0,fmtTimeout:0,maxPredRows:0,parallelLevel:3,workDir:null,mcIters:null,mcAgg:"mean",dmlCrossfitFolds:5,overfitPenalty:0.0,overfitTolerance:0.15,overfitMaxGap:1.0};
...
const ADDON_PRESETS = [
  ...
  {key:"reg_moderate",label:"Moderat",desc:"Nuisance-Penalty aus (Cross-Fitting + mc_iters fangen Overfitting ab). Final (FMT/CFT): 30% Penalty, 5% Gap toleriert.",group:"regularization",
    cfg:{overfitPenalty:0,overfitTolerance:0.15,fmtOverfitPenalty:0.3,fmtOverfitTolerance:0.05,cfOverfitPenalty:0.3,cfOverfitTolerance:0.05}},
  {key:"reg_strong",label:"Stark",desc:"Nuisance-Penalty aus. Final (FMT/CFT): 50% Penalty, 5% Gap — stärkere CATE-Regularisierung bei kleinen Datensätzen oder instabilen Vorläufen.",group:"regularization",
    cfg:{overfitPenalty:0,overfitTolerance:0.15,fmtOverfitPenalty:0.5,fmtOverfitTolerance:0.05,cfOverfitPenalty:0.5,cfOverfitTolerance:0.05}},
  // ── Stabilität ──
...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Tuning-Regularisierung" accent="#9B111E">
    <Info>Steuert, wie aggressiv Overfitting der CATE-Finalstufe (FMT/CFT) beim Tuning bestraft wird. Die Nuisance-Stufe (BLT) lassen die Presets bewusst unbestraft: Cross-Fitting + mc_iters fangen Nuisance-Overfitting bereits ab, eine zusätzliche Penalty dort würde Richtung Unterfitting drücken. Eine Nuisance-Penalty bleibt bei Bedarf über die YAML konfigurierbar. „Stark" erhöht die Final-Penalty (0.3 → 0.5), nicht die Toleranz.</Info>
    {(()=>{
      const REG_PRESETS = [
        {key:"moderate", label:"Moderat", desc:"Nuisance-Penalty aus (Cross-Fitting fängt Overfitting ab) · Final (FMT/CFT): 30% Penalty, 5% Gap. Empfohlen bei moderater Stichprobengröße.",
         blt_p:0, blt_t:0.15, fmt_p:0.3, fmt_t:0.05, cft_p:0.3, cft_t:0.05},
        {key:"strong", label:"Stark", desc:"Nuisance-Penalty aus · Final (FMT/CFT): 50% Penalty, 5% Gap. Stärkere CATE-Regularisierung für kleine Stichproben.",
         blt_p:0, blt_t:0.15, fmt_p:0.5, fmt_t:0.05, cft_p:0.5, cft_t:0.05},
      ];
...
const buildYaml = (cfg, sp, spFmt) => {
  ...
  if((cfg.models||[]).includes("CausalForestDML") || (cfg.models||[]).includes("CausalForest")) {
    ...
    if((cfg.cfOverfitPenalty||0)>0){a(`  overfit_penalty: ${cfg.cfOverfitPenalty}`);a(`  overfit_tolerance: ${cfg.cfOverfitTolerance===undefined?0.05:cfg.cfOverfitTolerance}`);a(`  overfit_max_penalized_gap: ${cfg.cfOverfitMaxGap===undefined?1.0:cfg.cfOverfitMaxGap}`);}
  ...
  if(cfg.tuningEnabled) {
    ...
    if(cfg.tuningSingleFold) a("  single_fold: true");if((cfg.overfitPenalty||0)>0){a(`  overfit_penalty: ${cfg.overfitPenalty}`);a(`  overfit_tolerance: ${cfg.overfitTolerance===undefined?0.15:cfg.overfitTolerance}`);a(`  overfit_max_penalized_gap: ${cfg.overfitMaxGap===undefined?1.0:cfg.overfitMaxGap}`);}
  ...
  if(cfg.fmtEnabled) {
    ...
    if((cfg.fmtOverfitPenalty||0)>0){a(`  overfit_penalty: ${cfg.fmtOverfitPenalty}`);a(`  overfit_tolerance: ${cfg.fmtOverfitTolerance===undefined?0.05:cfg.fmtOverfitTolerance}`);a(`  overfit_max_penalized_gap: ${cfg.fmtOverfitMaxGap===undefined?1.0:cfg.fmtOverfitMaxGap}`);}
...
function App() {
  ...
  const estimateFits = () => {
    ...
    if (cfg.tuningEnabled) {
      ...
      // EINMALIG vor den Trials berechnet (innerCv×2 OOF + bis zu 2 Fallback) — sie hängt
      // nur von den getunten Outcome-Nuisances ab, nicht von den Trial-Params. Pro Trial
      // laufen nur die K CATE-Regressoren über tc Folds.

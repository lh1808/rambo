...
- **Native Uplift-Plots:** Qini-Kurve (+ Perfect-Kurve, name-Parameter), Uplift-by-Percentile (+ strategy/kind/string_percentiles), Treatment-Balance (+ random-Parameter). Feature-complete gegenüber sklift v0.5.1.
- **CausalForest-Tuning (CFT):** Optuna TPE über 8-9 Wald-Parameter (n_estimators, max_depth, min_samples_leaf, etc.). CausalForestDML via est.score() (R-Loss), CausalForest via vorberechnete Residuen. Ersetzt EconML tune() Grid-Search.
- **Unified Tuning-Logging:** Alle 3 Wellen: `"XXX '%s': Starte %d Trials (parallel=%d, %s, n_jobs=%d)."` — konsistentes Format.
...
- **DRLearner Fit-Count:** `mc*cv*2+1` → `mc*cv*(1+K)+1` (propensity + K×regression pro Fold).
...
### Entfernt
- **scikit-uplift (sklift):** Komplett entfernt (Import, Dependencies, sklearn-Shim). Ersetzt durch native Plots.

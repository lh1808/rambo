...
**Numerische Referenz-Absicherung:** Die Metriken sind gegen unabhängige Referenzen gepinnt (`tests/test_uplift_metrics_reference.py`): Qini/AUUC exakt gegen die sklift-Kurvenflächen (Integral inkl. Ursprung; Toleranz nur für die dokumentierte Jitter-Präzisionsgrenze bei Score-Abständen < 1e-9·max|score|), `uplift_at_k` exakt gegen sklift, `policy_value` gegen die Differenz-in-Means-Handreferenz; die MT-Arm-Kurven sind bitidentisch zur BT-Maschinerie auf der jeweiligen Teilmenge und erben deren Verifikation transitiv.

**Konsistenz uplift@k ↔ Perzentil ↔ Qini-Kurve:** Alle drei nutzen dieselbe Sortierung und denselben Floor-Schnitt `n_size = int(k·n)` (identisch zu `sklift.uplift_at_k(strategy='overall')` und zu den `linspace`-Bin-Kanten des Perzentil-Plots). Dadurch gilt exakt — auch wenn die Eval-Set-Größe kein Vielfaches von `n_bins` ist:
- `uplift@10%` = erster Balken des Uplift-by-Percentile-Plots (`uplift@20%/50%` = kumulative Dezil-Kanten),
- Qini-Kurve am selben Schnitt: `Q(k) = Y_t(k) − Y_c(k)·N_t(k)/N_c(k) = N_treat(k)·(rate_t − rate_c) = N_treat(k)·uplift@k`.

Die drei Darstellungen sind damit ineinander überführbar. (Davon abzugrenzen ist die EconML-DRTester-Qini-Kurve — Policy-Value über „Percentage treated" —, die einer anderen Methodik folgt und nicht mit `uplift@k` übereinstimmt.)
...
Bei mehreren Eingabedateien kann über `data_prep.eval_file_index` eine oder mehrere Dateien als Evaluationsgrundlage festgelegt werden. Die DataPrep-Pipeline erzeugt dafür eine Boolean-Maske (`eval_mask.npy`), die in der Analyse-Pipeline über `data_files.eval_mask_file` geladen wird.

**Training auf allen Zeilen, Evaluation auf dem Mask-Subset:** TMES trainiert und
cross-predictet bewusst auf **allen** Zeilen — inklusive der Eval-Zeilen — für maximale
statistische Power (es gibt keinen Holdout-Split). Die `eval_mask` greift erst bei der
Evaluation: Uplift-Metriken, DRTester-Plots und Policy Values werden ausschließlich auf
den Mask-Zeilen berechnet. Die `Predictions_*`-Werte dort sind Out-of-Fold (das jeweilige
CV-Fold-Modell hat die Zeile nicht gesehen), aber die Zeilen selbst fließen in K−1
Fold-Trainings und in den Bundle-Refit ein — TMES ist damit **kein** Ersatz für ein
echtes externes Holdout. Für strikt ungesehene Eval-Daten ist der External-Eval-Pfad
(`validate_on: external`) zu verwenden. Die Calibration-Quantil-Cuts des DRTesters
werden aus der vollen `Train_*`-Spalte (alle Zeilen) gebildet; nur die Gruppen-Zuordnung
und Gate-Werte laufen über das Mask-Subset.

**MLflow-Log-Parameter:** `tmes_total_n` und `tmes_eval_n` werden beim Run geloggt, damit die Split-Größen nachvollziehbar sind.

**Die Maske überlebt** Subsampling (`df_frac`) und wird positionskonsistent angewendet. Leere oder all-True-Masken werden mit Warnung ignoriert. Ist `validate_on: external` gesetzt, wird eine zusätzlich vorhandene `eval_mask` mit Warnung ignoriert.

**Unterschied zu External Eval:**
- External Eval: separate `eval_x/t/y_file` Parquet-Dateien; Modelle sehen die Eval-Zeilen **nie** (echtes Holdout, Single-Fit auf Train)
- TMES: Maske über derselben Datenmenge; Training auf allen Zeilen, Evaluation OOF auf dem Mask-Subset
...
Mit `data_prep.balance_treatments: true` werden die Dateien per Random-Downsampling auf eine gemeinsame Ziel-Treatment-Rate angeglichen. Die Ziel-Rate wird nach **maximaler effektiver Stichprobe** `N_keep · p·(1−p)` gewählt — nicht nach minimalem Zeilenverlust. Hintergrund: Die Varianz eines Effekt-/Uplift-Schätzers skaliert mit `1/(N·p·(1−p))`, der kleinere Arm limitiert die Präzision. Eine balanciertere Rate kann daher gewinnen, obwohl sie mehr Zeilen verliert. Optionale Schranken begrenzen, wie klein ein Arm werden darf (`balance_min_arm_abs`/`_frac`), wie viel maximal verworfen wird (`balance_max_loss_frac`) und ob zusätzlich Zwischenraten Richtung 0.5 geprüft werden (`balance_target_grid_step`).
...
3. `evaluate_cate_with_plots(fitted_tester=...)` übernimmt die einmal berechneten Val-Nuisance-Outcomes (`dr_val_`, `ate_val`, `Dval`) und tauscht nur die CATE-Predictions (`cate_preds_val_`, `cate_preds_train_`) aus. Train-DR-Outcomes (`dr_train_`) werden bewusst **nicht** berechnet (`dr_train_=None`): EconMLs `evaluate_blp/cal/uplift` ziehen die Quantil-Cuts aus `cate_preds_train_` und die Gate-Werte aus `dr_val_`, lesen `dr_train_` aber nie (nur ein `hasattr`-Guard) — eine Train-DR-Berechnung wäre toter, fehleranfälliger Code.

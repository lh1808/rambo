...
const ADDON_PRESETS = [
  ...
  {key:"fmt",label:"FMT",desc:"CATE-Effektmodell optimieren (50 Trials)",group:"tuning",
    cfg:{fmtEnabled:true,fmtTrials:50,fmtModels:["NonParamDML","DRLearner"]}},
  {key:"fmt_intensiv",label:"FMT Intensiv",desc:"160 Trials – tiefe CATE-Optimierung",group:"tuning",
...
const PData = ({cfg,set,setCfg,activeBase,setActiveBase,activeAddons,setActiveAddons,setSp,setSpFmt,view}) => {
  ...
  const handleYamlImport = (text) => {
    try {
      ...
      if (_importedSpFmt) _importedMsgs.push("Suchraum FMT");
  ...
  return (
    <>
      {view==="files"&&<Sec title="Analysedaten">
        ...
        <Divider/>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
          <div style={{fontSize:13,fontWeight:600,color:C.dark}}>Evaluationsdaten (separater Datensatz, optional)</div>
          <Toggle label="External Eval aktivieren" checked={cfg.validateOn==="external"} onChange={v=>set({...cfg,validateOn:v?"external":"cross"})} help="Aktiviert die Evaluation auf einem separaten Datensatz anstelle von Cross-Validation auf den Trainingsdaten."/>
        </div>
        {cfg.validateOn==="external"
          ? <Info type="success">✓ External Eval aktiv. Training auf den obigen Daten, Evaluation auf den unten angegebenen Dateien. Kein Data-Leakage.</Info>
          : <Info>Alternativ kannst du die Eval-Dateien hier eintragen und anschließend „External Eval aktivieren" einschalten. Ohne Aktivierung werden die Dateien ignoriert und Cross-Validation auf den Trainingsdaten genutzt.</Info>}
        {tab==="Dateipfade" ? (
          <Row><Col>
            <Inp label="Eval X-Datei" placeholder="runs/data/X_eval.parquet" value={cfg.eval_x_file||""} onChange={v=>set({...cfg,eval_x_file:v,...(v&&cfg.validateOn!=="external"?{validateOn:"external"}:{})})}/>
            <Inp label="Eval T-Datei" placeholder="runs/data/T_eval.parquet" value={cfg.eval_t_file||""} onChange={v=>set({...cfg,eval_t_file:v,...(v&&cfg.validateOn!=="external"?{validateOn:"external"}:{})})}/>
          </Col><Col>
            <Inp label="Eval Y-Datei" placeholder="runs/data/Y_eval.parquet" value={cfg.eval_y_file||""} onChange={v=>set({...cfg,eval_y_file:v,...(v&&cfg.validateOn!=="external"?{validateOn:"external"}:{})})}/>
            ...
            {[{label:"Eval X-Datei (Features)",key:"eval_x_file"},{label:"Eval T-Datei (Treatment)",key:"eval_t_file"},{label:"Eval Y-Datei (Outcome)",key:"eval_y_file"},{label:"Eval S-Datei (optional)",key:"eval_s_file"}].map(f => {
              const uploadEvalFile = async (file) => {
                ...
                try {
                  ...
                  const data = await res.json();
                  if(data.status === "done" && data.path) {
                    // Bei eval_x/t/y-Upload automatisch external aktivieren
                    const autoExternal = ["eval_x_file","eval_t_file","eval_y_file"].includes(f.key) && cfg.validateOn!=="external";
                    set({...cfg, [f.key]: data.path, ...(autoExternal?{validateOn:"external"}:{})});
                  } else {
                    alert("Upload fehlgeschlagen: " + (data.message || "Unbekannter Fehler"));
                  }
                } catch(e) {
      ...
      {view==="files"&&<Sec title="Historischer Score (Benchmark)">
        <Info>Optional: Konfiguration für den historischen Score (S-Datei). Der Benchmark-Vergleich läuft automatisch, sobald eine S-Datei angegeben ist — kein zusätzliches Preset nötig.</Info>
        {cfg.s_file ? (<>
          <Info type="success">✓ Benchmark-Vergleich aktiv. Modelle werden gegen den historischen Score aus <code style={{background:"rgba(255,255,255,0.5)",padding:"1px 5px",borderRadius:3}}>{cfg.s_file}</code> gemessen (Policy-Value-Vergleich + Uplift-Metriken).</Info>
          <Row>
        ...
        </>) : <Info type="warn">Keine S-Datei angegeben. Kein Benchmark-Vergleich — fügst du oben bei „Optionale Dateien" eine S-Datei hinzu, wird der Vergleich automatisch aktiviert.</Info>}
      </Sec>}
...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Allgemein">
    ...
    <div style={{background:"#f6f8fa",border:"1px solid "+C.border,borderRadius:8,padding:"10px 14px",fontSize:11,fontFamily:"'JetBrains Mono',monospace",lineHeight:1.7}}>
      ...
      <div style={{marginTop:6,color:"#57606a",fontSize:10}}>MLflow: {workDirInfo.mlruns_dir}</div>
      <div style={{color:"#57606a",fontSize:10}}>Uploads: {workDirInfo.upload_dir}</div>
      <div style={{color:"#57606a",fontSize:10}}>Cache: {workDirInfo.cache_dir}</div>
    </div>
    <Inp label="work_dir (optional)" type="text" value={cfg.workDir||""} onChange={v=>set({...cfg,workDir:v||null})} help="Leer = Env-Variable oder ./runs"/>
    </>}
  </Sec>
...
const TuningPlanPreview = ({models, perRole, perLearner, trials, cv, isBoth, isRct}) => {
  ...
  const fitsForTask = (sig) => {
    const fam = sig.split("__")[0];
    if (fam==="grouped_outcome"||fam==="grouped_outcome_regression") return nCv * K;
  ...
  const sigLabel = sig => {
    ...
    if(fam==="grouped_outcome") return "Grouped Outcome (Cls.)";
  ...
  const fitsDetail = (sig) => {
    ...
    if (fam==="grouped_outcome"||fam==="grouped_outcome_regression") return nt+"T × "+nCv+"F × "+K+"G";
  ...
  return (
    <div>
      ...
      <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
        ...
        {isBoth && <span style={{display:"block",marginTop:6,color:"#7a5a00",background:"#fffbeb",padding:"4px 8px",borderRadius:6,border:"1px solid #e8d49c",fontSize:10.5}}><strong>CatBoost &amp; LGBM aktiv:</strong> FMT-Trials sind verdoppelt ({(fmtTrials||50)}T × 2 = {nTrials}T), damit beide Learner-Familien ausreichend Budget bekommen.</span>}
      </div>
      <div style={{border:"1px solid #ede6e7",borderRadius:8,overflow:"hidden"}}>
...
function App() {
  ...
  const estimateFits = () => {
    ...
    // "CatBoost & LGBM" verdoppelt auch die FMT-Trials
    if (cfg.fmtEnabled) {

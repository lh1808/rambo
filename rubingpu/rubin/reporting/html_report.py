...
class ReportCollector:
    ...
    def add_config(self, cfg) -> None:
        ...
        self.config_summary = {
            ...
            "tuning_metric": getattr(cfg.tuning, "metric", "pr_auc"),
...
def generate_html_report(collector: ReportCollector, output_path: str) -> str:
    ...
    for lbl, val in [
        ...
        ("Base Learner", {"both": "CatBoost & LGBM", "catboost": "CatBoost", "lgbm": "LightGBM"}.get(cs.get("base_learner", ""), cs.get("base_learner", "–"))),
        ("Validierung", (
    ...
    if cs.get("tuning_enabled") and collector.tuning_scores:
        ...
        # Config info
        h += '<div class="cg">'
        h += f'<div class="cd"><div class="cd-l">Metrik</div><div class="cd-v">{escape(str(cs.get("tuning_metric","pr_auc")))}</div></div>'
        h += f'<div class="cd"><div class="cd-l">Base Learner</div><div class="cd-v">{escape({"both": "CatBoost & LGBM", "catboost": "CatBoost", "lgbm": "LightGBM"}.get(cs.get("base_learner", ""), str(cs.get("base_learner","lgbm"))))}</div></div>'
        ...
        _is_both = (getattr(cfg.base_learner, "type", "") or "").lower() == "both"
        if _is_both:

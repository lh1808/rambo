...
**Internes Cross-Fitting:** Alle DML-Modelle (NonParamDML, ParamDML, CausalForestDML) und DRLearner verwenden intern `dml_crossfit_folds` (Default: 5) für die Nuisance-Residualisierung. Das ist höher als der EconML-Default (`cv=2`) und liefert stabilere Residuals, weil jedes Nuisance-Modell auf 80% statt nur 50% der Daten trainiert wird.

```yaml
data_processing:
  dml_crossfit_folds: 5   # Interne Cross-Fitting-Folds für Nuisance-Residuals (Y_resid, T_resid)
  mc_iters: null           # Monte-Carlo-Iterationen: null=1 Durchlauf, 2-3 empfohlen für stabilere Schätzungen
  mc_agg: "mean"           # Aggregation über mc_iters: "mean" (Standard) oder "median" (robuster)
```

- `dml_crossfit_folds`: Getrennt von `cross_validation_splits` (äußere Evaluation). Steuert, wie viele Folds EconML intern für die Nuisance-Residualisierung nutzt. Höher = stabilere Residuals, aber langsamer.

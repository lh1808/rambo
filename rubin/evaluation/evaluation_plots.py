...
def _native_uplift_by_percentile(
    ...
    try:
        ...
        if strategy == "by_group":
            ...
            treat_order = treat_idx[np.argsort(-(u_arr[treat_idx] + _rng_pct.uniform(-1e-10, 1e-10, len(treat_idx))))]
            ctrl_order = ctrl_idx[np.argsort(-(u_arr[ctrl_idx] + _rng_pct.uniform(-1e-10, 1e-10, len(ctrl_idx))))]
            ...
            order = np.argsort(-(u_arr + _rng_ov.uniform(-1e-10, 1e-10, len(u_arr))))
            y_s = y_arr[order]
            t_s = t_arr[order]
            n = len(y_s)
...
def _native_treatment_balance(
    ...
    try:
        ...
        jitter = rng.uniform(-1e-10, 1e-10, len(uplift))
        order = np.argsort(-(uplift + jitter))

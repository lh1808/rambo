...
const LGBM_LOGO=`data:image/svg+xml,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 48"><polygon points="2,2 38,2 2,22" fill="%23E4432C"/><polygon points="38,14 38,34 18,24" fill="%2371B74A"/><polygon points="2,26 22,36 2,46" fill="%232F9FD1"/><polygon points="18,34 38,46 2,46" fill="%23F3AE19"/></svg>')}`;
...
const ADDON_PRESETS = [
  ...
  // ── Stabilität ──
  {key:"mc_iters",label:"MC-Iterationen",desc:"Cross-Fitting 3× wiederholen – stabilere Residuals",group:"stability",
    cfg:{mcIters:3}},
  // ── Performance ──
...
const YAML_TO_CFG = {
  ...
  "data_processing.validate_on":"validateOn","data_processing.cross_validation_splits":"cvSplits","data_processing.dml_crossfit_folds":"dmlCv","data_processing.mc_iters":"mcIters","data_processing.mc_agg":"mcAgg",
...
const PData = ({cfg,set,setCfg,activeBase,setActiveBase,activeAddons,setActiveAddons,view}) => {
  ...
  return (
    <>
      ...
      {view==="template"&&<Sec title="Config importieren" accent="#D4A853">
        ...
        <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
          ...
          >
            ...
            <div style={{fontSize:13,fontWeight:600,color:C.dark}}>config.yml hierher ziehen</div>
          ...
          >
            <div style={{fontSize:28,marginBottom:6,opacity:0.4}}>📋</div>
            <div style={{fontSize:13,fontWeight:600,color:C.dark}}>YAML einfügen</div>
      ...
      {view==="template"&&<Sec title="2. Basis-Vorlage">
      ...
      {view==="template"&&<Sec title="3. Add-ons">
        ...
        {(() => {
          const G = [
            {group:"tuning",label:"Tuning",c:"#9B111E",bg:"#fdf2f3",bd:"#e8b4b8",abg:"#fce8ea"},
            {group:"production",label:"Production & Export",c:"#1a7f37",bg:"#e8f5ec",bd:"#a3d9b1",abg:"#d4edda"},
          ];
          const M = [
            {group:"feature",label:"Feature-Selektion",c:"#0891b2",bg:"#ecfeff",bd:"#a5f3fc",abg:"#cffafe"},
            {group:"interpret",label:"Erklärbarkeit",c:"#D4A853",bg:"#fffbeb",bd:"#e8d49c",abg:"#fff8dc"},
          ];
          const B = [
            {group:"validation",label:"Validierung",c:"#6366f1",bg:"#eef2ff",bd:"#c7d2fe",abg:"#e0e7ff"},
            {group:"stability",label:"Stabilität",c:"#0969da",bg:"#ddf4ff",bd:"#b6e3ff",abg:"#c8e6ff"},
            {group:"performance",label:"Performance",c:"#57606a",bg:"#f6f8fa",bd:"#d0d7de",abg:"#eaeef2"},
          ];
...
const PConfig = ({cfg,set}) => (<>
  <Sec title="Allgemein">
    <div style={{maxWidth:200,marginBottom:14}}>
      <Inp label="Random Seed" type="number" value={cfg.seed} onChange={v=>set({...cfg,seed:v})} icon="#" help="Reproduzierbarkeit: gleicher Seed = gleiche Ergebnisse bei identischen Daten"/>
    </div>
  ...
  <Sec title="Validierungsstrategie">
    ...
    <Inp label="Anzahl Folds (K)" type="number" value={cfg.cvSplits} onChange={v=>set({...cfg,cvSplits:v})} help={cfg.validateOn==="external" ? "Fold-Anzahl für Cross-Predictions und Tuning auf den Trainingsdaten." : "Bestimmt die Fold-Anzahl für Validierung, Tuning und Cross-Predictions."}/>
    ...
    <Info>Steuert das interne Cross-Fitting der DML-Modelle (NonParamDML, ParamDML, CausalForestDML, DRLearner). Getrennt von der äußeren K-Fold-Evaluation.</Info>
    ...
    {(cfg.mcIters||0)>0 && <div style={{fontSize:11,color:"#7a5a00",background:"#fffbeb",padding:"8px 12px",borderRadius:8,marginTop:6,lineHeight:1.5}}>Cross-Fitting wird <strong>{cfg.mcIters}×</strong> wiederholt. Nuisance-Fits pro Modell: {cfg.dmlCv||5} Folds × {cfg.mcIters} Iterationen = <strong>{(cfg.dmlCv||5)*(cfg.mcIters||1)}</strong></div>}
    {(cfg.mcIters||0)>0 && <Row>
    ...
    <Expander title="Erweiterte Cross-Fitting-Parameter">
      <Inp label="Interne Crossfit-Folds" type="number" value={cfg.dmlCv||5} onChange={v=>set({...cfg,dmlCv:Number(v)})} help="Folds für Nuisance-Residuals (Y_resid, T_resid). EconML-Default=2, empfohlen=5. Mehr Folds = stabilere Residuals, aber langsamer."/>
    </Expander>
    ...
    <Divider/><div style={{background:cfg.ensembleEnabled?"rgba(155,17,30,0.04)":"transparent",border:cfg.ensembleEnabled?"1.5px solid rgba(155,17,30,0.15)":"1.5px dashed #e0d6d7",borderRadius:10,padding:"12px 14px",transition:"all 0.2s"}}><Toggle label="Ensemble (Gleichgewichtet)" checked={cfg.ensembleEnabled||false} onChange={v=>set({...cfg,ensembleEnabled:v})} help="Mittelt die CATE-Vorhersagen aller trainierten Modelle. Nimmt an der Champion-Selektion teil."/>{cfg.ensembleEnabled&&<>{(()=>{const active=(cfg.models||[]).filter(m=>av.includes(m));return active.length>=2?<div style={{fontSize:11,color:"#999",marginTop:8,lineHeight:1.8}}>Ensemble aus {active.map((m,i)=><span key={m}>{i>0&&<span style={{color:"#ddd"}}> · </span>}<span style={{fontSize:10.5,padding:"2px 8px",borderRadius:8,background:"rgba(155,17,30,0.08)",color:C.ruby,fontWeight:600,whiteSpace:"nowrap"}}>{m}</span></span>)} <span style={{fontStyle:"italic",whiteSpace:"nowrap"}}>({active.length} Modelle, je 1/{active.length})</span></div>:null})()}{(cfg.models||[]).filter(m=>av.includes(m)).length<2&&<Info type="warn">Mindestens 2 aktive Modelle nötig.</Info>}</>}</div></Sec><Sec title="Base Learner"><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {[
            ...
            return (
              ...
              const totalFits = (hasCF?12:0)+(hasGRF?12:0);
              return (<>
                ...
                <div style={{border:"1px solid #ede6e7",borderRadius:8,overflow:"hidden",marginTop:12}}>
                  ...
                  {hasGRF && <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 80px",padding:"8px 14px",fontSize:12,background:hasCF?"#fdfbfb":"#fff"}}><span style={{fontWeight:600}}>CausalForest</span><span style={{fontSize:11,color:"#2d6a4f"}}>min_samples_leaf, max_depth, max_samples, criterion</span><span style={{fontFamily:"monospace",fontWeight:700,color:"#2d6a4f"}}>24</span></div>}
                </div>
                <div style={{fontSize:10.5,color:"#888",marginTop:6}}>{hasCF && "CausalForestDML: EconML tune() (2×3×2 = 12)."} {hasGRF && "CausalForest: R-Loss Grid-Search auf erstem CV-Fold (2×3×2×2 = 24)."}</div>
              </>);
...
const buildYaml = (cfg, sp, spFmt) => {
  ...
  if((cfg.dmlCv||5) !== 5) a(`  dml_crossfit_folds: ${cfg.dmlCv}`);
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",fontFamily:"'DM Sans',system-ui,-apple-system,sans-serif",fontSize:15,color:"#24292f",background:"#faf7f8"}}>
      <nav style={{width:280,minWidth:280,background:"linear-gradient(180deg,#4a080f 0%,#6B0D15 30%,#8a0f1a 100%)",color:"#fff",position:"sticky",top:0,height:"100vh",overflowY:"auto",flexShrink:0,display:"flex",flexDirection:"column",boxShadow:"4px 0 24px rgba(107,13,21,0.15)"}}>
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"24px 20px 14px"}}>
        ...
        <div style={{borderTop:"1px solid rgba(255,255,255,0.08)",margin:"4px 28px 8px"}}/>
        <div style={{flex:1,padding:"0 6px"}}>
          {pages.map((p,idx) => {
            ...
            return (
              <div key={p.key}>
                {showGroup && <div style={{fontSize:9,fontWeight:600,textTransform:"uppercase",letterSpacing:"1px",color:"rgba(255,255,255,0.2)",padding:"6px 20px 2px",marginTop:0}}>{p.group}</div>}
                <button onClick={()=>setPg(p.key)} style={{display:"flex",alignItems:"center",gap:8,width:"100%",textAlign:"left",padding:"6px 18px",background:active?"rgba(255,255,255,0.12)":"transparent",color:active?"#fff":"rgba(255,255,255,0.6)",border:"none",borderLeft:active?"3px solid rgba(255,255,255,0.85)":"3px solid transparent",borderRadius:active?"0 6px 6px 0":"0",cursor:"pointer",fontSize:13,fontWeight:active?600:400,fontFamily:"inherit",transition:"all 0.15s",letterSpacing:0.2,marginBottom:0}}
        ...
        <div style={{borderTop:"1px solid rgba(255,255,255,0.08)",margin:"6px 20px 0"}}/>

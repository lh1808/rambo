...
const ProgressTracker = ({currentStep,error,steps,enabled,completedSteps,stepProgress,stepDurations,totalElapsed}) => {
  ...
  return (
    <div style={{margin:"14px 0"}}>
      <style>{`
        @keyframes rubin-pulse {
          0%, 100% { opacity: 1; }
          50% { opacity: 0.4; }
        }
        @keyframes rubin-shimmer {
          0% { background-position: -200% 0; }
          100% { background-position: 200% 0; }
        }
        .rubin-active-step {
          animation: rubin-pulse 1.8s ease-in-out infinite;
        }
        .rubin-shimmer-bar {
          background: linear-gradient(90deg, transparent, rgba(155,17,30,0.15), transparent);
          background-size: 200% 100%;
          animation: rubin-shimmer 2s ease-in-out infinite;
        }
      `}</style>
      {/* Step counter (compact, no progress bar) */}
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
        <div style={{display:"flex",gap:4}}>
          {activeSteps.map(st => {
            ...
            const clr = failed ? "#cf222e" : done ? "#1a7f37" : active ? "#9B111E" : "#ddd";
            return <div key={st.id} style={{width:active?16:8,height:8,borderRadius:4,background:clr,transition:"all 0.4s"}} className={active&&!failed?"rubin-active-step":undefined}/>;
          })}
        </div>
        <span style={{fontSize:11.5,fontFamily:"monospace",color:allDone?"#1a7f37":"#6B0D15",fontWeight:600}}>{completedCount}/{totalCount} Schritte</span>
      </div>
      {/* Step list */}
      <div style={{borderRadius:10,overflow:"hidden",border:"1px solid #ede6e7"}}>
        {activeSteps.map((st) => {
          ...
          let bg="#fafafa",brd="#e8e2e3",col="#bbb",fw=400;
          if(done){bg="#f0faf3";brd="#1a7f37";col="#1a7f37";fw=500;}
          if(active&&!failed){bg="#FDF2F3";brd="#9B111E";col="#9B111E";fw=600;}
          if(failed){bg="#fff5f5";brd="#cf222e";col="#cf222e";fw=600;}
          return (
            <div key={st.id} style={{borderLeft:`3px solid ${brd}`,borderBottom:"1px solid #f5f0f0",background:bg,position:"relative",overflow:"hidden"}}>
              {active&&!failed&&<div className="rubin-shimmer-bar" style={{position:"absolute",inset:0,pointerEvents:"none"}}/>}
              <div style={{display:"flex",alignItems:"center",gap:14,padding:"10px 18px",position:"relative"}}>
                {done ? (
                  <svg width="18" height="18" viewBox="0 0 18 18"><circle cx="9" cy="9" r="8" fill="none" stroke="#1a7f37" strokeWidth="1.5"/><path d="M5.5 9.5l2 2 5-5" fill="none" stroke="#1a7f37" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                ) : active&&!failed ? (
                  <div className="rubin-active-step" style={{width:18,height:18,display:"flex",alignItems:"center",justifyContent:"center"}}>
                    <div style={{width:10,height:10,borderRadius:"50%",background:"#9B111E"}}/>
                  </div>
                ) : failed ? (
                  <svg width="18" height="18" viewBox="0 0 18 18"><circle cx="9" cy="9" r="8" fill="none" stroke="#cf222e" strokeWidth="1.5"/><path d="M6.5 6.5l5 5M11.5 6.5l-5 5" stroke="#cf222e" strokeWidth="1.5" strokeLinecap="round"/></svg>
                ) : (
                  <svg width="18" height="18" viewBox="0 0 18 18"><circle cx="9" cy="9" r="7" fill="none" stroke="#ddd" strokeWidth="1.5"/></svg>
                )}
                <span style={{fontSize:13.5,fontWeight:fw,color:active||done?col:"#888",flex:1}}>{st.label}</span>
                {active&&!failed&&<span style={{fontSize:10.5,color:"#9B111E",fontWeight:500,opacity:0.7}}>läuft …</span>}
                {done&&dur!=null&&<span style={{fontSize:10.5,color:"#1a7f37",fontWeight:500}}>{dur>=60?Math.floor(dur/60)+"m "+Math.round(dur%60)+"s":dur.toFixed(1)+"s"}</span>}
        ...
        <div style={{padding:"8px 18px 6px"}}>
          ...
          <div style={{marginTop:10,display:"flex",gap:3,justifyContent:"center"}}>
            {pages.filter(p=>p.id!=="overview").map(p=>{
              ...
              const clr=st.st==="done"?"#4ade80":st.st==="active"?"#fbbf24":"rgba(255,255,255,0.15)";
              return <div key={p.id} style={{width:st.st==="active"?12:6,height:6,borderRadius:3,background:clr,transition:"all 0.4s"}}/>;
            })}
          </div>
        </div>

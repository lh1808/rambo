...
class BaseLearnerTuner:
    ...
    def _tune_task(self, task: TuningTask, X: pd.DataFrame, Y: np.ndarray, T: np.ndarray, shared_params: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        _waves = max(1, _n_trials // max(1, _pj_start))
        _tlog.info(
            "BLT '%s': Starte %d Trials (parallel=%d, cv=%d, Wellen≈%d).",
            task.key, _n_trials, _pj_start, self.cfg.tuning.cv_splits, _waves,
        )
...
in allen Cross-Prediction-Folds wiederverwendet."""
    ...
    def tune_final_model(
        ...
        if name == "nonparamdml":
            ...
            # Timeout wird bei "both" deaktiviert (LGBM langsamer als CatBoost).
            ...
            if _fmt_is_both and _fmt_timeout:
                ...
                _fmt_timeout = None
            _tlog.info(
                "FMT '%s': Starte %d Trials (parallel=%d, outer_cv=%d, Wellen≈%d).",
                model_name, _fmt_nt, _fmt_pj, _outer_splits, max(1, _fmt_nt // max(1, _fmt_pj)),
            )
            study.optimize(objective, n_trials=_fmt_nt,
                           timeout=_fmt_timeout,
                           n_jobs=_fmt_pj, catch=(Exception,))
            _log_trial_diagnostics(study, model_name, n_jobs=_fmt_pj, stage="FMT")
            try:
...
def tune_causal_forest(
    ...
    - CFDML-interne Nuisance-Modelle: parallel_jobs=1, da CausalForestDML
      mit n_jobs=-1 bereits K Cross-Fitting-Folds parallel über joblib
      startet — parallel_jobs=-1 würde K×Cores Übersubskription erzeugen.
        ...
        if is_cfdml:
            ...
            for fold_i, (tr, va) in enumerate(split_iter):
                ...
                try:
                    ...
                    # parallel_jobs=1 für Nuisance: CFDML(n_jobs=-1) parallelisiert
                    # K Cross-Fitting-Folds intern via joblib. parallel_jobs=-1 würde
                    # K × Cores Übersubskription erzeugen. Siehe docs/architektur.md.
                    est = CausalForestDML(
    ...
    _tlog.info(
        "CFT '%s': Starte %d Trials (parallel=1, cv=%d, Seed=%d).%s",
    ...
    import gc as _gc
    _gc.collect()
    _tlog.info("CFT '%s': Study freigegeben, gc.collect() durchgeführt.", model_type)

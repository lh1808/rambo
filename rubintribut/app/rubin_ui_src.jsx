...
const LGBM={n_estimators:{min:50,max:1000,step:50,type:"int",dLow:200,dHigh:600},learning_rate:{min:0.001,max:0.3,step:0.005,type:"float",log:true,dLow:0.01,dHigh:0.15},num_leaves:{min:7,max:255,step:1,type:"int",dLow:15,dHigh:127},max_depth:{min:2,max:12,step:1,type:"int",dLow:3,dHigh:8},min_child_samples:{min:5,max:500,step:5,type:"int",dLow:10,dHigh:200},min_child_weight:{min:0.001,max:100,step:0.5,type:"float",log:true,dLow:0.01,dHigh:50},subsample:{min:0.3,max:1,step:0.05,type:"float",dLow:0.5,dHigh:1},colsample_bytree:{min:0.1,max:1,step:0.05,type:"float",dLow:0.3,dHigh:0.9},min_split_gain:{min:0,max:3,step:0.05,type:"float",dLow:0,dHigh:1},reg_alpha:{min:0.0000001,max:50,step:0.1,type:"float",log:true,dLow:0.000001,dHigh:20},reg_lambda:{min:0.0000001,max:50,step:0.1,type:"float",log:true,dLow:0.000001,dHigh:20},max_bin:{min:7,max:255,step:1,type:"int",dLow:15,dHigh:127},path_smooth:{min:0,max:20,step:0.5,type:"float",dLow:0,dHigh:10}};
...
const LGBM_DEFAULTS={boosting_type:"gbdt",n_estimators:500,learning_rate:0.05,num_leaves:63,max_depth:6,min_child_samples:20,min_child_weight:1.0,subsample:0.8,colsample_bytree:0.6,min_split_gain:0.0,reg_alpha:0.1,reg_lambda:1.0,path_smooth:0.0};
...
const LGBM_FINAL_DEFAULTS={boosting_type:"gbdt",n_estimators:200,learning_rate:0.03,num_leaves:20,max_depth:5,min_child_samples:100,min_child_weight:5.0,subsample:0.7,colsample_bytree:0.5,min_split_gain:0.5,reg_alpha:3.0,reg_lambda:15.0,path_smooth:5.0};
...
const ADDON_PRESETS = [
  ...
  {key:"feature_reduction",label:"Feature-Reduktion",desc:"Max. 60 Features via Importance + Korrelationsfilter",group:"feature",
    cfg:{fsEnabled:true,fsMethods:["catboost_importance","causal_forest"],fsCorrThresh:0.9,fsMaxFeatures:100}},
  // ── Erklärbarkeit ──
...
</>) : (<FixedParamsEditor params={cfg.fmtFixed||{}} defaults={(cfg.baseLearner||"catboost")==="catboost"?CB_FINAL_DEFAULTS:LGBM_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:v})} label="CATE-Parameter (Fixed)" accent="#D4A853"/>)}</>}</Sec>{((cfg.models||[]).includes("CausalForestDML")||(cfg.models||[]).includes("CausalForest"))&&<Sec title="CausalForest-Tuning" accent="#2d6a4f"><Info>Optimiert Wald-Parameter (max_depth, min_samples_leaf, max_features etc.) via Optuna. Bewertung per R-Loss auf Tuning-CV-Folds.</Info><Row><Col><Toggle label="CausalForest-Tuning aktivieren" checked={cfg.cfTune||false} onChange={v=>{const u={cfTune:v};if(v&&(cfg.cfTuneModels||[]).length===0){u.cfTuneModels=(cfg.models||[]).filter(m=>["CausalForestDML","CausalForest"].includes(m));}set({...cfg,...u});}}/></Col><Col><Toggle label="Single-Fold-Tuning" checked={cfg.cfSingleFold||false} onChange={v=>set({...cfg,cfSingleFold:v})} help="Bewertet jeden Trial auf 1 statt K Folds — schneller, aber verrauschtere R-Loss-Schätzung."/></Col></Row>{cfg.cfTune&&<><Divider/><div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:8}}>Tuning-Plan</div>{(() => {

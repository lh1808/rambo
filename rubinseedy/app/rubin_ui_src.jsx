...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Allgemein">
    ...
    <div style={{marginBottom:14}}>
      ...
      <input type="range" min={0} max={999} step={1} value={cfg.tuningSeed||18} onChange={e=>set({...cfg,tuningSeed:Number(e.target.value)})} style={{width:"100%",accentColor:"#D4A853"}}/>
          ...
          {pages.map((p,idx) => {
            ...
            return (
              <div key={p.key}>
                ...
                {st && st.st==="required" && (
                  <div style={{fontSize:9.5,color:"#ff8a8a",padding:"0 18px 3px 46px",lineHeight:1.3}}>{st.detail}</div>
                )}
                {st && st.st==="done" && !isOverview && (
                  <div style={{fontSize:9.5,color:"rgba(255,255,255,0.3)",padding:"0 18px 2px 46px",lineHeight:1.3}}>{st.detail}</div>
                )}
              </div>
            );
          })}

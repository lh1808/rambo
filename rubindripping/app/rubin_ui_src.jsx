...
const FinalTuningPlanPreview = ({models, fmtEnabled, fmtModels, fmtSingleFold, fmtTrials, fmtCv, outerCv: outerCvProp, mcIters, isBoth}) => {
  ...
  eligible.forEach(m => {
    ...
    // Pre-fit (einmalig) + Trials × Folds × 1 refit_final
    const preFit = outerFolds * nuisancePerFold;
    ...
    rows.push({
      ...
      trials: nTrials,
      totalFits: total,
      detail: `${outerFolds}F×${nuisancePerFold} + ${nTrials}T×${outerFolds}F`,
      note: `Pre-fit: ${outerFolds} Fold(s) × Nuisance (${nuisanceDesc}, cache_values). Pro Trial: ${outerFolds}× refit_final() + est.score(val).`
    });
  });
  ...
  return (
    <div>
      ...
      <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
        Nuisance-Modelle werden <strong>einmalig</strong> pro äußerem Fold mit cache_values gecacht. Trials fitten nur model_final via refit_final() auf gecachten Residuals (sequentiell, n_jobs=1). OOF-Evaluation via est.score(val).
...
<FixedParamsEditor params={(cfg.fmtFixed||{}).lgbm||{}} defaults={LGBM_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:{...(cfg.fmtFixed||{}),lgbm:v}})} label="LightGBM CATE-Parameter (Fixed)" accent="#D4A853"/>
              ...
              const cfdmlPreFit = evalFolds * fitsPerCfdmlFold;
                ...
                <div style={{border:"1px solid #ede6e7",borderRadius:8,overflow:"hidden"}}>
                  ...
                  {hasCF && <div style={{borderBottom:"1px solid #f0f0f0"}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 80px",padding:"8px 14px",fontSize:12}}>
                      ...
                      <span style={{fontSize:10.5,color:"#2d6a4f",fontFamily:MONO}}>{evalFolds}F×{fitsPerCfdmlFold} + {nTrials}T×{evalFolds}F</span>
                      <span style={{fontFamily:MONO,fontWeight:700,color:"#2d6a4f"}}>{cfdmlTotal.toLocaleString()}</span>
                    </div>
                    <div style={{fontSize:10.5,color:"#999",padding:"0 14px 6px",lineHeight:1.4}}>Pre-fit: {evalFolds}F × Nuisance ({dc}×model_y + {dc}×model_t + 1×Forest, cache_values). Pro Trial: {evalFolds}× refit_final() + est.score(val). 8 Wald-Params.</div>
                  </div>}
...
<div style={{fontSize:11,color:C.textSec,background:"#f9fafb",padding:"8px 12px",borderRadius:8,lineHeight:1.5,border:"1px solid #e5e7eb"}}>
  CausalForest-Tuning läuft <strong>sequentiell</strong> (CausalForestDML: Nuisance einmalig mit cache_values gecacht, Trials via set_params() + refit_final(). CausalForest: Nuisance-Residuen einmalig vorberechnet). Alle CPU-Kerne pro Fit (n_jobs=-1). <strong style={{color:trialColor}}>{trials}</strong> Trials insgesamt.
  {trials < 15 && " TPE kann bei weniger als 15 Trials kaum zwischen guten und schlechten Parametern unterscheiden."}
  {trials >= 15 && trials < 30 && " Grenzwertig — TPE beginnt erst nach den Startup-Trials zu lernen."}
  {trials >= 30 && " Genug Trials für stabile TPE-Exploration und Exploitation."}
</div>
...
function App() {
  ...
  const estimateFits = () => {
    ...
    // 3. FMT (Final-Model-Tuning) — cache_values: Nuisance EINMALIG + Trials nur model_final
    ...
    if (cfg.fmtEnabled) {
      ...
      (cfg.fmtModels||[]).forEach(x => {
        if (x==="NonParamDML") {
          // Pre-fit: fmtOuterFolds × Nuisance, Trials: fmtTr × fmtOuterFolds × 1 (model_final)
          f += fmtOuterFolds * fmtFitsPerDmlFit + fmtTr * fmtOuterFolds;
        } else if (x==="DRLearner") {
          f += fmtOuterFolds * fmtFitsPerDrFit + fmtTr * fmtOuterFolds;
        }
      });
    }

    // 4. CF Tuning — cache_values: Nuisance EINMALIG + Trials nur Forest/GRF refit
    if (cfg.cfTune) {
      const cfTrials = cfg.cfTrials || 50;
      const cfFolds = cfg.cfSingleFold ? 1 : innerCv;
      // CausalForestDML: Pre-fit cfFolds × Nuisance + cfTrials × cfFolds × 1 (forest refit)
      if ((cfg.cfTuneModels||[]).includes("CausalForestDML") && m.includes("CausalForestDML")) f += cfFolds * (mc*innerCv*2+1) + cfTrials * cfFolds;
      // CausalForest: Nuisance-Vorberechnung (cfFolds × 2) + cfTrials × cfFolds × 1 Forest

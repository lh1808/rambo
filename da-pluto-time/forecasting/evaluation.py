...
def compare_model_vs_baseline(
    ...
    Für jede Komponente werden MAE, sMAPE und WAPE berechnet und als
    Δ-Spalten (model − baseline) gegenübergestellt. Negative Δ-Werte
    bedeuten: Modell ist besser als Baseline.
    ...
    model_pred_ts
        Modell-Prognose.
    baseline_ts
        Baseline-Prognose (z. B. aus :func:`weekly_baseline_mean_last3`).
    output_chunk_length
        Prognosehorizont.
    ...
    """
    model_df = evaluate_multivariate(true_ts, model_pred_ts, output_chunk_length)
    base_df = evaluate_multivariate(true_ts, baseline_ts, output_chunk_length)

...
const DEFAULT_CFG = {expName:"rubin",seed:18,models:["NonParamDML","DRLearner","SLearner","TLearner"],baseLearner:"catboost",ensembleEnabled:false,tuningEnabled:true,tuningTrials:30,tuningSingleFold:false,tuningMetric:"log_loss",tuningMetricReg:"neg_mse",tuningPerRole:false,tuningPerLearner:false,tuningAutoml:"optuna",tuningModels:[],fmtEnabled:false,fmtModels:[],fmtSingleFold:false,fmtTrials:20,fmtMaxRows:0,fmtStabilityPenalty:0,fmtMethod:"rscorer",cfTune:false,cfTuneMaxRows:0,bundleEnabled:false,bundleDir:"runs/bundles",bundleChallengers:true,bundleMlflow:true,surrEnabled:false,surrMinLeaf:50,surrLeaves:31,surrDepth:0,fsEnabled:false,fsMethods:["lgbm_importance"],fsTopPct:15,fsCorrThresh:0.9,fsMaxFeatures:0,downsample:false,dfFrac:0.1,reduceMem:true,validateOn:"cross",cvSplits:5,treatmentType:"binary",refGroup:0,selMetric:"qini",higherBetter:true,refitChamp:true,manualChamp:null,hasNaN:false,nanCols:[],explEnabled:false,explMethod:"shap",explSampleSize:10000,explTopN:20,shapModels:[],shapBins:10,histScoreName:"historical_score",histScoreCol:"S",histScoreHigher:true,outputDir:"",blFixed:{},fmtFixed:{},cfFixed:{},tuningTimeout:0,tuningMaxRows:0,fmtTimeout:0,maxPredRows:0,savePreds:false,predsFormat:"csv",parallelLevel:2,workDir:null,mcIters:null,mcAgg:"mean"};
...
const ADDON_PRESETS = [
  ...
  {key:"bl_tuning_intensiv",label:"BL-Tuning Intensiv",desc:"120 Trials, Single-Fold – tiefere Suche",group:"tuning",
    cfg:{tuningEnabled:true,tuningTrials:120,tuningSingleFold:true}},
  {key:"fmt",label:"R-Score-Tuning (FMT)",desc:"CATE-Effektmodell optimieren (30 Trials)",group:"tuning",
    cfg:{fmtEnabled:true,fmtTrials:30,fmtModels:["NonParamDML","DRLearner"]}},
  {key:"grf_tuning",label:"GRF-Tuning",desc:"Grid-Search für CausalForest / CausalForestDML",group:"tuning",
    cfg:{cfTune:true}},
  // ── Production & Export ──
  ...
  <>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(5, 1fr)",gap:10,marginBottom:26}}>
      {[
        ...
        {v:"9+12",l:"Vorlagen & Add-ons",d:"9 Basis + 12 Add-ons",accent:"#6366f1"},
      ].map(c => (
...
const PData = ({cfg,set,setCfg,activeBase,setActiveBase,activeAddons,setActiveAddons,view}) => {
  ...
  const MUTEX = [["bl_tuning","bl_tuning_intensiv"]];
  const toggleAddon = (p) => {
...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Allgemein">
    <div style={{marginBottom:14}}>
      ...
      <input type="range" min={0} max={999} step={1} value={cfg.seed||18} onChange={e=>set({...cfg,seed:Number(e.target.value)})} style={{width:"100%",accentColor:C.ruby}}/>
    ...
    <Divider/><div style={{background:cfg.ensembleEnabled?"rgba(155,17,30,0.04)":"transparent",border:cfg.ensembleEnabled?"1.5px solid rgba(155,17,30,0.15)":"1.5px dashed #e0d6d7",borderRadius:10,padding:"12px 14px",transition:"all 0.2s"}}><Toggle label="Ensemble (Gleichgewichtet)" checked={cfg.ensembleEnabled||false} onChange={v=>set({...cfg,ensembleEnabled:v})} help="Mittelt die CATE-Vorhersagen aller trainierten Modelle. Nimmt an der Champion-Selektion teil."/>{cfg.ensembleEnabled&&<>{(()=>{const active=(cfg.models||[]).filter(m=>av.includes(m));return active.length>=2?<div style={{fontSize:11,color:"#999",marginTop:8,lineHeight:1.8}}>Ensemble aus {active.map((m,i)=><span key={m}>{i>0&&<span style={{color:"#ddd"}}> · </span>}<span style={{fontSize:10.5,padding:"2px 8px",borderRadius:8,background:"rgba(155,17,30,0.08)",color:C.ruby,fontWeight:600,whiteSpace:"nowrap"}}>{m}</span></span>)} <span style={{fontStyle:"italic",whiteSpace:"nowrap"}}>({active.length} Modelle, je 1/{active.length})</span></div>:null})()}{(cfg.models||[]).filter(m=>av.includes(m)).length<2&&<Info type="warn">Mindestens 2 aktive Modelle nötig.</Info>}</>}</div></Sec><Sec title="Base Learner"><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {[
            ...
            return (
              ...
              const cfTotal = cfNuisance + 12;  // + 12 Forest-Grid-Kombis
              const grfTotal = 24;  // Kein Nuisance, reiner Grid-Search
              ...
              return (<>
                <div style={{display:"flex",gap:10,marginTop:12,flexWrap:"wrap"}}>
                  ...
                  <div style={{padding:"8px 16px",background:"#faf7f7",borderRadius:8,border:"1px solid #ede6e7"}}><div style={{fontSize:22,fontWeight:700,color:"#333"}}>{hasCF&&hasGRF?"12+24":hasCF?"12":"24"}</div><div style={{fontSize:10,textTransform:"uppercase",letterSpacing:0.5,color:"#888"}}>Grid-Kombis</div></div>
                </div>
                <div style={{border:"1px solid #ede6e7",borderRadius:8,overflow:"hidden",marginTop:12}}>
                  ...
                  {hasCF && <div style={{borderBottom:"1px solid #f0f0f0"}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 80px",padding:"8px 14px",fontSize:12}}>
                      ...
                      <span style={{fontSize:11,color:"#2d6a4f"}}>{cfNuisance} Nuisance + 12 Forest-Grid</span>
                      <span style={{fontFamily:"monospace",fontWeight:700,color:"#2d6a4f"}}>{cfTotal}</span>
                    </div>
                    <div style={{fontSize:10.5,color:"#999",padding:"0 14px 6px",lineHeight:1.4}}>Nuisance: cv={dc} x 2 Modelle{mc>1?" x "+mc+" mc_iters":""} = {cfNuisance}. Dann 12 Forest-Kombis (2x3x2) auf den Residuals.</div>
                  </div>}
                  {hasGRF && <div style={{background:hasCF?"#fdfbfb":"#fff"}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 80px",padding:"8px 14px",fontSize:12}}>
                      <span style={{fontWeight:600}}>CausalForest</span>
                      <span style={{fontSize:11,color:"#2d6a4f"}}>24 Forest-Grid (kein Nuisance)</span>
                      <span style={{fontFamily:"monospace",fontWeight:700,color:"#2d6a4f"}}>24</span>
                    </div>
                    <div style={{fontSize:10.5,color:"#999",padding:"0 14px 6px",lineHeight:1.4}}>R-Loss Grid-Search auf erstem CV-Fold (2x3x2x2 = 24). Kein DML — keine Nuisance-Modelle.</div>
                  </div>}
                </div>
              </>);
...
export default function App() {
  ...
  const INIT_CFG = {expName:"rubin",seed:18,parallelLevel:2,workDir:null,x_file:"",t_file:"",y_file:"",s_file:"",treatmentType:"binary",refGroup:0,hasNaN:false,nanCols:[],validateOn:"cross",cvSplits:5,downsample:false,dfFrac:0.1,reduceMem:true,fsEnabled:false,fsMethods:["lgbm_importance"],fsTopPct:15,fsCorrThresh:0.9,outputDir:"",savePreds:false,predsFormat:"csv",models:["NonParamDML","DRLearner","SLearner","TLearner"],baseLearner:"catboost",tuningEnabled:true,tuningTrials:30,tuningSingleFold:false,tuningMetric:"log_loss",tuningMetricReg:"neg_mse",tuningPerRole:false,tuningPerLearner:false,tuningAutoml:"optuna",tuningModels:[],fmtEnabled:false,fmtModels:[],fmtSingleFold:false,fmtTrials:20,fmtMaxRows:0,cfTune:false,cfTuneMaxRows:0,selMetric:"qini",higherBetter:true,refitChamp:true,manualChamp:null,surrEnabled:false,surrMinLeaf:50,surrLeaves:31,surrDepth:0,bundleEnabled:false,bundleDir:"runs/bundles",bundleChallengers:true,bundleMlflow:true,explEnabled:true,explMethod:"shap",explSampleSize:10000,explTopN:20,shapModels:[],shapBins:10,histScoreName:"historical_score",histScoreCol:"S",histScoreHigher:true,fsMaxFeatures:0,maxPredRows:0,tuningTimeout:0,tuningMaxRows:0,fmtTimeout:0,blFixed:{},fmtFixed:{},cfFixed:{},ensembleEnabled:false,mcIters:null,mcAgg:"mean",fmtStabilityPenalty:0};
  ...
  const estimateFits = () => {
    ...
    if (cfg.tuningEnabled) {
      const tr = cfg.tuningTrials||30, tc = cfg.tuningSingleFold ? 1 : cv;
      const dml = m.some(x=>["NonParamDML","ParamDML","CausalForestDML"].includes(x));
      const dr = m.includes("DRLearner"), sl = m.includes("SLearner");
      const tl = m.includes("TLearner"), xl = m.includes("XLearner");
    ...
    if (cfg.cfTune) {
      if (m.includes("CausalForestDML")) f += dc*2*mc + 12;
      if (m.includes("CausalForest")) f += 24;
    }

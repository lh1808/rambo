...
2. **FMT-Zielfunktion ≠ Evaluationsmetrik:** FMT optimiert die negierte MSE der Residual-Regression (est.score()), nicht QINI oder AUUC. Die FMT-Zielfunktion und die Modellvergleichs-Metrik sind verschieden — bessere FMT-Scores garantieren keine besseren QINI-Werte. Es gibt keinen Evaluations-Leak.
...
- **`"TMEO (Mask-Holdout)"`** bei gesetztem `eval_mask_file` — die Summary-Bar zeigt zusätzlich `"Eval (TMEO): N Beob."` mit der Holdout-Größe

...
def patch_categorical_features(X: pd.DataFrame, base_learner_type: str = "lgbm"):
    ...
    try:
        ...
        if patch_catboost:
            ...
            def _convert_float_to_object(X, ci, np_mod):
                """Konvertiert numpy float-Array: kat. Spalten → int, Array → object-dtype."""
                ...
                for idx in ci:
                    if idx < X.shape[1]:
                        X[:, idx] = np_mod.nan_to_num(X[:, idx], nan=-1).astype(int)
                X_obj = np_mod.empty_like(X, dtype=object)
            ...
            def _make_fit_wrapper(original_fit):
                def _wrapped_fit(self, X, y=None, **kwargs):
                    ...
                    X_conv = _convert_float_to_object(X, ci, _np)
            ...
            def _make_predict_wrapper(original_predict):
                def _wrapped_predict(self, X, *args, **kwargs):
                    ...
                    X_conv = _convert_float_to_object(X, cat_indices, _np)

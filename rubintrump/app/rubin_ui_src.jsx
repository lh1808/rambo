...
const pages = [
  ...
  {label:"Datenpfade",key:"datafiles",group:"Daten"},
...
const LGBM={n_estimators:{min:50,max:1000,step:50,type:"int",dLow:200,dHigh:600},learning_rate:{min:0.001,max:0.3,step:0.005,type:"float",log:true,dLow:0.01,dHigh:0.15},num_leaves:{min:7,max:255,step:1,type:"int",dLow:15,dHigh:127},max_depth:{min:2,max:12,step:1,type:"int",dLow:3,dHigh:8},min_child_samples:{min:5,max:500,step:5,type:"int",dLow:10,dHigh:200},min_child_weight:{min:0.001,max:100,step:0.5,type:"float",log:true,dLow:0.01,dHigh:50},subsample:{min:0.3,max:1,step:0.05,type:"float",dLow:0.5,dHigh:1},colsample_bytree:{min:0.1,max:1,step:0.05,type:"float",dLow:0.3,dHigh:0.9},min_split_gain:{min:0,max:3,step:0.05,type:"float",dLow:0,dHigh:1},reg_alpha:{min:0.0000001,max:50,step:0.1,type:"float",log:true,dLow:0.000001,dHigh:20},reg_lambda:{min:0.0000001,max:50,step:0.1,type:"float",log:true,dLow:0.000001,dHigh:20},path_smooth:{min:0,max:20,step:0.5,type:"float",dLow:0,dHigh:10}};
const CB={iterations:{min:50,max:1000,step:50,type:"int",dLow:200,dHigh:600},learning_rate:{min:0.001,max:0.3,step:0.005,type:"float",log:true,dLow:0.01,dHigh:0.15},depth:{min:2,max:12,step:1,type:"int",dLow:4,dHigh:8},l2_leaf_reg:{min:0.1,max:80,step:0.5,type:"float",log:true,dLow:1,dHigh:30},random_strength:{min:0.001,max:20,step:0.1,type:"float",log:true,dLow:0.01,dHigh:10},subsample:{min:0.3,max:1,step:0.05,type:"float",dLow:0.5,dHigh:1},rsm:{min:0.1,max:1,step:0.05,type:"float",dLow:0.3,dHigh:0.9},min_data_in_leaf:{min:5,max:500,step:5,type:"int",dLow:10,dHigh:200},model_size_reg:{min:0,max:20,step:0.5,type:"float",dLow:0,dHigh:10},leaf_estimation_iterations:{min:1,max:15,step:1,type:"int",dLow:1,dHigh:10}};
const LGBM_FMT={n_estimators:{min:50,max:800,step:10,type:"int",dLow:100,dHigh:400},learning_rate:{min:0.001,max:0.2,step:0.005,type:"float",log:true,dLow:0.005,dHigh:0.12},num_leaves:{min:3,max:127,step:1,type:"int",dLow:7,dHigh:63},max_depth:{min:1,max:10,step:1,type:"int",dLow:2,dHigh:6},min_child_samples:{min:5,max:1000,step:5,type:"int",dLow:20,dHigh:500},min_child_weight:{min:0.1,max:150,step:0.5,type:"float",log:true,dLow:0.5,dHigh:80},subsample:{min:0.2,max:1,step:0.05,type:"float",dLow:0.4,dHigh:0.85},colsample_bytree:{min:0.1,max:1,step:0.05,type:"float",dLow:0.25,dHigh:0.7},max_bin:{min:7,max:255,step:1,type:"int",dLow:15,dHigh:127},min_split_gain:{min:0,max:5,step:0.1,type:"float",dLow:0,dHigh:3},reg_alpha:{min:0.01,max:100,step:0.5,type:"float",log:true,dLow:0.1,dHigh:50},reg_lambda:{min:0.1,max:200,step:1,type:"float",log:true,dLow:1,dHigh:100},path_smooth:{min:0,max:50,step:1,type:"float",dLow:0,dHigh:30}};
const CB_FMT={iterations:{min:50,max:800,step:10,type:"int",dLow:100,dHigh:400},learning_rate:{min:0.001,max:0.2,step:0.005,type:"float",log:true,dLow:0.005,dHigh:0.12},depth:{min:1,max:10,step:1,type:"int",dLow:2,dHigh:6},l2_leaf_reg:{min:0.5,max:150,step:1,type:"float",log:true,dLow:3,dHigh:80},random_strength:{min:0.1,max:30,step:0.5,type:"float",log:true,dLow:0.5,dHigh:15},subsample:{min:0.2,max:1,step:0.05,type:"float",dLow:0.4,dHigh:0.85},rsm:{min:0.1,max:1,step:0.05,type:"float",dLow:0.25,dHigh:0.7},min_data_in_leaf:{min:5,max:1000,step:5,type:"int",dLow:20,dHigh:500},model_size_reg:{min:0,max:50,step:0.5,type:"float",log:true,dLow:0.1,dHigh:30},leaf_estimation_iterations:{min:1,max:10,step:1,type:"int",dLow:1,dHigh:5}};
// Default fixed params per base learner (used when tuning is OFF)
const LGBM_DEFAULTS={boosting_type:"dart",drop_rate:0.1,skip_drop:0.5,max_drop:50,n_estimators:500,learning_rate:0.05,num_leaves:63,max_depth:6,min_child_samples:20,min_child_weight:1.0,subsample:0.8,colsample_bytree:0.6,min_split_gain:0.0,reg_alpha:0.1,reg_lambda:1.0,path_smooth:0.0};
const CB_DEFAULTS={iterations:500,learning_rate:0.05,depth:6,l2_leaf_reg:5.0,min_data_in_leaf:20,random_strength:1.0,subsample:0.8,rsm:0.6,model_size_reg:0.0,leaf_estimation_iterations:1};
// Final model (regressor) – konservative Defaults für CATE-Schätzung auf verrauschten Residuen
const LGBM_FINAL_DEFAULTS={boosting_type:"dart",drop_rate:0.15,skip_drop:0.5,n_estimators:200,learning_rate:0.03,num_leaves:20,max_depth:5,min_child_samples:100,min_child_weight:5.0,subsample:0.7,colsample_bytree:0.5,min_split_gain:0.5,reg_alpha:3.0,reg_lambda:15.0,path_smooth:5.0};
const CB_FINAL_DEFAULTS={iterations:200,learning_rate:0.03,depth:4,l2_leaf_reg:10.0,min_data_in_leaf:100,random_strength:3.0,subsample:0.7,rsm:0.5,model_size_reg:5.0,leaf_estimation_iterations:1};
// CausalForestDML + CausalForest Wald-Defaults (identisch mit _CFDML_DEFAULTS / _CF_DEFAULTS in model_registry.py)
const CF_FOREST_DEFAULTS = {n_estimators:200,max_depth:5,min_samples_leaf:200,min_samples_split:500,max_features:0.3,max_samples:0.45,min_var_fraction_leaf:0.01,min_impurity_decrease:0.0,criterion:"mse"};

// CF Search Space Ranges (für RSlider-Editor)
const CF_SS = {
  n_estimators:         {min:50,  max:600, step:10,  type:"int"},
  max_depth:            {min:2,   max:12,  step:1,   type:"int"},
  min_samples_leaf:     {min:10,  max:1000,step:10,  type:"int",  log:true},
  min_samples_split:    {min:20,  max:2000,step:10,  type:"int",  log:true},
  max_features:         {min:0.05,max:1.0, step:0.05,type:"float"},
  max_samples:          {min:0.1, max:1.0, step:0.05,type:"float"},
  min_var_fraction_leaf:{min:0.0001,max:0.2,step:0.001,type:"float",log:true},
  min_impurity_decrease:{min:0.0, max:0.05,step:0.001,type:"float"},
};
// ── Design System ──
            ...
            <div key={k} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 12px",background:changed?acBg:"#fff",borderRadius:R,border:`1px solid ${changed?acBorder:C.borderLight}`,transition:"all 0.15s"}}>
              ...
              <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"flex-end",gap:6}}>
                {isBool ? (
                  ...
                  <input type={isNum?"number":"text"} value={val===null?"":val} onChange={e=>{const v=e.target.value;set(k,isNum?(v===""?def:Number(v)):v===""?def:v)}} style={{width:80,height:30,padding:"0 8px",border:`1.5px solid ${changed?acBorder:C.border}`,borderRadius:6,fontSize:12,fontFamily:MONO,fontWeight:600,background:"#fff",outline:"none",boxSizing:"border-box",textAlign:"right",color:changed?ac:C.text}} onFocus={e=>{e.target.style.borderColor=ac;e.target.style.boxShadow=`0 0 0 2px ${ac}14`}} onBlur={e=>{e.target.style.borderColor=changed?acBorder:C.border;e.target.style.boxShadow="none"}}/>
                )}
...
const MC = ({value,label,desc,highlight}) => (
  <div style={{background:highlight?"linear-gradient(135deg,#6B0D15,#9B111E)":C.rose,borderRadius:12,padding:"20px 22px",textAlign:"center",flex:1,minWidth:130,transition:"transform 0.2s, box-shadow 0.2s",cursor:"default"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-3px)";e.currentTarget.style.boxShadow="0 8px 20px rgba(155,17,30,0.12)"}} onMouseLeave={e=>{e.currentTarget.style.transform="none";e.currentTarget.style.boxShadow="none"}}>
    <div style={{fontSize:30,fontWeight:800,color:highlight?"#fff":C.dark,letterSpacing:-0.5}}>{value}</div>
  ...
  return (
    <div style={{margin:"10px 0"}}>
      ...
      <div style={{position:"relative",paddingLeft:28}}>
        ...
        {activeIdx > 0 && <div style={{position:"absolute",left:9,top:4,height:`calc(${(activeIdx / Math.max(activeSteps.length-1, 1)) * 100}% - 4px)`,width:2,background:"linear-gradient(180deg, #1a7f37, #1a7f37)",borderRadius:1,transition:"height 0.6s ease"}}/>}
        {/* Active pulse segment */}
        {activeIdx >= 0 && !error && !allDone && <div style={{position:"absolute",left:9,top:`calc(${(activeIdx / Math.max(activeSteps.length-1, 1)) * 100}%)`,height:`calc(${(1 / Math.max(activeSteps.length-1, 1)) * 100}%)`,width:2,background:"linear-gradient(180deg, #1a7f37, #9B111E)",borderRadius:1,animation:"rubin-pulse-line 2.4s ease-in-out infinite"}}/>}
        {/* All-done: full green line */}
        {allDone && <div style={{position:"absolute",left:9,top:4,bottom:4,width:2,background:"#1a7f37",borderRadius:1,transition:"all 0.6s"}}/>}
            ...
            return (
              <div key={st.id} style={{position:"relative",display:"flex",alignItems:"center",gap:14,padding:"8px 0",transition:"opacity 0.4s",opacity:pending?0.4:1}}>
                ...
                <div style={{position:"absolute",left:-28,top:"50%",transform:"translateY(-50%)",width:20,display:"flex",justifyContent:"center"}}>
                  {done ? (
                    <div style={{width:7,height:7,borderRadius:"50%",background:"#1a7f37",boxShadow:"0 0 6px rgba(26,127,55,0.4)",transition:"all 0.3s"}}/>
                  ) : active&&!failed ? (
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:10,paddingTop:8,borderTop:"1px solid #f0eded"}}>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          {allDone ? (
            <div style={{width:6,height:6,borderRadius:"50%",background:"#1a7f37",boxShadow:"0 0 6px rgba(26,127,55,0.4)"}}/>
          ) : (
          ...
          <span style={{fontSize:11.5,color:allDone?"#1a7f37":"#666",fontWeight:500}}>{allDone?"Pipeline abgeschlossen":`${completedCount} von ${totalCount} Schritten`}</span>
        </div>
      </div>
    </div>
  );
...
const DEFAULT_CFG = {studyType:"rct",expName:"rubin",seed:42,tuningSeed:18,models:["NonParamDML","DRLearner","SLearner","TLearner","XLearner","ParamDML","CausalForestDML","CausalForest"],baseLearner:"catboost",ensembleEnabled:false,tuningEnabled:true,tuningTrials:50,tuningSingleFold:false,tuningMetric:"log_loss",tuningMetricReg:"neg_mse",tuningPerRole:false,tuningPerLearner:false,tuningModels:[],fmtEnabled:false,fmtModels:[],fmtSingleFold:false,fmtTrials:50,fmtMaxRows:0,fmtOverfitPenalty:0.0,fmtOverfitTolerance:0.05,cfTune:false,cfTrials:50,cfSingleFold:false,cfTuneModels:[],bundleEnabled:false,bundleDir:"runs/bundles",bundleChallengers:true,bundleMlflow:true,surrEnabled:false,surrMinLeaf:50,surrLeaves:31,surrDepth:0,fsEnabled:false,fsMethods:["catboost_importance"],fsTopPct:15,fsCorrThresh:0.9,fsMaxFeatures:0,downsample:false,dfFrac:0.1,reduceMem:true,validateOn:"cross",cvSplits:5,treatmentType:"binary",refGroup:0,selMetric:"qini",higherBetter:true,refitChamp:true,manualChamp:null,hasNaN:false,nanCols:[],explEnabled:false,explSampleSize:10000,explTopN:20,shapModels:[],shapBins:10,histScoreName:"historical_score",histScoreCol:"S",histScoreHigher:true,outputDir:"",blFixed:{},fmtFixed:{},cfFixed:{},tuningTimeout:0,tuningMaxRows:0,fmtTimeout:0,maxPredRows:0,parallelLevel:3,workDir:null,mcIters:null,mcAgg:"mean",dmlCrossfitFolds:5,overfitPenalty:0.0,overfitTolerance:0.15};



const ADDON_PRESETS = [
  // ── Tuning (Pipeline-Reihenfolge: BL → FMT → GRF) ──
  {key:"bl_tuning",label:"BL-Tuning",desc:"Nuisance-Hyperparameter optimieren (50 Trials)",group:"tuning",
    cfg:{tuningEnabled:true,tuningTrials:50}},
  {key:"bl_tuning_intensiv",label:"BL-Tuning Intensiv",desc:"100 Trials — intensivere Hyperparameter-Suche",group:"tuning",
    cfg:{tuningEnabled:true,tuningTrials:100}},
  {key:"fmt",label:"FM-Tuning",desc:"CATE-Effektmodell optimieren (50 Trials)",group:"tuning",
    cfg:{fmtEnabled:true,fmtTrials:50,fmtModels:["NonParamDML","DRLearner"]}},
  {key:"fmt_intensiv",label:"FM-Tuning Intensiv",desc:"100 Trials — intensivere CATE-Optimierung",group:"tuning",
    cfg:{fmtEnabled:true,fmtTrials:100,fmtModels:["NonParamDML","DRLearner"]}},
  {key:"grf_tuning",label:"CausalForest-Tuning",desc:"Wald-Parameter per Optuna optimieren (50 Trials)",group:"tuning",
    cfg:{cfTune:true,cfTuneModels:["CausalForestDML","CausalForest"]}},
  {key:"grf_tuning_intensiv",label:"CausalForest-Tuning Intensiv",desc:"100 Trials — intensivere Wald-Parameter-Suche",group:"tuning",
  ...
  // ── Feature-Selektion ──
  {key:"feature_reduction",label:"Feature-Reduktion",desc:"Importance-basierte Feature-Selektion (CatBoost + CausalForest)",group:"feature",
    cfg:{fsEnabled:true,fsMethods:["catboost_importance","causal_forest"],fsTopPct:15,fsCorrThresh:0.9}},
  // ── Erklärbarkeit ──
  ...
  {key:"reg_moderate",label:"Moderat",desc:"BLT: 15% Gap toleriert (Nuisance darf moderat overfitten). FMT: 5% Gap (CATE-Signal ist schwächer).",group:"regularization",
    cfg:{overfitPenalty:0.2,overfitTolerance:0.15,fmtOverfitPenalty:0.3,fmtOverfitTolerance:0.05}},
  {key:"reg_strong",label:"Stark",desc:"BLT: 8% Gap toleriert. FMT: 3% Gap. Konservativ — bei kleinen Datensätzen oder instabilen Vorläufen.",group:"regularization",
    cfg:{overfitPenalty:0.4,overfitTolerance:0.08,fmtOverfitPenalty:0.6,fmtOverfitTolerance:0.03}},
  // ── Stabilität ──
...
const POverview = ({setPg}) => (
  <>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(5, 1fr)",gap:10,marginBottom:26}}>
      {[
        {v:"8",l:"Kausale Modelle",d:"Meta-Learner, DML, DR, GRF",accent:"#9B111E"},
        ...
        {v:"14",l:"Pipeline-Optionen",d:"Tuning, Regularisierung, Bundles, SHAP u.v.m.",accent:"#6366f1"},
      ].map(c => (
        <div key={c.l} style={{background:"#fff",borderRadius:12,padding:"18px 14px",textAlign:"center",borderTop:`3px solid ${c.accent}`,boxShadow:"0 1px 4px rgba(0,0,0,0.04)",transition:"transform 0.2s",cursor:"default",display:"flex",flexDirection:"column",justifyContent:"center"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-2px)"}} onMouseLeave={e=>{e.currentTarget.style.transform="none"}}>
          ...
          {c.d && <div style={{fontSize:10,color:C.textFaint,marginTop:4}}>{c.d}</div>}
        </div>
      ))}
    </div>
    ...
    <Sec title="Einstieg">
      <Info><strong>Zwei Wege:</strong> Falls du Rohdaten aufbereiten musst (CSV/SAS → X/T/Y), starte mit der optionalen Datenvorbereitung. Falls du bereits vorbereitete Dateien hast, gehe direkt zu "Datenpfade".</Info>
      <div style={{display:"flex",gap:10,marginTop:12}}>
        <Btn secondary onClick={()=>setPg("dataprep")}>Datenvorbereitung (optional)</Btn>
        <Btn onClick={()=>setPg("datafiles")}>Direkt zu Datenpfade</Btn>
      </div>
    </Sec>
  </>
);
    ...
    <>
      ...
      <Sec title="1. Rohdaten laden" accent="#D4A853">
        ...
        {files.filter(f=>f.trim()).length > 1 && (<>
          ...
          <Divider/>
          <div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:8}}>Train Many, Evaluate One</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:(dp.targets||[""]).length>1?"1fr 1fr":"1fr",gap:6}}>
            {(dp.targets||[""]).map((t,i) => (
              <div key={i} style={{display:"flex",gap:6,alignItems:"center"}}>
                <input type="text" value={t} onChange={e=>{const nt=[...(dp.targets||[""])];nt[i]=e.target.value;setDp(prev=>({...prev,targets:nt}))}} placeholder="z.B. Y, OUTCOME, ..." style={{flex:1,height:34,padding:"0 10px",border:"1.5px solid #e0d6d7",borderRadius:6,fontSize:13,background:"#fff",outline:"none",boxSizing:"border-box"}}/>
              ...
              return (
                <div key={val} style={{display:"grid",gridTemplateColumns:"1fr 120px 1fr",padding:"9px 14px",borderBottom:"1px solid #f5f0f0",background:isControl?"#fffbeb":i%2===0?"#fff":"#fdfafa",alignItems:"center"}}>
                  ...
                  <div>
                    ...
                    }} style={{width:"100%",padding:"5px 8px",border:"1.5px solid #e0d6d7",borderRadius:6,fontSize:13,background:"#fff",cursor:"pointer",fontFamily:MONO,fontWeight:600,color:isControl?"#D4A853":"#9B111E"}}>
      ...
      <Sec title="3. Feature Dictionary laden (optional)" accent="#6366f1">
        ...
        <Row>
          <Col><Inp label="Feature Dictionary Pfad" placeholder="runs/exports/feature_dictionary.xlsx" value={dp.featurePath} onChange={v=>setDp(prev=>({...prev,featurePath:v}))}/></Col>
          <Col>
      ...
      <Sec title="Datenvorbereitung starten" accent="#D4A853">
        ...
        {dpDone && (<>
          ...
          <div style={{marginTop:12,display:"flex",gap:10}}>
            <Btn onClick={()=>setPg("datafiles")}>Weiter zu Datenpfade</Btn>
            <Btn small secondary onClick={()=>{setDpDone(false);setDpProgress(0)}}>Erneut starten</Btn>
          </div>
        </>)}
        {!dpRunning && !dpDone && <div style={{marginTop:12}}><Btn small secondary onClick={()=>setPg("datafiles")}>Überspringen – direkt zu Datenpfade</Btn></div>}
      </Sec>
    </>
  );
};
...
const YAML_TO_CFG = {
  ...
  "feature_selection.enabled":"fsEnabled","feature_selection.methods":"fsMethods","feature_selection.top_pct":"fsTopPct","feature_selection.correlation_threshold":"fsCorrThresh","feature_selection.max_features":"fsMaxFeatures",
  ...
  "causal_forest.use_econml_tune":"cfTune","causal_forest.search_space":"_cfSS","causal_forest.n_trials":"cfTrials","causal_forest.single_fold":"cfSingleFold","causal_forest.tune_models":"cfTuneModels",
...
const PData = ({cfg,set,setCfg,activeBase,setActiveBase,activeAddons,setActiveAddons,setSp,setSpFmt,view}) => {
  ...
  const toggleAddon = (p) => {
    ...
    if(next.has(p.key)) {
      ...
      next.add(p.key);
      // Mutual exclusion: deselect conflicting add-ons
      MUTEX.forEach(group => {
        if(group.includes(p.key)) {
          group.filter(k=>k!==p.key).forEach(k=>{
            if(next.has(k)){
              ...
              if(other){const r={};Object.keys(other.cfg).forEach(ck=>{r[ck]=DEFAULT_CFG[ck]});setCfg(prev=>({...prev,...r}));}
            }
          });
        }
      });
  ...
  return (
    <>
      {view==="files"&&<Sec title="Analysedaten">
        <Tabs tabs={["Dateipfade","Datei-Upload"]} active={tab} onSelect={setTab}/>

        {/* ── Trainingsdaten ── */}
        <div style={{fontSize:12,fontWeight:600,color:C.textSec,textTransform:"uppercase",letterSpacing:0.8,marginBottom:8}}>Trainingsdaten</div>
        {tab==="Dateipfade" ? (
          <Row><Col>
        ...
        {/* ── External Eval ── */}
        <div style={{marginTop:20,background:cfg.validateOn==="external"?"#fef2f2":"#fafafa",border:`1.5px solid ${cfg.validateOn==="external"?"#f5c2c7":"#e5e5e5"}`,borderRadius:12,padding:"14px 18px",transition:"all 0.2s"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:12,fontWeight:600,color:cfg.validateOn==="external"?"#991b1b":"#57606a",textTransform:"uppercase",letterSpacing:0.6}}>External Eval</span>
              {cfg.validateOn==="external" && (cfg.eval_x_file && cfg.eval_t_file && cfg.eval_y_file
                ? <span style={{fontSize:9,fontWeight:600,color:"#991b1b",background:"#fde8e8",padding:"2px 8px",borderRadius:8}}>Bereit</span>
                : <span style={{fontSize:9,fontWeight:600,color:"#92400e",background:"#fef3c7",padding:"2px 8px",borderRadius:8}}>Dateien fehlen</span>
              )}
            </div>
            <Toggle label="" checked={cfg.validateOn==="external"} onChange={v=>set({...cfg,validateOn:v?"external":"cross"})}/>
          </div>
          {cfg.validateOn==="external" && (<div style={{marginTop:10}}>
            <div style={{fontSize:11,color:"#57606a",lineHeight:1.5,marginBottom:10}}>Separater Holdout-Datensatz. Preprocessing leakage-frei auf Trainingsdaten gefittet.</div>
        ...
        {/* ── Historischer Score (Benchmark) ── */}
        {cfg.s_file && <div style={{marginTop:20,borderTop:"1px solid "+C.borderLight,paddingTop:16}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
            <div style={{fontSize:12,fontWeight:600,color:C.textSec,textTransform:"uppercase",letterSpacing:0.8}}>Benchmark</div>
            <span style={{fontSize:10,fontWeight:600,color:"#1a7f37",background:"#f0fdf4",border:"1px solid #a3d9b1",padding:"2px 10px",borderRadius:10}}>Aktiv</span>
          </div>
          <div style={{fontSize:12,color:C.textSec,lineHeight:1.5,marginBottom:12}}>Modelle werden gegen den historischen Score aus <code style={{background:"#f5f0f0",padding:"1px 5px",borderRadius:3,fontSize:11}}>{cfg.s_file.split("/").pop()}</code> verglichen. Läuft automatisch bei gesetzter S-Datei.</div>
          <Row>
      ...
      {view==="template"&&<Sec title="1. Treatment-Typ">
      ...
      {view==="template"&&<Sec title="2. Studientyp">
      ...
      {view==="template"&&<Sec title="4. Pipeline-Optionen">
...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Allgemein">
    ...
    <div style={{marginBottom:14}}>
      ...
      <div style={{fontSize:10.5,color:C.textMuted,marginTop:6,lineHeight:1.4}}>Tuning-CV-Folds nutzen einen eigenen Seed, damit Optuna die Hyperparameter auf <em>anderen</em> Folds bewertet als die spätere Cross-Prediction. Verhindert Val-Set-Overfitting.</div>
    ...
    <div style={{fontSize:12,fontWeight:600,color:C.text,marginBottom:8}}>Parallelisierungs-Level</div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(4, 1fr)",gap:8}}>
      {[{v:1,l:"Minimal",d:"1 Kern, sequentiell. Sicher, minimaler RAM."},{v:2,l:"Moderat",d:"Alle Kerne pro Fit, Folds sequentiell. Guter Kompromiss."},{v:3,l:"Hoch",d:"Folds und Trials parallel. DRTester reduziert. Mehr RAM."},{v:4,l:"Maximum",d:"Max. Parallelisierung. DRTester nur Champion. Höchster RAM."}].map(o=>{
        ...
        return(<button key={o.v} onClick={()=>set({...cfg,parallelLevel:o.v})} style={{padding:"12px 14px",borderRadius:10,border:active?"2px solid #9B111E":"1.5px solid #e0d6d7",background:active?"#FDF2F3":"#fff",cursor:"pointer",textAlign:"left",transition:"all 0.15s"}}>
    ...
    {workDirInfo && <><Divider/>
    <div style={{fontSize:12,fontWeight:600,color:C.text,marginBottom:8}}>Arbeitsverzeichnis</div>
    <div style={{fontSize:11,color:C.textSec,lineHeight:1.5,marginBottom:10}}>Alle erzeugten Artefakte landen hier. Konfigurierbar per Env <code style={{fontSize:10,background:C.rose,padding:"1px 6px",borderRadius:4}}>RUBIN_WORK_DIR</code> oder <code style={{fontSize:10,background:C.rose,padding:"1px 6px",borderRadius:4}}>constants.work_dir</code>.</div>
  ...
  <Sec title="Evaluierungsmodus">
    <Info>Bestimmt, wie die Modelle evaluiert werden: Cross-Validation (Standard) nutzt alle Daten effizient durch K-Fold Out-of-Fold-Predictions. External Eval trennt Training und Evaluation auf separate Datensätze.</Info>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
      {[{k:"cross",l:"Cross-Validation",d:"K-Fold OOF-Predictions – alle Daten effizient genutzt"},{k:"external",l:"Externer Datensatz",d:"Separater Eval-Datensatz – leakage-frei"}].map(o => {
        const active = cfg.validateOn === o.k;
        return (
          <button key={o.k} onClick={()=>set({...cfg,validateOn:o.k})} style={{padding:"14px 16px",borderRadius:10,border:active?"2px solid "+C.ruby:"1.5px solid "+C.border,background:active?C.rose:"#fff",cursor:"pointer",textAlign:"left",transition:"all 0.15s"}}
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <div>
                <div style={{fontSize:13,fontWeight:600,color:active?C.dark:"#555"}}>{o.l}</div>
    ...
    {cfg.validateOn==="external" && (
      cfg.eval_x_file && cfg.eval_t_file && cfg.eval_y_file
        ...
        : <Info type="warn">External Eval aktiviert, aber es fehlen noch Eval-Dateien. Diese kannst du auf der Seite <strong>Datenpfade</strong> eintragen{cfg.eval_x_file?` (Eval X bereits gesetzt: ${cfg.eval_x_file.split("/").pop()}, weitere fehlen)`:""}.</Info>
    )}
    {cfg.eval_mask_file && cfg.validateOn!=="external" && <Info type="warn"><strong>Train Many, Evaluate One</strong> aktiv: Mask-Rows werden <strong>vom Training ausgeschlossen</strong> und als Holdout für die Evaluation genutzt (Leakage-frei). Maske: <span style={{fontFamily:MONO,fontSize:11}}>{cfg.eval_mask_file.split("/").pop()}</span></Info>}
    {cfg.eval_mask_file && cfg.validateOn==="external" && <Info type="warn"><strong>Konflikt:</strong> <code style={{background:"rgba(255,255,255,0.5)",padding:"1px 5px",borderRadius:3}}>eval_mask_file</code> ist gesetzt, aber External Eval ist aktiv. Die Maske wird <strong>ignoriert</strong>, weil External-Eval-Dateien Vorrang haben. Entweder External Eval deaktivieren (nutze TMEO) oder die Mask-Datei entfernen.</Info>}
  </Sec>
  ...
  <Sec title="Tuning-Regularisierung" accent="#7c3aed">
    <Info>Steuert, wie aggressiv Overfitting beim Hyperparameter-Tuning bestraft wird. BLT (Nuisance) und FMT (CATE) nutzen bewusst unterschiedliche Einstellungen: BLT ist toleranter (DML-Orthogonalität kompensiert Nuisance-Overfitting), FMT strenger (CATE-Signal ist fragiler).</Info>
    {(()=>{
      const REG_PRESETS = [
        {key:"moderate", label:"Moderat", c:"#d97706",
         blt_p:0.2, blt_t:0.15, fmt_p:0.3, fmt_t:0.05},
        {key:"strong", label:"Stark", c:"#dc2626",
         blt_p:0.4, blt_t:0.08, fmt_p:0.6, fmt_t:0.03},
      ];
      const bp=cfg.overfitPenalty||0, bt=cfg.overfitTolerance===undefined?0.15:cfg.overfitTolerance;
      const fp=cfg.fmtOverfitPenalty||0, ft=cfg.fmtOverfitTolerance===undefined?0.05:cfg.fmtOverfitTolerance;
      const matched = REG_PRESETS.find(r => r.blt_p===bp && r.blt_t===bt && r.fmt_p===fp && r.fmt_t===ft);
      const activeKey = matched ? matched.key : (bp===0 && fp===0 ? null : "custom");
      const toggleReg = (r) => {
        if(activeKey===r.key) set({...cfg, overfitPenalty:0, overfitTolerance:0.15, fmtOverfitPenalty:0, fmtOverfitTolerance:0.05});
        else set({...cfg, overfitPenalty:r.blt_p, overfitTolerance:r.blt_t, fmtOverfitPenalty:r.fmt_p, fmtOverfitTolerance:r.fmt_t});
      };
      return (<>
        <div style={{display:"flex",gap:10,marginTop:4}}>
          {REG_PRESETS.map(r => {
            const active = activeKey===r.key;
            return (<button key={r.key} onClick={()=>toggleReg(r)} style={{flex:1,display:"flex",alignItems:"flex-start",gap:10,padding:"10px 14px",borderRadius:8,border:active?`1.5px solid ${r.c}`:`1.5px solid ${C.border}`,background:active?"rgba(124,58,237,0.03)":"#fff",cursor:"pointer",textAlign:"left",transition:"all 0.15s",fontFamily:"inherit"}}>
              <div style={{width:16,height:16,borderRadius:4,border:active?`2px solid ${r.c}`:"2px solid #ccc",background:active?r.c:"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:2}}>
                {active && <span style={{color:"#fff",fontSize:10,fontWeight:700}}>✓</span>}
              </div>
              <div>
                <div style={{fontSize:12.5,fontWeight:600,color:active?r.c:"#333"}}>{r.label}</div>
                <div style={{fontSize:10.5,color:"#888",marginTop:2,lineHeight:1.4}}>BLT: {Math.round(r.blt_t*100)}% Gap tol. · FMT: {Math.round(r.fmt_t*100)}% Gap tol.</div>
              </div>
            </button>);
          })}
        </div>
        ...
        {activeKey==="custom" && <div style={{fontSize:11,color:"#7c3aed",background:"#f5f3ff",padding:"8px 12px",borderRadius:8,marginTop:8,lineHeight:1.5,border:"1px solid #c4b5fd"}}>
  ...
  <Sec title="Feature-Selektion" accent="#0d9488">
    ...
    {cfg.fsEnabled&&<>
      <Divider/>
      <div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:10}}>1. Methoden auswählen</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
        {[
          {m:"catboost_importance",label:"CatBoost Gain",desc:"Native kategorische Features, Ordered Boosting → stabilere Importance."},
          {m:"lgbm_importance",label:"LightGBM Gain",desc:"Gain-Importance pro Feature über alle Splits."},
          {m:"causal_forest",label:"CausalForest",desc:"Kausale Relevanz: Feature-Heterogenität im Treatment-Effekt."},
      ...
      <Divider/>
      <div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:10}}>2. Parameter</div>
      <Row>
        <Col><Sld label="Top-% pro Methode" min={5} max={50} step={1} value={cfg.fsTopPct||15} onChange={v=>set({...cfg,fsTopPct:v})} help="Anteil der besten Features, die pro Methode behalten werden"/></Col>
        <Col><Sld label="Korrelations-Schwelle" min={0.5} max={1} step={0.05} value={cfg.fsCorrThresh||0.9} onChange={v=>set({...cfg,fsCorrThresh:v})} help="Bei Korrelation über dem Schwellenwert wird das weniger wichtige Feature entfernt"/></Col>
      </Row>
...
const rolesForModel = m => {
  ...
  if(n==="drlearner") return [
    ...
    {role:"model_regression",label:"Outcome-Regression (mit T)",sig:"outcome_regression__reg__with_t__all__Y"},
  ];
...
const computeTuningPlan = (models, perRole, perLearner, outerCv, innerCv) => {
  const tasks = {};
  (models || []).forEach(m => {
...
const TuningPlanPreview = ({models, perRole, perLearner, trials, cv, outerCv, innerCv: innerCvProp, isBoth, isRct}) => {
  const plan = computeTuningPlan(models, perRole, perLearner, outerCv, fullCv);
  ...
  const fitsForTask = (sig) => {
    ...
    // Pseudo-Effekt: Nuisance nutzt IMMER volle CV (nicht single_fold), CATE nutzt nCv
    if (fam==="pseudo_effect") return (nCv * K) + (fullCv * 2 + 2);
    return nCv;
  };

  const totalFits = plan.reduce((a,t) => a + trialsForTask(t.sig) * fitsForTask(t.sig), 0);
  const hasPropCap = isRct && nTrialsBase > RCT_PROP_CAP && plan.some(t => t.sig.split("__")[0] === "propensity");

  const sigColors = {"outcome":"#9B111E","outcome_regression":"#d97706","propensity":"#D4A853","grouped_outcome_regression":"#7c3aed","pseudo_effect":"#1a7f37"};
  const sigLabel = sig => {
    const fam = sig.split("__")[0];
  ...
  const fitsDetail = (sig) => {
    ...
    if (fam==="pseudo_effect") return nt+"T × ("+(nCv*K)+"cate + "+(fullCv*2+2)+"nuis)";
    return nt+"T × "+nCv+"F";
  };
  ...
  return (
    <div>
            ...
            <div key={t.key} style={{display:"grid",gridTemplateColumns:"36px 1fr 1fr 1fr 70px",padding:"7px 14px",borderBottom:"1px solid #f5f0f0",background:i%2===0?"#fff":"#fdfbfb",alignItems:"center",fontSize:12}}>
              ...
              <div>
                <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                  ...
                  {shared && <span style={{fontSize:10,color:"#1a7f37",fontWeight:600}}>shared</span>}
                  {t.sig.includes("__all__") && <span style={{fontSize:9,color:"#6b7280",fontWeight:500,background:"#f3f4f6",padding:"1px 6px",borderRadius:8,border:"1px solid #d1d5db"}}>Train: {Math.round((1-(1/(cv||5)))*100)}%</span>}
      ...
      {!perRole && !perLearner && plan.length < (models||[]).reduce((a,m)=>a+rolesForModel(m).length,0) && (
      ...
      {plan.some(t=>t.sig.includes("__all__")) && plan.some(t=>t.sig.includes("__all_direct__")) && (
        <div style={{fontSize:10.5,color:"#6b7280",marginTop:6,lineHeight:1.4}}>
          DML/DR-Nuisance-Tasks <span style={{background:"#f3f4f6",padding:"1px 5px",borderRadius:4,fontSize:9.5}}>Train: {Math.round((1-(1/(cv||5)))*100)}%</span> subsamplen die Trainingsmenge, um das DML-interne Cross-Fitting zu simulieren. Meta-Learner-Tasks trainieren auf der vollen Fold-Trainingsmenge.
        </div>
      )}
    </div>
  );
...
const FinalTuningPlanPreview = ({models, fmtEnabled, fmtModels, fmtSingleFold, fmtTrials, fmtCv, outerCv: outerCvProp, mcIters, isBoth}) => {
  ...
  return (
    <div>
      ...
      <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
        Jeder Trial fittet ein vollständiges kausales Modell inkl. internem Nuisance-Cross-Fitting (cv={internalCv}{mc>1?", mc_iters="+mc:""}). Ein Modell-Fit = {fitsPerDmlFit} interne Fits ({internalCv}×model_y + {internalCv}×model_t + 1×model_final). T = Trials, F = äußere Score-Folds.
          ...
          {vis.map(({m,d}) => {
            ...
            return (
              <button key={m} onClick={()=>{if(!ok)return;const s=new Set(cfg.models||[]);on?s.delete(m):s.add(m);set({...cfg,models:[...s]})}}
                style={{display:"flex",alignItems:"flex-start",gap:10,padding:"12px 14px",background:on?grp.bg:"#fff",border:on?`2px solid ${grp.color}`:`1.5px solid #e0d6d7`,borderRadius:10,cursor:ok?"pointer":"not-allowed",textAlign:"left",transition:"all 0.15s",opacity:ok?1:0.35}}>
...
<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
          {[
            ...
            {key:"lgbm",logos:[LGBM_LOGO],name:"LightGBM",sub:"Schneller, DART-Boosting als Default"},
            ...
            return (
              <button key={bl.key} onClick={()=>set({...cfg,baseLearner:bl.key,blFixed:{},fmtFixed:{}})}
                style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:6,padding:"18px 14px 14px",background:active?"#faf6f6":"#fff",border:active?"2px solid "+C.ruby:"1.5px solid #e0d6d7",borderRadius:12,cursor:"pointer",transition:"all 0.15s",position:"relative",minHeight:110}}
                onMouseEnter={e=>{if(!active){e.currentTarget.style.borderColor="#ccc";e.currentTarget.style.background="#fdfbfb"}}}
        ...
        </div>{_isBoth && <div style={{background:"#fff8e7",border:"1px solid #e8d49c",borderRadius:10,padding:"12px 16px",marginTop:12,fontSize:12,color:C.textSec,lineHeight:1.55}}><strong style={{color:"#7a5a00"}}>So funktioniert &quot;CatBoost &amp; LGBM&quot;:</strong> Dieser Modus entscheidet sich erst <strong>während des Tunings</strong> pro Task (model_y, model_t, model_final, ...) zwischen LightGBM und CatBoost. Das Tuning lernt, welcher Learner für die jeweilige Rolle besser abschneidet — am Ende kann model_y z.B. CatBoost nutzen und model_t LightGBM. Die Trial-Zahl wird dabei automatisch verdoppelt, damit beide Learner-Familien gleich viel Budget bekommen. <strong>Hinweis:</strong> Der Modus benötigt aktives Tuning. Ohne Tuning gibt es keine Entscheidungsgrundlage — dann wird CatBoost (globaler Default) verwendet.</div>}{!cfg.tuningEnabled && <><Divider/><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12,flexWrap:"wrap"}}><span style={{fontSize:13,fontWeight:600,color:C.dark}}>Base-Learner-Hyperparameter</span><span style={{fontSize:10,background:"#fffbeb",color:"#7a5a00",padding:"3px 12px",borderRadius:12,border:"1px solid #e8d49c",fontWeight:600,letterSpacing:0.2}}>Aktiv – Tuning ist deaktiviert</span></div><Info>Tuning ist deaktiviert. Diese Parameter werden direkt für alle Base-Learner-Modelle verwendet (Outcome, Propensity, CATE-Hilfsmodelle).</Info>{_isBoth ? (<>
...
</>) : (<FixedParamsEditor params={cfg.blFixed||{}} defaults={(cfg.baseLearner||"catboost")==="catboost"?CB_DEFAULTS:LGBM_DEFAULTS} onChange={v=>set({...cfg,blFixed:v})} label="Hyperparameter (Fixed)"/>)}{(()=>{const allTuned = cfg.tuningEnabled && ((cfg.tuningModels||[]).length===0);const someTuned = cfg.tuningEnabled && (cfg.tuningModels||[]).length>0;const noneTuned = !cfg.tuningEnabled;if(noneTuned) return <div style={{fontSize:11,color:"#059669",background:"#f0fdf4",padding:"6px 12px",borderRadius:8,marginTop:8,lineHeight:1.4,border:"1px solid #a7f3d0"}}>Alle Modelle verwenden diese Parameter direkt (kein Tuning aktiv).</div>;if(allTuned) return <div style={{fontSize:11,color:"#6b7280",background:"#f9fafb",padding:"6px 12px",borderRadius:8,marginTop:8,lineHeight:1.4,border:"1px solid #e5e7eb"}}>Tuning aktiv für alle Modelle — diese Parameter werden nur als Startpunkt für nicht-getunte Rollen verwendet.</div>;if(someTuned){const untuned=(cfg.models||[]).filter(m=>!(cfg.tuningModels||[]).includes(m));return <div style={{fontSize:11,color:"#d97706",background:"#fffbeb",padding:"6px 12px",borderRadius:8,marginTop:8,lineHeight:1.4,border:"1px solid #fbbf24"}}>Verwendet von: <strong>{untuned.join(", ")}</strong> (nicht im Tuning).</div>;}return null;})()}</>}</Sec><Sec title="Base-Learner-Tuning" accent="#C4343F"><Toggle label="Base-Learner-Tuning aktivieren" checked={cfg.tuningEnabled} onChange={v=>set({...cfg,tuningEnabled:v})}/>{_isBoth && !cfg.tuningEnabled && <div style={{fontSize:11.5,color:"#7a2e0e",background:"#fef2f2",padding:"10px 14px",borderRadius:8,border:"1px solid #e8b4b8",lineHeight:1.5,marginTop:8}}><strong>⚠ Hinweis:</strong> Im Modus &quot;CatBoost &amp; LGBM&quot; entscheidet Optuna pro Task, welcher Learner besser performt — dafür muss Tuning aktiv sein. Ohne Tuning kann keine Learner-Auswahl stattfinden, und es wird ausschließlich <strong>CatBoost</strong> als Fallback verwendet. Empfehlung: Tuning aktivieren, oder oben direkt den gewünschten Learner wählen.</div>}{cfg.tuningEnabled&&<><Divider/><div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:8}}>Tuning-Plan</div><TuningPlanPreview models={(cfg.tuningModels||[]).length>0?cfg.tuningModels:cfg.models} perRole={cfg.tuningPerRole||false} perLearner={cfg.tuningPerLearner||false} trials={cfg.tuningTrials||50} cv={cfg.tuningSingleFold?1:(cfg.dmlCrossfitFolds||5)} outerCv={cfg.cvSplits||5} innerCv={cfg.dmlCrossfitFolds||5} isBoth={_isBoth} isRct={(cfg.studyType||"rct")==="rct"}/><Divider/><Row><Col><Inp label="Trials" type="number" value={cfg.tuningTrials||50} onChange={v=>set({...cfg,tuningTrials:Number(v)})} help={_isBoth ? `Anzahl Kandidaten pro Study. Wird im "CatBoost & LGBM"-Modus automatisch verdoppelt (effektiv ${(cfg.tuningTrials||50)*2} Trials), um beiden Learner-Familien genug Budget zu geben.` : "Anzahl Kandidaten pro Study"}/></Col><Col><Toggle label="Single-Fold" checked={cfg.tuningSingleFold||false} onChange={v=>set({...cfg,tuningSingleFold:v})} help="1 Fold statt K pro Trial — bei K=5 also 5× schneller, aber verrauschtere Metrik."/></Col></Row><Row><Col><Sel label="Metrik (Klassifikation)" options={["log_loss","pr_auc","roc_auc","accuracy"]} value={cfg.tuningMetric||"log_loss"} onChange={v=>set({...cfg,tuningMetric:v})} help="Log-Loss (empfohlen): Misst die Kalibrierung der Nuisance-Vorhersagen. Gut kalibrierte P(Y|X)- und P(T|X)-Schätzungen liefern informativere Residuen für die CATE-Schätzung — das ist bei DML-Residualisierung entscheidend."/></Col><Col><Sel label="Metrik (Regression)" options={["neg_mse","neg_rmse","neg_mae","r2"]} value={cfg.tuningMetricReg||"neg_mse"} onChange={v=>set({...cfg,tuningMetricReg:v})} help="Neg. MSE (empfohlen): Binäres Y∈{0,1} wird in der EconML-Implementierung teilweise als Regression gelöst — MSE auf binären Targets entspricht dem Brier Score und misst direkt die Kalibrierung von P(Y|X,T)."/></Col></Row>{cfg.tuningSingleFold && <div style={{fontSize:11,color:C.textMuted,background:C.rose,padding:"6px 12px",borderRadius:8,marginTop:6,lineHeight:1.4}}>Single-Fold aktiv: Jeder Trial wird auf <strong style={{color:C.ruby}}>1</strong> statt {cfg.dmlCrossfitFolds||5} Folds evaluiert — {cfg.dmlCrossfitFolds||5}× schneller.</div>}{cfg.tuningSingleFold && (()=>{const K=cfg.dmlCrossfitFolds||5;if(dataStats){const ppf=Math.floor(dataStats.minority/K);if(ppf<100){const severe=ppf<50;return <div style={{fontSize:11,color:severe?"#991b1b":"#92400e",background:severe?"#fef2f2":"#fffbeb",padding:"8px 12px",borderRadius:8,marginTop:6,lineHeight:1.5,border:`1px solid ${severe?"#fca5a5":"#fbbf24"}`}}><strong>{severe?"⚠ Nicht empfohlen":"⚠ Grenzwertig"}:</strong> Bei Ihren Daten ({dataStats.n.toLocaleString()} Beob., {dataStats.nTreated.toLocaleString()} Treated, {dataStats.nPositive.toLocaleString()} Positive) landen nur <strong>{ppf}</strong> Minority-Fälle im Validierungs-Fold (empfohlen: ≥100, Minimum: ≥50). {severe?"Log-loss wird von einzelnen Samples dominiert — Tuning-Ergebnisse sind unzuverlässig. K-Fold CV dringend empfohlen.":"Die Metrik-Schätzung ist merklich verrauscht — K-Fold CV empfohlen."}</div>;}}else return <div style={{fontSize:11,color:"#6b7280",background:"#f9fafb",padding:"6px 12px",borderRadius:8,marginTop:6,lineHeight:1.5,border:"1px solid #e5e7eb"}}>Faustregel: min(n_treated, n_positive) / {K} ≥ 100 pro Val-Fold für zuverlässige Metrik-Schätzung (Collins et al.: min. 100, idealerweise 200+ Events für stabile Validation).</div>;return null;})()}<Divider/><div style={{fontSize:13,fontWeight:600,color:"#6B0D15",marginBottom:8}}>Modelle für BL-Tuning</div><Info>Standardmäßig werden alle ausgewählten Modelle getuned. Hier kann das Tuning auf bestimmte Modelle eingeschränkt werden — nicht ausgewählte Modelle nutzen die festen Hyperparameter (fixed_params).</Info><div style={{display:"flex",flexWrap:"wrap",gap:10,marginBottom:14}}>{(cfg.models||[]).map(m=>{const active=(cfg.tuningModels||[]).length===0||(cfg.tuningModels||[]).includes(m);return(<label key={m} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"6px 14px",borderRadius:8,border:active?"1.5px solid #C4343F":"1.5px solid "+C.border,background:active?"#FDF2F3":"#fff",cursor:"pointer",fontSize:12.5,fontWeight:active?600:400,transition:"all 0.15s"}}><input type="checkbox" checked={active} style={{accentColor:"#C4343F"}} onChange={e=>{const cur=cfg.tuningModels||[];if(cur.length===0){const s=new Set((cfg.models||[]).filter(x=>x!==m));if(s.size>0)set({...cfg,tuningModels:[...s]})}else{const s=new Set(cur);e.target.checked?s.add(m):s.delete(m);if(s.size===0)return;set({...cfg,tuningModels:s.size===(cfg.models||[]).length?[]:[...s]})}}}/>{m}</label>)})}</div>{(cfg.tuningModels||[]).length>0 && <div style={{fontSize:11,color:C.textMuted,background:C.rose,padding:"6px 12px",borderRadius:8,marginBottom:10,lineHeight:1.4}}>Eingeschränkt: Nur <strong style={{color:C.ruby}}>{(cfg.tuningModels||[]).join(", ")}</strong> werden getuned. Alle anderen nutzen fixed_params.</div>}<Divider/><div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:8}}>Task-Sharing</div><Info>Default: Nuisance-Modelle mit gleicher Signatur teilen sich ein Tuning. Per Role und Per Learner erzeugen spezialisierte Hyperparameter – genauer, aber rechenintensiver.</Info><Row><Col><Toggle label="Per Role" checked={cfg.tuningPerRole||false} onChange={v=>set({...cfg,tuningPerRole:v})} help="Separate Parameter je Rolle (model_y, model_t)"/></Col><Col><Toggle label="Per Learner" checked={cfg.tuningPerLearner||false} onChange={v=>set({...cfg,tuningPerLearner:v})} help="Separate Parameter je kausalem Modell"/></Col></Row><Divider/>{_isBoth ? (<>
...
</>) : (<FixedParamsEditor params={cfg.blFixed||{}} defaults={(cfg.baseLearner||"catboost")==="catboost"?CB_DEFAULTS:LGBM_DEFAULTS} onChange={v=>set({...cfg,blFixed:v})} label="Hyperparameter"/>)}</Expander><Expander title="Erweiterte Tuning-Einstellungen"><Row><Col><Inp label="Timeout (Sek., 0=unbegrenzt)" type="number" value={cfg.tuningTimeout||0} onChange={v=>set({...cfg,tuningTimeout:Number(v)})} help="Zeitlimit pro Study in Sekunden"/></Col></Row><Divider/><div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:6}}>Overfit-Penalty (Nuisance-Regularisierung)</div>
{(()=>{const p=cfg.overfitPenalty||0,t=cfg.overfitTolerance===undefined?0.15:cfg.overfitTolerance;const lbl=p===0?"Aus":p===0.2&&t===0.15?"Moderat":p===0.4&&t===0.08?"Stark":"Benutzerdefiniert";const clr=p===0?"#059669":p<=0.2?"#d97706":"#dc2626";return <div style={{fontSize:11,color:clr,background:p===0?"#f0fdf4":"#fffbeb",padding:"6px 12px",borderRadius:8,marginBottom:8,border:`1px solid ${clr}33`}}><strong>Aktuelle Einstellung: {lbl}</strong>{p>0&&` (Penalty ${p}, Tol. ${t})`} — global steuerbar unter Pipeline-Optionen → Tuning-Regularisierung.</div>;})()}<div style={{fontSize:11,color:C.textSec,lineHeight:1.5,marginBottom:10}}>Bestraft Hyperparameter-Konfigurationen, deren Nuisance-Modelle auf den Trainingsdaten deutlich besser performen als auf der Validierung (Train-Val-Gap). Dies verhindert, dass overfittende Nuisance-Modelle kausales Signal in den Residuals absorbieren. Bei 0 (Default) wird nur der Val-Score optimiert. Die Tolerance ist relativ: 0.15 = 15% Gap wird toleriert (für BLT höher als FMT, da DML-Orthogonalität moderates Nuisance-Overfitting kompensiert).</div><Row><Col><Sld label="Penalty-Faktor" min={0} max={1} step={0.05} value={cfg.overfitPenalty||0} onChange={v=>set({...cfg,overfitPenalty:v})} help="Stärke der Bestrafung. 0 = aus, 0.2 = moderat, 0.4 = stark."/></Col><Col><Sld label="Toleranz (Gap-Schwelle)" min={0} max={0.3} step={0.01} value={cfg.overfitTolerance===undefined?0.15:cfg.overfitTolerance} onChange={v=>set({...cfg,overfitTolerance:v})} help="Relativer Gap-Schwellwert: 0.15 = 15% des Score-Betrags. Für BLT höher als FMT, da DML-Orthogonalität moderate Nuisance-Overfitting kompensiert."/></Col></Row>{(cfg.overfitPenalty||0)>0 && <div style={{fontSize:11,color:"#3730a3",background:"#f0f4ff",padding:"8px 12px",borderRadius:8,marginTop:6,lineHeight:1.4,border:"1px solid #c7d2fe"}}><strong>Aktiv:</strong> Penalty={cfg.overfitPenalty}, Toleranz={cfg.overfitTolerance===undefined?0.15:cfg.overfitTolerance}. Relative Tolerance: {(cfg.overfitTolerance===undefined?5:Math.round(cfg.overfitTolerance*100))}% Gap toleriert. Penalty greift erst ab diesem relativen Train-Val-Unterschied.</div>}{_isBoth && (cfg.tuningTimeout||0)>0 && <Info type="warn">Timeout wird bei „CatBoost &amp; LGBM" automatisch ignoriert, damit beide Learner gleich viele Trials durchlaufen (LGBM-DART ist langsamer). Steuerung erfolgt über Trials.</Info>}</Expander></>}</Sec><Sec title="Final-Model-Tuning" accent="#D4A853"><Info>Optimiert das CATE-Effektmodell (model_final) via Optuna. Beide Modelle (NonParamDML, DRLearner) werden mit äußerer Cross-Validation bewertet: model_final wird auf Train gefittet und auf komplett Out-of-Fold-Val bewertet. NonParamDML wird via R-Loss bewertet (kausale Varianz), DRLearner via DR-MSE (doubly-robust).</Info><Toggle label="Final-Model-Tuning aktivieren" checked={cfg.fmtEnabled} onChange={v=>{const u={fmtEnabled:v};if(v&&(cfg.fmtModels||[]).length===0){u.fmtModels=(cfg.models||[]).filter(m=>["NonParamDML","DRLearner"].includes(m));}set({...cfg,...u});}} help="Optimiert model_final-Hyperparameter. Pro Trial wird ein volles DML/DR-Modell gefittet (inkl. Nuisance-Cross-Fitting)."/>{cfg.fmtEnabled&&<><Divider/><div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:8}}>Tuning-Plan</div><FinalTuningPlanPreview models={cfg.models} fmtEnabled={cfg.fmtEnabled} fmtModels={cfg.fmtModels||[]} fmtSingleFold={cfg.fmtSingleFold||false} fmtTrials={cfg.fmtTrials||50} fmtCv={cfg.dmlCrossfitFolds||5} outerCv={cfg.cvSplits||5} mcIters={cfg.mcIters||1} isBoth={_isBoth}/><Divider/><div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:8}}>Modelle für Final-Tuning</div><Info>Wähle, welche Modelle per OOF-CV optimiert werden. Nicht ausgewählte Modelle verwenden die festen Hyperparameter (unten).</Info><div style={{display:"flex",flexWrap:"wrap",gap:10,marginBottom:14}}>{(cfg.models||[]).filter(m=>["NonParamDML","DRLearner"].includes(m)).map(m=>{const active=(cfg.fmtModels||[]).includes(m);return(<label key={m} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"6px 14px",borderRadius:8,border:active?"1.5px solid #D4A853":"1.5px solid "+C.border,background:active?"#fffbeb":"#fff",cursor:"pointer",fontSize:12.5,fontWeight:active?600:400,transition:"all 0.15s"}}><input type="checkbox" checked={active} style={{accentColor:"#D4A853"}} onChange={e=>{const s=new Set(cfg.fmtModels||[]);e.target.checked?s.add(m):s.delete(m);set({...cfg,fmtModels:[...s]})}}/>{m}</label>)})}{!(cfg.models||[]).some(m=>["NonParamDML","DRLearner"].includes(m))&&<Info type="warn">Keine FMT-fähigen Modelle (NonParamDML/DRLearner) ausgewählt.</Info>}{(cfg.models||[]).some(m=>["NonParamDML","DRLearner"].includes(m)) && (cfg.fmtModels||[]).length===0 && <Info type="warn">Kein Modell für Final-Tuning ausgewählt. Alle verwenden die festen Hyperparameter.</Info>}</div><Row><Col><Inp label="Trials" type="number" value={cfg.fmtTrials||50} onChange={v=>set({...cfg,fmtTrials:Number(v)})}/></Col><Col><Toggle label="Single-Fold" checked={cfg.fmtSingleFold||false} onChange={v=>set({...cfg,fmtSingleFold:v})} help="1 OOF-Fold statt K pro Trial — bei K=5 also 5× schneller, aber verrauschtere Score-Schätzung (R-Loss bzw. DR-MSE)."/></Col></Row>{cfg.fmtSingleFold && <div style={{fontSize:11,color:C.textMuted,background:C.rose,padding:"6px 12px",borderRadius:8,marginTop:6,lineHeight:1.4}}>Single-Fold aktiv: Jeder Trial wird auf <strong style={{color:C.ruby}}>1</strong> statt {cfg.cvSplits||5} OOF-Folds evaluiert — {cfg.cvSplits||5}× schneller.</div>}{cfg.fmtSingleFold && (()=>{const K=cfg.cvSplits||5;if(dataStats){const ppf=Math.floor(dataStats.minority/K);if(ppf<100){const severe=ppf<50;return <div style={{fontSize:11,color:severe?"#991b1b":"#92400e",background:severe?"#fef2f2":"#fffbeb",padding:"8px 12px",borderRadius:8,marginTop:6,lineHeight:1.5,border:`1px solid ${severe?"#fca5a5":"#fbbf24"}`}}><strong>{severe?"⚠ Nicht empfohlen":"⚠ Grenzwertig"}:</strong> Bei Ihren Daten nur <strong>{ppf}</strong> Minority-Fälle im äußeren Val-Fold (empfohlen: ≥100, Minimum: ≥50). {severe?"OOF-Score ist nicht aussagekräftig. K-Fold CV dringend empfohlen.":"Die Score-Schätzung (R-Loss/DR-MSE) ist merklich verrauscht — K-Fold CV empfohlen."}</div>;}}else return <div style={{fontSize:11,color:"#6b7280",background:"#f9fafb",padding:"6px 12px",borderRadius:8,marginTop:6,lineHeight:1.5,border:"1px solid #e5e7eb"}}>Faustregel: min(n_treated, n_positive) / {K} ≥ 100 pro Val-Fold für zuverlässige Metrik-Schätzung (Collins et al.: min. 100, idealerweise 200+ Events für stabile Validation).</div>;return null;})()}<Divider/><Divider/>{_isBoth ? (<>
<div style={{fontSize:12,fontWeight:600,color:C.dark,marginBottom:6,marginTop:4}}>CatBoost</div>
<SSEditor bl="catboost" sp={spFmt||{lgbm:{},catboost:{}}} setSp={setSpFmt||setSp} fmt accent="#b8860b"/>
<div style={{fontSize:12,fontWeight:600,color:C.dark,marginBottom:6,marginTop:14}}>LightGBM</div>
<SSEditor bl="lgbm" sp={spFmt||{lgbm:{},catboost:{}}} setSp={setSpFmt||setSp} fmt accent="#b8860b"/>
</>) : (<SSEditor bl={cfg.baseLearner||"catboost"} sp={spFmt||{lgbm:{},catboost:{}}} setSp={setSpFmt||setSp} fmt accent="#b8860b"/>)}<Expander title="CATE-Modell-Defaults (Fixed)" accent="#b8860b"><div style={{fontSize:11,color:"#6b7280",lineHeight:1.5,marginBottom:10}}>Startpunkte für model_final. Bei aktivem FMT überschreibt Optuna die Parameter innerhalb des Suchraums — hier festgelegte Werte gelten nur für Parameter außerhalb des Suchraums.</div>{_isBoth ? (<>
<FixedParamsEditor params={(cfg.fmtFixed||{}).catboost||{}} defaults={CB_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:{...(cfg.fmtFixed||{}),catboost:v}})} label="CatBoost" accent="#b8860b"/>
<FixedParamsEditor params={(cfg.fmtFixed||{}).lgbm||{}} defaults={LGBM_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:{...(cfg.fmtFixed||{}),lgbm:v}})} label="LightGBM" accent="#b8860b"/>
</>) : (<FixedParamsEditor params={cfg.fmtFixed||{}} defaults={(cfg.baseLearner||"catboost")==="catboost"?CB_FINAL_DEFAULTS:LGBM_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:v})} label="CATE-Parameter" accent="#b8860b"/>)}</Expander><Expander title="Erweiterte Final-Tuning-Einstellungen"><Row><Col><Inp label="Timeout (Sek., 0=unbegrenzt)" type="number" value={cfg.fmtTimeout||0} onChange={v=>set({...cfg,fmtTimeout:Number(v)})} help="Zeitlimit pro Final-Model-Optuna-Study"/></Col></Row>{_isBoth && (cfg.fmtTimeout||0)>0 && <Info type="warn">Timeout wird bei „CatBoost &amp; LGBM" automatisch ignoriert (faire Trial-Allokation).</Info>}<Divider/><div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:6}}>Overfit-Penalty (model_final)</div>
{(()=>{const p=cfg.fmtOverfitPenalty||0,t=cfg.fmtOverfitTolerance===undefined?0.05:cfg.fmtOverfitTolerance;const lbl=p===0?"Aus":p===0.3&&t===0.05?"Moderat":p===0.6&&t===0.03?"Stark":"Benutzerdefiniert";const clr=p===0?"#059669":p<=0.3?"#d97706":"#dc2626";return <div style={{fontSize:11,color:clr,background:p===0?"#f0fdf4":"#fffbeb",padding:"6px 12px",borderRadius:8,marginBottom:8,border:`1px solid ${clr}33`}}><strong>Aktuelle Einstellung: {lbl}</strong>{p>0&&` (Penalty ${p}, Tol. ${t})`} — global steuerbar unter Pipeline-Optionen → Tuning-Regularisierung.</div>;})()}<div style={{fontSize:11,color:C.textSec,lineHeight:1.5,marginBottom:10}}>Bestraft model_final-Konfigurationen, deren Score auf den Trainingsdaten deutlich besser ist als auf der Validierung (OOF). Ein großer Train-Val-Gap bedeutet, dass model_final Rauschen statt echte Heterogenität gelernt hat. Bei 0 (Default) wird nur der OOF-Score optimiert. Tolerance ist strenger als bei BLT (5% statt 10%), weil CATE-Signal schwächer ist als Outcome-Signal.</div><Row><Col><Sld label="Penalty-Faktor" min={0} max={1} step={0.05} value={cfg.fmtOverfitPenalty||0} onChange={v=>set({...cfg,fmtOverfitPenalty:v})} help="Stärke der Bestrafung. 0 = aus, 0.3 = moderat, 0.6 = stark."/></Col><Col><Sld label="Toleranz (Gap-Schwelle)" min={0} max={0.2} step={0.01} value={cfg.fmtOverfitTolerance===undefined?0.05:cfg.fmtOverfitTolerance} onChange={v=>set({...cfg,fmtOverfitTolerance:v})} help="Relativer Gap-Schwellwert: 0.05 = 5% des Score-Betrags. Skalen-sicher über R-Loss und DR-MSE."/></Col></Row>{(cfg.fmtOverfitPenalty||0)>0 && <div style={{fontSize:11,color:"#92400e",background:"#fffbeb",padding:"8px 12px",borderRadius:8,marginTop:6,lineHeight:1.4,border:"1px solid #fbbf24"}}><strong>Aktiv:</strong> Penalty={cfg.fmtOverfitPenalty}, Toleranz={cfg.fmtOverfitTolerance===undefined?0.05:cfg.fmtOverfitTolerance}</div>}</Expander></>}{!cfg.fmtEnabled && (cfg.models||[]).some(m=>["NonParamDML","DRLearner"].includes(m)) && <><Divider/><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12,flexWrap:"wrap"}}><span style={{fontSize:13,fontWeight:600,color:C.dark}}>Final-Modell-Hyperparameter</span><span style={{fontSize:10,background:"#fffbeb",color:"#7a5a00",padding:"3px 12px",borderRadius:12,border:"1px solid #e8d49c",fontWeight:600,letterSpacing:0.2}}>Direkt verwendet</span></div><Info>Final-Model-Tuning ist deaktiviert. Diese Parameter werden direkt für das CATE-Effektmodell (model_final) von NonParamDML und DRLearner verwendet.</Info>{_isBoth ? (<>
<div style={{fontSize:12,fontWeight:600,color:C.dark,marginBottom:6,marginTop:4}}>CatBoost-Parameter (Final-Modell)</div>
<FixedParamsEditor params={(cfg.fmtFixed||{}).catboost||{}} defaults={CB_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:{...(cfg.fmtFixed||{}),catboost:v}})} label="CatBoost CATE-Parameter (Fixed)" accent="#b8860b"/>
<div style={{fontSize:12,fontWeight:600,color:C.dark,marginBottom:6,marginTop:14}}>LightGBM-Parameter (Final-Modell)</div>
<FixedParamsEditor params={(cfg.fmtFixed||{}).lgbm||{}} defaults={LGBM_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:{...(cfg.fmtFixed||{}),lgbm:v}})} label="LightGBM CATE-Parameter (Fixed)" accent="#b8860b"/>
</>) : (<FixedParamsEditor params={cfg.fmtFixed||{}} defaults={(cfg.baseLearner||"catboost")==="catboost"?CB_FINAL_DEFAULTS:LGBM_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:v})} label="CATE-Parameter (Fixed)" accent="#b8860b"/>)}</>}</Sec>{((cfg.models||[]).includes("CausalForestDML")||(cfg.models||[]).includes("CausalForest"))&&<Sec title="CausalForest-Tuning" accent="#2d6a4f"><Info>CausalForest und CausalForestDML werden per Optuna über Wald-Parameter (max_depth, min_samples_leaf, max_features, criterion etc.) optimiert. Jeder Trial wird via R-Loss auf Tuning-CV-Folds bewertet — unabhängig vom Nuisance-Tuning (BLT) und CATE-Tuning (FMT).</Info><Toggle label="CausalForest-Tuning aktivieren" checked={cfg.cfTune||false} onChange={v=>{const u={cfTune:v};if(v&&(cfg.cfTuneModels||[]).length===0){u.cfTuneModels=(cfg.models||[]).filter(m=>["CausalForestDML","CausalForest"].includes(m));}set({...cfg,...u});}} help="Optimiert Wald-Parameter (max_depth, min_samples_leaf, max_features etc.) per Optuna-TPE. Jeder Trial wird via R-Loss auf Tuning-CV-Folds bewertet. Normal: 50 Trials, Intensiv: 100 Trials."/>{cfg.cfTune&&<><Divider/><div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:8}}>Tuning-Plan</div>{(() => {
              ...
              const cfdmlTotal = nTrials * evalFolds * fitsPerCfdmlFold;
              const cfTotal = nTrials * evalFolds + evalFolds * 2;
              return (<>
                <div style={{display:"flex",gap:10,marginBottom:12,flexWrap:"wrap"}}>
                ...
                <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
                  Jeder Trial konstruiert ein Forest-Modell mit Kandidaten-Params, fittet auf {dc-1} von {dc} Folds{cfSF?" (Single-Fold: nur 1. Fold bewertet)":""} (tuning_cv_seed) und bewertet via est.score() (R-Loss) auf dem Validierungs-Fold. TPE-Sampler mit MedianPruner. CausalForestDML nutzt BLT-getunte Nuisance-Modelle.
                </div>
                <div style={{border:"1px solid #ede6e7",borderRadius:8,overflow:"hidden"}}>
                  ...
                  {hasCF && <div style={{borderBottom:"1px solid #f0f0f0"}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 80px",padding:"8px 14px",fontSize:12}}>
                      ...
                      <span style={{fontSize:10.5,color:"#2d6a4f",fontFamily:MONO}}>{nTrials}T × {evalFolds}F × {fitsPerCfdmlFold}</span>
                      <span style={{fontFamily:MONO,fontWeight:700,color:"#2d6a4f"}}>{cfdmlTotal.toLocaleString()}</span>
                    </div>
                    <div style={{fontSize:10.5,color:"#999",padding:"0 14px 6px",lineHeight:1.4}}>Pro Fold: est.fit() = {dc}×model_y + {dc}×model_t + 1×Forest{mc>1?" × "+mc+" mc_iters":""} = {fitsPerCfdmlFold} Fits. 8 Wald-Params.</div>
                  </div>}
                  {hasGRF && <div style={{background:hasCF?"#fdfbfb":"#fff"}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 80px",padding:"8px 14px",fontSize:12}}>
                      <span style={{fontWeight:600}}>CausalForest</span>
                      <span style={{fontSize:10.5,color:"#2d6a4f",fontFamily:MONO}}>{nTrials}T × {evalFolds}F + {evalFolds}×2 Nuisance</span>
                      <span style={{fontFamily:MONO,fontWeight:700,color:"#2d6a4f"}}>{cfTotal.toLocaleString()}</span>
                    </div>
                    <div style={{fontSize:10.5,color:"#999",padding:"0 14px 6px",lineHeight:1.4}}>1 Forest-Fit pro Fold + {evalFolds}×2 Nuisance-Modelle (einmalig vorberechnet für R-Loss). 9 Wald-Params (+ criterion).</div>
                  </div>}
                </div>
              </>);
            })()}<Divider/><div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:8}}>Modelle für CausalForest-Tuning</div><Info>Wähle, welche Forest-Modelle per Optuna optimiert werden. Nicht ausgewählte Modelle verwenden Default-Parameter.</Info><div style={{display:"flex",flexWrap:"wrap",gap:10,marginBottom:14}}>{(cfg.models||[]).filter(m=>["CausalForestDML","CausalForest"].includes(m)).map(m=>{const active=(cfg.cfTuneModels||[]).includes(m);return(<label key={m} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"6px 14px",borderRadius:8,border:active?"1.5px solid #2d6a4f":"1.5px solid "+C.border,background:active?"#e8f5ec":"#fff",cursor:"pointer",fontSize:12.5,fontWeight:active?600:400,transition:"all 0.15s"}}><input type="checkbox" checked={active} style={{accentColor:"#2d6a4f"}} onChange={e=>{const s=new Set(cfg.cfTuneModels||[]);e.target.checked?s.add(m):s.delete(m);set({...cfg,cfTuneModels:[...s]})}}/>{m}</label>)})}{!(cfg.models||[]).some(m=>["CausalForestDML","CausalForest"].includes(m))&&<Info type="warn">Keine Forest-Modelle (CausalForestDML/CausalForest) in der Modellauswahl.</Info>}{(cfg.models||[]).some(m=>["CausalForestDML","CausalForest"].includes(m)) && (cfg.cfTuneModels||[]).length===0 && <Info type="warn">Kein Modell für CausalForest-Tuning ausgewählt. Alle verwenden Default-Parameter.</Info>}</div><Divider/><Row><Col><Inp label="Trials" type="number" value={cfg.cfTrials||50} onChange={v=>set({...cfg,cfTrials:Number(v)})} help="Anzahl Optuna-Trials für Wald-Parameter-Optimierung."/></Col><Col><Toggle label="Single-Fold" checked={cfg.cfSingleFold||false} onChange={v=>set({...cfg,cfSingleFold:v})} help="1 Fold statt K pro Trial — bei K=5 also 5× schneller, aber verrauschtere R-Loss-Schätzung."/></Col></Row>{cfg.cfSingleFold && <div style={{fontSize:11,color:C.textMuted,background:C.rose,padding:"6px 12px",borderRadius:8,marginTop:6,lineHeight:1.4}}>Single-Fold aktiv: Jeder Trial wird auf <strong style={{color:"#2d6a4f"}}>1</strong> statt {cfg.dmlCrossfitFolds||5} Folds evaluiert — {cfg.dmlCrossfitFolds||5}× schneller.</div>}</>}{cfg.cfTune && <><Divider/>
{(()=>{
  const cfSS = cfg._cfSS || {};
  const g = k => ({low: cfSS[k]?.low ?? ({n_estimators:100,max_depth:3,min_samples_leaf:50,min_samples_split:100,max_features:0.1,max_samples:0.3,min_var_fraction_leaf:0.001,min_impurity_decrease:0.0})[k], high: cfSS[k]?.high ?? ({n_estimators:400,max_depth:8,min_samples_leaf:500,min_samples_split:1000,max_features:0.5,max_samples:0.9,min_var_fraction_leaf:0.1,min_impurity_decrease:0.01})[k]});
  const s = (k,l,h) => set({...cfg, _cfSS:{...cfSS,[k]:{low:l,high:h}}});
  return (<div>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
      <span style={{fontSize:13.5,fontWeight:700,color:"#2d6a4f"}}>Wald-Parameter Suchraum</span>
      <span style={{fontSize:10.5,color:"#2d6a4f",opacity:0.7,background:"#2d6a4f12",padding:"2px 10px",borderRadius:10,border:"1px solid #2d6a4f30"}}>Optuna low – high</span>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 14px"}}>
    ...
    <div style={{fontSize:11,color:"#6b7280",marginTop:10,lineHeight:1.5}}>
      TPE-Sampler mit MedianPruner. Evaluation via est.score() (R-Loss) auf Tuning-CV-Folds.
    </div>
  </div>);
})()}
<Divider/><Expander title="Wald-Parameter (Defaults / Fixed)" accent="#2d6a4f"><div style={{fontSize:11,color:"#6b7280",lineHeight:1.5,marginBottom:10}}>Diese Parameter werden als Ausgangswerte verwendet. Bei aktivem Tuning überschreibt Optuna die Parameter innerhalb des Suchraums — hier festgelegte Werte gelten dann nur für Parameter, die nicht im Suchraum liegen.</div><FixedParamsEditor params={cfg.cfFixed||{}} defaults={CF_FOREST_DEFAULTS} onChange={v=>set({...cfg,cfFixed:v})} label="Wald-Parameter" accent="#2d6a4f"/></Expander>
</>}
{!cfg.cfTune && <><Divider/><Expander title="Wald-Parameter (Defaults / Fixed)" accent="#2d6a4f"><div style={{fontSize:11,color:"#059669",background:"#f0fdf4",padding:"6px 12px",borderRadius:8,lineHeight:1.4,marginBottom:10,border:"1px solid #a7f3d0"}}>Tuning deaktiviert — diese Parameter werden direkt für alle Forest-Modelle verwendet.</div><FixedParamsEditor params={cfg.cfFixed||{}} defaults={CF_FOREST_DEFAULTS} onChange={v=>set({...cfg,cfFixed:v})} label="Wald-Parameter" accent="#2d6a4f"/></Expander></>}</Sec>}</>);
};
...
const PSelection = ({cfg,set}) => {
  ...
  return (<>
    ...
    <Sec title="Bundle-Export" accent="#1a7f37">
  ...
  {cfg.explEnabled&&<Sec title="Modellauswahl" accent="#D4A853">
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:10,marginTop:4}}>
      {(cfg.models||[]).map(m => {
        ...
        return (
          <label key={m} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:8,border:checked?"1.5px solid #D4A853":"1.5px solid "+C.border,background:checked?"#fffbeb":"#fff",cursor:"pointer",fontSize:13,fontWeight:checked?600:400,transition:"all 0.15s"}}>
    ...
    {(cfg.shapModels||[]).length > 0 && <div style={{fontSize:11,color:C.textMuted,marginTop:8}}>Zusätzlich zum Champion werden {(cfg.shapModels||[]).length} Modell(e) analysiert.</div>}
  </Sec>}
</>);
...
const buildYaml = (cfg, sp, spFmt) => {
  ...
  a("constants:");
  a(`  SEED: ${cfg.seed||42}`);
  a(`  tuning_seed: ${cfg.tuningSeed||18}`);
  ...
  if(cfg.tuningEnabled) {
    ...
    if(cfg.tuningSingleFold) a("  single_fold: true");if((cfg.overfitPenalty||0)>0){a(`  overfit_penalty: ${cfg.overfitPenalty}`);a(`  overfit_tolerance: ${cfg.overfitTolerance===undefined?0.15:cfg.overfitTolerance}`);}
  ...
  if(cfg.fmtEnabled) {
    ...
    if((cfg.fmtOverfitPenalty||0)>0){a(`  overfit_penalty: ${cfg.fmtOverfitPenalty}`);a(`  overfit_tolerance: ${cfg.fmtOverfitTolerance===undefined?0.05:cfg.fmtOverfitTolerance}`);}
...
const PPreview = ({cfg,sp,spFmt}) => {const y=buildYaml(cfg,sp,spFmt),issues=validate(cfg);const mt=cfg.treatmentType==="multi";return(<>
  <Row gap={12}>
    ...
    <MC value={cfg.tuningEnabled?(cfg.tuningTrials||50)+"T":"Aus"} label="Tuning"/>
    ...
    <MC value={["Min","Mod","Hoch","Max"][(cfg.parallelLevel||3)-1]} label="Parallel"/>
  </Row>
  <div style={{height:16}}/>
        ...
        {!running && !done && issues.length === 0 && (
          <>
            ...
            <Row gap={12}>
              ...
              <MC value={(cfg.models||[]).length} label="Modelle"/>
              <MC value={cfg.tuningEnabled?(cfg.tuningTrials||50)+"T":"Aus"} label="Tuning"/>
              <MC value={cfg.validateOn==="external"?"External":cfg.eval_mask_file?"TMEO":"CV-"+(cfg.cvSplits||5)} label="Validierung"/>
              <MC value={cfg.bundleEnabled?"An":"Aus"} label="Bundle"/>
              <MC value={["Min","Mod","Hoch","Max"][(cfg.parallelLevel||3)-1]} label="Parallel"/>
            </Row>
          </>
...
function App() {
  ...
  const INIT_CFG = {studyType:"rct",expName:"rubin",seed:42,tuningSeed:18,parallelLevel:3,workDir:null,x_file:"",t_file:"",y_file:"",s_file:"",treatmentType:"binary",refGroup:0,hasNaN:false,nanCols:[],validateOn:"cross",cvSplits:5,downsample:false,dfFrac:0.1,reduceMem:true,fsEnabled:false,fsMethods:["catboost_importance"],fsTopPct:15,fsCorrThresh:0.9,outputDir:"",models:["NonParamDML","DRLearner","SLearner","TLearner","XLearner","ParamDML","CausalForestDML","CausalForest"],baseLearner:"catboost",tuningEnabled:true,tuningTrials:50,tuningSingleFold:false,tuningMetric:"log_loss",tuningMetricReg:"neg_mse",tuningPerRole:false,tuningPerLearner:false,tuningModels:[],fmtEnabled:false,fmtModels:[],fmtSingleFold:false,fmtTrials:50,fmtMaxRows:0,cfTune:false,cfTrials:50,cfSingleFold:false,cfTuneModels:[],selMetric:"qini",higherBetter:true,refitChamp:true,manualChamp:null,surrEnabled:false,surrMinLeaf:50,surrLeaves:31,surrDepth:0,bundleEnabled:false,bundleDir:"runs/bundles",bundleChallengers:true,bundleMlflow:true,explEnabled:false,explSampleSize:10000,explTopN:20,shapModels:[],shapBins:10,histScoreName:"historical_score",histScoreCol:"S",histScoreHigher:true,fsMaxFeatures:0,maxPredRows:0,tuningTimeout:0,tuningMaxRows:0,fmtTimeout:0,blFixed:{},fmtFixed:{},cfFixed:{},ensembleEnabled:false,mcIters:null,mcAgg:"mean",fmtOverfitPenalty:0.0,fmtOverfitTolerance:0.05,dmlCrossfitFolds:5,overfitPenalty:0.0,overfitTolerance:0.15};
  ...
  const pageStatus = (() => {
    ...
    if(!cfg.expName) s.config = {st:"required",detail:"Experiment-Name fehlt"};
      ...
      const valLabel = cfg.validateOn==="cross" ? cfg.cvSplits+"F Cross-Val" : cfg.validateOn==="external" ? "External Eval" : "TMEO";
    ...
    if(!cfg.explEnabled) s.explain = {st:"open",detail:"Deaktiviert"};
      ...
      const nShapModels = (cfg.shapModels||[]).length || "alle";
      s.explain = {st:"done",detail:"SHAP, "+cfg.explSampleSize+" Samples, "+nShapModels+" Modelle"};
    }
  ...
  const estimateFits = () => {
    ...
    const fitsPerFit = (x) => {
      if (["NonParamDML","ParamDML","CausalForestDML","DRLearner"].includes(x)) return mc*innerCv*2+1;
    ...
    if (cfg.tuningEnabled) {
      ...
      // Task 7: Pseudo-Effekt (XL cate_models) — Nuisance + CATE
      if (hasXl) f += tr * ((tc * K) + (innerCv * 2 + 2));
    }
    ...
    // 4. CF Tuning — Optuna-basiert
    if (cfg.cfTune) {
      const cfTrials = cfg.cfTrials || 50;
      const cfFolds = cfg.cfSingleFold ? 1 : innerCv;
      // CausalForestDML: jeder Trial = cfFolds × est.fit(), jedes est.fit() = mc*innerCv*2+1 interne Fits
      if ((cfg.cfTuneModels||[]).includes("CausalForestDML") && m.includes("CausalForestDML")) f += cfTrials * cfFolds * (mc*innerCv*2+1);
      // CausalForest: jeder Trial = cfFolds × 1 Forest-Fit + Nuisance-Vorberechnung (cfFolds × 2)
      if ((cfg.cfTuneModels||[]).includes("CausalForest") && m.includes("CausalForest")) f += cfTrials * cfFolds + cfFolds * 2;
    }

    // 5. Training
    // Cross/TMEO: K-Fold CV + Train-Predictions-Fit (+1 bei SHAP für keep-fold)
    // External: nur 1 Fit auf Trainingsdaten, Predictions via predict() auf Holdout
    // TMEO (eval_mask_file gesetzt) läuft seit dem Leakage-Fix wie External
    // (Mask-Rows → Holdout, Training auf ~mask → nur 1 Fit pro Modell)
    const isHoldoutMode = cfg.validateOn === "external" || !!cfg.eval_mask_file;
    const isExternal = isHoldoutMode;  // Alias für Training-Loop-Klarheit
    m.forEach(x => {
    ...
    // 6. DRTester Nuisance
    // Cross/TMEO: has_train=True → fit_nuisance auf Train UND Val (4×5 = 20 Fits @ cv=5)
    // External: has_train=False (Train_-Spalten = NaN) → nur Val (2×5 = 10 Fits)
    f += isExternal ? 10 : 20;
    ...
    // Ohne Bundle: Champion-Refit
    else if (cfg.refitChamp) {
      f += fitsPerFit(m[0] || "SLearner");
    }

    // 9. Ensemble-Refit: alle Modelle brauchen Refit
    if (cfg.refitChamp && cfg.ensembleEnabled && !bundle) {
      m.forEach(x => { f += fitsPerFit(x); });
    }
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",fontFamily:FONT,fontSize:15,color:"#24292f",background:"#faf7f8"}}>
      ...
      <main style={{flex:1,maxWidth:960,padding:"36px 52px 80px",margin:"0 auto"}}>
        ...
        {pg==="datafiles" && <PData cfg={cfg} set={set} setCfg={set} activeBase={activeBase} setActiveBase={setActiveBase} activeAddons={activeAddons} setActiveAddons={setActiveAddons} setSp={setSp} setSpFmt={setSpFmt} view="files"/>}
        {pg==="template" && <PData cfg={cfg} set={set} setCfg={set} activeBase={activeBase} setActiveBase={setActiveBase} activeAddons={activeAddons} setActiveAddons={setActiveAddons} setSp={setSp} setSpFmt={setSpFmt} view="template"/>}
        {pg==="config" && <PConfig cfg={cfg} set={set}/>}
        {pg==="models" && <PModels cfg={cfg} set={set} sp={sp} setSp={setSp} spFmt={spFmt} setSpFmt={setSpFmt} dataStats={dataStats}/>}
        {pg==="selection" && <PSelection cfg={cfg} set={set}/>}
        {pg==="explain" && <PExplain cfg={cfg} set={set}/>}
        {pg==="preview" && <PPreview cfg={cfg} sp={sp} spFmt={spFmt}/>}
        <div key={"run-"+resetKey} style={{display: pg==="run" ? "block" : "none"}}><PRun cfg={cfg} setPg={setPg} sp={sp} spFmt={spFmt} onRunningChange={setAnalysisRunning} onDoneChange={setAnalysisDone}/></div>

...
Wird das Ensemble Champion und `refit_champion_on_full_data: true`, werden beim Bundle-Export **alle Einzelmodelle** auf vollen Daten refittet und anschließend ein neues `EnsembleCateEstimator` mit den refitteten Modellen erstellt. Das Ensemble selbst hat kein eigenes `fit()` — es aggregiert die Vorhersagen seiner Mitglieder.

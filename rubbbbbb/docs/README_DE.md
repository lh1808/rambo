...
- **Analyse-Pipeline**: Trainieren, Feature-Selektion (Korrelation + Importance-Union mit Umverteilung, exaktes Feature-Budget), 3-stufiges Tuning (BLT via Optuna + FMT mit OOF-CV + CFT für CausalForest, jeweils mit Overfit-Penalty und Skill Scores), Evaluieren (DRTester + native Uplift-Plots: Qini, Uplift-by-Percentile, Treatment-Balance + ATE-Barplot), Surrogate-Einzelbaum (Champion), Explainability (SHAP), HTML-Report mit klickbaren Plots (Lightbox).

...
class BaseLearnerTuner:
    ...
    def _objective_all_regression(self, params: Dict[str, Any], X_mat: np.ndarray, target: np.ndarray, strata: np.ndarray, train_ratio: float = 1.0, trial=None) -> float:
        """Tuning-Objective für Regressoren auf dem Gesamt-Datensatz (SLearner, TLearner, XLearner Outcome).
        ...
        if name == "nonparamdml":
            ...
            for fold_i, (tr, va) in enumerate(split_iter):
                if fold_i >= _n_cache_folds:
                    break
                est = NonParamDML(
                    # parallel_jobs=-1: Nuisance-Pre-Fit läuft sequentiell (1 Fold nach dem anderen),
                    # daher alle Kerne pro Fit nutzbar (kein paralleles Optuna, das Kerne braucht).
                    ...
                    model_t=build_base_learner(base_type, {**base_fixed_params, **tuned_roles.get("model_t", {})}, seed=self.seed, task="classifier", parallel_jobs=-1),
            ...
            _tlog.info(
                ...
                "Trials fitten nur noch model_final via refit_final().",
                model_name, len(_cached_folds), int(self.cfg.final_model_tuning.cv_splits),
            )
            ...
            for fold_i, (tr, va) in enumerate(split_iter):
                if fold_i >= _n_cache_folds:
                    break
                est = DRLearner(
                    model_propensity=build_base_learner(base_type, {**base_fixed_params, **tuned_roles.get("model_propensity", {})}, seed=self.seed, task="classifier", parallel_jobs=-1),
...
_CF_SEARCH_SPACE = {
    ...
    "min_samples_leaf":     {"type": "int",   "low": 5,     "high": 200,   "log": True},  # EconML Default: 5
    "min_samples_split":    {"type": "int",   "low": 10,    "high": 500,   "log": True},  # EconML Default: 10
    ...
    "min_var_fraction_leaf":{"type": "float", "low": 1e-6,  "high": 0.05,  "log": True},  # EconML Default: None → 1e-6 ≈ None
...
def tune_causal_forest(
    ...
    if not is_cfdml:
        ...
        for _fi, (_tr, _va) in enumerate(_splits_pre[:_n_pre]):
            ...
            # Laufen sequentiell (Fold für Fold) — jeder Fit nutzt alle CPUs.
            # Beide Nuisance sind Classifier (discrete_outcome=True, discrete_treatment=True),
            # konsistent mit NonParamDML/CausalForestDML model_registry.
    ...
    if is_cfdml:
        ...
        _tlog.info(
            ...
            "Trials fitten nur noch den CausalForest via refit_final().",
            model_type, len(_cfdml_cached_folds), inner_cv,
        )
    ...
    def objective(trial):
        ...
        if is_cfdml:
            ...
            for fold_i, (est, tr, va) in enumerate(_cfdml_cached_folds):
                try:
                    est.set_params(**{k: v for k, v in params.items() if k not in ("criterion",)})
                    est.refit_final()

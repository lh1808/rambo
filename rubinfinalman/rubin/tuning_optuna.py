    ...
    def tune_final_model(
        ...
        if name == "nonparamdml":
            ...
            if _fmt_is_both and _fmt_timeout:
                ...
                _fmt_timeout = None
            _tlog.info(
                "FMT '%s': Starte %d Trials (parallel=%d, %s, n_jobs=%d).",
                model_name, _fmt_nt, _fmt_pj, "Single-Fold" if _sf else f"{_outer_splits}-Fold", self._parallel_jobs_per_fit(),
            )
...
def tune_causal_forest(
        ...
        if is_cfdml:
            ...
            for fold_i, (tr, va) in enumerate(split_iter):
                ...
                try:
                    ...
                    # parallel_jobs=-1 für Nuisance: EconML's Cross-Fitting-Loop
                    # ist sequentiell (_OrthoLearner, kein ray). Nur EIN Fit läuft
                    # gleichzeitig → volle CPU-Auslastung, keine Übersubskription.
                    # CFDML(n_jobs=-1) parallelisiert den CausalForest-Baum-Fit,
                    # NICHT die Nuisance-Folds.
                    est = CausalForestDML(
    ...
    _tlog.info(
        "CFT '%s': Starte %d Trials (parallel=1, %s, n_jobs=-1).",

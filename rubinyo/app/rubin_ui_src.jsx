  ...
  eligible.forEach(m => {
    ...
    if(mn === "nonparamdml") {
      ...
      rows.push({model:m, method:"RScorer", trials:nTrials, fitsPerTrial:internalFits, totalFits:total,
        ...
        note:"Pro Trial: 1 NonParamDML-Fit (cv="+dc+": "+dc+"x model_y + "+dc+"x model_t"+(mc>1?" x "+mc+" mc_iters":"")+", + 1x model_final = "+internalFits+")."});
    }
    if(mn === "drlearner") {
      const drFolds = fmtSingleFold ? 1 : nCv;
      const internalPerFold = mc*dc*2+1;
      ...
      rows.push({model:m, method:fmtSingleFold?"score() 1-Fold":"score() + CV", trials:nTrials, fitsPerTrial, totalFits:total,
        ...
        note:"Pro Trial: "+drFolds+" Fold(s), je 1 DRLearner-Fit (cv="+dc+": "+dc+"x propensity + "+dc+"x regression"+(mc>1?" x "+mc+" mc_iters":"")+", + 1x model_final = "+internalPerFold+")."});
    }
  });

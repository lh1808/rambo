...
**Score-Konvention:** `est.score()` gibt in EconML die MSE zurück (lower=better):
- **NonParamDML:** MSE der Residual-auf-Residual-Regression (R-Loss): `E[(Y_resid - τ(X) × T_resid)²]`
- **DRLearner:** MSE der DR-Pseudo-Outcome-Regression: `E[(Y_DR - θ(X))²]`

Da Optuna `direction="maximize"` verwendet (konsistent mit dem BLT, wo log_loss negiert wird), werden die FMT-Scores ebenfalls negiert (`-MSE`), sodass höhere Werte bessere Modelle anzeigen.

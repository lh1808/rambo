...
class BaseLearnerTuner:
    ...
    def _create_study(self, study_key: str):
        ...
        _blt_cv_folds = 1 if self.cfg.tuning.single_fold else (self.cfg.tuning.cv_splits or 5)
        pj = self._tuning_n_jobs(_blt_cv_folds)
    ...
    def _fit_model(self, params: Dict[str, Any], X_train: np.ndarray, y_train: np.ndarray, estimator_task: str):
        ...
        if pl <= 1:
            ...
            n_trial_workers = self._tuning_n_jobs(_blt_cv_folds)
    ...
    def _tune_task(self, task: TuningTask, X: pd.DataFrame, Y: np.ndarray, T: np.ndarray, shared_params: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        _pj = self._tuning_n_jobs(_blt_cv_folds)
    ...
    def tune_all(self, model_names: List[str], X: pd.DataFrame, Y: np.ndarray, T: np.ndarray) -> Dict[str, Dict[str, Dict[str, Any]]]:
        ...
        _tlog.info(
            ...
            self._tuning_n_jobs(1 if self.cfg.tuning.single_fold else (self.cfg.tuning.cv_splits or 5)),
            self.cfg.constants.parallel_level,
            max(1, _n_effective // max(1, self._tuning_n_jobs(1 if self.cfg.tuning.single_fold else (self.cfg.tuning.cv_splits or 5)))),
            _cpu_diag,
        )
        if skipped:
            _tlog.info(
        ...
        if not self.cfg.tuning.single_fold and self.cfg.constants.parallel_level >= 3:
            ...
            _red_pj = self._tuning_n_jobs(self.cfg.tuning.cv_splits or 5)
            if _red_pj < _full_pj:
                _tlog.info(
...
Score-Konvention:
est.score() gibt MSE zurück (lower=better):
  NonParamDML: MSE der Residual-auf-Residual-Regression (R-Loss)
  DRLearner: MSE der DR-Pseudo-Outcome-Regression
Da Optuna direction="maximize" verwendet, werden die Scores negiert (-MSE),
sodass höhere Werte bessere Modelle anzeigen. Konsistent mit dem BLT,
wo log_loss ebenfalls negiert wird.
...
in allen Cross-Prediction-Folds wiederverwendet."""
    ...
    def _apply_overfit_penalty(self, val_score: float, train_score: float,
        ...
        Formel: adjusted = val_score - penalty * max(0, gap - tolerance)
        wobei gap = train_score - val_score (positiv = Train besser als Val).

        Alle Scores müssen "higher=better" sein (nach Negierung).
        """
        if penalty <= 0:
            return val_score
        gap = train_score - val_score  # positiv = Overfitting
    ...
    def _parallel_jobs_per_fit(self) -> int:
        ...
        if pl <= 2:
            return -1
        n_cpus = available_cpu_count()
        n_trial_workers = self._tuning_n_jobs(1 if self.cfg.tuning.single_fold else (self.cfg.tuning.cv_splits or 5))
        ...
        if name == "nonparamdml":
            ...
            def objective(trial):
                ...
                for fold_i, (tr, va) in enumerate(split_iter):
                    ...
                    # est.score() gibt MSE zurück (lower=better). Negieren für
                    # direction="maximize" Kompatibilität: -MSE → higher=better.
                    # Konsistent mit BLT, wo log_loss ebenfalls negiert wird.
        ...
        if name == "drlearner":
            ...
            def objective(trial):
                ...
                for fold_i, (tr, va) in enumerate(split_iter):
                    ...
                    est.fit(Y_tune[tr], T_tune[tr], X=X_tune.iloc[tr])
                    # est.score() gibt MSE der DR-Pseudo-Outcome-Regression zurück
                    # (lower=better). Negieren für direction="maximize".
                    val_score = -float(est.score(Y_tune[va], T_tune[va], X=X_tune.iloc[va]))
                    fold_scores_raw.append(val_score)

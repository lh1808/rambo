...
- `use_econml_tune`: Wenn `true`, wird vor dem finalen Training ein **Optuna-basiertes Tuning** über
  8 Wald-Parameter (n_estimators, max_depth, min_samples_leaf, min_samples_split, max_features,
  max_samples, min_var_fraction_leaf, min_impurity_decrease) durchgeführt.
  CausalForestDML: Optuna-Trials evaluieren via `est.score()` (R-Loss).
  CausalForest: Nuisance-Residuen werden vorberechnet, Optuna optimiert nur Forest-Parameter via R-Loss.
- `n_trials`: Anzahl Optuna-Trials für CausalForest-Tuning (Default: 50).
  Nur wirksam bei `use_econml_tune: true`.
- `tune_models`: Liste der Modelle, für die CausalForest-Tuning ausgeführt wird.
  ...
  nur CausalForestDML, CausalForest behält Defaults.

Wichtig:

- **BLT-Optuna** optimiert die **Base Learner** (also `model_y`/`model_t`) — auch beim `CausalForestDML`.
- **CFT-Optuna** optimiert die **Wald-Parameter** der Forest-Stufe — getrennt vom BLT.
- Beide nutzen TPE (Tree-structured Parzen Estimator) mit `multivariate=True`.

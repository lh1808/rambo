...
**CausalForest-Tuning (CFT):** Optimiert Wald-Parameter (max_depth, min_samples_leaf, max_features, n_estimators etc.) für CausalForestDML und CausalForest via Optuna TPE. Bewertung per R-Loss auf Tuning-CV-Folds. Trials laufen sequentiell (n_jobs=1), da jeder Forest intern alle Kerne nutzt.

- **CausalForestDML:** Jeder Trial konstruiert einen vollständigen CausalForestDML-Estimator (inkl. Nuisance-Cross-Fitting), fittet auf K-1 Folds und bewertet via `est.score()` (R-Loss).
- **CausalForest:** Nuisance-Residuen (model_y, model_t) werden einmalig vorberechnet. Jeder Trial fittet nur den Forest auf den vorberechneten Residuen und bewertet via R-Loss.
...
3. **Uplift-Plots (alle Modelle):** Qini-Kurve, Uplift-by-Percentile, Treatment-Balance — immer für alle Modelle, da schnell (~2-5s). Alle Plots sind native rubin-Implementierungen mit rubin-Farbpalette .
...
FMT-Trials sind deutlich schwerer als BLT-Trials: Jeder Trial fittet ein vollständiges DML/DR-Modell mit internem Cross-Fitting. NonParamDML: K Folds × (model_y + model_t) + model_final = 11 Fits. DRLearner: K Folds × (propensity + K×regression) + model_final = 16 Fits. Bei `cores//4` würden zu viele schwere Trials gleichzeitig laufen → RAM-Überlastung und CPU-Kontextwechsel.

...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Validierungsstrategie">
    ...
    <Info>{(cfg.validateOn === "external" || cfg.eval_mask_file)
      ? `Evaluation auf ${cfg.validateOn === "external" ? "separatem Holdout-Datensatz" : "dem Mask-Subset (TMEO)"}. Jedes Modell wird EINMAL auf ${cfg.eval_mask_file && cfg.validateOn !== "external" ? "den ~mask-Rows" : "den Trainingsdaten"} gefittet und direkt auf dem Eval-Set evaluiert — kein äußeres CV. DML/DR-Modelle nutzen intern Cross-Fitting (cv=2, EconML-Default) für die Nuisance-Residualisierung.`
      : `Alle Modelle durchlaufen externe ${cfg.cvSplits||5}-Fold Cross-Validation für echte Out-of-Fold CATE-Predictions. DML/DR-Modelle nutzen zusätzlich internes Cross-Fitting (cv=2, EconML-Default) für die Nuisance-Residualisierung innerhalb jedes äußeren Folds.`
    }</Info>
...
function App() {
  ...
  const estimateFits = () => {
    ...
    const bltCv = 2;                        // BLT innere CV (EconML-Default)
    const dmlCv = 2;                        // DML internes Cross-Fitting (EconML-Default)
    const fmtCv = 2;                        // FMT internes CV (EconML-Default)

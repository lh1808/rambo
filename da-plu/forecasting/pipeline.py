...
def _optuna_objective_for_horizon(
    ...
    Das Ziel ist eine im Rolling-Backtest gemessene Metrik (Standard:
    sMAPE), berechnet per :func:`rolling_block_forecast` mit
    leakage-freier Pro-Block-Transformation.

    Parameters
    ----------
    trial
        Laufender Optuna-Trial.
    ts_y_static
        Zielreihe inkl. One-Hot-kodierter Static Covariates.
    ts_X
        Past-Kovariaten (augmentiert).
    cov_ts
        Future-Kovariaten (bis Horizont-Ende verlängert).
    hcfg
        Horizont-Konfiguration.
    cfg
        Globale Konfiguration.
    tuning_cfg
        Tuning-Konfiguration dieses Horizonts.

    Returns
    -------
    float
        Score des Trials (je niedriger, desto besser bei
        ``direction="minimize"``). ``float("inf")`` bei zu wenigen
        Beobachtungen oder leerem Backtest.
    """
    ...
    # Der Callback meldet train_loss an Optuna; der MedianPruner
    # vergleicht gegen abgeschlossene Trials und pruned bei schlechter
    # Performance.
    pruning_callbacks = []
    try:
        ...
        pass  # optuna[integration] nicht installiert → kein Pruning

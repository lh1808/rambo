...
- Es wird eine CV (Cross Validation) mit `cv_splits` durchgeführt (Default: 2, EconML-Default). Dieser Wert steuert die innere Tuning-CV und ist bewusst **getrennt** von `cross_validation_splits` (äußere Evaluation, Default: 5). Die innere CV muss nicht der äußeren entsprechen — EconML nutzt intern cv=2 als Standard für Nuisance-Cross-Fitting.
...
Die internen CVs sind bewusst getrennt:

| Stufe | Config-Feld | Default | Beschreibung |
|---|---|---|---|
| BLT Tuning | `tuning.cv_splits` | 2 | Innere CV pro Tuning-Trial |
| FMT Tuning (intern) | `final_model_tuning.cv_splits` | 2 | Nuisance-Cross-Fitting im FMT |
| FMT DRLearner (äußer) | `cross_validation_splits` | 5 | Score-Folds für DRLearner-FMT |
| DML/DR Produktion | `dml_crossfit_folds` | 2 | Internes Nuisance-Cross-Fitting |

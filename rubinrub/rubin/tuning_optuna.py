...
persistiert und in weiteren Folds wiederverwendet."""
    ...
    def tune_final_model(
        ...
        if name == "nonparamdml":
            ...
            self._logger.info("FMT '%s': %d/%d Trials abgeschlossen (parallel=%d).", model_name, _fmt_ok, len(study.trials), _fmt_pj)
            if _fmt_fail > 0:
                self._logger.warning("FMT '%s': %d Trials fehlgeschlagen.", model_name, _fmt_fail)
            try:

...
const LGBM = {
  ...
  learning_rate                 : {min:0.001,max:0.3,step:0.005,type:"float",log:true,dLow:0.01,dHigh:0.15},
  ...
  min_child_weight              : {min:0.001,max:100,step:0.5,type:"float",dLow:0.01,dHigh:50},
...
const CB = {
  ...
  l2_leaf_reg                   : {min:0.1,max:80,step:0.5,type:"float",dLow:1,dHigh:30},
  random_strength               : {min:0.001,max:20,step:0.1,type:"float",dLow:0.01,dHigh:10},
  subsample                     : {min:0.3,max:1,step:0.05,type:"float",dLow:0.5,dHigh:1},
...
const LGBM_FMT = {
  ...
  learning_rate                 : {min:0.001,max:0.2,step:0.005,type:"float",log:true,dLow:0.005,dHigh:0.12},
  ...
  min_child_weight              : {min:0.1,max:50,step:0.5,type:"float",dLow:0.5,dHigh:20},
...
const CB_FMT = {
  ...
  l2_leaf_reg                   : {min:0.5,max:60,step:1,type:"float",dLow:3,dHigh:30},
  random_strength               : {min:0.1,max:20,step:0.5,type:"float",dLow:0.5,dHigh:10},
  subsample                     : {min:0.2,max:1,step:0.05,type:"float",dLow:0.4,dHigh:0.85},
  ...
  model_size_reg                : {min:0,max:30,step:0.5,type:"float",dLow:0.1,dHigh:10},
...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Tuning-Regularisierung" accent="#9B111E">
    ...
    {(()=>{
      const REG_PRESETS = [
        {key:"moderate", label:"Moderat", desc:"BLT: 15% Gap toleriert · FMT/CFT: 5% Gap toleriert. Empfohlen bei moderater Stichprobengröße.",
         blt_p:0.2, blt_t:0.15, fmt_p:0.3, fmt_t:0.05, cft_p:0.3, cft_t:0.05},
        {key:"strong", label:"Stark", desc:"BLT: 8% Gap toleriert · FMT/CFT: 3% Gap toleriert. Für kleine Stichproben oder starkes Overfitting.",
         blt_p:0.4, blt_t:0.08, fmt_p:0.6, fmt_t:0.03, cft_p:0.6, cft_t:0.03},
      ];
      ...
      const cp=cfg.cfOverfitPenalty||0, ct=cfg.cfOverfitTolerance===undefined?0.05:cfg.cfOverfitTolerance;
      ...
      const toggleReg = (r) => {
        if(activeKey===r.key) set({...cfg, overfitPenalty:0, overfitTolerance:0.15, fmtOverfitPenalty:0, fmtOverfitTolerance:0.05, cfOverfitPenalty:0, cfOverfitTolerance:0.05});
...
<Expander title="Overfit-Penalty (CausalForest)"><div style={{fontSize:11,color:C.textSec,lineHeight:1.5,marginBottom:10}}>Bestraft Konfigurationen, deren In-Sample-Score (Train) deutlich besser ist als der OOF-Score (Val). Bei 0 (Default) wird nur der OOF-Score optimiert — kein zusätzlicher Rechenaufwand. Bei Penalty &gt; 0 wird pro Fold zusätzlich effect(X_train) berechnet (~50% mehr Rechenzeit).</div><Row><Col><Sld label="Penalty-Faktor" min={0} max={1} step={0.05} value={cfg.cfOverfitPenalty||0} onChange={v=>set({...cfg,cfOverfitPenalty:v})} help="Stärke der Bestrafung. 0 = aus, 0.3 = moderat, 0.6 = stark."/></Col><Col><Sld label="Toleranz (Gap-Schwelle)" min={0} max={0.2} step={0.01} value={cfg.cfOverfitTolerance===undefined?0.05:cfg.cfOverfitTolerance} onChange={v=>set({...cfg,cfOverfitTolerance:v})} help="Relativer Gap-Schwellwert: 0.05 = 5% des Score-Betrags."/></Col></Row>{(cfg.cfOverfitPenalty||0)>0 && <div style={{fontSize:11,color:"#92400e",background:"#fffbeb",padding:"8px 12px",borderRadius:8,marginTop:6,lineHeight:1.4,border:"1px solid #fbbf24"}}><strong>Aktiv:</strong> Penalty={cfg.cfOverfitPenalty}, Toleranz={cfg.cfOverfitTolerance===undefined?0.05:cfg.cfOverfitTolerance}. Zusätzlicher Rechenaufwand: ~50% pro Trial (Train-Predictions).</div>}</Expander>
</>}
...
const buildYaml = (cfg, sp, spFmt) => {
  ...
  if((cfg.models||[]).includes("CausalForestDML") || (cfg.models||[]).includes("CausalForest")) {
    ...
    if((cfg.cfOverfitPenalty||0)>0){a(`  overfit_penalty: ${cfg.cfOverfitPenalty}`);a(`  overfit_tolerance: ${cfg.cfOverfitTolerance===undefined?0.05:cfg.cfOverfitTolerance}`);}

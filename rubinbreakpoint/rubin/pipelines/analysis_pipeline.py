...
class AnalysisPipeline:
    ...
    def _run_training(self, cfg, X, T, Y, tuned_params_by_model, holdout_data, mlflow, _progress_cb=None):
        ...
        _last_label = None
        _cft_shared_scorers = []  # RScorer-Cache für CFT (wiederverwendbar zwischen CFDML und GRF)
            ...
            if name in ("CausalForestDML", "CausalForest") and getattr(cfg.causal_forest, "use_econml_tune", False) and (not getattr(cfg.causal_forest, "tune_models", []) or name in getattr(cfg.causal_forest, "tune_models", [])):
                try:
                    ...
                    result = tune_causal_forest(
                        ...
                        precomputed_scorers=_cft_shared_scorers if _cft_shared_scorers else None,
                    )
                    # Scorer vom ersten CFT-Modell für das zweite wiederverwenden
                    if not _cft_shared_scorers and result.get("scorers"):
                        _cft_shared_scorers = result["scorers"]
        ...
        if _cft_shared_scorers:
            del _cft_shared_scorers
            gc.collect()
    ...
    def _train_and_evaluate_surrogate(self, cfg, X, T, Y, teacher_name, preds, holdout_data, models, eval_summary, mlflow, surrogate_name=None, run_drtester=False, fitted_tester=None, eval_mask=None, S=None):
        ...
        if holdout_data is None:
            ...
            if is_mt:
                ...
                fold_aligned = champion_df.attrs.get("fold_aligned_preds")
                fold_splits = champion_df.attrs.get("fold_splits")
                    ...
                    try:
                        ...
                        try:
                            ...
                            for _mn in model_names:
                                _mdf = preds.get(_mn)
                                if _mdf is not None and hasattr(_mdf, 'attrs'):
                                    _mfa = _mdf.attrs.get("fold_aligned_preds")
                                    _mfs = _mdf.attrs.get("fold_splits")
                                    if _mfa and _mfs:
                            ...
                            if _n_with_folds >= 2 and _ens_fold_splits:
                                ...
                                ensemble_df.attrs["fold_aligned_preds"] = _ens_fold_aligned
                                ensemble_df.attrs["fold_splits"] = _ens_fold_splits
                                self._logger.info(

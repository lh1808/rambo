...
def select_features_by_importance(
    ...
    Das Budget wird gleichmäßig aufgeteilt: jede Methode liefert
    ``ceil(max_features / n_methods)`` Features. Die Union wird anschließend
    auf ``max_features`` gekappt (Rank-Aggregation als Tiebreaker).
    ...
    # Union auf max_features kappen (mittlere Rank-Position als Tiebreaker)
    if len(all_keep) > int(max_features):
    ...
    _logger.info(
        "Feature-Selection: %d / %d Features behalten (max_features=%d, %d Methoden × %d pro Methode).",
        len(keep_ordered), n_total, max_features, n_methods, per_method,
    )

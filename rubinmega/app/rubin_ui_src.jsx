...
const FinalTuningPlanPreview = ({models, fmtEnabled, fmtModels, fmtSingleFold, fmtTrials, fmtCv, mcIters}) => {
  const nTrials = fmtTrials || 50;
  const nCv = fmtCv || 3;
  const dc = fmtCv || 5;
  const mc = mcIters || 1;
...
const PModels = ({cfg,set,sp,setSp,spFmt,setSpFmt}) => {
  return (<><Sec title="Base Learner"><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {[
            {key:"catboost",logo:CB_LOGO,name:"CatBoost"},
            {key:"lgbm",logo:LGBM_LOGO,name:"LightGBM"},
          ].map(bl => {
            const active = (cfg.baseLearner||"catboost") === bl.key;
            return (
              <button key={bl.key} onClick={()=>set({...cfg,baseLearner:bl.key,blFixed:{},fmtFixed:{}})}
                style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:6,padding:"18px 14px 14px",background:active?"#faf6f6":"#fff",border:active?"2px solid "+C.ruby:"1.5px solid #e0d6d7",borderRadius:12,cursor:"pointer",transition:"all 0.15s",position:"relative",minHeight:80}}
                onMouseEnter={e=>{if(!active){e.currentTarget.style.borderColor="#ccc";e.currentTarget.style.background="#fdfbfb"}}}

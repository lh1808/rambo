...
class CausalForestTuner:
    ...
    Alle anderen Wald-Parameter auf EconML-Defaults fixiert.
    ...
    def tune(
        ...
        if not is_cfdml:
            ...
            _rct_suffix = " model_t = DummyClassifier (RCT)." if _cft_is_rct else ""
            _tlog.info("GRF: %d Fold-Splits vorberechnet.%s", len(_grf_fold_data), _rct_suffix)
        ...
        if is_cfdml:
            ...
            gc.collect()
            _tlog.info(
                "CFT '%s': Nuisance gecacht für %d Folds. Trials fitten nur noch Forest via refit_final().",
                model_type, len(_cfdml_cached_folds),
            )

        # ── RScorer: Cache prüfen oder neu erstellen ──
        if self._scorer_cache:
            _cft_fold_scorers = self._scorer_cache
            _tlog.info("CFT '%s': Gecachte RScorer wiederverwendet (%d).", model_type, len(_cft_fold_scorers))
        else:
            _cft_fold_scorers = []
            _scorer_folds = (
            ...
            _tlog.info("CFT '%s': RScorer erstellt + gecacht (%d).", model_type, len(_cft_fold_scorers))
        ...
        # ── Study erstellen + optimieren ──
        _cft_n_startup = max(5, n_trials // 5)
        _cft_n_startup = min(_cft_n_startup, max(3, n_trials // 2))
        sampler = optuna.samplers.TPESampler(
            seed=tuning_cv_seed, multivariate=True, group=True,
            consider_endpoints=True, n_startup_trials=_cft_n_startup,
        )
        ...
        try:
            ...
            _tlog.warning("CFT '%s': Keine abgeschlossenen Trials.", model_type)
            best = {}
            best_score = None

        if best_score is not None and best_score <= -1e11:
            _tlog.warning("CFT '%s': Alle Trials fehlgeschlagen (best_score=%.2g).", model_type, best_score)
            best = {}
            best_score = None
        ...
        gc.collect()
        _tlog.info("CFT '%s': Study + Cache freigegeben.", model_type)

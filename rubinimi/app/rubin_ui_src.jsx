...
const PConfig = ({cfg,set}) => (<>
  ...
  <Sec title="Validierungsstrategie">
    ...
    <Info>DML/DR-Modelle (inkl. CausalForestDML) werden einmal auf allen Daten gefittet — ihr internes Cross-Fitting (cv={cfg.cvSplits||5}) erzeugt bereits OOF-Residuals. Meta-Learner und CausalForest nutzen externe {cfg.cvSplits||5}-Fold Cross-Validation.</Info>
    <Toggle label="Monte-Carlo-Iterationen aktivieren" checked={(cfg.mcIters||0)>0} onChange={v=>set({...cfg,mcIters:v?3:null})} help="Wiederholt das interne Cross-Fitting mehrfach und mittelt die Residuals. Stabilere CATE-Schätzungen, aber proportional längere Laufzeit."/>
    {(cfg.mcIters||0)>0 && <div style={{fontSize:11,color:"#7a5a00",background:"#fffbeb",padding:"8px 12px",borderRadius:8,marginTop:6,lineHeight:1.5}}>Cross-Fitting wird <strong>{cfg.mcIters}x</strong> wiederholt. Interne Fits pro DML/DR-Modell: ({cfg.cvSplits||5} Folds x 2 Nuisance + 1 Final) x {cfg.mcIters} mc = <strong>{(((cfg.cvSplits||5)*2+1)*(cfg.mcIters||1))}</strong> Base-Learner-Fits. DML/DR-Modelle (inkl. CausalForestDML) werden einmal auf allen Daten gefittet (internes OOF). Meta-Learner und CausalForest nutzen weiterhin externe CV.</div>}
    {(cfg.mcIters||0)>0 && <Row>
...
const FinalTuningPlanPreview = ({models, fmtEnabled, fmtModels, fmtSingleFold, fmtTrials, fmtCv, mcIters}) => {
  ...
  eligible.forEach(m => {
    ...
    if(mn === "nonparamdml") {
      const internalFits = (dc*2+1)*mc;
      const total = nTrials * internalFits;
      rows.push({model:m, method:"RScorer", trials:nTrials, fitsPerTrial:internalFits, totalFits:total,
        detail:nTrials+"T x "+internalFits+" intern",
        note:"Pro Trial: 1 NonParamDML-Fit (cv="+dc+": "+dc+"x model_y + "+dc+"x model_t + 1x model_final = "+(dc*2+1)+(mc>1?" x "+mc+" mc_iters = "+internalFits:"")+")."});
    }
    if(mn === "drlearner") {
      const drFolds = fmtSingleFold ? 1 : nCv;
      const internalPerFold = (dc*2+1)*mc;
      const fitsPerTrial = drFolds * internalPerFold;
      ...
      rows.push({model:m, method:fmtSingleFold?"score() 1-Fold":"score() + CV", trials:nTrials, fitsPerTrial, totalFits:total,
        detail:nTrials+"T x "+drFolds+"F x "+internalPerFold+" intern",
        note:"Pro Trial: "+drFolds+" Fold(s), je 1 DRLearner-Fit (cv="+dc+": "+dc+"x propensity + "+dc+"x regression + 1x model_final = "+(dc*2+1)+(mc>1?" x "+mc+" mc_iters = "+internalPerFold:"")+")."});
    }
  });
...
export default function App() {
  ...
  const estimateFits = () => {
    ...
    const fitsPerFit = (x) => {
      if (["NonParamDML","ParamDML","CausalForestDML","DRLearner"].includes(x)) return (dc*2+1)*mc;
      if (x==="TLearner") return K;
      if (x==="XLearner") return 1+2*K;
      return 1; // SLearner, CausalForest
    };
    const isOOF = (x) => ["NonParamDML","ParamDML","CausalForestDML","DRLearner"].includes(x);
    ...
    // 3. FMT (volle kausale Modell-Fits pro Trial, gleiche cv/mc wie Training)
    if (cfg.fmtEnabled) {
      const fmtInternal = (dc*2+1)*mc;  // Identisch mit Training
      (cfg.fmtModels||[]).forEach(x => {
        if (x==="NonParamDML") f += (cfg.fmtTrials||20) * fmtInternal;
        if (x==="DRLearner") {
    ...
    m.forEach(x => {
      ...
      if (isOOF(x)) {
        // DML/DR (+ CausalForestDML): Single-Fit auf allen Daten (internes Cross-Fitting)
        // Deckt Training + Train-Preds + SHAP + Refit/Bundle in einem Fit ab
        f += pf;
      } else {
        // Meta-Learner: Externe CV + Train-Preds + ggf. SHAP
    ...
    // 8. Bundle: nur Meta-Learner + CausalForest brauchen Refit (DML/DR bereits gefittet)
    if (bundle) {
      m.forEach(x => { if (!isOOF(x)) f += fitsPerFit(x); });
      if (cfg.surrEnabled) f += 1;
    }
    // Ohne Bundle: Champion-Refit nur für Meta-Learner
    else if (cfg.refitChamp) {
      const hasOofChamp = m.some(x=>isOOF(x));
      if (!hasOofChamp) f += 1;  // Meta-Learner Champion Refit
    }

    // 9. Ensemble-Refit: nur Meta-Learner brauchen Refit

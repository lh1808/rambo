...
CausalForestDML wird einmal auf allen Daten gefittet (Single-Fit mit internem Cross-Fitting). Externe CV-Folds entfallen. CausalForest nutzt weiterhin externe CV-Folds, diese laufen aber immer sequentiell — der interne GRF (Generalized Random Forest) nutzt joblib-Prozesse für die Baum-Parallelisierung, die in Threads zu Deadlocks führen würden. Jeder Fold bekommt alle CPU-Kerne.
...
- `cross_validation_splits`: **Zentrale Fold-Anzahl.** Steuert die externe Cross-Validation für Meta-Learner (SLearner, TLearner, XLearner) und CausalForest sowie das interne Cross-Fitting der DML/DR-Modelle (NonParamDML, ParamDML, CausalForestDML, DRLearner). DML/DR-Modelle werden einmal auf allen Daten gefittet — ihre internen OOF-Residuals nutzen diese Fold-Anzahl. Wird auch für BL-Tuning, FMT und CausalForest-Tune verwendet. Standard: 5.
...
**Hinweis zu `ensemble`:**  
Nutzt EconMLs `EnsembleCateEstimator` mit gleichgewichteten (`1/N`) Vorhersagen aller trainierten Modelle. DML/DR-Modelle liefern Predictions aus dem Single-Fit (intern OOF), Meta-Learner und CausalForest aus externer Cross-Validation. Das Ensemble nimmt an der Champion-Selektion teil. Wird das Ensemble Champion, werden beim Bundle-Export die Meta-Learner und CausalForest auf vollen Daten refittet (DML/DR sind bereits auf vollen Daten gefittet).
...
**Internes Cross-Fitting (Single-Fit für DML/DR):**

DML-Modelle (NonParamDML, ParamDML, CausalForestDML) und DRLearner werden einmal auf allen Daten gefittet — ohne externe Cross-Validation. EconMLs internes Cross-Fitting erzeugt OOF-Nuisance-Residuals, die den Confounding-Anteil bereits bereinigen. model_final trainiert zwar auf allen Residuals (technisch in-sample), aber die DML-Theorie (Chernozhukov et al., 2018) garantiert die Korrektheit:

- Die Neyman-Orthogonalitätsbedingung sorgt dafür, dass Nuisance-Fehler nur quadratisch klein in die CATE-Schätzung eingehen. model_final sieht nur das kausale Signal plus Rauschen — Confounding-Information wurde in den OOF-Residuals bereits eliminiert.
- Ein überangepasstes model_final erzeugt verrauschtere (nicht zu optimistische) Predictions. QINI und AUUC messen Ranking-Qualität, die durch Rauschen verschlechtert, nicht verbessert wird.
- Dies ist das Standardverfahren in der DML-Literatur und in EconMLs eigener Evaluation (RScorer, DRTester).

Meta-Learner (SLearner, TLearner, XLearner) und CausalForest besitzen keinen internen OOF-Mechanismus und nutzen weiterhin externe K-Fold Cross-Validation.

```yaml
data_processing:
  cross_validation_splits: 5  # Externe CV (Meta-Learner/CausalForest) + internes DML Cross-Fitting
...
- `stability_penalty`: Bestraft instabile CATE-Schätzungen mit hoher Streuung relativ zum Median-Effekt. **0.0 = Default** (reiner R-Score). 0.3–0.5 = moderate Regularisierung. 1.0+ = starke Stabilität (CATEs werden konservativ, ähnlich CausalForestDML). Formel: `score = R_score − λ · log(1 + CV)`, wobei CV = std(CATE) / |median(CATE)|.

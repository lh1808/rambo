...
1. **Holdout-Prinzip:** ~20% der Daten (erster Fold Val) werden von keinem Tuning-Schritt gesehen. Bei DML/DR-Single-Fit kommen diese 20% erstmals im finalen Training zum Einsatz — die Hyperparameter wurden nicht auf ihnen optimiert.
...
5. **DML-Theorie sichert Konsistenz:** Die Nuisance-OOF-Residuals, auf denen model_final trainiert, sind von Confounding bereinigt (Neyman-Orthogonalität). model_final kann keinen spuriösen Zusammenhang ausnutzen, weil diese Information in den Residuals nicht mehr enthalten ist. Ein überangepasstes model_final erzeugt Rauschen (schadet dem Ranking), keinen Bias (verbessert das Ranking nicht).

### Schritt 5: Training

rubin verwendet zwei Training-Pfade, abhängig davon ob ein Modell internes Cross-Fitting besitzt:

**DML/DR-Modelle (NonParamDML, ParamDML, CausalForestDML, DRLearner) — Single-Fit:**
Diese Modelle werden einmal auf allen Daten gefittet. EconMLs internes Cross-Fitting (cv=`cross_validation_splits`) erzeugt automatisch Out-of-Fold-Nuisance-Residuals (Y_res, T_res). Eine externe CV-Schleife wäre redundant und würde identische Nuisance-Fits wiederholen. Das gefittete Modell wird direkt für Predictions, SHAP, Bundle-Export und Refit wiederverwendet — kein separater Refit nötig.

**Meta-Learner (SLearner, TLearner, XLearner) + CausalForest — Externe K-Fold CV:**
Diese Modelle haben keinen internen OOF-Mechanismus. Für jede Beobachtung wird eine Prediction aus einem Modell erzeugt, das diese Beobachtung nicht gesehen hat (K-Fold Out-of-Fold). CausalForest-CV-Folds laufen immer sequentiell, da der interne GRF joblib-Prozesse für die Baum-Parallelisierung spawnt — in Threads führt das zu Deadlocks.

#### Warum Single-Fit für DML/DR-Modelle korrekt ist

Bei DML-Modellen besteht ein Fit aus zwei Stufen:

1. **Nuisance-Stufe (OOF):** Outcome- und Propensity-Modelle werden per K-Fold Cross-Fitting trainiert. Jede Beobachtung erhält Nuisance-Predictions aus einem Modell, das sie nicht gesehen hat. Die resultierenden Residuals Y_res = Y − Ŷ und T_res = T − T̂ sind genuinely Out-of-Fold.

2. **model_final (auf allen Daten):** Das CATE-Effektmodell wird auf allen Residuals trainiert und predictet auf denselben Daten. Technisch ist das in-sample.

Auf den ersten Blick erscheint die in-sample model_final-Prediction problematisch für faire Modellvergleiche. Die DML-Theorie (Chernozhukov et al., 2018) zeigt jedoch, warum das kein Problem ist:

- **Neyman-Orthogonalität:** Die Residuals sind durch das OOF-Cross-Fitting von Confounding bereinigt. model_final kann keinen spuriösen Zusammenhang zwischen Treatment und Outcome "erlernen", da dieser bereits in den Residuals eliminiert wurde. Was model_final sieht, ist nur noch das kausale Signal plus Rauschen.

- **Fehler-Propagation zweiter Ordnung:** Selbst bei imperfekten Nuisance-Modellen propagieren deren Fehler nur quadratisch klein in die CATE-Schätzung. Die CATE konvergiert mit √n-Rate — unabhängig davon, ob model_final in-sample oder OOF predictet.

- **Overfitting schadet, hilft nicht:** Ein überangepasstes model_final erzeugt verrauschtere Predictions. Da QINI und AUUC die Ranking-Qualität messen, verschlechtert Rauschen das Ranking — ein overfittendes model_final kann sich nicht durch In-Sample-Bias bei der Evaluation "schönen".

- **Standard in Theorie und Praxis:** EconMLs eigene Evaluationsmethoden (RScorer, DRTester) basieren auf Single-Fit-Predictions. Kein Paper aus der DML-Literatur verwendet externe CV für die CATE-Evaluation von DML-Modellen — der einzelne Fit auf allen Daten ist die vorgesehene Methode.

Die externe CV bei Meta-Learnern (SLearner, TLearner, XLearner) und CausalForest bleibt hingegen essentiell: Diese Modelle haben kein internes Cross-Fitting, d.h. sowohl Nuisance- als auch Effektschätzung sind vollständig in-sample. Ohne externe CV wären ihre Predictions systematisch zu optimistisch.

### Schritt 5b: Ensemble (optional)
Bei `models.ensemble: true` und mindestens 2 trainierten Modellen wird ein gleichgewichtetes Ensemble (`EnsembleCateEstimator` aus EconML) erstellt. Die Ensemble-Predictions sind der Mittelwert der individuellen Vorhersagen (DML/DR: aus Single-Fit, Meta-Learner: aus Cross-Predictions). Das Ensemble nimmt regulär an der Champion-Selektion teil. CausalForest ist über den `CausalForestAdapter` (`BaseCateEstimator`-kompatibel) vollwertig ensemble-fähig.

Wird das Ensemble Champion und `refit_champion_on_full_data: true`, werden beim Bundle-Export nur die Meta-Learner und CausalForest auf vollen Daten refittet (DML/DR-Modelle sind bereits auf vollen Daten gefittet). Anschließend wird ein neues `EnsembleCateEstimator` mit allen gefitteten Modellen erstellt.

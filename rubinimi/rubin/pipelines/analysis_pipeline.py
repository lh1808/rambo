...
from rubin.training import _predict_effect, train_and_crosspredict_bt_bo, train_single_fit_oof, INTERNAL_OOF_MODELS, is_multi_treatment, SurrogateTreeWrapper, SURROGATE_MODEL_NAME
...
class AnalysisPipeline:
    ...
    def _run_tuning(self, cfg, X, T, Y, mlflow):
        ...
        if cfg.tuning.enabled:
            ...
            best_avg = sum(tuner.best_scores.values()) / max(n_tasks, 1)
            self._logger.info(
                "Base-Learner-Tuning: %d Tasks abgeschlossen (%d Trials/Task). Ø Score: %.6g",
                n_tasks, cfg.tuning.n_trials, best_avg,
            )
    ...
    def _run_training(self, cfg, X, T, Y, tuned_params_by_model, holdout_data, mlflow):
        ...
        for name in cfg.models.models_to_train:
            ...
            # DML/DR-Modelle (+ CausalForestDML): internes Cross-Fitting = externe CV-Splits
            # Damit sind die internen OOF-Predictions konsistent mit der
            # Granularität, die sonst durch externe Cross-Predictions erzeugt würde.
            ...
            if name == "CausalForest" and getattr(cfg.causal_forest, "use_econml_tune", False):
                try:
                    ...
                    self._logger.info(
                        "CausalForest Tune: %d Kombis auf Fold 1 (train=%d, val=%d) → best=%s, R-Loss=%.6g",
                        result.get("n_combos", 0), len(X_tr), len(X_va), best_p, best_loss or 0,
                    )
                except Exception:
            ...
            if holdout_data is None:
                if name in INTERNAL_OOF_MODELS:
                    # ── DML/DR (+ CausalForestDML): Einmal auf allen Daten fitten ──
                    # Internes Cross-Fitting erzeugt OOF-Nuisance-Residuals.
                    # Externe CV wäre redundant und würde nur identische
                    # Nuisance-Fits wiederholen.
                    df_pred, fitted_model = train_single_fit_oof(
                        model=model, X=X, Y=Y, T=T, model_name=name,
                    )
                    current_model = fitted_model
                    if keep_fold:
                        # Für SHAP: der gefittete Model ist direkt nutzbar
                        fold_models[name] = (fitted_model, None)
                else:
                    # ── Meta-Learner + CausalForest: Externe CV nötig ──
            ...
            if is_mt:
                ...
                if pred_col in dfp.columns:
                    ...
                    eval_summary[mname] = {
                        ...
                        "policy_value": float(policy_value(y=y, t=t, score=s, threshold=0.0)),
                    }
                    for key, val in eval_summary[mname].items():
    ...
    def _evaluate_historical_score(self, cfg, X, T, Y, holdout_data, preds, hist_score_eval, eval_summary, policy_values_dict, mlflow, fitted_tester=None, eval_mask=None):
        ...
        eval_summary[hist_name] = {
            ...
            "policy_value": float(policy_value(y=eval_y, t=eval_t, score=hist_score, threshold=0.0)),
        }
        ...
        try:
            ...
            if is_mt:
                ...
                eval_summary[sname] = {
                    ...
                    "uplift_at_20pct": float(uplift_at_k(curve, k_fraction=0.20)),
                    "uplift_at_50pct": float(uplift_at_k(curve, k_fraction=0.50)),
                    "policy_value": float(policy_value(y=y_s, t=t_s, score=s, threshold=0.0)),
                }
    ...
    def _run_bundle_export(self, cfg, models, eval_summary, X, T, Y, X_full, T_full, Y_full, selected_feature_columns, holdout_data, export_bundle, bundle_dir, bundle_id, mlflow):
        ...
        for mname, mobj in models_to_export.items():
            ...
            elif not holdout_data and mname in INTERNAL_OOF_MODELS:
                # DML/DR (+ CausalForestDML): Bereits auf allen Daten gefittet (train_single_fit_oof).
                # Kein erneuter Fit nötig — Modell kann direkt exportiert werden.
                obj_to_write = mobj
                if mname == champion:
                    champion_refit_on_full_data = True
                    champion_refit_rows = int(len(X))
                    champion_fitted_obj = obj_to_write
                self._logger.info("%s: Bereits auf allen Daten gefittet — kein Refit noetig.", mname)
    ...
    def _run_explainability(self, cfg, X, T, Y, models, eval_summary, mlflow, report, holdout_data=None, fold_models=None):
        ...
        if champion_name in fold_models:
            ...
            if val_idx is not None:
                ...
                # Single-Fit OOF-Modell (DML/DR): Internes Cross-Fitting macht
                # Nuisance-Residuals OOF. Alle Daten können verwendet werden.
                X_expl = X.copy()
                self._logger.info(
                    "Explainability für Champion '%s': Single-Fit-Modell (internes OOF), "

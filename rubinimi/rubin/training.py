...
# ── Modelle mit internem OOF-Mechanismus ──
# Diese Modelle produzieren bei einem einzigen fit() auf allen Daten
# bereits OOF-Nuisance-Residuals (DML cross-fitting). Externe
# Cross-Validation ist für sie redundant.
# CausalForest hat kein internes Cross-Fitting (keine Nuisance-Modelle),
# daher braucht es externe CV für faire Vergleichbarkeit.
INTERNAL_OOF_MODELS = frozenset({
    "NonParamDML", "ParamDML", "CausalForestDML", "DRLearner",
})


def train_single_fit_oof(
    model: Any,
    X: pd.DataFrame,
    Y: np.ndarray,
    T: np.ndarray,
    ...
    """Trainiert ein DML/DR/CF-Modell einmal auf allen Daten.

    Diese Modelle verwenden intern Cross-Fitting (cv-Fold Nuisance-Residualisierung)
    oder honest splitting. Die resultierenden CATE-Predictions sind dadurch bereits
    out-of-fold für die Nuisance-Komponenten. Eine externe CV-Schleife ist
    nicht nötig und würde nur redundante Nuisance-Fits erzeugen.

    Gibt (DataFrame, fitted_model) zurück. Der fitted_model kann direkt
    für SHAP, Bundle-Export und Refit wiederverwendet werden.
    """
    _logger.info("%s: Single-Fit auf allen Daten (internes Cross-Fitting).", model_name)
    fitted = copy.deepcopy(model)
    fitted.fit(Y, T, X=X)

    preds = _predict_effect(fitted, X)
    ...
    if is_mt:
        ...
        for k in range(n_effects):
            ...
            out[f"Train_{model_name}_T{k+1}"] = preds[:, k]  # Identisch — gleicher Fit
        best_effect = np.nanmax(preds, axis=1)
        best_arm = np.nanargmax(preds, axis=1) + 1
        out[f"OptimalTreatment_{model_name}"] = np.where(best_effect > 0, best_arm, 0)
    else:
        out[f"Predictions_{model_name}"] = preds
        out[f"Train_{model_name}"] = preds  # Identisch — gleicher Fit

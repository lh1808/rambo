...
_PLOT_EXPLANATIONS = {
    "cate_distribution": ("CATE-Verteilung", "Histogramm der vorhergesagten Treatment-Effekte. DML/DR: aus Single-Fit (internes OOF). Meta-Learner/CausalForest: aus externer CV. Zeigt, ob das Modell heterogene Effekte erkennt oder zum Intercept kollabiert."),
...
def generate_html_report(collector: ReportCollector, output_path: str) -> str:
    ...
    for lbl, val in [
        ...
        ("Validierung", f'external (separater Datensatz)' if cs.get("validate_on") == "external" else f'{cs.get("validate_on","cross")} ({cs.get("cv_splits",5)} Folds, DML/DR: internes OOF)'),
    ...
    if collector.model_metrics:
        ...
        h = '<p class="expl">' + ('Uplift-Metriken basierend auf dem <strong>externen Evaluationsdatensatz</strong>. Modelle wurden auf den Trainingsdaten trainiert und hier auf ungesehenen Daten evaluiert.' if is_external else 'Uplift-Metriken basierend auf Out-of-Fold-Predictions. DML/DR-Modelle: internes Cross-Fitting (Single-Fit). Meta-Learner/CausalForest: externe K-Fold CV.') + ' Champion ist hervorgehoben. <span class="best-marker-legend">Bester Wert</span> je Metrik ist markiert.</p>'

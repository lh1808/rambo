...
class BaseLearnerTuner:
    ...
    def _tune_task_flaml(self, task: TuningTask, X: pd.DataFrame, Y: np.ndarray, T: np.ndarray,
        ...
        timeout = self.cfg.tuning.timeout_seconds or 120
        n_trials = int(self.cfg.tuning.n_trials)
    ...
    def compute_combined_loss_diagnostic(
        ...
        Die Combined Loss ist das Produkt der MSE-Werte von Outcome- und
        Propensity-Modell. Sie korreliert mit der Qualität der kausalen Schätzung
        und dient als aggregierte Güte-Kennzahl nach dem Base-Learner-Tuning.

        Returns:
            Dict mit combined_loss pro Modell, z.B. {"NonParamDML": 0.023, ...}
        """
        import logging
        _log = logging.getLogger("rubin.tuning")
        ...
        for mname in model_names:
            if mname not in dml_models:
                continue

            roles = tuned_params.get(mname, {})
            # Outcome-Rolle bestimmen
            ...
            try:
                ...
                for tr, va in _iter_stratified_or_kfold(T.astype(int), n_splits, seed):
                    ...
                    out_model = build_base_learner(base_type, {**fixed, **out_params}, seed=seed,
                                                   task="classifier" if mname != "DRLearner" else "regressor",
                                                   parallel_jobs=pj)
                    out_model.fit(X_np[tr], Y[tr])
                    if mname != "DRLearner" and hasattr(out_model, "predict_proba"):
                ...
                results[mname] = combined
                _log.info(
                    "Combined Loss Diagnostic [%s]: outcome=%.6g, propensity=%.6g, combined=%.6g",
                    mname, avg_out, avg_prop, combined,
                )
            except Exception as e:
                _log.warning("Combined Loss Diagnostic [%s]: Fehler – %s", mname, e)

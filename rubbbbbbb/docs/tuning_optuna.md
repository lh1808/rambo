...
adjusted = val_qini - penalty × scale × max(0, gap - tolerance)
```

Die Formel ist metrisch-agnostisch: Ein 20%-Gap wird identisch bestraft, ob R-Score (~0.3) oder Qini (~0.0001). Bei `overfit_penalty: 0` (Default) wird kein Train-Qini berechnet — kein Laufzeit-Overhead.

  ...
  return (
    <div>
      ...
      {bl==="catboost" && <div style={{fontSize:10.5,color:"#6b7280",lineHeight:1.4,marginBottom:10,padding:"6px 10px",background:"#f9fafb",borderRadius:6,border:"1px solid #e5e7eb"}}>
        <strong style={{color:"#374151"}}>GPU-Hinweis:</strong> Bei erkannter NVIDIA GPU wird <code style={{fontSize:10,background:"#f3f4f6",padding:"1px 4px",borderRadius:3}}>rsm</code> automatisch aus dem Suchraum entfernt (auf GPU nur für Pairwise-Modes unterstützt). Alle anderen Parameter werden normal optimiert.
      </div>}
...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Tuning-Regularisierung" accent="#9B111E">
    ...
    {(()=>{
      const REG_PRESETS = [
        {key:"moderate", label:"Moderat", desc:"BLT: 15% Gap toleriert · FMT: 5% Gap toleriert. Empfohlen bei moderater Stichprobengröße.",
         blt_p:0.2, blt_t:0.15, fmt_p:0.3, fmt_t:0.05},
        {key:"strong", label:"Stark", desc:"BLT: 8% Gap toleriert · FMT: 3% Gap toleriert. Für kleine Stichproben oder starkes Overfitting.",
...
<FixedParamsEditor params={(cfg.fmtFixed||{}).lgbm||{}} defaults={LGBM_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:{...(cfg.fmtFixed||{}),lgbm:v}})} label="LightGBM CATE-Parameter (Fixed)" accent="#b8860b"/>
              ...
              return (<>
                ...
                <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
                  Jeder Trial konstruiert ein Forest-Modell mit Kandidaten-Params, fittet auf {dc-1} von {dc} Folds{cfSF?" (Single-Fold: nur 1. Fold bewertet)":""} (tuning_cv_seed) und bewertet via R-Loss auf dem Validierungs-Fold. CausalForestDML nutzt est.score() (intern residualisiert), CausalForest berechnet den R-Loss manuell über vorberechnete Nuisance-Residuen.
                </div>
                <div style={{border:"1px solid #ede6e7",borderRadius:8,overflow:"hidden"}}>
                  ...
                  {hasCF && <div style={{borderBottom:"1px solid #f0f0f0"}}>
                    ...
                    <div style={{fontSize:10.5,color:"#999",padding:"0 14px 6px",lineHeight:1.4}}>Pro Fold: est.fit() = {dc}×model_y + {dc}×model_t + 1×Forest{mc>1?" × "+mc+" mc_iters":""} = {fitsPerCfdmlFold} Fits. Scoring via est.score() (R-Loss). 8 Wald-Params.</div>
                  </div>}
                  {hasGRF && <div style={{background:hasCF?"#fdfbfb":"#fff"}}>
                    ...
                    <div style={{fontSize:10.5,color:"#999",padding:"0 14px 6px",lineHeight:1.4}}>1 Forest-Fit pro Fold. Scoring via manuellem R-Loss: {evalFolds}×2 BLT-getunte Nuisance-Modelle einmalig vorberechnet. 9 Wald-Params (+ criterion).</div>
                  </div>}
                </div>
              </>);

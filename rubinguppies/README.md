...
> Ohne sie scheitert die PyPI-Auflösung mit `invalid peer certificate: UnknownIssuer`.
>
> Am besten in `.devboxrc` oder `devbox-setup` hinterlegen, damit sie beim Workspace-Start
> automatisch gesetzt werden:
>
> ```bash
> # .devboxrc oder devbox-setup
> export PIXI_TLS_ROOT_CERTS="all"   # Conda-Channels über Nexus
> export UV_NATIVE_TLS=true            # PyPI-Pakete über Nexus (uv/rustls)
> ```

```bash
cd rubin_repo
# Falls nicht über .devboxrc gesetzt:
...
```bash
pixi run dataprep -- --config configs/config_full.yml
# oder: python run_dataprep.py --config configs/config_full.yml
```
...
## GPU-Beschleunigung (CatBoost)

Wenn eine NVIDIA GPU verfügbar ist, nutzt rubin automatisch GPU-Training für alle CatBoost-Operationen (Feature-Selektion, BLT, FMT, CF-Tuning, Training). LightGBM und GRF laufen weiterhin auf CPU.

**Voraussetzungen:**
- NVIDIA GPU mit Compute Capability ≥ 3.5 (z.B. Tesla V100, T4, A100)
- NVIDIA-Treiber ≥ 450.80.02
- CatBoost über **PyPI** installiert (nicht conda-forge — das conda-Paket bringt eine eigene CUDA-Runtime mit, die zu Treiber-Inkompatibilitäten führen kann)

**Automatisches Verhalten:**
- GPU-Erkennung: Zweistufig (Hardware + CUDA-Kompatibilitätstest beim ersten CatBoost-Aufruf)
- `rsm`/`colsample_bylevel`: Wird automatisch entfernt (auf GPU nicht für Classification/Regression unterstützt)
- Parallelisierung: CatBoost-Trials sequentiell (`n_jobs=1`, GPU-Exklusivsperre), LGBM-Trials parallel
- Fallback: Bei CUDA-Inkompatibilität automatisch CPU-Modus

Siehe `docs/tuning_optuna.md` → GPU-Beschleunigung für Details.
...
- Kategorische Features: Automatisches Patching für EconML-Kompatibilität — LightGBM/CatBoost nutzen native kategoriale Splits (fit, predict, predict_proba), auch wenn EconML X intern zu numpy float64 konvertiert. dtypes.json wird automatisch erkannt.
...
- Die Evaluation enthält pro-Arm-Qini/AUUC/Uplift-Werte, pro-Arm-Policy-Values
  (`policy_value_T{k}`) sowie einen globalen IPW-basierten `policy_value`.
- Für die Champion-Auswahl wird `metric: policy_value` empfohlen (alternativ:
  `policy_value_T1`, `qini_T1`, etc.).

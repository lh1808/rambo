...
def _blt_fits_per_task(family: str, n_trials: int, cv: int, full_cv: int, K: int,
    ...
    once = (full_cv * 2 + 2) if family == "pseudo_effect" else 0
...
def _render_tuning(collector, cs) -> str:
    ...
    # Bundle-Refit (nur cross): exportierte Modelle werden am Ende auf vollen Daten
    # refittet (konservativ alle Modelle gezählt).
    _fb_bundle = 0 if _fb_external else _fb_n
    ...
    h += (
        ...
        f'{"" if _fb_external else f" + {_fb_bundle} Bundle-Refit"}) — '
...
def _render_cft(collector, cs) -> str:
    ...
    _cft_mode = cft.get("mode", "cache_values")
    ...
    _cft_scorer = cft.get("scorer", "rscore")
    if _cft_scorer == "qini":
    ...
    # Fit-Schätzung (analog FMT, cache_values-bewusst): Pre-Fit einmal pro äußerem
    # Fold (CFDML inkl. Nuisances + Scorer) + n_trials × refit_final pro Fold.
    # WICHTIG: CFT nutzt für die ÄUSSEREN OOF-Folds dieselbe Zahl wie für das innere
    # Cross-Fitting — beide = data_processing.dml_crossfit_folds (causal_forest.py:140,200).
    # Das unterscheidet CFT von FMT, dessen äußere Folds cross_validation_splits sind.
    _cft_xfit = int(cs.get("dml_crossfit_folds", 5) or 5)
    _cft_outer = 1 if _cft_sf else _cft_xfit
    _cft_mc = int(cs.get("mc_iters", 1) or 1)
    _cft_per_fit = (_cft_mc * _cft_xfit * 2 + 1) if "DML" in (_cft_model or "") else 1
    _cft_scorer_fits = 0 if _cft_scorer == "qini" else 4  # RScorer: cv=2 × (model_y+model_t)
    _cft_prefit = _cft_outer * (_cft_per_fit + _cft_scorer_fits)
    _cft_trialfits = int(_cft_nt) * _cft_outer
    _cft_total_fits = _cft_prefit + _cft_trialfits
    h += f'<div class="cd hl"><div class="cd-l">Fits (geschätzt)</div><div class="cd-v sm">{_cft_total_fits:,}</div></div>'
    h += '</div>'
    h += (
        f'<p style="font-size:13px;color:var(--text-l);margin:4px 28px 14px;font-family:var(--mono)">'
        f'{_cft_outer} Fold(s) × ({_cft_per_fit}+{_cft_scorer_fits}) Pre-Fit + {_cft_nt}T × {_cft_outer} refit_final '
        f'= <strong style="color:var(--ruby)">{_cft_total_fits:,} Fits</strong></p>'
    )

    _cft_score = cft.get("best_score")
    if _cft_score is not None:
        _cft_score_lbl = "Qini" if _cft_scorer == "qini" else "R-Score"
        h += f'<h3>Best CFT-Score</h3>'
        if _cft_scorer == "qini":
            h += '<p class="expl">OOF-Qini (aggregierter Qini-Koeffizient). Qini &gt; 0 = Modell-Ranking besser als Zufall.</p>'
        else:
            h += '<p class="expl">R-Score via externem RScorer (unabhängige Nuisance, 2-Fold T×Y). R-Score &gt; 0 = echte Heterogenität, ≈ 0 = Intercept-Only, &lt; 0 = Overfitting.</p>'
        h += f'<div class="tbl-scroll"><table class="dt"><thead><tr><th>Modell</th><th>{_cft_score_lbl}</th></tr></thead><tbody>'

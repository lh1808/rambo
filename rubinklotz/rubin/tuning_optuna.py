...
class BaseLearnerTuner:
    ...
    def tune_all(self, model_names: List[str], X: pd.DataFrame, Y: np.ndarray, T: np.ndarray) -> Dict[str, Dict[str, Dict[str, Any]]]:
        ...
        _tlog = logging.getLogger("rubin.tuning")
        # CPU-Diagnose: os.cpu_count() kann durch cgroup-Limits reduziert sein
        _n_cpus = os.cpu_count() or 1
        _n_affinity = None
        try:
            _n_affinity = len(os.sched_getaffinity(0))
        ...
        try:
            ...
            with open("/sys/fs/cgroup/cpu.max") as _f:
                ...
                if _parts[0] != "max":
                    _cgroup_quota = int(_parts[0]) // int(_parts[1])
        except Exception:
            try:
                # cgroup v1
                ...
                pass
        _cpu_diag = f"os.cpu_count={_n_cpus}"
        if _n_affinity is not None and _n_affinity != _n_cpus:
            _cpu_diag += f", sched_affinity={_n_affinity}"
        if _cgroup_quota is not None and _cgroup_quota != _n_cpus:
            _cpu_diag += f", cgroup_quota={_cgroup_quota}"
        _tlog.info(
            ...
            max(1, _n_cpus // max(1, self._tuning_n_jobs())),
            _cpu_diag,
        )
        if skipped:
            _tlog.info(

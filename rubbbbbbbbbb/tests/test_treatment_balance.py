...
def test_max_loss_frac_zero_is_respected(caplog):
    # Regression: balance_max_loss_frac=0.0 (strengster Deckel) ist falsy und wurde
    # früher via `or 1.0` stillschweigend zu 1.0 (Deckel aus). Jetzt korrekt behandelt:
    # jeder Kandidat verliert Zeilen → kein feasibler → Verlust-Deckel-Warnung.
    inst = _make_pipeline()
    with caplog.at_level(logging.WARNING, logger="rubin.test.balance"):
        out = inst._check_and_balance_treatments(

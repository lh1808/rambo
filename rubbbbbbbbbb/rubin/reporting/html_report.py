...
def _render_tuning(collector, cs) -> str:
    ...
    h += f'<div class="cd hl"><div class="cd-l">BLT-Fits (nur Nuisance-Tuning)</div><div class="cd-v sm">{total_fits:,}</div></div>'
    h += '</div>'
    ...
    # Die obige BLT-Zahl zählt nur Nuisance-EINZELfits des Base-Learner-Tunings.
    # Sie ist NICHT die Pipeline-Gesamtheit. Hier die ehrliche Übersicht in
    # ZWEI Einheiten (nicht summierbar): Einzel-Fits (BLT) vs. Top-Level-Modell-
    # Fits (Training/Bundle, jeder DML-Fit macht intern ~fits_per_dml Sub-Fits).
    ...
    h += (
        ...
        f'<strong>{total_fits:,}</strong> Nuisance-Einzelfits. Davon getrennt (andere Einheit): '
...
def _render_cft(collector, cs) -> str:
    ...
    # Fit-Schätzung (analog FMT, cache_values-bewusst): Pre-Fit einmal pro Fold
    # (CFDML inkl. Nuisances + Scorer) + n_trials × refit_final pro Fold.
    _cft_outer = 1 if _cft_sf else int(cs.get("cv_splits", 5) or 5)

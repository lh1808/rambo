...
def build_tft(hcfg: HorizonConfig, tft_cfg: TftConfig) -> TFTModel:
    ...
    - PyTorch Lightning wird per ``pl_trainer_kwargs`` ruhiggestellt:
      ...
      Ohne diese Einstellung legt jeder ``fit()``-Aufruf ein eigenes
      Log-Verzeichnis unter ``./darts_logs/`` an, was in Backtests mit
      Optuna-Tuning schnell zu Hunderten Log-Ordnern führt. Für gezielte
      Feedback-Läufe kann ``verbose=True`` an ``fit()`` / ``predict()``

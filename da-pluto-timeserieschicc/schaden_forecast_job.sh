...
# WICHTIG: torch ZUERST und SEPARAT in einer treiber-kompatiblen Variante
# installieren. Das Extra "u8darts[torch]" würde sonst die neueste
# torch-Version ziehen, die für eine neuere CUDA-Version gebaut ist, als der
# NVIDIA-Treiber auf der Maschine unterstützt → Laufzeitfehler
# "RuntimeError: The NVIDIA driver on your system is too old".
#
# Der cu121-Build (CUDA 12.1) hat breite Treiber-Kompatibilität und läuft auf
# allen neueren Treibern (>= ~530). Anschließend u8darts OHNE [torch]-Extra,
# damit das bereits installierte torch NICHT überschrieben wird.
pip install torch --index-url https://download.pytorch.org/whl/cu121
pip install u8darts icalendar optuna pyyaml

# --- Falls KEINE GPU genutzt werden soll / der GPU-Build weiter zickt ---
# CPU-only-Variante (garantiert ohne CUDA-Abhängigkeit, langsamer):

...
def evaluate_multivariate(
    ...
    pd.DataFrame
        ...
        ``abs_error_sum``, ``abs_actual_sum``.

        ``abs_error_sum`` (= Σ|Ist − Prognose|) und ``abs_actual_sum``
        (= Σ|Ist|) erlauben eine *gepoolte* WAPE über mehrere Komponenten:
        ``Σ abs_error_sum / Σ abs_actual_sum`` – im Gegensatz zum
        ungewichteten Mittel der Einzel-WAPE gewichtet das automatisch
        nach Volumen.
    """
    ...
    for component in true_aligned.components:
        ...
        abs_error_sum = float(np.sum(np.abs(y_true - y_pred)))
        abs_actual_sum = float(np.sum(np.abs(y_true)))
        mae = float(np.mean(np.abs(y_true - y_pred)))
        mse = float(np.mean((y_true - y_pred) ** 2))
        rmse = float(np.sqrt(mse))
        wape = float(abs_error_sum / (abs_actual_sum + 1e-8) * 100.0)

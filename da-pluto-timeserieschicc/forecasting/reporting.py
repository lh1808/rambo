...
- **Status**   — ``NEU`` (Neuschaden) / ``LAUFEND`` (laufend)
...
def _readable_name(component: str) -> str:
    ...
    # Status case-insensitiv und sowohl für Kurzcodes (NEU/LAUFEND, früher
    # LFD) als auch die langen Wörter (Neuschaden/Folgebearbeitung) abbilden.
    # Unbekannte Codes werden roh angezeigt statt fälschlich auf "Folge".
    s = parts[2].strip().upper()
    if s.startswith("NEU"):
        status = "Neu"
    elif s.startswith(("LAUFEND", "LFD", "FOLGE")):
        status = "Folge"
    else:
        status = parts[2]
    orga = parts[3]
...
# ======================================================================
# Tiefen-Report als PDF (Liniendiagramme je Ebene + je Endausprägung)
# ======================================================================

def _level_matrix(
    df: pd.DataFrame, dim_map: pd.DataFrame, dim: str, freq: str
) -> pd.DataFrame:
    """Zeit × Dimensionswert: aggregiertes Volumen je Ebene (Lücken = NaN)."""
    res = df.resample(freq).sum(min_count=1)
    cols = {}
    for val, comps in dim_map.groupby(dim).groups.items():
        cols[val] = res[list(comps)].sum(axis=1, min_count=1)
    return pd.DataFrame(cols).sort_index(axis=1)


def _plot_level_lines(ax, mat: pd.DataFrame, title: str, max_lines: int = 15) -> None:
    """Linien je Dimensionswert über die Zeit; NaN erzeugt sichtbare Lücken."""
    totals = mat.sum(min_count=1).sort_values(ascending=False)
    cols = list(totals.head(max_lines).index)
    for col in cols:
        ax.plot(mat.index, mat[col].values, lw=1.1, label=str(col))
    ax.set_title(title, fontsize=11)
    ax.grid(alpha=0.3)
    ax.legend(fontsize=7, ncol=2, loc="upper left")
    hidden = len(mat.columns) - len(cols)
    if hidden > 0:
        ax.set_xlabel(f"(weitere {hidden} volumenschwächere ausgeblendet)",
                      fontsize=8)


def _detect_loading_anomalies(
    df: pd.DataFrame, comp_weekly: pd.DataFrame
) -> pd.DataFrame:
    """
    Heuristische Auffälligkeiten zur Fehlersuche in der DB-Beladung, je
    Komponente: fehlende Daten, Lücken innerhalb des Zeitraums, negative
    Werte, durchgehend Null, Spätstart, vorzeitiges Ende.
    """
    if comp_weekly.shape[0] == 0:
        return pd.DataFrame()
    g_first = comp_weekly.index.min()
    g_last = comp_weekly.index.max()
    ...
    for comp in df.columns:
        ...
        if len(data) == 0:
            rows.append({
                "komponente": comp,
                "auffaelligkeit": "keine Daten",
                "erste_woche": "", "letzte_woche": "", "luecken_wochen": 0,
            })
            continue

        first, last = data.index.min(), data.index.max()
        within = s.loc[first:last]
        n_gap = int(within.isna().sum())
        has_neg = bool((df[comp].dropna() < 0).any())
        all_zero = bool((data == 0).all())
        late = (first - g_first).days > 28
        early = (g_last - last).days > 28

        if all_zero:
            reasons.append("durchgehend Null")
        if n_gap > 0:
            reasons.append(f"{n_gap} Lücken-Wochen")
        if has_neg:
            reasons.append("negative Werte")
        if late:
            reasons.append("Spätstart")
        if early:
            reasons.append("endet vorzeitig")

        if reasons:
            rows.append({
                "komponente": comp,
                "auffaelligkeit": "; ".join(reasons),
                "erste_woche": first.date().isoformat(),
                "letzte_woche": last.date().isoformat(),
                "luecken_wochen": n_gap,
            })
    return pd.DataFrame(rows)


def _pdf_text_page(pdf, lines: List[str], title: str) -> None:
    """Einfache Textseite (A4 quer)."""
    fig = plt.figure(figsize=(11.7, 8.3))
    fig.suptitle(title, fontsize=14, y=0.96)
    body = "\n".join(lines)
    fig.text(0.06, 0.88, body, fontsize=10, va="top", family="monospace")
    pdf.savefig(fig)
    plt.close(fig)


def _pdf_table_pages(pdf, table: pd.DataFrame, title: str,
                     rows_per_page: int = 26) -> None:
    """Rendert ein DataFrame als (ggf. mehrseitige) Tabelle ins PDF."""
    if table.empty:
        fig = plt.figure(figsize=(11.7, 8.3))
        fig.suptitle(title, fontsize=14)
        fig.text(0.5, 0.5, "Keine Auffälligkeiten gefunden.",
    ...
    # Spaltenbreiten proportional zur max. Textlänge je Spalte, damit
    # lange Komponentennamen nicht abgeschnitten werden.
    widths = []
    for col in table.columns:
        maxlen = max([len(str(col))] + [len(str(v)) for v in table[col]])
        widths.append(maxlen)
    wsum = float(sum(widths)) or 1.0
    col_widths = [w / wsum for w in widths]
    for p in range(pages):
        chunk = table.iloc[p * rows_per_page:(p + 1) * rows_per_page]
        fig, ax = plt.subplots(figsize=(11.7, 8.3))
        ax.axis("off")
        ax.set_title(f"{title}  (Seite {p + 1}/{pages}, {n} gesamt)",
...
def _pdf_component_pages(pdf, comp_weekly: pd.DataFrame,
                         ncols: int = 4, nrows: int = 5,
                         freq_label: str = "wöchentlich") -> None:
    """Small-Multiples: je Endausprägung ein Linienverlauf (Lücken sichtbar)."""
    comps = sorted(comp_weekly.columns)
    per = ncols * nrows
    pages = (len(comps) + per - 1) // per
    for p in range(pages):
        sub = comps[p * per:(p + 1) * per]
        fig, axes = plt.subplots(nrows, ncols, figsize=(16, 11), squeeze=False)
        for i, ax in enumerate(axes.flat):
            if i < len(sub):
                comp = sub[i]
                s = comp_weekly[comp]
                ax.plot(s.index, s.values, lw=0.9, color="#4C78A8")
                ax.set_title(_readable_name(comp), fontsize=7)
                ax.tick_params(labelsize=6)
                ax.grid(alpha=0.25)
            else:
                ax.axis("off")
        fig.suptitle(
            f"Endausprägungen – Verlauf je Komponente ({freq_label}) – "
            f"Seite {p + 1}/{pages}",
            fontsize=13,
        )
        fig.tight_layout(rect=[0, 0, 1, 0.97])
...
def write_data_report_pdf(
    ...
    """
    Erzeugt einen tiefen Daten-Report als **ein** PDF (``data_report.pdf``)
    mit Liniendiagrammen – fachbereichsfreundlich und für die Fehlersuche
    in der DB-Beladung:

    1. Titel + Gesamtverlauf,
    2. Aggregierte Verläufe je Ebene (Kennzahl, Sparte, Status, Produkt, ORGA),
    3. Auffälligkeiten (Lücken, negative Werte, Spät-/Früh-Ende, …),
    4. je Endausprägung (Komponente) ein eigener Verlauf.

    Lücken in der Beladung bleiben als Linienunterbrechung sichtbar (leere
    Zeiträume werden NICHT mit 0 aufgefüllt), damit echte Nullen von
    fehlenden Daten unterscheidbar sind. Aggregation auf ``freq``
    (Default wöchentlich) hält die ~5 Jahre lesbar.
    """
    from matplotlib.backends.backend_pdf import PdfPages

    if df_daily is None or df_daily.empty:
        if logger is not None:
            logger.warning("PDF-Report übersprungen: keine Daten.")
        return None
    ...
    comp_weekly = df.resample(freq).sum(min_count=1)
    total = comp_weekly.sum(axis=1, min_count=1)
    total_volume = float(np.nansum(df.values))

    # Auffälligkeiten auch als CSV ablegen.
    anomalies = _detect_loading_anomalies(df, comp_weekly)
    anomalies.to_csv(run_dir / "data_report_anomalien.csv", index=False)

    pdf_path = run_dir / "data_report.pdf"
    with PdfPages(pdf_path) as pdf:
        # --- Seite 1: Titel + Gesamtverlauf ---
        fig, ax = plt.subplots(figsize=(11.7, 8.3))
        ax.plot(total.index, total.values, lw=1.2, color="#4C78A8",
                label="Gesamtvolumen")
        roll = total.rolling(13, min_periods=1).mean()
        ax.plot(roll.index, roll.values, lw=2.0, color="#E45756",
                label="gleitendes Mittel (13 Wochen)")
        ax.set_title("Gesamtverlauf Termineingänge", fontsize=12)
        ax.grid(alpha=0.3)
        ax.legend(fontsize=9)
        fig.suptitle(
            f"Daten-Report DB-Historie · {df.index.min().date()} – "
            f"{df.index.max().date()} · {df.shape[1]} Komponenten · "
            f"Gesamtvolumen {total_volume:,.0f} · "
            f"{len(anomalies)} auffällige Komponenten",
            fontsize=12, y=0.98,
        )
        ...
        # --- Seiten 2–3: Aggregierte Ebenen ---
        page_groups = [
            [("kennzahl", "Verlauf je Kennzahl"),
             ("sparte", "Verlauf je Sparte")],
            [("status", "Verlauf je Schadenstatus"),
             ("orga", "Verlauf je ORGA")],
            [("produkt", "Verlauf je Produkt")],
        ]
        for grp in page_groups:
            fig, axes = plt.subplots(len(grp), 1, figsize=(11.7, 8.3),
                                     squeeze=False)
            for ax, (dim, title) in zip(axes.flat, grp):
                mat = _level_matrix(df, dim_map, dim, freq)
                _plot_level_lines(ax, mat, title)
            fig.tight_layout()
            pdf.savefig(fig)
            plt.close(fig)
        ...
        _pdf_table_pages(
            pdf, anomalies,
            "Auffälligkeiten (Heuristik zur DB-Beladungs-Fehlersuche)",
        )

        # --- je Endausprägung ein Verlauf ---
        _pdf_component_pages(pdf, comp_weekly)

    if logger is not None:
        logger.info(
            "PDF-Daten-Report geschrieben (%d Komponenten, %d auffällig): %s",
            df.shape[1], len(anomalies), pdf_path,
        )
    return pdf_path

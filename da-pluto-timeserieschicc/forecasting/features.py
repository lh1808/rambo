...
- :func:`build_target_and_past_covariates` – Trennung in Zielreihe (nur
  "echte" ``KENNZAHL__PRODUKT__STATUS__ORGA``-Komponenten) und Past-Kovariaten
...
Spaltenkonvention
-----------------
Target-Spalten folgen dem Namensmuster ``KENNZAHL__PRODUKT__STATUS__ORGA``,
z. B. ``TERM_EINGANG_SCHRIFTST__KFZ_Vollkasko__NEU__ORG12``. Aus dem Namen
werden im :func:`add_static_covariates`-Schritt vier statische Merkmale
(Kennzahl, Sparte, Status, Orga) extrahiert.
"""
...
def add_product_aggregates_to_df(
    ...
    Erwartet Spaltennamen der Form ``KENNZAHL__PRODUKT__STATUS__ORGA``. Für jedes
    ...
    df
        ...
        ``KENNZAHL__PRODUKT__STATUS__ORGA``-Struktur erkennbar ist.
    prep_cfg
...
def augment_X_with_target_past_features_safe(
    ...
    # Alle neuen Spalten in einem Dict sammeln und am Ende per pd.concat
    # in einem Schritt anhängen. Das vermeidet DataFrame-Fragmentierung
    # (hunderte einzelne Spalten-Inserts erzeugen sonst massive
    # PerformanceWarnings bei pandas).
    ...
    for col in base_cols:
        ...
        if prep_cfg.add_lags and lags:
            for L in lags:
                extra_cols[f"{col}_lag{L}"] = df[col].shift(L)

        if prep_cfg.add_ma and rolling_windows:
            for W in rolling_windows:
                extra_cols[f"{col}_ma{W}"] = (
                    df[col].rolling(window=W, min_periods=prep_cfg.min_periods).mean()
                )

        if prep_cfg.add_std and rolling_windows:
            for W in rolling_windows:
                extra_cols[f"{col}_std{W}"] = (
                    df[col].rolling(window=W, min_periods=prep_cfg.min_periods).std()
                )

        if prep_cfg.add_yoy:
            if prep_cfg.yoy_mode == "diff":
                extra_cols[f"{col}_yoy"] = df[col] - df[col].shift(YOY_SHIFT_WEEKS)
            elif prep_cfg.yoy_mode == "log":
                with np.errstate(divide="ignore", invalid="ignore"):
                    extra_cols[f"{col}_yoy"] = np.log1p(df[col]) - np.log1p(
                        df[col].shift(YOY_SHIFT_WEEKS)
                    )
            else:
                extra_cols[f"{col}_yoy"] = df[col] / df[col].shift(YOY_SHIFT_WEEKS) - 1.0

    if extra_cols:
        new_df = pd.concat([df, pd.DataFrame(extra_cols, index=df.index)], axis=1)
    else:
        new_df = df
...
def build_target_and_past_covariates(
    ...
    ``KENNZAHL__PRODUKT__STATUS__ORGA``-Muster entsprechen
    (``component_n_parts - 1`` Vorkommen des Separators). Aggregat- und abgeleitete Spalten wandern in die
    Past-Kovariaten.
    ...
    expected_seps = prep_cfg.component_n_parts - 1

    target_cols_all = [
        c for c in numeric_cols
        if isinstance(c, str) and c.count(sep) == expected_seps
    ]
    if len(target_cols_all) == 0:
        raise ValueError(
            "Es wurden keine gültigen Target-Spalten gefunden mit genau "
            f"{expected_seps} '{sep}'-Trennern (component_n_parts="
            f"{prep_cfg.component_n_parts})."
        )

    # Guardrail 1: dünn besetzte KPSO-Zellen verwerfen. Sie bleiben in den
    # Produkt-Aggregaten enthalten (die wurden in preprocess_dataframe vor
    # dem Drop berechnet), nur als eigene Zielreihe fallen sie weg.
    dropped_cols: List[str] = []
    if prep_cfg.min_component_total > 0:
        totals = df[target_cols_all].abs().sum()
        target_cols = [c for c in target_cols_all if totals[c] >= prep_cfg.min_component_total]
        dropped_cols = [c for c in target_cols_all if c not in target_cols]
        if dropped_cols:
            logging.getLogger(__name__).warning(
                "min_component_total=%.3g: %d von %d KPSO-Zielzellen verworfen "
                "(zu geringes Volumen): %s",
                prep_cfg.min_component_total, len(dropped_cols),
                len(target_cols_all), sorted(dropped_cols),
            )
    else:
    ...
    if len(target_cols) == 0:
        raise ValueError(
            "Nach dem min_component_total-Filter sind keine Target-Spalten "
            "übrig – Schwellwert vermutlich zu hoch."
        )

    # Guardrail 2: Auswahl der Roh-Spalten für die Past-Kovariaten. Leere
    # Liste = alle (bisheriges Verhalten). Mit z. B. ["AGG_PROD_"] fließen
    # nur die Produkt-Aggregate ein, wodurch die Kovariatenbreite von der
    # ORGA-Kardinalität entkoppelt wird. Verworfene Zielzellen werden in
    # keinem Fall als Past-Kovariate verwendet.
    if prep_cfg.past_covariate_select:
        pc_cols = [
            c for c in numeric_cols
            if any(pat in c for pat in prep_cfg.past_covariate_select)
            and c not in dropped_cols
        ]
        if not pc_cols:
            raise ValueError(
                "past_covariate_select hat keine Spalte getroffen: "
                f"{prep_cfg.past_covariate_select}"
            )
    else:
        pc_cols = [c for c in numeric_cols if c not in dropped_cols]

    ts_y_base = TimeSeries.from_dataframe(df[target_cols], freq=cfg.data.freq)
    ts_X_raw = TimeSeries.from_dataframe(df[pc_cols], freq=cfg.data.freq)
...
def extract_static_for_component(
    ...
    """
    Leitet die statischen Merkmale (Kennzahl, Sparte, Status, Orga) aus
    einem Komponenten-Namen ``KENNZAHL__PRODUKT__STATUS__ORGA`` ab.
    ...
    orga = parts_main[prep_cfg.static_orga_index]
...
def add_static_covariates(
    ...
    """
    Hängt pro Komponente vier statische kategoriale Merkmale (Kennzahl,
    Sparte, Status, Orga) an und fittet einen One-Hot-Transformer darauf.

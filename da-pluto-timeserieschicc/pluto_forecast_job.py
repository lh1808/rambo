...
def run_pluto_multivariate_forecast_job() -> Dict[int, Dict[str, object]]:
    ...
    # Gemeinsamer Zeitstempel für alle Artefakte dieses Laufs (Daten-
    # Überblick + Reporting), damit die Run-Unterverzeichnisse zusammenpassen.
    run_ts = datetime.now()
    ...
    try:
        ...
        # Strukturierter Überblick der historischen DB-Daten (Wo/Wann/Menge).
        # Wird je Lauf in ein eigenes Unterverzeichnis geschrieben.
        try:
            overview_dir = write_data_overview(
                df_daily, runs_dir, run_timestamp=run_ts, logger=logger
            )
            if overview_dir is not None:
                logger.info("Daten-Überblick unter %s", overview_dir)
            # Tiefer Report als ein PDF (Liniendiagramme je Ebene + je
            # Endausprägung, inkl. Auffälligkeiten für die Beladungs-Prüfung).
            pdf_path = write_data_report_pdf(
                df_daily, runs_dir, run_timestamp=run_ts, logger=logger
            )
            if pdf_path is not None:
                logger.info("Daten-Report (PDF) unter %s", pdf_path)
        except Exception as e:
            # Der Überblick ist Diagnostik – ein Fehler hier darf den
            # eigentlichen Forecast-Job nicht abbrechen.
            logger.warning("Daten-Überblick/-Report fehlgeschlagen (übersprungen): %s", e)
        ...
        reporter = RunReporter(output_dir=runs_dir, run_timestamp=run_ts)
        ...
        # Nur Zukunftsdaten schreiben: alles nach dem letzten Ist-Datum.
        # Die Forecast-Serie startet normalerweise bereits nach dem
        # letzten Datenpunkt, aber als explizite Absicherung filtern wir
        # hier nochmal — damit niemals Ist-Zeiträume überschrieben werden.
        data_end = df_daily.index.max()
        future_mask = df_combined.index > data_end
        df_future = df_combined.loc[future_mask]

        if df_future.empty:
            logger.warning(
                "Keine Forecast-Zeilen nach dem letzten Ist-Datum (%s). "
                "Nichts zu schreiben.",
                data_end.date(),
            )
        else:
            # Optionale Tages-Disaggregation des Wochen-Forecasts.
            # Reporting/Evaluation oben bleiben wöchentlich; nur der in die
            # DB geschriebene Forecast wird ggf. auf Tageswerte gebrochen.
            if cfg.disaggregation.enabled:
                weights_df, global_w, _levels = compute_weekday_weights(
                    df_daily,
                    window_weeks=cfg.disaggregation.window_weeks,
                    min_window_volume=cfg.disaggregation.min_window_volume,
                    logger=logger,
                )
                df_daily_fc = disaggregate_weekly_to_daily(
                    df_future,
                    weights_df,
                    global_w,
                    weekend_policy=cfg.disaggregation.weekend_policy,
                    logger=logger,
                )
                # Auch auf Tagesebene sicherstellen: nie Ist-Zeiträume
                # überschreiben (das erste Wochenfenster kann den aktuellen
                # Wochenrest berühren).
                df_write = df_daily_fc.loc[df_daily_fc.index > data_end]
                granularitaet = "Tage"
            else:
                df_write = df_future
                granularitaet = "Wochen"

            if df_write.empty:
                logger.warning(
                    "Nach Filterung (> %s) keine Schreibzeilen übrig.",
                    data_end.date(),
                )
            else:
                # Hinweis: bei aktiver Disaggregation vervielfacht sich die
                # Zeilenzahl grob um Faktor 7 (Tage) × Komponentenzahl; das
                # DELETE+INSERT (executemany) wächst entsprechend.
                logger.info(
                    "Schreibe Forecast: %d %s ab %s bis %s "
                    "(letztes Ist-Datum: %s).",
                    len(df_write),
                    granularitaet,
                    df_write.index.min().date(),
                    df_write.index.max().date(),
                    data_end.date(),
                )

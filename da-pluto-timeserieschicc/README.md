...
- `NEU` (Neuschaden)
- `LAUFEND` (laufend / Folgebearbeitung)
...
TERM_EINGANG_SONST__HUS_Hausrat__LAUFEND__UNBEKANNT
```
...
│   ├── data_report.pdf              ← TIEFER Report: alles in einer Datei
...
│   └── data_report_anomalien.csv    ← geflaggte Komponenten (Beladungs-Prüfung)
├── 2025-04-21T08-30_h13_model/
│   ├── run_metadata.json
│   ├── config.yaml
│   ├── metrics.csv
...
Zusätzlich entsteht `data_report.pdf` – ein tiefer, fachbereichsfreundlicher
Report in **einer** Datei (nur diese öffnen): Gesamtverlauf, Linien­diagramme
je Ebene (Kennzahl, Sparte, Status, Produkt, ORGA), eine Auffälligkeitsseite
zur Fehlersuche in der DB-Beladung (Lücken, negative Werte, Spät-/Früh-Ende,
durchgehend Null) und je Endausprägung (Komponente) ein eigener Verlauf.
Lücken bleiben als Linienunterbrechung sichtbar (leere Zeiträume werden nicht
mit 0 aufgefüllt), damit echte Nullen von fehlenden Daten unterscheidbar
sind. Aggregation standardmäßig wöchentlich, damit die ~5 Jahre lesbar
bleiben. Die geflaggten Komponenten liegen zusätzlich als
`data_report_anomalien.csv` vor.
...
| `status` | `NEU` | NEU (Neuschaden) vs. LAUFEND (laufend) |

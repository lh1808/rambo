...
const ADDON_PRESETS = [
  ...
  // ── Feature-Selektion ──
  {key:"feature_reduction",label:"Feature-Reduktion",desc:"Max. 60 Features via Importance + Korrelationsfilter",group:"feature",
    cfg:{fsEnabled:true,fsMethods:["catboost_importance","causal_forest"],fsCorrThresh:0.9,fsMaxFeatures:60}},
  // ── Erklärbarkeit ──
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {files.map((f,i) => (
              <div key={i}
                ...
                <div style={{fontWeight:600,color:f.trim()?"#1a7f37":"#6B0D15",fontSize:12.5}}>{f.trim() ? "✓ "+f.split("/").pop() : "Datei "+(i+1)+" hochladen"}</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {(dp.evalFiles||[""]).map((f,i) => (
                <div key={i}
                  ...
                  <div style={{fontWeight:600,color:f.trim()?"#1a7f37":"#6366f1",fontSize:12.5}}>{f.trim() ? "✓ "+f.split("/").pop() : `Eval-Datei ${i+1} hochladen`}</div>
                  <div style={{fontSize:10.5,color:"#999",marginTop:3}}>{f.trim() ? "Klicken zum Ersetzen" : "Klicken zum Hochladen"}</div>
                </div>
              ))}
            </div>
      ...
      <Sec title="3. Feature Dictionary laden (optional)" accent="#6366f1">
        ...
        {dpTab==="Dateipfade" ? (
          ...
          <div style={{border:"1.5px solid "+(dp.featurePath?"#86efac":"#c4b5fd"),borderRadius:10,padding:"20px 16px",textAlign:"center",background:dp.featurePath?"#f0fdf4":"#f5f3ff",cursor:"pointer",transition:"all 0.15s"}}
            ...
            <div style={{fontWeight:600,color:dp.featurePath?"#1a7f37":"#6366f1",fontSize:12.5}}>{dp.featurePath ? "✓ "+dp.featurePath.split("/").pop() : "Dictionary hochladen (.xlsx / .csv)"}</div>
...
const YAML_TO_CFG = {
  ...
  "tuning.enabled":"tuningEnabled","tuning.n_trials":"tuningTrials","tuning.cv_splits":"dmlCrossfitFolds","tuning.single_fold":"tuningSingleFold","tuning.overfit_penalty":"overfitPenalty","tuning.overfit_tolerance":"overfitTolerance",
  ...
  return (
    <>
      {view==="files"&&<Sec title="Analysedaten">
        ...
        {/* ── Evaluationsmodus (konsistent mit DataPrep) ── */}
        <Info>{cfg.dpRunName
        ...
        {/* ── External Eval (nur bei external-Modus) ── */}
        {cfg.validateOn==="external" && (<div style={{marginTop:20,background:"#f5f3ff",border:"1.5px solid #c4b5fd",borderRadius:12,padding:"14px 18px"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
            <span style={{fontSize:12,fontWeight:600,color:"#6366f1",textTransform:"uppercase",letterSpacing:0.6}}>External Eval</span>
            {(cfg.eval_x_file && cfg.eval_t_file && cfg.eval_y_file)
              ? <span style={{fontSize:9,fontWeight:600,color:"#059669",background:"#f0fdf4",padding:"2px 8px",borderRadius:8}}>Bereit</span>
              ...
              {[{label:"X (Features)",key:"eval_x_file"},{label:"Y (Outcome)",key:"eval_y_file"},{label:"T (Treatment)",key:"eval_t_file"},{label:"S (Score, optional)",key:"eval_s_file"}].map(f => {
                ...
                return (
                  <div key={f.key}
                    ...
                    <div style={{fontWeight:600,color:cfg[f.key]?"#1a7f37":"#6366f1",fontSize:12.5}}>{cfg[f.key] ? "✓ "+cfg[f.key].split("/").pop() : f.label}</div>
...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Feature-Selektion" accent="#0d9488">
    <Info>Reduziert die Feature-Matrix auf die relevantesten Spalten. Der Prozess läuft in drei Stufen: (1) Korrelationsfilter entfernt redundante Features, (2) Importance-Berechnung pro Methode, (3) Top-N pro Methode auswählen und per Union zusammenführen.</Info>
    ...
    {cfg.fsEnabled&&<>
      <Divider/>
      <div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:10}}>1. Importance-Methoden</div>
      ...
      <Divider/>
      <div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:10}}>2. Feature-Budget</div>
      <Inp label="Max. Features" type="number" value={cfg.fsMaxFeatures||60} onChange={v=>set({...cfg,fsMaxFeatures:Number(v)})} help={`Maximale Anzahl Features nach Selektion. Bei ${(cfg.fsMethods||["catboost_importance"]).length} Methode(n): je ${Math.ceil((cfg.fsMaxFeatures||60)/Math.max(1,(cfg.fsMethods||["catboost_importance"]).length))} Features pro Methode, dann Union auf ${cfg.fsMaxFeatures||60} gekappt.`}/>
      {(()=>{
        const nM = (cfg.fsMethods||["catboost_importance"]).length;
        const maxF = cfg.fsMaxFeatures||60;
        ...
        return <div style={{fontSize:11,color:C.textSec,background:"#f9fafb",padding:"8px 12px",borderRadius:8,marginTop:6,lineHeight:1.5,border:"1px solid #e5e7eb"}}>
          <strong>{nM}</strong> Methode{nM>1?"n":""} × <strong>{perM}</strong> Features = max. <strong>{nM*perM}</strong> in der Union → gekappt auf <strong>{maxF}</strong>.
          {nM>1 && " Overlaps zwischen Methoden reduzieren die Gesamtzahl — je mehr Overlap, desto stabiler die Selektion."}
          {nM>1 && " Bei Überschreitung entscheidet Rank-Aggregation (mittlere Position über alle Methoden), welche Features bleiben."}
        </div>;
      })()}
      <Divider/>
      <div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:10}}>3. Korrelationsfilter</div>
...
const TuningPlanPreview = ({models, perRole, perLearner, trials, cv, outerCv, innerCv: innerCvProp, isBoth, isRct}) => {
  ...
  const fitsDetail = (sig) => {
    ...
    if (fam==="pseudo_effect") return nt+"T × ("+nCv+"F×"+K+"G cate + "+fullCv+"CV×2+2 nuis)";
    return nt+"T × "+nCv+"F";
  };
  ...
  return (
    <div>
      ...
      {plan.some(t=>t.sig.split("__")[0]==="pseudo_effect") && (
        <div style={{fontSize:10.5,color:"#6b7280",marginTop:6,lineHeight:1.4}}>
          <strong style={{color:"#374151"}}>Pseudo-Effekt:</strong> Pro Trial werden zuerst Nuisance-Modelle (μ₀, μ₁) mit den <em>getunten</em> Params aus dem Grouped-Outcome-Task berechnet ({fullCv}CV × 2 Gruppen + 2 Fallback = {fullCv*2+2} Fits). Darauf werden die CATE-Regressoren auf den resultierenden Pseudo-Outcomes evaluiert ({nCv}F × {K}G = {nCv*K} Fits).
        </div>
      )}
    </div>
  );
...
const buildYaml = (cfg, sp, spFmt) => {
  ...
  if(cfg.fsEnabled) {
    ...
    a(`  max_features: ${cfg.fsMaxFeatures||60}`);
...
const PPreview = ({cfg,sp,spFmt}) => {const y=buildYaml(cfg,sp,spFmt),issues=validate(cfg);const mt=cfg.treatmentType==="multi";return(<>
  <Row gap={12}>
    ...
    <MC value={cfg.validateOn==="external"?"External":cfg.eval_mask_file?"TMES":"CV-"+(cfg.cvSplits||5)} label="Validierung"/>
...
function App() {
  ...
  const estimateFits = () => {
    ...
    // 6. DRTester Nuisance
    // Cross/TMES: has_train=True → fit_nuisance auf Train UND Val (4×5 = 20 Fits @ cv=5)

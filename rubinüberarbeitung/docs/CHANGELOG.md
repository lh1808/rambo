...
- **HTML-Report TMES-Erkennung:** Der Report zeigt jetzt alle drei Validierungsmodi korrekt an (`cross (5 Folds)`, `external (separater Datensatz)`, `TMES (Mask-Holdout)`). Summary-Bar zeigt bei TMES zusätzlich `Eval (TMES): N Beob.`.

...
class BaseLearnerTuner:
    ...
    def _tune_task(self, task: TuningTask, X: pd.DataFrame, Y: np.ndarray, T: np.ndarray, shared_params: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        _tlog.info(
            "BLT '%s': Starte %d Trials (parallel=%d, cv=%d).",
            task.key, _n_trials, _pj_start, self.cfg.tuning.cv_splits,
        )
...
in allen Cross-Prediction-Folds wiederverwendet."""
    ...
    def _log_trial_diagnostics(self, study, label: str, n_jobs: int = 1):
        ...
        n_pruned = _states.get("PRUNED", 0)
        _tlog.info(
            "FMT '%s': %d/%d Trials abgeschlossen (%d fehlgeschlagen, %d gepruned, parallel=%d).",
            label, n_complete, len(study.trials), n_fail, n_pruned, n_jobs,
        )
        if n_fail > 0:
            error_groups: dict = {}
            for tr in study.trials:
    ...
    def tune_final_model(
        ...
        if name == "nonparamdml":
            ...
            self._log_trial_diagnostics(study, model_name, n_jobs=_fmt_pj)
            try:
...
def tune_causal_forest(
    ...
    _tlog.info(
        "CFT '%s': Starte %d Trials (%d Folds, Seed=%d, parallel=1).%s",
    ...
    # ── Trial-Diagnose (konsistent mit BLT/FMT) ──
    _states = {}
    for tr in study.trials:
        sname = tr.state.name
        _states[sname] = _states.get(sname, 0) + 1
    ...
    n_pruned = _states.get("PRUNED", 0)
    _tlog.info(
        "CFT '%s': %d/%d Trials abgeschlossen (%d fehlgeschlagen, %d gepruned, parallel=1).",
        model_type, n_complete, n_trials, n_fail, n_pruned,
    )
    if n_fail > 0:
        error_groups: dict = {}
        for tr in study.trials:
            if tr.state.name == "FAIL":
                reason = (tr.system_attrs or {}).get("fail_reason", "Unbekannt")
                err_lines = [l.strip() for l in reason.strip().split("\n") if l.strip()]
                short = err_lines[-1] if err_lines else "Unbekannt"
                key = short[:120]
                if key not in error_groups:
                    error_groups[key] = {"count": 0, "full_example": reason}
                error_groups[key]["count"] += 1
        _tlog.warning("CFT '%s': %d/%d Trials FEHLGESCHLAGEN. Fehlertypen:", model_type, n_fail, n_trials)
        for err_key, info in sorted(error_groups.items(), key=lambda x: -x[1]["count"]):
            _tlog.warning("  [%d×] %s", info["count"], err_key)
        most_common = max(error_groups.values(), key=lambda x: x["count"])
        _tlog.warning("CFT '%s': Häufigster Fehler — vollständiger Traceback:\n%s", model_type, most_common["full_example"][:2000])

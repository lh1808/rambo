...
class BaseLearnerTuner:
    ...
    def _role_signature(self, model_name: str, role: str) -> Tuple[str, str, bool, str, str]:
        ...
        Für das Tuning-Sharing werden "all" und "all_direct" zusammengelegt (in _task_key),
        wenn äußere CV-Folds == interne DML-Folds (Default: beide 5). Bei gleicher
        Fold-Anzahl sind die effektiven Trainingsmengen ähnlich genug. Bei unterschiedlicher
        Fold-Anzahl bleiben die Scopes getrennt. "group_specific_shared_params" bleibt immer separat."""
    ...
    def _task_key(self, model_name: str, role: str) -> str:
        ...
        # Scope-Merge: "all_direct" (Meta-Learner) und "all" (DML/DR Cross-Fitting)
        # werden zusammengelegt, WENN äußere CV-Folds == interne DML-Folds.
        # Bei gleicher Fold-Anzahl ist die effektive Trainingsmenge ähnlich genug,
        # dass dieselben Hyperparameter für beide Scopes optimal sind.
        # Bei unterschiedlicher Fold-Anzahl bleiben die Scopes getrennt.
        _outer_cv = self.cfg.data_processing.cross_validation_splits
        _inner_cv = self.cfg.data_processing.dml_crossfit_folds
        if _outer_cv == _inner_cv and sample_scope in ("all", "all_direct"):
            effective_scope = "all"
        else:
    ...
    def tune_all(self, model_names: List[str], X: pd.DataFrame, Y: np.ndarray, T: np.ndarray) -> Dict[str, Dict[str, Dict[str, Any]]]:
        ...
        _outer = self.cfg.data_processing.cross_validation_splits
        _inner = self.cfg.data_processing.dml_crossfit_folds
        _scope_merged = _outer == _inner
        _tlog.info(
            "Tuning-Plan: %d Tasks für %d Modelle%s.",
            len(plan), len(active_models),
            f" (Scope-Merge aktiv: äußere CV={_outer} == interne CV={_inner})" if _scope_merged else
            f" (Scopes getrennt: äußere CV={_outer} ≠ interne CV={_inner})",
        )

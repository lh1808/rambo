        ...
        {keys.map(k => {
          ...
          return (
            <div key={k} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 12px",background:changed?"#FDF2F3":"#fff",borderRadius:R,border:`1px solid ${changed?"#e0b5b9":C.borderLight}`,transition:"all 0.15s"}}>
              <code style={{fontSize:11,fontWeight:600,color:changed?C.ruby:C.textSec,minWidth:120,fontFamily:MONO}}>{k}</code>
              <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"flex-end",gap:6}}>
                {isBool ? (
                  ...
                  <input type={isNum?"number":"text"} value={val===null?"":val} onChange={e=>{const v=e.target.value;set(k,isNum?(v===""?def:Number(v)):v===""?def:v)}} style={{width:80,height:30,padding:"0 8px",border:`1.5px solid ${changed?"#d4a0a6":C.border}`,borderRadius:6,fontSize:12,fontFamily:MONO,fontWeight:600,background:"#fff",outline:"none",boxSizing:"border-box",textAlign:"right",color:changed?C.ruby:C.text}} onFocus={e=>{e.target.style.borderColor=C.ruby;e.target.style.boxShadow="0 0 0 2px rgba(155,17,30,0.08)"}} onBlur={e=>{e.target.style.borderColor=changed?"#d4a0a6":C.border;e.target.style.boxShadow="none"}}/>
                )}
  ...
  return (
    <div style={{marginBottom:8,padding:"9px 14px",background:"#faf7f7",borderRadius:R,border:`1px solid ${C.borderLight}`}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
        ...
        <span style={{fontSize:11,fontFamily:MONO,color:C.ruby,background:"#fff",padding:"1px 8px",borderRadius:4,border:`1px solid ${C.borderLight}`,fontWeight:600}}>{f(low)} – {f(high)}</span>
      </div>
...
const ADDON_PRESETS = [
  ...
  {key:"grf_tuning",label:"CausalForest-Tuning",desc:"RScorer-Grid-Search für CausalForest / CausalForestDML (je 12 Kombis)",group:"tuning",
    cfg:{cfTune:true,cfTuneModels:["CausalForestDML","CausalForest"]}},
  {key:"grf_tuning_intensiv",label:"CausalForest-Tuning Intensiv",desc:"Erweitertes Grid mit je 48 Kombis (+ criterion mse/het, erweiterte Wertebereiche)",group:"tuning",
...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Allgemein">
    ...
    <div style={{marginBottom:14}}>
      ...
      <style>{`
        .tuning-seed-slider { width:100%; accent-color:#D4A853; }
  ...
  <Sec title="Evaluierungsmodus">
    ...
    <Inp label="Äußere CV-Folds (K)" type="number" value={cfg.cvSplits} onChange={v=>set({...cfg,cvSplits:v})} help="Äußere Cross-Validation für Out-of-Fold CATE-Predictions. Interne CVs (BLT, FMT, DML/DR-Cross-Fitting) laufen unabhängig davon."/>
...
const computeTuningPlan = (models, perRole, perLearner, outerCv, innerCv) => {
  const tasks = {};
  const scopeMerge = (outerCv||5) === (innerCv||5);
  (models || []).forEach(m => {
    rolesForModel(m).forEach(({role, label, sig}) => {
      // Scope-Merge: all_direct → all wenn äußere CV == interne CV (Backend-Logik spiegeln)
      let key = scopeMerge ? sig.replace("all_direct","all") : sig;
      if(perRole) key += "__" + role.toLowerCase();
      if(perLearner) key += "__" + m.toLowerCase();
      if(!tasks[key]) tasks[key] = {key, label, sig: scopeMerge ? sig.replace("all_direct","all") : sig, models: [], roles: []};
  ...
  return (
    <div>
      ...
      <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
        ...
        {(outerCv||5)===(fullCv) && plan.length < (models||[]).reduce((a,m)=>a+rolesForModel(m).length,0) && <span style={{display:"block",marginTop:6,color:"#059669",background:"#f0fdf4",padding:"4px 8px",borderRadius:6,border:"1px solid #a7f3d0",fontSize:10.5}}><strong>Scope-Merge aktiv</strong> (äußere CV={outerCv||5} = interne CV={fullCv}): Meta-Learner und DML/DR-Tasks mit gleicher Signatur werden zusammengefasst.</span>}
...
<FixedParamsEditor params={(cfg.fmtFixed||{}).catboost||{}} defaults={CB_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:{...(cfg.fmtFixed||{}),catboost:v}})} label="CatBoost CATE-Parameter (Fixed)"/>
<div style={{fontSize:12,fontWeight:600,color:C.dark,marginBottom:6,marginTop:14}}>LightGBM-Parameter (Final-Modell)</div>
<FixedParamsEditor params={(cfg.fmtFixed||{}).lgbm||{}} defaults={LGBM_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:{...(cfg.fmtFixed||{}),lgbm:v}})} label="LightGBM CATE-Parameter (Fixed)"/>
</>) : (<FixedParamsEditor params={cfg.fmtFixed||{}} defaults={(cfg.baseLearner||"catboost")==="catboost"?CB_FINAL_DEFAULTS:LGBM_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:v})} label="CATE-Parameter (Fixed)"/>)}</>}</Sec>{((cfg.models||[]).includes("CausalForestDML")||(cfg.models||[]).includes("CausalForest"))&&<Sec title="CausalForest-Tuning" accent="#2d6a4f"><Info>CausalForest und CausalForestDML werden per RScorer-Grid-Search über Wald-Parameter optimiert — unabhängig vom Optuna-Tuning der Nuisance-Modelle (BLT) und des CATE-Modells (FMT).</Info><Toggle label="RScorer-Grid-Search aktivieren" checked={cfg.cfTune||false} onChange={v=>{const u={cfTune:v};if(v&&(cfg.cfTuneModels||[]).length===0){u.cfTuneModels=(cfg.models||[]).filter(m=>["CausalForestDML","CausalForest"].includes(m));}set({...cfg,...u});}} help="Optimiert Wald-Parameter (max_depth, min_weight_fraction_leaf etc.) per Grid-Search. Die beste Kombination wird via RScorer (R-Learner-Loss) bewertet. Normal: 12 Kombis, Intensiv: 48 Kombis."/>{cfg.cfTune&&<><Divider/><div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:8}}>Tuning-Plan</div>{(() => {
              ...
              return (<>
                <div style={{display:"flex",gap:10,marginBottom:12,flexWrap:"wrap"}}>
                  <div style={{padding:"8px 16px",background:"linear-gradient(135deg,#2d6a4f,#40916c)",borderRadius:8,color:"#fff"}}><div style={{fontSize:22,fontWeight:700}}>{(hasCF?1:0)+(hasGRF?1:0)}</div><div style={{fontSize:10,textTransform:"uppercase",letterSpacing:0.5,opacity:0.7}}>Grid-Searches</div></div>
                ...
                <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
                  RScorer-basierter Grid-Search über Wald-Parameter. Beide Modelle fitten vorab Nuisance-Modelle für den RScorer (cv={dc}×2). CausalForestDML nutzt den internen RScorer{mc>1?" mit mc="+mc:""}, CausalForest einen externen RScorer mit BLT-getunten Nuisance-Modellen.
                </div>
                <div style={{border:"1px solid #ede6e7",borderRadius:8,overflow:"hidden"}}>
                  ...
                  {hasCF && <div style={{borderBottom:"1px solid #f0f0f0"}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 80px",padding:"8px 14px",fontSize:12}}>
                      ...
                      <span style={{fontFamily:MONO,fontWeight:700,color:"#2d6a4f"}}>{cfTotal}</span>
                    </div>
                  ...
                  {hasGRF && <div style={{background:hasCF?"#fdfbfb":"#fff"}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 80px",padding:"8px 14px",fontSize:12}}>
                      ...
                      <span style={{fontFamily:MONO,fontWeight:700,color:"#2d6a4f"}}>{grfTotal}</span>
                    </div>
...
{(()=>{
  ...
  return (<div style={{background:"#faf8f8",borderRadius:10,border:`1px solid ${C.borderLight}`,padding:"16px 20px"}}>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px 12px"}}>
      {grids.map(g => (
        <div key={g.param} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 12px",background:"#fff",borderRadius:8,border:`1px solid ${C.borderLight}`}}>
          <code style={{fontSize:11,fontWeight:600,color:"#2d6a4f",minWidth:130,fontFamily:MONO}}>{g.param}</code>
          <div style={{flex:1,display:"flex",gap:4,justifyContent:"flex-end",flexWrap:"wrap"}}>
            {g.values.map(v => (
              <span key={v} style={{fontSize:11,padding:"1px 8px",borderRadius:6,background:"#f0fdf4",border:"1px solid #d1fae5",color:"#2d6a4f",fontFamily:MONO,fontWeight:500}}>{v}</span>
            ))}
          </div>
        </div>
      ))}
    ...
    <div style={{fontSize:11,color:"#6b7280",marginTop:10,lineHeight:1.5}}>
      Nicht-getunte Parameter verwenden Default-Werte (n_estimators=200, honest=true, min_samples_leaf=5, max_samples=0.45).
    </div>
  </div>);
})()}
</>}
{!cfg.cfTune && <><Divider/><div style={{fontSize:11,color:"#6b7280",lineHeight:1.5}}>Alle Wald-Parameter verwenden Default-Werte. Aktiviere den RScorer-Grid-Search oben für automatische Optimierung.</div></>}</Sec>}</>);
};
...
function App() {
  ...
  const estimateFits = () => {
    ...
    if (cfg.tuningEnabled) {
      ...
      const scopeMerge = outerCv === innerCv;
      // Task 1: Outcome (Classifier, DML model_y)
      if (hasDml) f += tr * tc;
      // Task 2: Propensity — mit Scope-Merge: XL + DML/DR in 1 Task
      if (scopeMerge) {
        if (hasDml||hasDr||hasXl) f += propTr * tc;           // 1 merged Task
      } else {
        if (hasDml||hasDr) f += propTr * tc;                  // propensity (all)
        if (hasXl) f += propTr * tc;                          // propensity (all_direct)
      }
      // Task 3: Outcome Regression — mit Scope-Merge: SL + DR in 1 Task
      if (scopeMerge) {
        if (hasDr||hasSl) f += tr * tc;                       // 1 merged Task
      } else {
        if (hasDr) f += tr * tc;                              // outcome_regression (all)
        if (hasSl) f += tr * tc;                              // outcome_regression (all_direct)
      }
      // Task 4: Grouped Outcome Regression (TL + XL models)
      if (hasTl||hasXl) f += tr * tc * K;
      // Task 5: Pseudo-Effekt (XL cate_models) — Nuisance + CATE

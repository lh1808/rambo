...
const pages = [
  ...
  {label:"Modelle & Vorlagen",key:"template",group:"Experiment"},
  {label:"BL & FM Tuning",key:"models"},
...
const DEFAULT_CFG = {studyType:"rct",classBalance:false,expName:"rubin",seed:42,models:["NonParamDML","DRLearner","SLearner","TLearner","XLearner","ParamDML","CausalForestDML","CausalForest"],baseLearner:"catboost",ensembleEnabled:false,tuningEnabled:true,tuningTrials:100,tuningSingleFold:false,tuningMetric:"pr_auc",tuningMetricReg:"neg_mse",tuningPerRole:false,tuningPerLearner:false,tuningAutoml:"optuna",tuningModels:[],fmtEnabled:false,fmtModels:[],fmtSingleFold:false,fmtTrials:50,fmtMaxRows:0,fmtStabilityPenalty:0,fmtMethod:"rscorer",cfTune:false,cfTuneIntensive:false,bundleEnabled:false,bundleDir:"runs/bundles",bundleChallengers:true,bundleMlflow:true,surrEnabled:false,surrMinLeaf:50,surrLeaves:31,surrDepth:0,fsEnabled:false,fsMethods:["lgbm_importance"],fsTopPct:15,fsCorrThresh:0.9,fsMaxFeatures:0,downsample:false,dfFrac:0.1,reduceMem:true,validateOn:"cross",cvSplits:5,treatmentType:"binary",refGroup:0,selMetric:"qini",higherBetter:true,refitChamp:true,manualChamp:null,hasNaN:false,nanCols:[],explEnabled:false,explMethod:"shap",explSampleSize:10000,explTopN:20,shapModels:[],shapBins:10,histScoreName:"historical_score",histScoreCol:"S",histScoreHigher:true,outputDir:"",blFixed:{},fmtFixed:{},cfFixed:{},tuningTimeout:0,tuningMaxRows:0,fmtTimeout:0,maxPredRows:0,savePreds:false,predsFormat:"csv",parallelLevel:3,workDir:null,mcIters:null,mcAgg:"mean"};
...
const ADDON_PRESETS = [
  ...
  {key:"grf_tuning_intensiv",label:"GRF-Tuning Intensiv",desc:"Erweitertes Grid mit je 48 Kombis (+ criterion, max_depth)",group:"tuning",
    cfg:{cfTune:true,cfTuneIntensive:true}},
  // ── Production & Export ──
  ...
  // ── Imbalance ──
  {key:"imbalance",label:"Stark imbalancierte Zielvariable",desc:"Auto-Balancing + Recalibration für seltene Events (z.B. 1:1000)",group:"imbalance",
    cfg:{classBalance:true}},
  // ── Erklärbarkeit ──
  ...
  <>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(5, 1fr)",gap:10,marginBottom:26}}>
      {[
        ...
        {v:"2",l:"Base Learner",d:"LightGBM & CatBoost",accent:"#C4343F"},
        ...
        {v:"9+14",l:"Vorlagen & Add-ons",d:"9 Basis + 14 Add-ons",accent:"#6366f1"},
      ].map(c => (
    ...
    <>
      ...
      <Sec title="2. Pflicht-Spalten zuweisen">
        ...
        <div style={{marginTop:0}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:(dp.targets||["Y"]).length>1?"1fr 1fr":"1fr",gap:6}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:12,marginTop:6}}>
            ...
            {(dp.targets||["Y"]).filter(t=>t.trim()).length > 1 && <span style={{fontSize:10.5,color:"#7a5a00",background:"#fffbeb",padding:"3px 10px",borderRadius:6,border:"1px solid #e8d49c"}}>Y = {(dp.targets||[]).filter(t=>t.trim()).join(" + ")}</span>}
          </div>
        </div>

        <div style={{marginTop:14}}>
          <Inp label="Treatment-Spalte (T)" value={dp.treatment!=null?dp.treatment:"T"} onChange={v=>setDp(prev=>({...prev,treatment:v}))} help="Name der Spalte mit dem Treatment-Indikator"/>
        </div>
        ...
        {simCols && simCols.length > 0 && (() => {
          ...
          const warns = [];
          (dp.targets||["Y"]).filter(t=>t.trim()).forEach(t => {
            if (!colSet.has(t.toUpperCase())) warns.push({col: t, role: "Target (Y)"});
          });
          const treat = (dp.treatment||"T").trim();
      ...
      <Sec title="5. Weitere Optionen">
        {(() => {
          ...
          {(() => {
            ...
            const reserved = new Set([...((dp.targets||["Y"]).map(t=>(t||"").toUpperCase())), (dp.treatment||"T").toUpperCase(), ...((dp.scoreName||"").trim() ? [(dp.scoreName||"").toUpperCase()] : [])]);
...
const YAML_TO_CFG = {
  "study_type":"studyType","class_balance":"classBalance","mlflow.experiment_name":"expName","constants.SEED":"seed","constants.parallel_level":"parallelLevel","constants.work_dir":"workDir",
  ...
  "data_processing.validate_on":"validateOn","data_processing.cross_validation_splits":"cvSplits","data_processing.mc_iters":"mcIters","data_processing.mc_agg":"mcAgg",
  ...
  "tuning.enabled":"tuningEnabled","tuning.n_trials":"tuningTrials","tuning.single_fold":"tuningSingleFold","tuning.metric":"tuningMetric","tuning.metric_regression":"tuningMetricReg",
  ...
  "final_model_tuning.enabled":"fmtEnabled","final_model_tuning.models":"fmtModels","final_model_tuning.single_fold":"fmtSingleFold","final_model_tuning.stability_penalty":"fmtStabilityPenalty","final_model_tuning.n_trials":"fmtTrials","final_model_tuning.method":"fmtMethod",
    ...
    <>
      {view==="files"&&<Sec title="Analysedaten">
        ...
        <div style={{marginTop:20,borderTop:"1px solid "+C.borderLight,paddingTop:16}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:cfg.validateOn==="external"?12:0}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <div style={{fontSize:12,fontWeight:600,color:C.textSec,textTransform:"uppercase",letterSpacing:0.8}}>External Eval</div>
              {cfg.validateOn==="external" && (cfg.eval_x_file && cfg.eval_t_file && cfg.eval_y_file
                ? <span style={{fontSize:10,fontWeight:600,color:"#1a7f37",background:"#f0fdf4",border:"1px solid #a3d9b1",padding:"2px 10px",borderRadius:10}}>Konfiguriert</span>
                : <span style={{fontSize:10,fontWeight:600,color:"#7a5a00",background:"#fffbeb",border:"1px solid #e8d49c",padding:"2px 10px",borderRadius:10}}>Unvollständig</span>
              )}
            </div>
            <Toggle label="" checked={cfg.validateOn==="external"} onChange={v=>set({...cfg,validateOn:v?"external":"cross"})} help="Evaluation auf separatem Datensatz statt Cross-Validation"/>
          </div>
          {cfg.validateOn==="external" && (<>
            <div style={{fontSize:12,color:C.textSec,lineHeight:1.5,marginBottom:12}}>Training auf den obigen Daten, Evaluation auf separatem Datensatz. Kein Data-Leakage — Preprocessing wird getrennt auf den Trainingsdaten gefittet.</div>
            {tab==="Dateipfade" ? (
              <Row><Col>
                <Inp label="Eval X-Datei" placeholder="runs/data/X_eval.parquet" value={cfg.eval_x_file||""} onChange={v=>set({...cfg,eval_x_file:v})}/>
                <Inp label="Eval T-Datei" placeholder="runs/data/T_eval.parquet" value={cfg.eval_t_file||""} onChange={v=>set({...cfg,eval_t_file:v})}/>
              </Col><Col>
                <Inp label="Eval Y-Datei" placeholder="runs/data/Y_eval.parquet" value={cfg.eval_y_file||""} onChange={v=>set({...cfg,eval_y_file:v})}/>
                <Inp label="Eval S-Datei (optional)" placeholder="runs/data/S_eval.parquet" value={cfg.eval_s_file||""} onChange={v=>set({...cfg,eval_s_file:v})}/>
              </Col></Row>
            ) : (
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
      ...
      {view==="template"&&<Sec title="2. Studientyp">
        ...
        {(cfg.studyType||"rct") !== "rct" && (
          <div style={{marginTop:10}}><Info type="warn"><strong>Confounding-Korrektur essentiell:</strong> Es werden nur Modelle mit Doubly-Robust- oder Orthogonalisierungs-Ansatz empfohlen: <strong>NonParamDML, ParamDML, DRLearner, CausalForestDML</strong>. SLearner, TLearner und CausalForest sind als Baseline auswählbar, aber nicht gegen Confounding geschützt.</Info></div>
        )}
      </Sec>}

      {view==="template"&&<PModelsOnly cfg={cfg} set={set}/>}

      {view==="template"&&<Sec title="4. Add-ons">
        ...
        {(() => {
          const R1 = [
            ...
            {group:"imbalance",label:"Datenqualität",c:"#b45309",bg:"#fffbeb",bd:"#fcd34d",abg:"#fef3c7"},
...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Validierungsstrategie">
    ...
    <Inp label="Anzahl Folds (K)" type="number" value={cfg.cvSplits} onChange={v=>set({...cfg,cvSplits:v})} help="Zentrale Fold-Anzahl: Externe CV für Meta-Learner/CausalForest + internes DML-Cross-Fitting + Tuning. Ein Wert für alles."/>
    {cfg.validateOn==="external" && (
    ...
    <Divider/>
    <div style={{fontSize:12,fontWeight:600,textTransform:"uppercase",letterSpacing:".5px",color:"#999",marginBottom:8}}>DML Cross-Fitting</div>
    <Info>{(cfg.validateOn === "external" || cfg.eval_mask_file)
      ? `Evaluation auf ${cfg.validateOn === "external" ? "separatem Holdout-Datensatz" : "dem Mask-Subset (TMEO)"}. Jedes Modell wird EINMAL auf ${cfg.eval_mask_file && cfg.validateOn !== "external" ? "den ~mask-Rows" : "den Trainingsdaten"} gefittet und direkt auf dem Eval-Set evaluiert — kein äußeres CV. DML/DR-Modelle nutzen intern Cross-Fitting (cv=${cfg.cvSplits||5}) für die Nuisance-Residualisierung.`
      : `Alle Modelle durchlaufen externe ${cfg.cvSplits||5}-Fold Cross-Validation für echte Out-of-Fold CATE-Predictions. DML/DR-Modelle nutzen zusätzlich internes Cross-Fitting (cv=${cfg.cvSplits||5}) für die Nuisance-Residualisierung innerhalb jedes äußeren Folds.`
    }</Info>
    ...
    {(cfg.mcIters||0)>0 && <div style={{fontSize:11,color:"#7a5a00",background:"#fffbeb",padding:"8px 12px",borderRadius:8,marginTop:6,lineHeight:1.5}}>Cross-Fitting wird <strong>{cfg.mcIters}x</strong> wiederholt. Interne Fits pro DML/DR-Fold: ({cfg.cvSplits||5} Folds x 2 Nuisance + 1 Final) x {cfg.mcIters} mc = <strong>{(((cfg.cvSplits||5)*2+1)*(cfg.mcIters||1))}</strong> Base-Learner-Fits. {(cfg.validateOn === "external" || cfg.eval_mask_file) ? `Evaluation auf ${cfg.validateOn === "external" ? "separatem Holdout-Datensatz" : "Mask-Subset (TMEO)"} — kein äußeres CV, jedes Modell wird einmal auf ${cfg.eval_mask_file && cfg.validateOn !== "external" ? "~mask-Rows" : "Trainingsdaten"} gefittet.` : `Zusätzlich ${cfg.cvSplits||5} äußere CV-Folds für OOF-CATE-Predictions.`}</div>}
    {(cfg.mcIters||0)>0 && <Row>
  ...
  <Sec title="Ausgabe">
    <Info>Ergebnisse werden automatisch in <strong>MLflow</strong> geloggt (Metriken, Plots, Report). Hier kannst du optional Cross-Predictions als Datei exportieren.</Info>
    <Row>
      <Col>
...
const TuningPlanPreview = ({models, perRole, perLearner, trials, cv, isBoth, isRct}) => {
  ...
  const sigLabel = sig => {
    const fam = sig.split("__")[0];
    if(fam==="outcome_regression") return "Outcome (Regressor)";
    if(fam==="outcome") return "Outcome (Classifier)";
    if(fam==="propensity") return "Propensity";
          ...
          return (
            <div key={t.key} style={{display:"grid",gridTemplateColumns:"36px 1fr 1fr 1fr 70px",padding:"7px 14px",borderBottom:"1px solid #f5f0f0",background:i%2===0?"#fff":"#fdfbfb",alignItems:"center",fontSize:12}}>
              ...
              <div>
                ...
                {isPropCapped && <span style={{fontSize:9,color:"#6366f1",marginLeft:6,fontWeight:600,background:"#f6f7ff",padding:"1px 6px",borderRadius:8,border:"1px solid #c7d2fe"}}>RCT: {trialsForTask(t.sig)}T</span>}
              </div>
...
const FinalTuningPlanPreview = ({models, fmtEnabled, fmtModels, fmtSingleFold, fmtTrials, fmtCv, mcIters, isBoth}) => {
  const bothMultiplier = isBoth ? 2 : 1;
  ...
  eligible.forEach(m => {
    ...
    if(mn === "nonparamdml") {
      const total = nTrials;
      rows.push({model:m, method:"RScorer", trials:nTrials, fitsPerTrial:1, totalFits:total,
        detail:nTrials+"T × 1",
        note:"RScorer wird einmal vorab gefittet (berechnet Residuen mit CV). Danach 1 Kandidaten-Fit pro Trial, Bewertung über vorberechnete Residuen."});
    }
    if(mn === "drlearner") {
      const drFolds = fmtSingleFold ? 1 : nCv;
      const fitsPerTrial = drFolds;
      const total = nTrials * fitsPerTrial;
      rows.push({model:m, method:fmtSingleFold?"score() 1-Fold":"score() + CV", trials:nTrials, fitsPerTrial, totalFits:total,
        detail:nTrials+"T × "+drFolds+"F",
        note:"Pro Trial: "+drFolds+" Fold(s), je 1 DRLearner-Fit (score). Kein RScorer verfügbar (andere Nuisance-Struktur)."});
    }
  });
  ...
  return (
    <div>
      ...
      <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
        Jeder Trial fittet ein vollstaendiges kausales Modell mit internem Cross-Fitting (cv={dc}{mc>1?", mc_iters="+mc:""}). T = Trials, F = aeussere Folds. Die internen Fits entsprechen den Trainings-Einstellungen, damit die gefundenen Hyperparameter optimal passen.
...
const PModelsOnly = ({cfg,set}) => {
  ...
  return (<Sec title="3. Kausale Modelle">{mt&&<Info type="warn"><strong>Multi-Treatment:</strong> Nur DML-Modelle verfügbar.</Info>}{cfg.hasNaN&&<Info type="warn"><strong>Fehlende Werte:</strong> CausalForestDML und CausalForest werden automatisch übersprungen (GRF-basierte Modelle können keine fehlenden Werte verarbeiten).</Info>}
    {[
      ...
      {label:"DML / Doubly Robust",color:"#D4A853",bg:"#fffbeb",models:[
        {m:"NonParamDML",d:"Flexibelstes DML – nicht-lineares CATE-Modell"},
        {m:"ParamDML",d:"Lineares CATE-Modell (LinearDML)"},
        {m:"DRLearner",d:"Doubly Robust – stabil bei Modell-Fehlspezifikation"},
      ]},
...
<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
        ...
        </div>{_isBoth && <div style={{background:"#fff8e7",border:"1px solid #e8d49c",borderRadius:10,padding:"12px 16px",marginTop:12,fontSize:12,color:C.textSec,lineHeight:1.55}}><strong style={{color:"#7a5a00"}}>So funktioniert &quot;CatBoost &amp; LGBM&quot;:</strong> Dieser Modus entscheidet sich erst <strong>während des Tunings</strong> pro Task (model_y, model_t, model_final, ...) zwischen LightGBM und CatBoost. Das Tuning lernt, welcher Learner für die jeweilige Rolle besser abschneidet — am Ende kann model_y z.B. CatBoost nutzen und model_t LightGBM. Die Trial-Zahl wird dabei automatisch verdoppelt, damit beide Learner-Familien gleich viel Budget bekommen. <strong>Hinweis:</strong> Der Modus benötigt aktives Tuning. Ohne Tuning gibt es keine Entscheidungsgrundlage — dann wird CatBoost (globaler Default) verwendet.</div>}{!cfg.tuningEnabled && <><Divider/><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12,flexWrap:"wrap"}}><span style={{fontSize:13,fontWeight:600,color:C.dark}}>Nuisance-Hyperparameter</span><span style={{fontSize:10,background:"#fffbeb",color:"#7a5a00",padding:"3px 12px",borderRadius:12,border:"1px solid #e8d49c",fontWeight:600,letterSpacing:0.2}}>Aktiv – Tuning ist deaktiviert</span></div><Info>Tuning ist deaktiviert. Diese Parameter werden direkt für alle Nuisance-Modelle (Outcome, Propensity) verwendet.</Info>{_isBoth ? (<>
              ...
              return (<>
                ...
                <div style={{border:"1px solid #ede6e7",borderRadius:8,overflow:"hidden",marginTop:12}}>
                  ...
                  {hasCF && <div style={{borderBottom:"1px solid #f0f0f0"}}>
                    ...
                    <div style={{fontSize:10.5,color:"#999",padding:"0 14px 6px",lineHeight:1.4}}>Nuisance: cv={dc} x 2 Modelle{mc>1?" x "+mc+" mc_iters":""} = {cfNuisance}. Dann {grk} Forest-Kombis{cfg.cfTuneIntensive?" (intensiv)":""} auf den Residuals.</div>
                  </div>}
...
const buildYaml = (cfg, sp, spFmt) => {
  ...
  if(cfg.classBalance) a("class_balance: true");
  a("");
  a("mlflow:");
  ...
  a(`  dml_crossfit_folds: ${cfg.cvSplits||5}`);
  ...
  if(cfg.tuningEnabled) {
    ...
    a(`  metric: ${cfg.tuningMetric||"pr_auc"}`);if((cfg.tuningMetricReg||"neg_mse")!=="neg_mse")a(`  metric_regression: ${cfg.tuningMetricReg}`);
...
function App() {
  ...
  const INIT_CFG = {studyType:"rct",classBalance:false,expName:"rubin",seed:42,parallelLevel:3,workDir:null,x_file:"",t_file:"",y_file:"",s_file:"",treatmentType:"binary",refGroup:0,hasNaN:false,nanCols:[],validateOn:"cross",cvSplits:5,downsample:false,dfFrac:0.1,reduceMem:true,fsEnabled:false,fsMethods:["lgbm_importance"],fsTopPct:15,fsCorrThresh:0.9,outputDir:"",savePreds:false,predsFormat:"csv",models:["NonParamDML","DRLearner","SLearner","TLearner","XLearner","ParamDML","CausalForestDML","CausalForest"],baseLearner:"catboost",tuningEnabled:true,tuningTrials:100,tuningSingleFold:false,tuningMetric:"pr_auc",tuningMetricReg:"neg_mse",tuningPerRole:false,tuningPerLearner:false,tuningAutoml:"optuna",fmtEnabled:false,fmtModels:[],fmtSingleFold:false,fmtTrials:50,fmtMaxRows:0,cfTune:false,cfTuneIntensive:false,selMetric:"qini",higherBetter:true,refitChamp:true,manualChamp:null,surrEnabled:false,surrMinLeaf:50,surrLeaves:31,surrDepth:0,bundleEnabled:false,bundleDir:"runs/bundles",bundleChallengers:true,bundleMlflow:true,explEnabled:true,explMethod:"shap",explSampleSize:10000,explTopN:20,shapModels:[],shapBins:10,histScoreName:"historical_score",histScoreCol:"S",histScoreHigher:true,fsMaxFeatures:0,maxPredRows:0,tuningTimeout:0,tuningMaxRows:0,fmtTimeout:0,blFixed:{},fmtFixed:{},cfFixed:{},ensembleEnabled:false,mcIters:null,mcAgg:"mean",fmtStabilityPenalty:0};
  ...
  const estimateFits = () => {
    ...
    const cv = cfg.cvSplits || 5, dc = cfg.cvSplits || 5, mc = cfg.mcIters || 1;
    const K = 2; // Binary Treatment
    ...
    const fitsPerFit = (x) => {
      if (["NonParamDML","ParamDML","CausalForestDML","DRLearner"].includes(x)) return mc*dc*2+1;
    ...
    // 2. BL-Tuning (1 Base-Learner-Fit pro Trial pro Fold)
    // tuningModels einschränkt, WELCHE Modelle getuned werden
    // "CatBoost & LGBM"-Modus verdoppelt die Trials intern (Optuna wählt den Learner kategorisch)
    const bothMult = (cfg.baseLearner||"catboost") === "both" ? 2 : 1;
    if (cfg.tuningEnabled) {
      const bltM = (cfg.tuningModels||[]).length > 0 ? cfg.tuningModels : m;
      const tr = (cfg.tuningTrials||100) * bothMult, tc = cfg.tuningSingleFold ? 1 : cv;
      const dml = bltM.some(x=>["NonParamDML","ParamDML","CausalForestDML"].includes(x));
      const dr = bltM.includes("DRLearner"), sl = bltM.includes("SLearner");
      const tl = bltM.includes("TLearner"), xl = bltM.includes("XLearner");
      if (dml) f += tr * tc;                            // outcome (cls): DML model_y
      if (sl||dr) f += tr * tc;                         // outcome_regression (reg): SL, DR
      if (tl||xl) f += tr * tc * K;                     // grouped_outcome_reg: K fits/fold
      if (dml||dr||xl) f += tr * tc;                    // propensity (cls)
      if (xl) f += tr * ((tc * K) + (cv * 2 + 2));     // pseudo_effect: nuisance + objective
    }

    // 3. FMT (Final-Model-Tuning)
    // "CatBoost & LGBM" verdoppelt auch die FM-Tuning-Trials
    if (cfg.fmtEnabled) {
      const fmtInternal = mc*dc*2+1;
      const fmtTr = (cfg.fmtTrials||50) * bothMult;
      (cfg.fmtModels||[]).forEach(x => {
        if (x==="NonParamDML") {
          // RScorer: 1 Vorab-Fit (Residuen-Berechnung mit CV) + 1 Kandidat pro Trial
          f += fmtInternal + fmtTr;
        }
        if (x==="DRLearner") {
          // score() + CV: jeder Fold pro Trial = voller Modell-Fit
          const fmtFolds = cfg.fmtSingleFold ? 1 : cv;
          f += fmtTr * fmtFolds * fmtInternal;
        }
      });
    }
    ...
    if (cfg.cfTune) {
      ...
      if (m.includes("CausalForestDML")) f += dc*2*mc + _grk;
      if (m.includes("CausalForest")) f += _grk;
    }

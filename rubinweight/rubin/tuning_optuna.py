...
def build_base_learner(base_type: str, params: Dict[str, Any], seed: int, task: str = "classifier", parallel_jobs: int = -1, class_balance: bool = False):
...
class_balance: Bei True werden Classifier automatisch mit Class-Weighting trainiert
  (LightGBM: is_unbalance=True, CatBoost: auto_class_weights='Balanced').
  Sinnvoll bei stark imbalancierter Zielvariable (z.B. 1:1000 Conversion-Rate).
...
CatBoost mit Warnung (passend zum globalen Default base_learner.type="catboost")."""
    ...
    # Class-Balancing: bei stark imbalancierter Zielvariable automatisch
    # Klassen-Gewichtung aktivieren. Nur für Classifier — Regressor-Balancing
    # erfolgt über sample_weight in _fit_model.
    if class_balance and task == "classifier":
        if base_type == "lgbm":
    ...
    if base_type == "lgbm":
        model = _build_lgbm_classifier(params, seed, parallel_jobs) if task == "classifier" else _build_lgbm_regressor(params, seed, parallel_jobs)
    elif base_type == "catboost":
        model = _build_catboost_classifier(params, seed, parallel_jobs) if task == "classifier" else _build_catboost_regressor(params, seed, parallel_jobs)
    else:
        raise ValueError(f"Unbekannter base_learner.type={base_type!r}. Erwartet: 'lgbm', 'catboost' oder 'both'.")

    # Classifier-Recalibration: Bei class_balance=True sind die predict_proba-Werte
    # durch Klassen-Gewichtung infliert (z.B. P≈0.08 statt P≈0.001 bei 1:1000).
    # CalibratedClassifierCV lernt eine monotone Mapping-Funktion (Isotonic Regression)
    # per internem 3-Fold-CV, die die wahren Wahrscheinlichkeiten wiederherstellt.
    # Das ist essentiell für DML-Residualisierung: Y − E[Y|X] braucht kalibrierte E[Y|X].
    if class_balance and task == "classifier":
...
class BaseLearnerTuner:
    ...
    def _score_classifier(self, y_true: np.ndarray, proba: np.ndarray, multiclass: bool = False) -> float:
        ...
        - pr_auc (Default): Average Precision (PR-AUC). Robust bei Class Imbalance —
          misst Ranking-Qualität statt Kalibrierung. Empfohlen für Nuisance-Modelle
          bei unbalancierten Outcomes (Conversion, Churn). Bei Multiclass: OvR-Mittel.
        - log_loss: Negierter Log-Loss (höher = besser, da direction=maximize).
          Misst Kalibrierung — bei starker Imbalance tendiert Log-Loss dazu,
          Modelle nah an der Basisrate zu bevorzugen (flache Residuen).
        - roc_auc: ROC-AUC, bei Multiclass roc_auc_ovr (weighted). Bei stark
          unbalancierten Daten weniger aussagekräftig als PR-AUC.
        - accuracy: Accuracy auf Basis der Klasse mit höchster Wahrscheinlichkeit
        """
        metric = getattr(self.cfg.tuning, "metric", "pr_auc")
    ...
    def _fit_model(self, params: Dict[str, Any], X_train: np.ndarray, y_train: np.ndarray, estimator_task: str):
        ...
        _cb = getattr(self.cfg, "class_balance", False)
        model = build_base_learner(self.cfg.base_learner.type, params, seed=self.seed, task=estimator_task, parallel_jobs=pj, class_balance=_cb)
        # Regression mit binärem Y + class_balance: Sample-Weighting
        if _cb and estimator_task == "regressor":
            unique_y = np.unique(y_train)
            if len(unique_y) <= 2 and set(unique_y).issubset({0, 1, 0.0, 1.0}):
                n_pos = (y_train == 1).sum()
                n_neg = (y_train == 0).sum()
                if n_pos > 0 and n_neg > 0:
                    w = np.where(y_train == 1, n_neg / n_pos, 1.0)
                    w = w / w.mean()  # Normalisierung auf mean=1
    ...
    def _should_recalibrate(self, target: np.ndarray) -> bool:
        """Prüft ob Recalibration nötig ist: class_balance aktiv + binäres Y."""
        if not getattr(self.cfg, "class_balance", False):
            return False
        unique = np.unique(target)
        return len(unique) <= 2 and set(unique.astype(int)).issubset({0, 1})

    def _recalibrate_predictions(self, pred: np.ndarray, y_val: np.ndarray) -> np.ndarray:
        """Isotonic Regression: stellt kalibrierte Wahrscheinlichkeiten nach Balancing wieder her.

        Bei class_balance=True sind die Rohvorhersagen des Regressors durch
        Sample-Weighting infliert (Optimum verschoben von P(Y=1|X) auf einen
        höheren Wert). Isotonic Regression lernt eine monotone Mapping-Funktion
        auf dem Val-Fold, die die wahre Prävalenz wiederherstellt.
        """
    ...
    def _objective_all_regression(self, params: Dict[str, Any], X_mat: np.ndarray, target: np.ndarray, trial=None) -> float:
        ...
        scores: List[float] = []
        _recal = self._should_recalibrate(target)
        for fold_i, (tr, va) in enumerate(self._cv_splits(target.astype(float), single_fold=self.cfg.tuning.single_fold)):
            ...
            if _recal:
                pred = self._recalibrate_predictions(pred, target[va])
    ...
    def _objective_grouped_regression(self, params: Dict[str, Any], X_mat: np.ndarray, Y: np.ndarray, T: np.ndarray, trial=None) -> float:
        ...
        scores: List[float] = []
        K = len(np.unique(T))
        _recal = self._should_recalibrate(Y)
        ...
        for fold_i, (tr, va) in enumerate(self._cv_splits(strat_labels, single_fold=self.cfg.tuning.single_fold)):
            ...
            for group in range(K):
                ...
                if _recal:
                    pred = self._recalibrate_predictions(pred, Y[va][va_mask])
    ...
    def _tune_task(self, task: TuningTask, X: pd.DataFrame, Y: np.ndarray, T: np.ndarray, shared_params: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        _tlog.info(
            "BLT '%s': %d/%d Trials abgeschlossen (parallel=%d, Wellen≈%d).",
            task.key, n_complete, len(study.trials), _pj, max(1, n_complete // max(1, _pj)),
        )
        if n_fail > 0:
            _tlog.warning("Tuning '%s': %d/%d Trials erfolgreich, %d fehlgeschlagen.",
        ...
        if _is_rct and task.objective_family == "propensity":
            try:
                ...
                current_metric = getattr(self.cfg.tuning, "metric", "pr_auc") or "pr_auc"
    ...
    def _tune_task_flaml(self, task: TuningTask, X: pd.DataFrame, Y: np.ndarray, T: np.ndarray,
        ...
        # Metric-Mapping
        if is_classifier:
            metric = self.cfg.tuning.metric or "pr_auc"
...
persistiert und in weiteren Folds wiederverwendet."""
    ...
    def tune_final_model(
        ...
        if name == "nonparamdml":
            ...
            _fmt_ok = len([t for t in study.trials if t.state == self.optuna.trial.TrialState.COMPLETE])
            _fmt_fail = len(study.trials) - _fmt_ok
            _tlog.info("FMT '%s': %d/%d Trials abgeschlossen (parallel=%d).", model_name, _fmt_ok, len(study.trials), _fmt_pj)
            if _fmt_fail > 0:
                _tlog.warning("FMT '%s': %d Trials fehlgeschlagen.", model_name, _fmt_fail)
            try:

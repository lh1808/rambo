...
class AnalysisPipeline:
    ...
    def _run_training(self, cfg, X, T, Y, tuned_params_by_model, holdout_data, mlflow, _progress_cb=None):
        ...
        for name in _models_order:
            ...
            # für Nuisance-Residualisierung. Wird auf cross_validation_splits gesetzt.
            _dml_cv = cfg.data_processing.cross_validation_splits
            ...
            if name.lower() == "causalforestdml" and getattr(cfg.causal_forest, "use_econml_tune", False):
                try:
                    # Gleicher T×Y-stratifizierter Split wie Cross-Predictions (erster Fold = Train)
                    ...
                    n_sp = int(cfg.data_processing.cross_validation_splits)
                    strata_counts = pd.Series(strata).value_counts(dropna=False)
                    eff_sp = min(n_sp, int(strata_counts.min())) if not strata_counts.empty else n_sp
                    if eff_sp >= 2:
                        cv = StratifiedKFold(n_splits=eff_sp, shuffle=True, random_state=cfg.constants.random_seed)
                        split_iter = cv.split(np.zeros(len(X)), strata)
                    else:
                        cv = KFold(n_splits=max(2, min(n_sp, len(X))), shuffle=True, random_state=cfg.constants.random_seed)
                    ...
                    self._logger.info(
                        "CausalForestDML EconML tune(): %d Grid-Kombis%s, train=%d Beobachtungen.",
                        _cfdml_n, " (intensiv)" if _intensive else "", len(X_tune),
                    )
                except Exception:
            ...
            # CausalForest: Eigener Grid-Search über Wald-Parameter (R-Loss auf erstem Fold)
            # Verwendet denselben T×Y-stratifizierten Split wie die Cross-Predictions.
            if name == "CausalForest" and getattr(cfg.causal_forest, "use_econml_tune", False):
                try:
                    ...
                    if best_loss is not None:
                        mlflow.log_metric("causal_forest_pure__tune_best_r_loss", best_loss)
                    self._logger.info(
                        "CausalForest Tune: %d Kombis%s auf Fold 1 (train=%d, val=%d) → best=%s, R-Loss=%.6g",
                        result.get("n_combos", 0), " (intensiv)" if _intensive else "",
                        len(X_tr), len(X_va), best_p, best_loss or 0,
                    )
                except Exception:
    ...
    def _run_evaluation(self, cfg, X, T, Y, S, holdout_data, preds, models, tuned_params_by_model, mlflow, eval_mask=None):
        ...
        try:
            ...
            model_reg = build_base_learner(cfg.base_learner.type, dr_params_y, seed=cfg.constants.random_seed, task="classifier", parallel_jobs=pj, class_balance=cfg.class_balance)
            model_prop = build_base_learner(cfg.base_learner.type, dr_params_t, seed=cfg.constants.random_seed, task="classifier", parallel_jobs=pj, class_balance=cfg.class_balance)
            ...
            if not is_mt:
                ...
                def _fit_arm(arm):
                    ...
                    tester = fit_drtester_nuisance(
                        model_regression=build_base_learner(cfg.base_learner.type, dr_params_y, seed=cfg.constants.random_seed, task="classifier", parallel_jobs=pj, class_balance=cfg.class_balance),
                        model_propensity=build_base_learner(cfg.base_learner.type, dr_params_t, seed=cfg.constants.random_seed, task="classifier", parallel_jobs=pj, class_balance=cfg.class_balance),
    ...
    def _evaluate_historical_score(self, cfg, X, T, Y, holdout_data, preds, hist_score_eval, eval_summary, policy_values_dict, mlflow, fitted_tester=None, eval_mask=None):
        ...
        try:
            ...
            if ft_hist is not None:
                ...
                pj = 1 if cfg.constants.parallel_level <= 1 else -1
                model_reg = build_base_learner(cfg.base_learner.type, params_y, seed=cfg.constants.random_seed, task="classifier", parallel_jobs=pj, class_balance=cfg.class_balance)
                model_prop = build_base_learner(cfg.base_learner.type, params_t, seed=cfg.constants.random_seed, task="classifier", parallel_jobs=pj, class_balance=cfg.class_balance)
                bundle_h = evaluate_cate_with_plots(
    ...
    def _build_surrogate_regressor(self, cfg):
        ...
        return build_base_learner(base_type, params, seed=seed, task="regressor", parallel_jobs=1 if cfg.constants.parallel_level <= 1 else -1, class_balance=cfg.class_balance)
            ...
            try:
                ...
                if pred_col_s in surrogate_df.columns:
                    ...
                    if ft_surr is not None:
                        ...
                        pj = 1 if cfg.constants.parallel_level <= 1 else -1
                        model_reg = build_base_learner(cfg.base_learner.type, {}, seed=cfg.constants.random_seed, task="classifier", parallel_jobs=pj, class_balance=cfg.class_balance)
                        model_prop = build_base_learner(cfg.base_learner.type, {}, seed=cfg.constants.random_seed, task="classifier", parallel_jobs=pj, class_balance=cfg.class_balance)
                        bundle_s = evaluate_cate_with_plots(
                            model_regression=model_reg, model_propensity=model_prop,
                            X_val=eval_X, T_val=t_s, Y_val=y_s,
                            cate_preds_val=cate_vals,
            ...
            with patch_categorical_features(X, base_learner_type=cfg.base_learner.type) as cat_indices:
                ...
                if eval_summary:
                    ...
                    if _champ:
                        ...
                        mlflow.set_tag("rubin.base_learner", cfg.base_learner.type or "catboost")

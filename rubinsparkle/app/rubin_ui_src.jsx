...
<FixedParamsEditor params={(cfg.fmtFixed||{}).lgbm||{}} defaults={LGBM_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:{...(cfg.fmtFixed||{}),lgbm:v}})} label="LightGBM CATE-Parameter (Fixed)" accent="#b8860b"/>
              ...
              const cfTotal = nTrials * evalFolds;
              return (<>
                <div style={{display:"flex",gap:10,marginBottom:12,flexWrap:"wrap"}}>
                ...
                <div style={{border:"1px solid #ede6e7",borderRadius:8,overflow:"hidden"}}>
                  ...
                  {hasGRF && <div style={{background:hasCF?"#fdfbfb":"#fff"}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 80px",padding:"8px 14px",fontSize:12}}>
                      ...
                      <span style={{fontSize:10.5,color:"#2d6a4f",fontFamily:MONO}}>{nTrials}T × {evalFolds}F × 1</span>
                      <span style={{fontFamily:MONO,fontWeight:700,color:"#2d6a4f"}}>{cfTotal.toLocaleString()}</span>
                    </div>
                    <div style={{fontSize:10.5,color:"#999",padding:"0 14px 6px",lineHeight:1.4}}>Kein internes Cross-Fitting — 1 Forest-Fit pro Fold. 9 Wald-Params (+ criterion).</div>
                  </div>}
                </div>
              </>);
...
function App() {
  ...
  const estimateFits = () => {
    ...
    // 1. Feature-Selektion
    if (cfg.fsEnabled) (cfg.fsMethods||[]).forEach(x => { if (x==="lgbm_importance") f++; if (x==="causal_forest") f++; });
    ...
    if (cfg.cfTune) {
      ...
      // CausalForest: jeder Trial = cfFolds × 1 Forest-Fit
      if ((cfg.cfTuneModels||[]).includes("CausalForest") && m.includes("CausalForest")) f += cfTrials * cfFolds;
    }

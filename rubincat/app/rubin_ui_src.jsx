...
const ADDON_PRESETS = [
  ...
  {key:"mc_iters",label:"MC-Iterationen",desc:"Cross-Fitting 3x wiederholen – stabilere Residuals",group:"stability",
    cfg:{mcIters:3}},
  {key:"exploration",label:"Exploration",desc:"10% Sampling + 3 CV-Folds – schnelles Iterieren",group:"stability",
  ...
  return (
    <>
      ...
      {view==="template"&&<Sec title="2. Studientyp">
        <div style={{display:"flex",gap:0,borderRadius:8,overflow:"hidden",border:"1.5px solid "+C.border,width:"fit-content",marginBottom:12}}>
          {[["rct","RCT (randomisiert)"],["observational","Beobachtungsdaten"]].map(([k,l])=>{const sel=(cfg.studyType||"rct")===k;return(
            <button key={k} onClick={()=>{const upd={...cfg,studyType:k};if(k==="rct"){upd.models=["NonParamDML","DRLearner","SLearner","TLearner","XLearner","ParamDML","CausalForestDML","CausalForest"]}else{upd.models=["NonParamDML","ParamDML","DRLearner","CausalForestDML"]}set(upd)}} style={{padding:"9px 20px",fontSize:12.5,fontWeight:sel?700:400,background:sel?"linear-gradient(135deg,#6B0D15,#9B111E)":"#fff",color:sel?"#fff":C.text,border:"none",cursor:"pointer",transition:"all 0.15s"}}>{l}</button>
          )})}
        </div>
        {(cfg.studyType||"rct") !== "rct" ? (
          <Info>Bei Beobachtungsdaten ist Confounding-Korrektur essentiell. Es werden nur Modelle mit Doubly-Robust- oder Orthogonalisierungs-Ansatz empfohlen: <strong>NonParamDML, ParamDML, DRLearner, CausalForestDML</strong>. SLearner, TLearner und CausalForest sind als Baseline auswählbar, aber nicht gegen Confounding geschützt.</Info>
        ) : (
          <Info>Treatment-Zuweisung ist zufällig — alle Modelle und Evaluationen sind direkt valide. SLearner und TLearner sind einfach und effizient. DML/DR-Modelle funktionieren ebenfalls, die Propensity-Schätzung ist aber weniger kritisch.</Info>
        )}
      </Sec>}

      {view==="template"&&<PModelsOnly cfg={cfg} set={set}/>}

      {view==="template"&&<Sec title="3. Add-ons">
...
const buildYaml = (cfg, sp, spFmt) => {
  ...
  if(bl === "both") {
    const lgbmF2 = fmtF.lgbm||{}, cbF2 = fmtF.catboost||{};
    if(Object.keys(lgbmF2).length > 0 || Object.keys(cbF2).length > 0) {
      a("  fixed_params:");

...
- Als Metrik wird für **Classifier-Tasks** (Nuisance: `model_y`, `model_t`, Propensity) standardmäßig `pr_auc` (Average Precision) verwendet. PR-AUC misst Ranking-Qualität statt Kalibrierung und ist bei Class Imbalance (typisch für Conversion/Churn) deutlich aussagekräftiger als Log-Loss. Alternativ: `log_loss` (misst Kalibrierung), `roc_auc`, `accuracy`. Für **Regressor-Tasks** (Meta-Learner Outcome: `overall_model`, `models`, `model_regression`) wird automatisch **neg. MSE** als Metrik genutzt.
...
### Warum PR-AUC als Default?

Bei unbalancierten Binary-Outcomes (z.B. 5% Positive Rate) tendiert Log-Loss-Tuning dazu, Modelle nah an der Basisrate zu bevorzugen — die Vorhersagen werden "flach" (wenig Varianz). Im DML-Framework führt das zu entsprechend flachen Residuen `Y - E[Y|X]`, aus denen die CATE-Schätzung kaum Struktur extrahieren kann. PR-AUC zwingt das Modell dagegen dazu, Positives über Negatives zu ranken — die resultierenden Vorhersagen haben echte Spread, und die Residuen kodieren tatsächlich, wo das Outcome anders war als erwartet. Das gibt dem CATE-Modell eine informative Grundlage.

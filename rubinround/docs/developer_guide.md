...
CausalForestDML und CausalForest werden via Optuna TPE über 8–9 Wald-Parameter getunt (n_estimators, max_depth, min_samples_leaf, min_samples_split, max_features, max_samples, min_var_fraction_leaf, min_impurity_decrease, criterion). CausalForestDML: Evaluation via `est.score()` (R-Loss). CausalForest: Nuisance-Residuen vorberechnet, R-Loss manuell. Trials laufen sequentiell (n_jobs=1), da jeder Forest intern alle Kerne nutzt. Siehe `docs/tuning_optuna.md → CausalForest-Tuning (Optuna)` für Details.

...
class CausalForestAdapter(BaseCateEstimator):
    ...
    def tune(self, X, T, Y, intensive=False, rscorer=None, model_y=None, model_t=None, nuisance_cv=5):
        ...
        gegenüber einem konstanten Effekt erklärt. Identisch mit der R-Loss-Metrik,
        die im CFT (CausalForest-Tuning via Optuna) verwendet wird.
...
erzeugt."""
    ...
    # (min_samples_leaf=5, max_depth=None), leicht konservativer für Stabilität.
    # CFT (CausalForest-Tuning via Optuna) kann diese Parameter weiter optimieren.
    _CFDML_DEFAULTS = {
        "n_estimators": 200,
        "max_depth": None,            # EconML Default: unbegrenzt — CFT kann optimieren
        "min_samples_leaf": 10,       # EconML Default: 5 — leicht konservativer
        "min_samples_split": 20,      # EconML Default: 10 — leicht konservativer
        "max_features": "auto",       # EconML Default: 'auto' = n_features (alle Features pro Split)
        "max_samples": 0.45,
        "min_var_fraction_leaf": None, # EconML Default: None — keine Extra-Restriktion
        "min_impurity_decrease": 0.0,
    }
    def _sanitize_forest_params(defaults: dict, tuned: dict) -> dict:
        """Merged Forest-Params mit Validierung: n_estimators muss durch subforest_size=4 teilbar sein."""
    ...
    # Defaults orientieren sich an EconML (min_samples_leaf=5, max_depth=None),
    # leicht konservativer für Stabilität. CFT kann diese weiter optimieren.
    _CF_DEFAULTS = {
        "n_estimators": 200,
        "criterion": "mse",
        ...
        "max_features": "auto",        # EconML Default: 'auto' = n_features (alle Features pro Split)
        "max_samples": 0.45,
        "min_var_fraction_leaf": None,  # EconML Default: None
        "min_impurity_decrease": 0.0,
    }
    reg.register(
        "CausalForest",
        lambda ctx: CausalForestAdapter(
            ...
            **{k: v for k, v in _sanitize_forest_params(_CF_DEFAULTS, ctx.params_for("grf")).items()

...
def generate_html_report(collector: ReportCollector, output_path: str) -> str:
    ...
    _is_tmeo = bool(cs.get("eval_mask_file")) and cs.get("validate_on") != "external"
    is_external = cs.get("validate_on") == "external" or _is_tmeo  # beide nutzen Holdout-Pfad
    ...
    if is_external and eds.get("n_rows"):
        _eval_label = "Eval (TMEO)" if _is_tmeo else "Eval"
    ...
    if cs.get("final_tuning_enabled"):
        ...
        h = '<p class="expl">Optimiert das CATE-Effektmodell (model_final) via Optuna. Beide Modelle (NonParamDML, DRLearner) werden mit äußerer OOF-CV bewertet: model_final wird auf Train gefittet und auf Out-of-Fold-Val per est.score() evaluiert. Dies verhindert optimistische R-Score-Schätzungen.</p>'

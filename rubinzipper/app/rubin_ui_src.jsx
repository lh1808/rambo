  ...
  <div style={{maxHeight:360,overflowY:"auto",border:"1px solid #ede6e7",borderRadius:8}}>
    ...
    {columns.map((col,i) => (
      <div key={col} style={{display:"grid",gridTemplateColumns:"180px 1fr",padding:"6px 14px",borderBottom:"1px solid #f5f0f0",background:i%2===0?"#fff":"#fdfbfb",alignItems:"center"}}>
        <span style={{fontSize:13,fontWeight:500,fontFamily:MONO,color:"#333"}}>{col}</span>
        ...
        <div style={{display:"flex",gap:10,marginTop:4}}>
          {REG_PRESETS.map(r => {
            ...
            return (<div key={r.key} onClick={()=>toggleReg(r)} style={{flex:1,padding:"12px 16px",borderRadius:10,border:active?`2px solid ${r.c}`:`1.5px solid #e5e7eb`,background:active?"#fff":"#fafafa",cursor:"pointer",transition:"all 0.15s",boxShadow:active?`0 0 0 3px ${r.c}22`:"none"}}>
              ...
              {active && <div style={{fontSize:10,color:r.c,marginTop:4,fontFamily:MONO}}>BLT p={r.blt_p}/t={r.blt_t} | FMT p={r.fmt_p}/t={r.fmt_t}</div>}
            </div>);
          })}
        </div>

...
const YAML_TO_CFG = {
  ...
  "tuning.enabled":"tuningEnabled","tuning.n_trials":"tuningTrials","tuning.cv_splits":"bltCvSplits","tuning.single_fold":"tuningSingleFold","tuning.metric":"tuningMetric","tuning.metric_regression":"tuningMetricReg",
  ...
  "final_model_tuning.enabled":"fmtEnabled","final_model_tuning.models":"fmtModels","final_model_tuning.cv_splits":"fmtCvSplits","final_model_tuning.single_fold":"fmtSingleFold","final_model_tuning.stability_penalty":"fmtStabilityPenalty","final_model_tuning.n_trials":"fmtTrials","final_model_tuning.method":"fmtMethod",
...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Validierungsstrategie">
    ...
    <Inp label="Anzahl Folds (K)" type="number" value={cfg.cvSplits} onChange={v=>set({...cfg,cvSplits:v})} help="Äußere Cross-Validation für Out-of-Fold CATE-Predictions. Interne CVs (BLT, FMT, DML-Cross-Fitting) laufen unabhängig davon mit EconML-Default (2 Folds)."/>
    ...
    <Info>{(cfg.validateOn === "external" || cfg.eval_mask_file)
      ? `Evaluation auf ${cfg.validateOn === "external" ? "separatem Holdout-Datensatz" : "dem Mask-Subset (TMEO)"}. Jedes Modell wird EINMAL auf ${cfg.eval_mask_file && cfg.validateOn !== "external" ? "den ~mask-Rows" : "den Trainingsdaten"} gefittet und direkt auf dem Eval-Set evaluiert — kein äußeres CV. DML/DR-Modelle nutzen intern Cross-Fitting (cv=${cfg.dmlCrossfitFolds||2}) für die Nuisance-Residualisierung.`
      : `Alle Modelle durchlaufen externe ${cfg.cvSplits||5}-Fold Cross-Validation für echte Out-of-Fold CATE-Predictions. DML/DR-Modelle nutzen zusätzlich internes Cross-Fitting (cv=${cfg.dmlCrossfitFolds||2}) für die Nuisance-Residualisierung innerhalb jedes äußeren Folds.`
    }</Info>
    <Row><Col><Inp label="Internes Cross-Fitting (cv)" type="number" value={cfg.dmlCrossfitFolds||2} onChange={v=>set({...cfg,dmlCrossfitFolds:Number(v)})} help="Interne Folds für DML/DR Nuisance-Residualisierung in Produktion (EconML-Default=2). Höhere Werte stabilisieren Residuals, reduzieren aber die Trainingsmenge pro Nuisance-Modell."/></Col></Row>
...
const FinalTuningPlanPreview = ({models, fmtEnabled, fmtModels, fmtSingleFold, fmtTrials, fmtCv, mcIters, isBoth}) => {
  ...
  const internalCv = fmtCv || 2;  // FMT internes Nuisance-CV (EconML-Default=2)
  const outerCv = 5;              // Äußere Score-Folds für DRLearner (= cross_validation_splits)
  const mc = mcIters || 1;
  ...
  eligible.forEach(m => {
    ...
    if(mn === "nonparamdml") {
      ...
      rows.push({model:m, method:"RScorer", trials:nTrials, fitsPerTrial:perTrial, totalFits:total,
        ...
        note:`RScorer-Setup: ${internalCv}×2 = ${setupFits} Nuisance-Fits (einmalig). Pro Trial: NonParamDML.fit() mit cv=${internalCv} = ${perTrial} Base-Learner-Fits (${internalCv}×model_y + ${internalCv}×model_t + 1×model_final).`});
    }
    if(mn === "drlearner") {
  ...
  return (
    <div>
      ...
      <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
        Jeder Trial fittet ein vollstaendiges kausales Modell mit internem Cross-Fitting (cv={internalCv}{mc>1?", mc_iters="+mc:""}). T = Trials, F = aeussere Folds. Die internen Fits entsprechen den Trainings-Einstellungen, damit die gefundenen Hyperparameter optimal passen.
...
</>) : (<SSEditor bl={cfg.baseLearner||"catboost"} sp={spFmt||{lgbm:{},catboost:{}}} setSp={setSpFmt||setSp} fmt/>)}<Expander title="Erweiterte Final-Tuning-Einstellungen"><Row><Col><Inp label="Innere CV-Folds" type="number" value={cfg.fmtCvSplits||2} onChange={v=>set({...cfg,fmtCvSplits:Number(v)})} help="Inneres Nuisance-Cross-Fitting pro FMT-Trial (EconML-Default=2). DRLearner äußere Score-Folds nutzen die äußere CV-Anzahl (cross_validation_splits)."/></Col><Col><Inp label="Timeout (Sek., 0=unbegrenzt)" type="number" value={cfg.fmtTimeout||0} onChange={v=>set({...cfg,fmtTimeout:Number(v)})} help="Zeitlimit pro Final-Model-Optuna-Study"/></Col></Row>{_isBoth && (cfg.fmtTimeout||0)>0 && <Info type="warn">Timeout wird bei „CatBoost &amp; LGBM" automatisch ignoriert (faire Trial-Allokation).</Info>}</Expander><Divider/><div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:8}}>Tuning-Plan</div><FinalTuningPlanPreview models={cfg.models} fmtEnabled={cfg.fmtEnabled} fmtModels={cfg.fmtModels||[]} fmtSingleFold={cfg.fmtSingleFold||false} fmtTrials={cfg.fmtTrials||50} fmtCv={cfg.fmtCvSplits||2} mcIters={cfg.mcIters||1} isBoth={_isBoth}/></>}{!cfg.fmtEnabled && (cfg.models||[]).some(m=>["NonParamDML","DRLearner"].includes(m)) && <><Divider/><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12,flexWrap:"wrap"}}><span style={{fontSize:13,fontWeight:600,color:C.dark}}>Final-Modell-Hyperparameter</span><span style={{fontSize:10,background:"#fffbeb",color:"#7a5a00",padding:"3px 12px",borderRadius:12,border:"1px solid #e8d49c",fontWeight:600,letterSpacing:0.2}}>Direkt verwendet</span></div><Info>R-Score-Tuning ist deaktiviert. Diese Parameter werden direkt für das CATE-Effektmodell (model_final) von NonParamDML und DRLearner verwendet.</Info>{_isBoth ? (<>
...
<FixedParamsEditor params={(cfg.fmtFixed||{}).lgbm||{}} defaults={LGBM_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:{...(cfg.fmtFixed||{}),lgbm:v}})}/>
              ...
              const dc = cfg.dmlCrossfitFolds||2, mc = cfg.mcIters||1;
              const cfNuisance = dc*2*mc;  // Nuisance-Vorab: cv=2 × (model_y + model_t) × mc_iters
...
const buildYaml = (cfg, sp, spFmt) => {
  ...
  if((cfg.dmlCrossfitFolds||2) !== 2) a(`  dml_crossfit_folds: ${cfg.dmlCrossfitFolds}`);
  ...
  if(cfg.tuningEnabled) {
    ...
    if((cfg.bltCvSplits||2) !== 2) a(`  cv_splits: ${cfg.bltCvSplits}`);
  ...
  if(cfg.fmtEnabled) {
    ...
    if((cfg.fmtCvSplits||2) !== 2) a(`  cv_splits: ${cfg.fmtCvSplits}`);
...
function App() {
  ...
  const estimateFits = () => {
    ...
    const bltCv = cfg.bltCvSplits || 2;    // BLT innere CV
    const dmlCv = cfg.dmlCrossfitFolds || 2; // DML internes Cross-Fitting
    const fmtCv = cfg.fmtCvSplits || 2;    // FMT internes CV
    ...
    const fitsPerFit = (x) => {
      if (["NonParamDML","ParamDML","CausalForestDML","DRLearner"].includes(x)) return mc*dmlCv*2+1;
    ...
    if (cfg.tuningEnabled) {
      ...
      const tr = (cfg.tuningTrials||100) * bothMult, tc = cfg.tuningSingleFold ? 1 : bltCv;
    ...
    if (cfg.cfTune) {
      ...
      if (m.includes("CausalForestDML")) f += dmlCv*2*mc + _grk;

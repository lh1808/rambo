...
const RSlider = ({label,min,max,step,low,high,onLow,onHigh,type="float",accent}) => {
  ...
  const uid = label.replace(/[^a-z0-9]/gi,"_");
...
const ADDON_PRESETS = [
  ...
  // ── Feature-Selektion ──
  {key:"feature_reduction",label:"Feature-Reduktion",desc:"Importance-basierte Feature-Selektion (LightGBM + GRF)",group:"feature",
  ...
  <Sec title="Feature-Selektion" accent="#0d9488">
    ...
    {cfg.fsEnabled&&<>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        {[
          {m:"lgbm_importance",label:"LightGBM Gain",desc:"Misst die Gain-Importance pro Feature über alle Splits."},
          ...
          return (
            <label key={m} style={{display:"flex",flexDirection:"column",padding:"12px 14px",borderRadius:10,border:checked?"1.5px solid "+C.ruby:"1.5px solid "+C.border,background:checked?"rgba(155,17,30,0.03)":"#fff",opacity:blocked?0.35:1,cursor:blocked?"not-allowed":"pointer",transition:"all 0.15s"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                <input type="checkbox" checked={checked} disabled={blocked} style={{accentColor:C.ruby}} onChange={e=>{const s=new Set(cfg.fsMethods||["lgbm_importance"]);e.target.checked?s.add(m):s.delete(m);set({...cfg,fsMethods:[...s]})}}/>

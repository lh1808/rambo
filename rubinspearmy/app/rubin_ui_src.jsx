...
const ADDON_PRESETS = [
  ...
  // ── Feature-Selektion ──
  {key:"feature_reduction",label:"Feature-Reduktion",desc:"Max. 100 Features via Importance + Korrelationsfilter",group:"feature",
...
const FinalTuningPlanPreview = ({models, fmtEnabled, fmtModels, fmtSingleFold, fmtTrials, fmtCv, outerCv: outerCvProp, mcIters, isBoth}) => {
  ...
  const mc = mcIters || 1;
  const K = 2; // Binary Treatment (MT: K = Anzahl Treatment-Arme)
  const eligible = (models||[]).filter(m => ["NonParamDML","DRLearner"].includes(m) && (fmtModels === undefined || fmtModels === null || fmtModels.includes(m)));

  // Interne Fits pro DML/DR .fit() Aufruf
  // NonParamDML: mc × (cv × 2 Nuisance: model_y + model_t) + 1 model_final
  // DRLearner:   mc × (cv × (1+K) Nuisance: propensity + K×regression) + 1 model_final
  const fitsPerDmlFit = mc * internalCv * 2 + 1;
  const fitsPerDrFit = mc * internalCv * (1+K) + 1;
  ...
  eligible.forEach(m => {
    ...
    const fpf = isDr ? fitsPerDrFit : fitsPerDmlFit;
    // Äußere CV-Folds × est.fit(train) + est.score(val) pro Trial
    const perTrial = outerFolds * fpf;
    const total = nTrials * perTrial;
    const modelLabel = isDr ? "DRLearner" : "NonParamDML";
    const nuisanceDesc = isDr
      ? `${internalCv}×model_propensity + ${internalCv}×${K}×model_regression + 1×model_final`
      : `${internalCv}×model_y + ${internalCv}×model_t + 1×model_final`;
    rows.push({
      model: m,
      ...
      totalFits: total,
      detail: `${nTrials}T × ${outerFolds}F × ${fpf}`,
      note: `Pro Trial: ${outerFolds} äußere Fold(s) × ${fpf} Fits/Fold (${modelLabel}.fit() mit cv=${internalCv}: ${nuisanceDesc}). OOF-Evaluation via est.score(val).`
    });
  });
  ...
  return (
    <div>
      ...
      <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
        Jeder Trial fittet ein vollständiges kausales Modell inkl. internem Nuisance-Cross-Fitting (cv={internalCv}{mc>1?", mc_iters="+mc:""}). NonParamDML: {fitsPerDmlFit} Fits/Fold ({internalCv}×model_y + {internalCv}×model_t + 1×model_final). DRLearner: {fitsPerDrFit} Fits/Fold ({internalCv}×propensity + {internalCv}×{K}×regression + 1×model_final). T = Trials, F = äußere Score-Folds.
...
{par > 0 ? (<>
  <div style={{display:"flex",gap:8,marginBottom:8}}>
    {[{w:3,l:"Schnell",d:"Grenzwertig, aber schnell"},{w:5,l:"Standard",d:"Solide TPE-Exploration"},{w:8,l:"Gründlich",d:"Hohe Suchqualität"}].map(p => {
      const active = waves === p.w;
      return <button key={p.w} onClick={()=>setByWaves(p.w)} style={{flex:1,padding:"10px 12px",borderRadius:10,border:active?"1.5px solid #D4A853":"1.5px solid "+C.border,background:active?"#fffbeb":"#fff",cursor:"pointer",textAlign:"left",transition:"all 0.15s"}}>
        <div style={{fontSize:13,fontWeight:600,color:active?"#7a5a00":C.dark}}>{p.l}</div>
        <div style={{fontSize:11,color:active?"#7a5a00":C.textMuted,marginTop:2}}>{p.w} Wellen = {par*p.w} Trials</div>
      </button>;
    })}
  </div>
  <div style={{fontSize:11,color:C.textSec,background:"#f9fafb",padding:"8px 12px",borderRadius:8,lineHeight:1.5,border:"1px solid #e5e7eb"}}>
    <strong>{par}</strong> parallele Trials ({nCores} Kerne / 8, da FMT-Trials schwerer) × <strong style={{color:waveColor}}>{waves} Wellen</strong> = <strong>{trials}</strong> Trials.
    {waves < 3 && " TPE kann bei weniger als 3 Wellen kaum zwischen guten und schlechten Parametern unterscheiden."}
    {waves >= 3 && waves < 5 && " Grenzwertig — TPE beginnt erst nach den Startup-Trials zu lernen."}
    {waves >= 5 && " Genug Wellen für stabile TPE-Exploration und Exploitation."}
  </div>
  <Expander title="Manuell anpassen">
    <Row><Col><Inp label="Trials (manuell)" type="number" value={trials} onChange={v=>set({...cfg,fmtTrials:Number(v)})}/></Col><Col><div style={{fontSize:11,color:C.textMuted,marginTop:24}}>= {par} parallel × {waves} Wellen</div></Col></Row>
  </Expander>
</>) : (
  <Inp label="Trials" type="number" value={trials} onChange={v=>set({...cfg,fmtTrials:Number(v)})} help="Server-Verbindung nötig für Wellen-Berechnung."/>
)}
</>);
...
function App() {
  ...
  const estimateFits = () => {
    ...
    const fitsPerFit = (x) => {
      ...
      if (x==="DRLearner") return mc*innerCv*(1+K)+1; // propensity + K×regression pro Fold
    ...
    const fmtFitsPerDmlFit = mc * innerCv * 2 + 1;   // NonParamDML: model_y + model_t
    const fmtFitsPerDrFit = mc * innerCv * (1+K) + 1; // DRLearner: propensity + K×regression
    if (cfg.fmtEnabled) {
      ...
      (cfg.fmtModels||[]).forEach(x => {
        if (x==="NonParamDML") {
          ...
          f += fmtTr * fmtOuterFolds * fmtFitsPerDrFit;
        }
      });
    }

...
const pages = [
  ...
  {label:"Training & Evaluation",key:"traineval"},
...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="DML/DR Cross-Fitting">
    <Info>Steuert die Anzahl der internen Folds für die Nuisance-Residualisierung bei DML/DR-Modellen (NonParamDML, ParamDML, CausalForestDML, DRLearner). Gilt für Training, Cross-Predictions und Evaluation — überall wo ein DML/DR-Modell gefittet wird.</Info>
    <Row><Col><Inp label="Internes Cross-Fitting (cv)" type="number" value={cfg.dmlCrossfitFolds||2} onChange={v=>set({...cfg,dmlCrossfitFolds:Number(v)})} help="Anzahl der internen Folds für Nuisance-Cross-Fitting (EconML-Default=2). Bei cv=K trainiert jedes Nuisance-Modell auf (K-1)/K der Daten. Höhere Werte bedeuten mehr Trainingsdaten pro Nuisance-Modell und stabilere Residuals, kosten aber proportional mehr Rechenzeit."/></Col></Row>
    {(cfg.dmlCrossfitFolds||2) > 2 && <div style={{fontSize:11,color:"#7a5a00",background:"#fffbeb",padding:"8px 12px",borderRadius:8,marginTop:6,lineHeight:1.5}}>cv={cfg.dmlCrossfitFolds}: Jedes Nuisance-Modell trainiert auf {Math.round((1-1/(cfg.dmlCrossfitFolds||2))*100)}% der Daten (statt 50% bei cv=2). Stabilere Residuals, aber {cfg.dmlCrossfitFolds||2}× statt 2× Nuisance-Fits pro DML/DR-Modell.</div>}
  </Sec>
...
<FixedParamsEditor params={(cfg.fmtFixed||{}).lgbm||{}} defaults={LGBM_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:{...(cfg.fmtFixed||{}),lgbm:v}})}/>
              ...
              return (<>
                ...
                <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
                  Grid-Search über Wald-Parameter. CausalForestDML benötigt vorab Nuisance-Fits (cv={dc}×2{mc>1?" × mc="+mc:""}). CausalForest arbeitet direkt auf den Daten (kein Nuisance).
                </div>
                <div style={{border:"1px solid #ede6e7",borderRadius:8,overflow:"hidden"}}>
...
const PTrainEval = ({cfg,set}) => (<>
  ...
  <Sec title="DML/DR Cross-Fitting">
    ...
    <div style={{fontSize:11,color:C.textSec,lineHeight:1.5,marginBottom:8}}>Die Anzahl der internen Cross-Fitting-Folds ist unter <strong>Pipeline-Einstellungen</strong> konfigurierbar (Default: 2, EconML-Standard).</div>
    ...
    {(cfg.mcIters||0)>0 && <div style={{fontSize:11,color:"#7a5a00",background:"#fffbeb",padding:"8px 12px",borderRadius:8,marginTop:6,lineHeight:1.5}}>Cross-Fitting wird <strong>{cfg.mcIters}x</strong> wiederholt. Interne Fits pro DML/DR-Fold: ({cfg.dmlCrossfitFolds||2} Folds x 2 Nuisance + 1 Final) x {cfg.mcIters} mc = <strong>{((((cfg.dmlCrossfitFolds||2))*2+1)*(cfg.mcIters||1))}</strong> Nuisance-Fits. {(cfg.validateOn === "external" || cfg.eval_mask_file) ? `Evaluation auf ${cfg.validateOn === "external" ? "separatem Holdout-Datensatz" : "Mask-Subset (TMEO)"} — kein äußeres CV, jedes Modell wird einmal auf ${cfg.eval_mask_file && cfg.validateOn !== "external" ? "~mask-Rows" : "Trainingsdaten"} gefittet.` : `Zusätzlich ${cfg.cvSplits||5} äußere CV-Folds für OOF-CATE-Predictions.`}</div>}
    {(cfg.mcIters||0)>0 && <Row>
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",fontFamily:"'DM Sans',system-ui,-apple-system,sans-serif",fontSize:15,color:"#24292f",background:"#faf7f8"}}>
      ...
      <main style={{flex:1,maxWidth:960,padding:"36px 52px 80px",margin:"0 auto"}}>
        ...
        {pg==="traineval" && <PTrainEval cfg={cfg} set={set}/>}

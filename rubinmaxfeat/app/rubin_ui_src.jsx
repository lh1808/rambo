...
const ADDON_PRESETS = [
  ...
  // ── Feature-Selektion ──
  {key:"feature_reduction",label:"Feature-Reduktion",desc:"Max. 50 Features via Importance + Korrelationsfilter",group:"feature",
    cfg:{fsEnabled:true,fsMethods:["catboost_importance","causal_forest"],fsCorrThresh:0.9,fsMaxFeatures:50}},
  // ── Erklärbarkeit ──
...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Feature-Selektion" accent="#0d9488">
    ...
    {cfg.fsEnabled&&<>
      ...
      <Inp label="Max. Features" type="number" value={cfg.fsMaxFeatures||50} onChange={v=>set({...cfg,fsMaxFeatures:Number(v)})} help={`Maximale Anzahl Features nach Selektion. Bei ${(cfg.fsMethods||["catboost_importance"]).length} Methode(n): je ${Math.ceil((cfg.fsMaxFeatures||50)/Math.max(1,(cfg.fsMethods||["catboost_importance"]).length))} Features pro Methode, dann Union auf ${cfg.fsMaxFeatures||50} gekappt.`}/>
      {(()=>{
        const nM = (cfg.fsMethods||["catboost_importance"]).length;
        const maxF = cfg.fsMaxFeatures||50;
...
const buildYaml = (cfg, sp, spFmt) => {
  ...
  if(cfg.fsEnabled) {
    ...
    a(`  max_features: ${cfg.fsMaxFeatures||50}`);

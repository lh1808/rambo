...
2. FMT (Final-Model-Tuning): Optimiert model_final (CATE-Regressor) via OOF-CV
   mit gecachten Nuisance-Residuals (cache_values + refit_final).
...
- TPE-Sampler: multivariate=True, consider_endpoints=True, n_startup gekappt.
...
# Die folgenden Helper-Funktionen werden nicht mehr in aktiven Scoring-Pfaden
# verwendet, bleiben aber als Utilities im Code.
...
def _create_fold_scorer(
    ...
    if build_base_learner_fn is None:
        build_base_learner_fn = build_base_learner  # Gleiche Datei (tuning_optuna.py)
...
class BaseLearnerTuner:
    ...
    TPE-Sampler: multivariate=True, consider_endpoints=True, constant_liar=True.
...
in allen Cross-Prediction-Folds wiederverwendet."""
    ...
    def _apply_overfit_penalty(self, val_score: float, train_score: float,
        ...
        Funktioniert identisch für R-Score (normalisiert) unabhängig von Absolutskala.
        """
        if penalty <= 0:
            return val_score
        gap = train_score - val_score  # positiv = Overfitting
    ...
    def tune_final_model(
        ...
        if name == "nonparamdml":
            ...
            if _fmt_is_both and _fmt_timeout:
                ...
                _fmt_timeout = None
            _tlog.info(
                "FMT '%s': Starte %d Trials (cache_values, R-Score, %s, parallel_jobs=-1).",
            ...
            import gc as _gc
            _gc.collect()
            _tlog.info("FMT '%s': Study + Cache + Scorers freigegeben, gc.collect() durchgeführt.", model_name)
            return result
            ...
            # ── RScorer pro Fold: Cache-Check + ggf. Erstellen ──
            # DRLearner: model_regression ≈ model_y, model_propensity ≈ model_t
            _cache_key = (_outer_splits, self.tuning_cv_seed, _sf, len(Y_tune), _op > 0)
            if self._scorer_cache_key == _cache_key and self._scorer_cache_val:
                _fold_scorers = self._scorer_cache_val
                _fold_train_scorers = self._scorer_cache_train
...
def tune_causal_forest(
    ...
    - max_depth: Baumtiefe (kategorisch: 3, 5, 7, 10, 15, None)
    - min_weight_fraction_leaf: Blattgröße als Anteil der Gesamtdaten (log-skaliert)
    - min_var_fraction_leaf: Mindest-Treatment-Varianz in Blättern (log-skaliert)
    - criterion: Split-Qualität (mse/het)
    Alle anderen Wald-Parameter werden auf EconML-Defaults fixiert (n_estimators=100,
    Alle anderen Wald-Parameter werden auf EconML-Defaults fixiert (n_estimators=100
    beim Tuning, 500 bei Production; min_samples_leaf=5, max_samples=0.45 etc.),
    was den Suchraum von 9 auf 4 Dimensionen reduziert.

    Scoring: Externer EconML RScorer (unabhängige Nuisance, 2-Fold T×Y Cross-Fitting).
    Der RScorer fittet eigene Nuisance-Modelle auf den Val-Daten und bewertet den
    Estimator via scorer.score(est) → R-Score. Scorer werden zwischen Modellen
    wiederverwendet (precomputed_scorers).

    Zwei Pfade:
    - CausalForestDML: Nuisance wird einmalig gecacht (cache_values=True),
      Trials ändern nur die Forest-Parameter via setattr() + refit_final().
    - CausalForest (GRF): Pro Trial neuer CausalForestAdapter mit Params im
      Konstruktor. Scoring via scorer.score(adapter).

    Bei RCT (study_type="rct") wird model_t als DummyClassifier(strategy="prior")
    im Nuisance-Caching und im RScorer verwendet → konstante Propensity.
    ...
    - CFDML: CausalForestDML(n_jobs=-1) parallelisiert den CausalForest-Baum-Fit.

    Args:
        precomputed_scorers: Optional vorberechnete RScorer-Liste. Wird zwischen
            CFDML- und GRF-Calls wiederverwendet (gleiche Folds, gleiche Daten).
            Erste Aufruf: None → erstellt Scorer + gibt sie in result["scorers"] zurück.
            Zweiter Aufruf: vorherige Scorer übergeben → spart ~80s.

    Returns:
        Dict mit 'best_params', 'best_score', 'n_trials_completed', 'scorers'.
    """
    ...
    # UI-Chip-Selektoren: depth_choices / criterion_choices überschreiben choices
    _depth_choices = getattr(cfg.causal_forest, "depth_choices", None)
    if _depth_choices and isinstance(_depth_choices, list) and len(_depth_choices) > 0:
        # "None" (String aus JSX) → None (Python) konvertieren
        effective_ss["max_depth"]["choices"] = [
            None if v == "None" else v for v in _depth_choices
        ]
    _criterion_choices = getattr(cfg.causal_forest, "criterion_choices", None)
    ...
    # Fixe Defaults + User-Overrides aus forest_fixed_params (YAML-Config)
    _effective_fixed = dict(_CF_FIXED_DEFAULTS)
    _user_fixed = getattr(cfg.causal_forest, "forest_fixed_params", {}) or {}
    if isinstance(_user_fixed, dict):
        _effective_fixed.update(_user_fixed)
    # Tuning immer mit kleinen Forests — EconML tune() Konvention.
    # Production-n_estimators (z.B. 500) wird erst im finalen Modell-Fit angewendet.
    _effective_fixed["n_estimators"] = _CFT_TUNING_N_ESTIMATORS

    # ── GRF (nicht-DML): Fold-Splits vorberechnen ──
    # Nuisance-Residuen werden NICHT mehr manuell vorberechnet — der RScorer
    # handhabt eigene unabhängige Nuisance. Nur die Fold-Indices (tr, va)
    # werden gespeichert, um im Trial-Loop CausalForestAdapter zu fitten + scoren.
    _grf_fold_data = None
    ...
    if not is_cfdml:
        ...
        _tlog.info(
            "GRF: %d Fold-Splits vorberechnet (%s Base-Learner für RScorer).%s",
    ...
    # via setattr() + refit_final(). Die äußeren Folds sind über alle Trials identisch
    # (gleicher tuning_cv_seed) → Nuisance-Fitting entfällt komplett in der Trial-Schleife.
    _cfdml_cached_folds = None
    from .model_registry import CausalForestAdapter  # Benötigt für GRF scorer.score(adapter)
    if is_cfdml:
        ...
        _n_pre = 1 if single_fold else inner_cv
        _cfdml_cached_folds = []
        # Default-GRF-Params: Fixe Defaults + erste Werte aus dem Suchraum
        ...
        # Keys die explizit im CausalForestDML-Konstruktor gesetzt werden
        # müssen entfernt werden (sonst "got multiple values for keyword argument").
        # Kann passieren wenn User z.B. n_jobs in forest_fixed_params setzt.
        ...
        for _fi, (_tr, _va) in enumerate(_splits_pre[:_n_pre]):
            ...
            est = CausalForestDML(
                ...
                random_state=seed,
                n_jobs=-1,
                inference=False,  # Tuning braucht keine CI (EconML tune()-Konvention)
                **_default_forest,
            )
    ...
    # ── RScorer pro Fold: precomputed oder neu erstellen ──
    if precomputed_scorers is not None and len(precomputed_scorers) > 0:
        _cft_fold_scorers = precomputed_scorers
        _tlog.info(
            "CFT '%s': Precomputed RScorer wiederverwendet (%d Scorers).",
            model_type, len(_cft_fold_scorers),
        )
    else:
        _cft_fold_scorers = []
        # Fold-Indices für Scorer-Erstellung: CFDML nutzt cached_folds, GRF nutzt _grf_fold_data
        _scorer_folds = (
        ...
        _tlog.info(
            "CFT '%s': RScorer (unabhängige Nuisance, 2-Fold T×Y) pro Fold erstellt (%d Scorers).",
            model_type, len(_cft_fold_scorers),
        )

    def objective(trial):
        # Suggest params aus effektivem Suchraum (4 Parameter: max_depth, min_weight_fraction_leaf,
        # min_var_fraction_leaf, criterion). Alle anderen → fixe EconML-Defaults.
        params = {}
        ...
        # Fixe Defaults für nicht-getunte Parameter einmischen
        all_params = dict(_effective_fixed)
        all_params.update(params)
        # Tuning immer mit kleinen Forests (EconML tune() Konvention)
        all_params["n_estimators"] = _CFT_TUNING_N_ESTIMATORS
        # Keys entfernen die explizit im Konstruktor/setattr gesetzt werden.
        # Verhindert "got multiple values for keyword argument" Fehler wenn
        # User z.B. n_jobs in forest_fixed_params setzt.
        ...
        if is_cfdml:
            ...
            for fold_i, (est, tr, va) in enumerate(_cfdml_cached_folds):
                try:
                    _forest_params = {k: v for k, v in all_params.items()}
            ...
            # GRF: CausalForestAdapter (BaseCateEstimator) → scorer.score(adapter)
    ...
    _cft_mode = "cache_values + refit_final" if is_cfdml else "CausalForestAdapter + RScorer"
    _tlog.info(
        "CFT '%s': Starte %d Trials (%s, R-Score, %s, parallel_jobs=-1).",
        model_type, n_trials, _cft_mode, _fold_lbl,
    )
    ...
    if best_score is not None and best_score <= -1e11:
        _tlog.warning(
            "CFT '%s': Alle Trials fehlgeschlagen (best_score=%.2g). "
            "Mögliche Ursachen: zu wenige Samples pro Fold, ungültige Daten, "
            "oder CausalForest-Fehler. Debug-Log prüfen (logging.DEBUG).",
            model_type, best_score,
        )
        best = {}
        best_score = None

    del study
    # Gecachte Estimatoren + Scorers freigeben

...
# Schlankes Scoring-Environment (ohne mlflow/optuna/matplotlib/shap/flaml):
pixi run -e prod score -- --bundle runs/bundles/<bundle_id> --x new_X.parquet --out scores.csv
# Produktivsetzung (Config-getrieben, SAS-XPT + Monitoring-JSON, Domino-Job):
# siehe production/README.md (Referenz) und production/PRODUKTIVSETZUNG.md (Schritt-für-Schritt)
...
Bei Datensätzen mit vielen kategorischen Features (>20) setzt rubin automatisch eine Performance-Optimierung:

- **`max_ctr_complexity=1`**: Keine automatischen Feature-Kombinationen. Default (4) generiert bei 74 kat. Features über 1 Mio. Kombinationen — extrem langsam. Individuelle Ordered Target Statistics und Ordered Boosting bleiben vollständig erhalten.

Diese Einstellung gilt für BLT, FMT, CF-Tuning, Feature-Selektion und Training.
...
- CausalForest-Tuning (CFT) mit Optuna TPE über Forest-Parameter

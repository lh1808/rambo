...
class TestSchema:
    ...
    def test_validate_extra_columns_fail_strict(self, sample_schema):
        ...
        assert result.ok is False
    def test_validate_object_str_string_equivalent(self):
        """pandas-2/3-Kompatibilität: object/str/string sind im Dtype-Vergleich äquivalent.

        pandas 3 (String-Migration) meldet für Textspalten "str" statt "object";
        Schemata aus pandas-2-Bundles persistieren "object" (und umgekehrt).
        Beide Richtungen dürfen keinen Mismatch erzeugen."""
        df = pd.DataFrame({"c": ["x", "y"]})
        actual = str(df["c"].dtype)  # "object" (p2) bzw. "str" (p3)
        for expected_dtype in ("object", "str", "string"):
            schema = Schema(columns=["c"], dtypes={"c": expected_dtype}, categorical_columns=[])
            result = validate_schema(df, schema)
            assert result.ok is True, (expected_dtype, actual, result.dtype_mismatches)
        # Echte Mismatches werden weiterhin erkannt:
        schema = Schema(columns=["c"], dtypes={"c": "float64"}, categorical_columns=[])
        assert validate_schema(df, schema).ok is False

...
### Synchronisationsworkflow bei Änderungen

Bei UI-Änderungen an `app/rubin_ui_src.jsx` müssen zwei abhängige Artefakte mitaktualisiert werden
(Synchronität automatisch prüfbar via `pixi run -e dev ui-sync` bzw. `python scripts/check_ui_sync.py`):

1. **`app/frontend/index.html`** — Inline-Version für den Live-App-Betrieb. Der `<script type="text/babel">`-Block wird 1:1 mit dem JSX-Quellcode ersetzt.
2. **`rubin_overview.html`** — Base64-eingebettetes iframe für die Dokumentation. Muss nach jedem JSX-Update neu gebaut werden, sonst zeigt die Doku-Preview veralteten Code.

Der typische Rebuild-Flow:

```python
# 1. Validieren: JSX kompiliert?
node -e "require('./app/frontend/lib/babel.min.js').transform(
         require('fs').readFileSync('app/rubin_ui_src.jsx','utf-8'),
         {presets:['react']})"

# 2. JSX → index.html embedden (Script-Block ersetzen)
# 3. rubin_overview.html: Base64(CSS + React + ReactDOM + Babel + Mock + JSX) neu bauen
```

Bei UI-Änderungen wurden beide Artefakte bei jedem Sync erneuert. **Nur JSX anzupassen reicht nicht** — `index.html` und `rubin_overview.html` würden dann outdated bleiben.

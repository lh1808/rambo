...
- Treatment-Balance-Prüfung bei mehreren Dateien (Warnung bei >5pp Unterschied, optionales Downsampling per `balance_treatments`, Auswahl nach effektiver Stichprobe N·p·(1−p))
...
Optimiert Nuisance-Modelle (Outcome, Propensity, Regression) über Optuna TPE mit signatur-basiertem Task-Sharing. Jeder Task wird als eigenständige Optuna-Study mit fester Metrik (log_loss für Klassifikation, neg_mse für Regression) und optionaler Overfit-Penalty (BLT: nur Meta-Learner-Tasks; DML/DR-Nuisances werden nie bestraft) evaluiert. Zusätzlich werden Skill Scores (Verbesserung gegenüber naivem Baseline-Modell) berechnet und geloggt. Details in `docs/tuning_optuna.md`.
...
CausalForestDML ist bewusst nicht Teil des FMT: Sein „model_final" ist der GRF-Forest selbst — keine LightGBM/CatBoost-Hyperparameter, die per Optuna tuned werden könnten. Die Forest-Parameter werden stattdessen über CFT (CausalForestTuner, Optuna TPE) optimiert.

**CausalForest-Tuning (CFT):** Optimiert 4 kausale Parameter (max_depth, min_weight_fraction_leaf, min_var_fraction_leaf, criterion) für CausalForestDML und CausalForest via Optuna TPE (aligniert mit EconML tune()). Restliche Forest-Parameter auf EconML-Defaults fixiert. Trials laufen sequentiell (n_jobs=1), Forest intern nutzt alle Kerne.
...
4. **Kein struktureller Vorteil durch FMT:** Meta-Learner haben kein model_final → kein FMT nötig. CausalForestDML nutzt eigenes CFT (Optuna-basiertes Forest-Parameter-Tuning) statt FMT. CausalForest hat weder Nuisance noch model_final. Der Vergleich ist fair, da FMT ein inhärentes Feature der DML-Architektur ist — nicht ein Evaluations-Vorteil.
...
Im Cross-Modus durchlaufen alle Modelle externe K-Fold Cross-Validation. Für jede Beobachtung wird eine CATE-Prediction aus einem Modell erzeugt, das diese Beobachtung nicht gesehen hat (Out-of-Fold); im External-Modus stattdessen Single-Fit auf Train mit Evaluation auf den separaten Eval-Dateien. Zusätzlich wird pro Fold auf dem gesamten Datensatz predictet (Fold-Aligned Predictions in `DataFrame.attrs["fold_aligned_preds"]`) für leakage-freies Surrogate-Training.
...
Bei `models.ensemble: true` und mindestens 2 trainierten Modellen wird ein gleichgewichtetes Ensemble (`ShapeSafeEnsembleCate`, rubin-eigen) erstellt — eine rubin-eigene Alternative zu EconMLs `EnsembleCateEstimator`, dessen `const_marginal_effect` bei gemischten Modellfamilien an heterogenen Output-Shapes crasht; die Mitglieder-Outputs werden über `_predict_effect` normalisiert und erst dann gemittelt. Die Ensemble-Predictions sind der Mittelwert der individuellen Out-of-Fold-Vorhersagen aller beitragenden Modelle; Mitglied wird nur, wer vollständige Cross-Predictions liefert (BT: `Predictions_*`, MT: alle Arm-Spalten). Die Zusammensetzung wird als `member_names` am Ensemble persistiert und ist die Single Source of Truth für den Bundle-Export. Das Ensemble nimmt regulär an der Champion-Selektion teil. CausalForest ist über den `CausalForestAdapter` (`BaseCateEstimator`-kompatibel) vollwertig ensemble-fähig.

Beim Bundle-Export werden **alle** trainierten Modelle auf vollen Daten refittet und das Ensemble anschließend self-contained aus den refitteten Mitgliedern neu erstellt — unabhängig davon, welches Modell Champion ist. Da EconML die Mitglieder in die Ensemble-Pickle serialisiert, ist das gebündelte Ensemble in Production immer mit gefitteten Mitgliedern einsatzbereit.

Für Explainability wird bei einem Ensemble-Champion automatisch das beste Einzelmodell (nach Selektionsmetrik) herangezogen, da das Ensemble kein natives SHAP unterstützt.
...
Bestes Modell anhand der konfigurierten Metrik (Standard: Qini). Optional manuell festlegbar. Vor dem Export werden **alle** trainierten Modelle auf allen Daten refittet, sodass in Production frei zwischen ihnen gewählt werden kann. Das Ensemble wird stets self-contained aus den refitteten Mitgliedern neu erstellt. Im Holdout-Modus unterbleibt der Voll-Refit (die Modelle bleiben auf Train gefittet), um die Evaluation nicht zu entwerten. Bundle-Export schreibt alle Production-Artefakte synchron.
...
### Reproduzierbarkeit (warum Läufe nicht immer bit-identisch sind)

Alle **Zufallsquellen sind mit Seeds fixiert**: globale `np.random.seed`/`random.seed` (Pipeline-Start, Worker, Feature-Selektion), alle CV-Splits (`StratifiedKFold`/`KFold` mit `random_state`), der Optuna-`TPESampler` (pro Study aus `tuning_cv_seed` abgeleitet), EconML-Schätzer (`random_state`) und der DRTester-Bootstrap. Identische Ergebnisse sind dennoch **nicht in jedem Fall garantiert** — die Ursache ist **Parallelität, nicht ein fehlender Seed**, und das Ergebnis kann von der Anzahl der dem Prozess zugeteilten CPU-Kerne abhängen. Maßgeblich ist `available_cpu_count()` (cgroup-Quota bzw. CPU-Affinität, sonst `os.cpu_count()`): unterschiedliche Maschinen, Container oder Instanzgrößen liefern eine andere Kernzahl und damit potenziell andere Ergebnisse.

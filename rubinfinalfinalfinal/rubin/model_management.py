...
def _safe_float(x: Any) -> Optional[float]:
    try:
        if x is None:
            return None
        v = float(x)
        # NaN/Inf ausschließen: Ein Modell mit nicht-endlicher Selektionsmetrik
        # (z. B. NaN-Qini bei leeren Quantil-Bins / geringer CATE-Varianz) darf
        # NICHT als Champion gewählt werden. Ohne diesen Guard verfälscht NaN den
        # Sort (NaN-Vergleiche liefern False) und kann fälschlich „gewinnen".

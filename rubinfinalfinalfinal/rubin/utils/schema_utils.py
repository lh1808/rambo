...
def validate_schema(df: pd.DataFrame, expected: Schema, strict: bool = False) -> SchemaValidationResult:
    ...
    # object/str/string sind über pandas-Versionen hinweg äquivalent (pandas-3-
    # String-Migration meldet "str", ältere Schemata persistieren "object").
    _obj_like = {"object", "str", "string"}
    _norm = lambda d: "object" if d in _obj_like else d
    ...
    for c in expected.columns:
        if c in df.columns:
            ...
            act = str(df[c].dtype)
            if exp is not None and _norm(exp) != _norm(act):

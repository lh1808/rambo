...
- Metadaten (JSON) und Dokumentation (Snapshot der Konfiguration) sauber daneben liegen."""
    def __init__(self, base_dir: str = "runs/bundles") -> None:
...
- Im Fehlerfall (oder bei Audits) ist schnell nachvollziehbar, wann das Bundle erzeugt wurde."""
        ...
        # ML-Stack-Versionen zum Export-Zeitpunkt: Bundles sind Pickles — das
        # Entpickeln über abweichende econml-/lightgbm-/…-Versionen hinweg ist
        # riskant. ProductionPipeline vergleicht beim Laden gegen die Laufzeit
        # und warnt bei Abweichungen.
        if "ml_package_versions" not in enriched:
            from importlib.metadata import version as _pkg_version
            versions = {}
            for pkg in ("econml", "lightgbm", "catboost", "scikit-learn", "pandas", "numpy"):
                try:
                    versions[pkg] = _pkg_version(pkg)
                except Exception:
                    versions[pkg] = None
            enriched["ml_package_versions"] = versions

...
class CausalForestAdapter(BaseCateEstimator):
    ...
    Propensity-/Outcome-Schätzung im Forest), ohne vorgelagerte Nuisance-Modelle.
    ...
    und ist als BaseCateEstimator vollwertig ensemble-fähig (ShapeSafeEnsembleCate).
    """
    ...
    def effect(self, X):
        ...
        pred = self._cf.predict(X_np)
        # CausalForest gibt (n, 1) zurück — squeeze auf (n,) für BT-Konsistenz
        while pred.ndim > 1 and pred.shape[-1] == 1:
    ...
    def tune(self, X, T, Y, intensive=False, rscorer=None, model_y=None, model_t=None, nuisance_cv=5):
        """Grid-Search über CausalForest-Parameter mit RScorer-Evaluation.
...
(n_estimators muss durch subforest_size=4 teilbar sein)."""
    ...
    # (model_y, model_t) verwendet. Forest-Parameter orientieren sich an EconML-Defaults.
    ...
    # Kein Base Learner nötig — der Forest schätzt Treatment-Effekte direkt.

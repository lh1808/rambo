...
### CFT: Warum n_jobs=-1, aber parallel_jobs=1 bei CFDML-Nuisance

`CausalForestDML(n_jobs=-1)` parallelisiert intern **zwei Phasen**:
1. **Nuisance-Cross-Fitting:** EconML's `_OrthoLearner._fit_nuisances` nutzt joblib `Parallel(n_jobs=self.n_jobs)` — bei n_jobs=-1 laufen alle K Cross-Fitting-Folds gleichzeitig.
2. **Forest-Building:** Die Bäume des CausalForest werden parallel gebaut.

Deshalb **muss** `parallel_jobs=1` für die Nuisance-Modelle (model_y, model_t) gesetzt sein: Bei n_jobs=-1 auf dem Estimator laufen bereits K Nuisance-Fits parallel. Hätte jeder Fit ebenfalls alle CPUs (parallel_jobs=-1), entstünde K × Cores Übersubskription → Crashes und massive Verlangsamung.

```
CausalForestDML(n_jobs=-1, model_y=CatBoost(thread_count=1)):
  Phase 1 — Nuisance (K Folds parallel via EconML joblib):
    Fold 1: model_y.fit() + model_t.fit() → je 1 CPU  ┐
    Fold 2: model_y.fit() + model_t.fit() → je 1 CPU  │ K gleichzeitig
    ...                                                 ┘
  Phase 2 — Forest (alle CPUs):
    CausalForest.fit(residuals) → alle CPUs
```

**Standalone Nuisance-Fits** (GRF Residuen-Vorberechnung vor dem Optuna-Loop) laufen hingegen komplett isoliert — kein EconML-Estimator managt die Parallelität, kein Forest läuft gleichzeitig. Daher `parallel_jobs=-1`.

```
tune_causal_forest():
  ┌─ GRF Residuen-Vorberechnung (VOR Optuna) ─┐
  │  model_y.fit()  → parallel_jobs=-1  (alle CPUs, kein Forest aktiv)
  │  model_t.fit()  → parallel_jobs=-1  (alle CPUs, kein Forest aktiv)
  └────────────────────────────────────────────┘
  
  ┌─ Optuna study.optimize (n_jobs=1, sequentiell) ─┐
  │  Trial N:                                        │
  │    CausalForestDML(                              │
  │      model_y → parallel_jobs=1  (CFDML-intern)   │
  │      model_t → parallel_jobs=1  (CFDML-intern)   │

...
class ReportCollector:
    ...
    def add_config(self, cfg) -> None:
        ...
        self.config_summary = {
            ...
            "tuning_metric_regression": "neg_mse",
...
def generate_html_report(collector: ReportCollector, output_path: str) -> str:
    ...
    if cs.get("tuning_enabled") and collector.tuning_scores:
        ...
        # Config info
        h += '<div class="cg">'
        h += f'<div class="cd"><div class="cd-l">Metrik (Clf)</div><div class="cd-v">log_loss (fest)</div></div>'
        h += f'<div class="cd"><div class="cd-l">Metrik (Reg)</div><div class="cd-v">neg_mse (fest)</div></div>'
        ...
        if _is_both:
            h += '<h3>Best Scores pro Task</h3><div class="tbl-scroll"><table class="dt"><thead><tr><th>Task</th><th>Learner</th><th>Best Score</th></tr></thead><tbody>'
            for task, score in sorted(collector.tuning_scores.items()):
                ...
                h += f'<tr><td><code>{escape(task)}</code></td><td><strong>{escape(_lt_disp)}</strong></td><td>{score:.6f}</td></tr>'
            h += '</tbody></table></div>'
        else:
    ...
    if cs.get("final_tuning_enabled"):
        ...
        if fmt.get("best_scores"):
            ...
            h += '<h3>Best FMT-Scores</h3><div class="tbl-scroll"><table class="dt"><thead><tr><th>Modell</th><th>Score (−MSE)</th>'
            if pen_scores:

...
class BaseLearnerTuner:
    ...
    def _tune_task_flaml(self, task: TuningTask, X: pd.DataFrame, Y: np.ndarray, T: np.ndarray,
        ...
        # MLflow-Isolation: FLAML triggert intern sklearn/catboost-Autologging,
        # das Parameter in den aktiven MLflow-Run schreibt. Bei mehreren Tasks
        # crasht das mit "Changing param values is not allowed".
        # Lösung: MLflow-Tracking temporär auf ein Temp-Verzeichnis umleiten,
        # damit FLAML's Autologging ins Leere schreibt. Nach dem Fit wird
        # die Original-URI wiederhergestellt. rubin macht sein eigenes Logging.
        _orig_tracking_uri = None
        _flaml_tmp = None
        try:
            import mlflow as _mlflow
            _orig_tracking_uri = _mlflow.get_tracking_uri()
            import tempfile
            _flaml_tmp = tempfile.mkdtemp(prefix="rubin_flaml_")
            _mlflow.set_tracking_uri(f"file://{_flaml_tmp}")
            # Zusätzlich alle Autolog-Varianten deaktivieren
            try:
        ...
        try:
            ...
            # MLflow Tracking URI wiederherstellen (egal ob Erfolg oder Fehler)
            if _orig_tracking_uri is not None:
                try:
                    _mlflow.set_tracking_uri(_orig_tracking_uri)
                except Exception:
                    pass
            # FLAML-Temp-Verzeichnis aufräumen
            try:
                import shutil
                if _flaml_tmp and os.path.isdir(_flaml_tmp):
                    shutil.rmtree(_flaml_tmp, ignore_errors=True)
            except Exception:
                pass

...
const LGBM={n_estimators:{min:200,max:600,step:50,type:"int"},learning_rate:{min:0.01,max:0.15,step:0.005,type:"float",log:true},num_leaves:{min:15,max:127,step:1,type:"int"},max_depth:{min:3,max:8,step:1,type:"int"},min_child_samples:{min:10,max:200,step:5,type:"int"},min_child_weight:{min:0.01,max:50,step:0.5,type:"float",log:true},subsample:{min:0.5,max:1,step:0.05,type:"float"},colsample_bytree:{min:0.3,max:0.9,step:0.05,type:"float"},min_split_gain:{min:0,max:1,step:0.05,type:"float"},reg_alpha:{min:0.000001,max:20,step:0.1,type:"float",log:true},reg_lambda:{min:0.000001,max:20,step:0.1,type:"float",log:true},path_smooth:{min:0,max:10,step:0.5,type:"float"}};
const CB={iterations:{min:200,max:600,step:50,type:"int"},learning_rate:{min:0.01,max:0.15,step:0.005,type:"float",log:true},depth:{min:4,max:8,step:1,type:"int"},l2_leaf_reg:{min:1,max:30,step:0.5,type:"float",log:true},random_strength:{min:0.01,max:10,step:0.1,type:"float",log:true},subsample:{min:0.5,max:1,step:0.05,type:"float"},rsm:{min:0.3,max:0.9,step:0.05,type:"float"},min_data_in_leaf:{min:10,max:200,step:5,type:"int"},model_size_reg:{min:0,max:10,step:0.5,type:"float"},leaf_estimation_iterations:{min:1,max:10,step:1,type:"int"}};
const LGBM_FMT={n_estimators:{min:100,max:400,step:10,type:"int"},learning_rate:{min:0.005,max:0.12,step:0.005,type:"float",log:true},num_leaves:{min:7,max:63,step:1,type:"int"},max_depth:{min:2,max:6,step:1,type:"int"},min_child_samples:{min:20,max:500,step:5,type:"int"},min_child_weight:{min:0.5,max:80,step:0.5,type:"float",log:true},subsample:{min:0.4,max:0.85,step:0.05,type:"float"},colsample_bytree:{min:0.25,max:0.7,step:0.05,type:"float"},max_bin:{min:15,max:127,step:1,type:"int"},min_split_gain:{min:0,max:3,step:0.1,type:"float"},reg_alpha:{min:0.1,max:50,step:0.5,type:"float",log:true},reg_lambda:{min:1,max:100,step:1,type:"float",log:true},path_smooth:{min:0,max:30,step:1,type:"float"}};
const CB_FMT={iterations:{min:100,max:400,step:10,type:"int"},learning_rate:{min:0.005,max:0.12,step:0.005,type:"float",log:true},depth:{min:2,max:6,step:1,type:"int"},l2_leaf_reg:{min:3,max:80,step:1,type:"float",log:true},random_strength:{min:0.5,max:15,step:0.5,type:"float",log:true},subsample:{min:0.4,max:0.85,step:0.05,type:"float"},rsm:{min:0.25,max:0.7,step:0.05,type:"float"},min_data_in_leaf:{min:20,max:500,step:5,type:"int"},model_size_reg:{min:0.1,max:30,step:0.5,type:"float",log:true},leaf_estimation_iterations:{min:1,max:5,step:1,type:"int"}};
...
const CB_FINAL_DEFAULTS={iterations:200,learning_rate:0.03,depth:4,l2_leaf_reg:10.0,min_data_in_leaf:100,random_strength:3.0,subsample:0.7,rsm:0.5,model_size_reg:5.0};
...
const STEPS=[{id:"load",label:"Daten laden & Preprocessing"},{id:"fs",label:"Feature-Selektion"},{id:"tune",label:"Base-Learner-Tuning"},{id:"fmt",label:"Final-Model-Tuning"},{id:"grf",label:"GRF-Tuning"},{id:"train",label:"Training & Predictions"},{id:"eval",label:"Evaluation & Metriken"},{id:"surrogate",label:"Surrogate-Tree"},{id:"bundle",label:"Bundle-Export"},{id:"explain",label:"Explainability"},{id:"report",label:"HTML-Report"}];
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 14px"}}>
        {Object.entries(defs).map(([k,d])=>{const r=g(k);const lbl=d.log?`${k} (log)`:k;return (<RSlider key={k} label={lbl} min={d.min} max={d.max} step={d.step} low={r.low} high={r.high} type={d.type} onLow={v=>s(k,v,r.high)} onHigh={v=>s(k,r.low,v)}/>);})}
      </div>
    </div>
  );
};
...
// ── Progress Tracker ──
const _sidebarStyles = `@keyframes rubin-spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } } @keyframes rubin-sweep { 0% { left: -30%; } 50% { left: 100%; } 100% { left: -30%; } }`;
    ...
    <div style={{margin:"10px 0"}}>
            ...
            return (
              <div key={st.id} style={{position:"relative",display:"flex",alignItems:"center",gap:14,padding:"8px 0",transition:"opacity 0.4s",opacity:pending?0.4:1}}>
                ...
                <div style={{position:"absolute",left:-28,top:"50%",transform:"translateY(-50%)",width:20,display:"flex",justifyContent:"center"}}>
                  {done ? (
                    <div style={{width:12,height:12,borderRadius:"50%",background:"#1a7f37",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.3s"}}>
                      <svg width="8" height="8" viewBox="0 0 8 8"><path d="M1.5 4.2l1.5 1.5 3.5-3.5" fill="none" stroke="#fff" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg>
                    </div>
                  ) : active&&!failed ? (
                    <div style={{width:12,height:12,borderRadius:"50%",background:"#9B111E",boxShadow:"0 0 0 4px rgba(155,17,30,0.1)",animation:"rubin-pulse-line 2.4s ease-in-out infinite",transition:"all 0.3s"}}/>
                  ) : failed ? (
                    <div style={{width:12,height:12,borderRadius:"50%",background:"#cf222e",display:"flex",alignItems:"center",justifyContent:"center"}}>
                      <svg width="7" height="7" viewBox="0 0 7 7"><path d="M1.5 1.5l4 4M5.5 1.5l-4 4" stroke="#fff" strokeWidth="1.3" strokeLinecap="round"/></svg>
                    </div>
                  ) : (
                    <div style={{width:8,height:8,borderRadius:"50%",background:"#e0e0e0",transition:"all 0.3s"}}/>
                  )}
                </div>
                {/* Content */}
                <div style={{flex:1,display:"flex",alignItems:"center",gap:10,minHeight:24}}>
                  ...
                  {done&&dur!=null&&<span style={{fontSize:10,fontFamily:"'JetBrains Mono',monospace",color:"#b0b0b0",background:"#f7f7f7",padding:"2px 8px",borderRadius:10}}>{fmtDur(dur)}</span>}
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:10,paddingTop:8,borderTop:"1px solid #f0eded"}}>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          {allDone ? (
            <svg width="14" height="14" viewBox="0 0 14 14"><circle cx="7" cy="7" r="6" fill="#1a7f37" opacity="0.15"/><path d="M4 7.2l2 2 4-4" fill="none" stroke="#1a7f37" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg>
          ) : (
...
const DEFAULT_CFG = {studyType:"rct",expName:"rubin",seed:42,models:["NonParamDML","DRLearner","SLearner","TLearner","XLearner","ParamDML","CausalForestDML","CausalForest"],baseLearner:"catboost",ensembleEnabled:false,tuningEnabled:true,tuningTrials:100,tuningSingleFold:false,tuningMetric:"log_loss",tuningMetricReg:"neg_mse",tuningPerRole:false,tuningPerLearner:false,tuningModels:[],fmtEnabled:false,fmtModels:[],fmtSingleFold:false,fmtTrials:50,fmtMaxRows:0,fmtOverfitPenalty:0.0,fmtOverfitTolerance:0.05,cfTune:false,cfTuneIntensive:false,cfTuneModels:[],bundleEnabled:false,bundleDir:"runs/bundles",bundleChallengers:true,bundleMlflow:true,surrEnabled:false,surrMinLeaf:50,surrLeaves:31,surrDepth:0,fsEnabled:false,fsMethods:["lgbm_importance"],fsTopPct:15,fsCorrThresh:0.9,fsMaxFeatures:0,downsample:false,dfFrac:0.1,reduceMem:true,validateOn:"cross",cvSplits:5,treatmentType:"binary",refGroup:0,selMetric:"qini",higherBetter:true,refitChamp:true,manualChamp:null,hasNaN:false,nanCols:[],explEnabled:false,explSampleSize:10000,explTopN:20,shapModels:[],shapBins:10,histScoreName:"historical_score",histScoreCol:"S",histScoreHigher:true,outputDir:"",blFixed:{},fmtFixed:{},cfFixed:{},tuningTimeout:0,tuningMaxRows:0,fmtTimeout:0,maxPredRows:0,parallelLevel:3,workDir:null,mcIters:null,mcAgg:"mean",dmlCrossfitFolds:5,overfitPenalty:0.0,overfitTolerance:0.15};

const BT_PRESETS = [
  {key:"bt_quickstart",label:"Quickstart",desc:"2 Meta-Learner – schnellster Einstieg",
    cfg:{models:["SLearner","TLearner"],tuningEnabled:false,fmtEnabled:false,cfTune:false,bundleEnabled:false,surrEnabled:false,downsample:false}},
  {key:"bt_exploration",label:"Exploration",desc:"4 Modelle, 10% Sampling – schnell iterieren",
    cfg:{models:["SLearner","TLearner","NonParamDML","DRLearner"],tuningEnabled:false,fmtEnabled:false,cfTune:false,bundleEnabled:false,surrEnabled:false,downsample:true,dfFrac:0.1,cvSplits:3}},
  {key:"bt_standard",label:"Standard",desc:"4 Modelle – solide Basis (2 DML + 2 Meta)",
    cfg:{models:["NonParamDML","DRLearner","SLearner","TLearner"],tuningEnabled:false,fmtEnabled:false,cfTune:false}},
  {key:"bt_dml_focus",label:"DML-Fokus",desc:"4 Modelle – DML-Familie + Causal Forests",
    cfg:{models:["NonParamDML","DRLearner","CausalForestDML","CausalForest"],tuningEnabled:false,fmtEnabled:false,cfTune:false}},
  {key:"bt_forest_focus",label:"GRF-Fokus",desc:"4 Modelle – Forest-zentriert + DML-Vergleich",
    cfg:{models:["CausalForestDML","CausalForest","NonParamDML","DRLearner"],tuningEnabled:false,fmtEnabled:false,cfTune:false}},
  {key:"bt_full",label:"Vollständig",desc:"Alle 8 Modelle – maximaler Vergleich",
    cfg:{models:["NonParamDML","DRLearner","SLearner","TLearner","XLearner","ParamDML","CausalForestDML","CausalForest"],tuningEnabled:false,fmtEnabled:false,cfTune:false}},
];

const MT_PRESETS = [
  {key:"mt_exploration",label:"MT Exploration",desc:"2 Modelle, 10% Sampling – schnell iterieren",
    cfg:{models:["NonParamDML","DRLearner"],tuningEnabled:false,fmtEnabled:false,cfTune:false,bundleEnabled:false,downsample:true,dfFrac:0.1,cvSplits:3,selMetric:"policy_value"}},
  {key:"mt_standard",label:"MT Standard",desc:"2 DML-Modelle",
    cfg:{models:["NonParamDML","DRLearner"],tuningEnabled:false,fmtEnabled:false,cfTune:false,selMetric:"policy_value"}},
  {key:"mt_intensiv",label:"MT Erweitert",desc:"4 Modelle inkl. CausalForests",
    cfg:{models:["NonParamDML","DRLearner","CausalForestDML","CausalForest"],tuningEnabled:false,fmtEnabled:false,cfTune:false,selMetric:"policy_value"}},
];

const ADDON_PRESETS = [
  // ── Tuning (Pipeline-Reihenfolge: BL → FMT → GRF) ──
  {key:"bl_tuning",label:"BL-Tuning",desc:"Nuisance-Hyperparameter optimieren (100 Trials)",group:"tuning",
    cfg:{tuningEnabled:true,tuningTrials:100}},
  {key:"bl_tuning_intensiv",label:"BL-Tuning Intensiv",desc:"160 Trials – 10 TPE-Wellen bei voller Parallelisierung",group:"tuning",
    cfg:{tuningEnabled:true,tuningTrials:160}},
  ...
  {key:"fmt_intensiv",label:"FM-Tuning Intensiv",desc:"160 Trials – tiefe CATE-Optimierung",group:"tuning",
    cfg:{fmtEnabled:true,fmtTrials:160,fmtModels:["NonParamDML","DRLearner"]}},
  {key:"grf_tuning",label:"GRF-Tuning",desc:"Grid-Search für CausalForest / CausalForestDML (je 12 Kombis)",group:"tuning",
    cfg:{cfTune:true,cfTuneModels:["CausalForestDML","CausalForest"]}},
  {key:"grf_tuning_intensiv",label:"GRF-Tuning Intensiv",desc:"Erweitertes Grid mit je 48 Kombis (+ criterion, max_depth)",group:"tuning",
    cfg:{cfTune:true,cfTuneIntensive:true,cfTuneModels:["CausalForestDML","CausalForest"]}},
  // ── Production & Export ──
...
const PDataPrep = ({dp,setDp,cfg,setCfg,setPg}) => {
  ...
  const targetU = ((dp.targets||[""])[0]||"Y").toUpperCase();
      ...
      <Sec title="2. Pflicht-Spalten zuweisen">
        ...
        {simCols && simCols.length > 0 && (() => {
          ...
          return (<Info type="warn"><strong>{warns.length === 1 ? "Spalte" : warns.length + " Spalten"} nicht in den Daten gefunden:</strong>{" "}
            {warns.map((w, i) => <span key={i}>{i > 0 ? ", " : ""}<span style={{fontFamily:"monospace",fontWeight:600}}>{w.col}</span> ({w.role})</span>)}
          ...
          <div>
            ...
            {(() => {
              ...
              return (<>
                ...
                {hasData && !isBinary && !dp.binaryTarget && <Info type="warn">Nicht-binäre Werte: <span style={{fontFamily:"monospace",fontWeight:600}}>{tv.slice(0,8).join(", ")}{tv.length>8?"...":""}</span></Info>}
...
const YAML_TO_CFG = {
  "study_type":"studyType","mlflow.experiment_name":"expName","constants.SEED":"seed","constants.parallel_level":"parallelLevel","constants.work_dir":"workDir",
  ...
  "causal_forest.use_econml_tune":"cfTune","causal_forest.tune_intensive":"cfTuneIntensive","causal_forest.tune_models":"cfTuneModels",
        ...
        {importMode && (
          <div style={{marginTop:10}}>
            <textarea
              ...
              rows={6}
              style={{width:"100%",fontFamily:"'JetBrains Mono',monospace",fontSize:11.5,padding:"10px 14px",border:`1.5px solid ${C.border}`,borderRadius:8,resize:"vertical",background:"#1e1e2e",color:"#cdd6f4",outline:"none",boxSizing:"border-box",lineHeight:1.6}}
...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Allgemein">
    <div style={{marginBottom:14}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
        ...
        <span style={{color:C.ruby,fontFamily:"'JetBrains Mono',monospace",fontSize:13,background:C.rose,padding:"2px 12px",borderRadius:6,fontWeight:700,minWidth:36,textAlign:"center"}}>{cfg.seed}</span>
      </div>
      <input type="range" min={0} max={999} step={1} value={cfg.seed||42} onChange={e=>set({...cfg,seed:Number(e.target.value)})} style={{width:"100%",accentColor:C.ruby}}/>
      <div style={{fontSize:10.5,color:C.textMuted,marginTop:6,lineHeight:1.4}}>Reproduzierbarkeit: gleicher Seed = gleiche Ergebnisse bei identischen Daten</div>
    </div>
    ...
    <div style={{background:"#f6f8fa",border:"1px solid "+C.border,borderRadius:8,padding:"10px 14px",fontSize:11,fontFamily:"'JetBrains Mono',monospace",lineHeight:1.7}}>
  ...
  <Sec title="Evaluierungsmodus">
    ...
    <Inp label="Äußere CV-Folds (K)" type="number" value={cfg.cvSplits} onChange={v=>set({...cfg,cvSplits:v})} help="Äußere Cross-Validation für Out-of-Fold CATE-Predictions. Interne CVs (BLT, FMT, DML-Cross-Fitting) laufen unabhängig davon."/>
    ...
    {cfg.eval_mask_file && cfg.validateOn!=="external" && <Info type="warn"><strong>Train Many, Evaluate One</strong> aktiv: Mask-Rows werden <strong>vom Training ausgeschlossen</strong> und als Holdout für die Evaluation genutzt (Leakage-frei). Maske: <span style={{fontFamily:"monospace",fontSize:11}}>{cfg.eval_mask_file.split("/").pop()}</span></Info>}
    ...
    {(()=>{
      ...
      return (<>
          ...
          {REG_PRESETS.map(r => {
            ...
            return (<div key={r.key} onClick={()=>toggleReg(r)} style={{flex:1,padding:"12px 16px",borderRadius:10,border:active?`2px solid ${r.c}`:`1.5px solid #e5e7eb`,background:active?"#fff":"#fafafa",cursor:"pointer",transition:"all 0.15s",boxShadow:active?`0 0 0 3px ${r.c}22`:"none"}}>
              ...
              <div style={{fontSize:11,color:"#6b7280",marginTop:4,lineHeight:1.6}}>
                <div>BLT: {Math.round(r.blt_t*100)}% Gap tol. (p={r.blt_p})</div>
                <div>FMT: {Math.round(r.fmt_t*100)}% Gap tol. (p={r.fmt_p})</div>
              </div>
              {active && <div style={{fontSize:10,color:r.c,marginTop:4,fontFamily:"var(--mono)"}}>BLT p={r.blt_p}/t={r.blt_t} | FMT p={r.fmt_p}/t={r.fmt_t}</div>}
            </div>);
          })}
        </div>
        ...
        {activeKey==="custom" && <div style={{fontSize:11,color:"#7c3aed",background:"#f5f3ff",padding:"8px 12px",borderRadius:8,marginTop:8,lineHeight:1.5,border:"1px solid #c4b5fd"}}>
          <strong>Benutzerdefiniert:</strong><br/>BLT: Penalty={bp}, Tol.={bt} | FMT: Penalty={fp}, Tol.={ft}<br/><span style={{fontSize:10,color:"#9ca3af"}}>Anpassbar unter Learner &amp; Tuning → Erweiterte Einstellungen</span>
        </div>}
      </>);
    })()}
  </Sec>
    ...
    {cfg.fsEnabled&&<>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
        {[
          ...
          {m:"lgbm_permutation",label:"LightGBM Permutation",desc:"Misst den Metrik-Verfall bei zufälliger Feature-Permutation."},
...
const TuningPlanPreview = ({models, perRole, perLearner, trials, cv, isBoth, isRct}) => {
  ...
  const fitsForTask = (sig) => {
    ...
    if (fam==="pseudo_effect") return (nCv * K) + (nCv * 2 + 2);
    return nCv;
  };
  ...
  const fitsDetail = (sig) => {
    ...
    if (fam==="pseudo_effect") return nt+"T × ("+(nCv*K)+"obj + "+(nCv*2+2)+"nuis)";
    return nt+"T × "+nCv+"F";
  };
        ...
        {plan.map((t,i) => {
          ...
          return (
            <div key={t.key} style={{display:"grid",gridTemplateColumns:"36px 1fr 1fr 1fr 70px",padding:"7px 14px",borderBottom:"1px solid #f5f0f0",background:i%2===0?"#fff":"#fdfbfb",alignItems:"center",fontSize:12}}>
              ...
              <span style={{fontSize:10.5,color:"#888",fontFamily:"monospace"}}>{fitsDetail(t.sig)}</span>
              <span style={{fontFamily:"monospace",color:"#C4343F",fontWeight:600}}>{rowFits.toLocaleString()}</span>
            </div>
          );
        })}
      </div>
        ...
        {rows.map((r,i) => (
          <div key={r.model} style={{borderBottom:"1px solid #f5f0f0",background:i%2===0?"#fff":"#fdfbfb"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 70px",padding:"8px 14px",alignItems:"center",fontSize:12.5}}>
              ...
              <span style={{fontSize:10.5,color:"#888",fontFamily:"monospace"}}>{r.detail}</span>
              <span style={{fontFamily:"monospace",color:"#b8860b",fontWeight:700}}>{r.totalFits.toLocaleString()}</span>
            </div>
...
<FixedParamsEditor params={(cfg.fmtFixed||{}).lgbm||{}} defaults={LGBM_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:{...(cfg.fmtFixed||{}),lgbm:v}})}/>
              ...
              const grfNuisance = dc*2;
              const grk = cfg.cfTuneIntensive ? 48 : 12;
              const cfTotal = cfNuisance + grk;
              const grfTotal = grfNuisance + grk;
              const totalFits = (hasCF?cfTotal:0)+(hasGRF?grfTotal:0);
              return (<>
                <div style={{display:"flex",gap:10,marginBottom:12,flexWrap:"wrap"}}>
                  <div style={{padding:"8px 16px",background:"linear-gradient(135deg,#2d6a4f,#40916c)",borderRadius:8,color:"#fff"}}><div style={{fontSize:22,fontWeight:700}}>{(hasCF?1:0)+(hasGRF?1:0)}</div><div style={{fontSize:10,textTransform:"uppercase",letterSpacing:0.5,opacity:0.7}}>Grid-Studies</div></div>
                  <div style={{padding:"8px 16px",background:"#e8f5ec",borderRadius:8,border:"1px solid #a3d9b1"}}><div style={{fontSize:22,fontWeight:700,color:"#2d6a4f"}}>{totalFits}</div><div style={{fontSize:10,textTransform:"uppercase",letterSpacing:0.5,color:"#888"}}>Modell-Fits</div></div>
                  <div style={{padding:"8px 16px",background:"#faf7f7",borderRadius:8,border:"1px solid #ede6e7"}}><div style={{fontSize:22,fontWeight:700,color:"#333"}}>{grk}</div><div style={{fontSize:10,textTransform:"uppercase",letterSpacing:0.5,color:"#888"}}>Kombis / Modell</div></div>
                </div>
                <div style={{fontSize:10.5,color:"#888",marginBottom:10,lineHeight:1.5}}>
                  RScorer-basierter Grid-Search über Wald-Parameter. Beide Modelle fitten vorab Nuisance-Modelle für den RScorer (cv={dc}×2). CausalForestDML nutzt EconMLs internen RScorer{mc>1?" mit mc="+mc:""}, CausalForest einen externen RScorer mit BLT-getunten Nuisance-Modellen.
                </div>
                <div style={{border:"1px solid #ede6e7",borderRadius:8,overflow:"hidden"}}>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 80px",fontSize:11,fontWeight:600,padding:"8px 14px",background:"#2d6a4f",color:"#fff"}}><span>Modell</span><span>Aufschlüsselung</span><span>Fits</span></div>
                  {hasCF && <div style={{borderBottom:"1px solid #f0f0f0"}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 80px",padding:"8px 14px",fontSize:12}}>
                      ...
                      <span style={{fontSize:11,color:"#2d6a4f"}}>{cfNuisance} Nuisance + {grk} Forest-Grid</span>
                      <span style={{fontFamily:"monospace",fontWeight:700,color:"#2d6a4f"}}>{cfTotal}</span>
                    </div>
                    <div style={{fontSize:10.5,color:"#999",padding:"0 14px 6px",lineHeight:1.4}}>Nuisance: cv={dc} × 2 Modelle{mc>1?" × "+mc+" mc_iters":""} = {cfNuisance}. Dann {grk} Forest-Kombis{cfg.cfTuneIntensive?" (intensiv)":""} auf den Residuals.</div>
                  </div>}
                  {hasGRF && <div style={{background:hasCF?"#fdfbfb":"#fff"}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 80px",padding:"8px 14px",fontSize:12}}>
                      <span style={{fontWeight:600}}>CausalForest</span>
                      <span style={{fontSize:11,color:"#2d6a4f"}}>{grfNuisance} RScorer-Nuisance + {grk} Forest-Grid</span>
                      <span style={{fontFamily:"monospace",fontWeight:700,color:"#2d6a4f"}}>{grfTotal}</span>
                    </div>
                    <div style={{fontSize:10.5,color:"#999",padding:"0 14px 6px",lineHeight:1.4}}>RScorer: cv={dc} × 2 Nuisance-Modelle = {grfNuisance}. Dann {grk} Forest-Kombis{cfg.cfTuneIntensive?" inkl. criterion":""}.</div>
                  </div>}
                </div>
              </>);
            })()}<Divider/><div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:8}}>Modelle für GRF-Tuning</div><Info>Wähle, welche Forest-Modelle per RScorer-Grid-Search optimiert werden. Nicht ausgewählte Modelle verwenden EconML-Defaults.</Info><div style={{display:"flex",flexWrap:"wrap",gap:10,marginBottom:14}}>{(cfg.models||[]).filter(m=>["CausalForestDML","CausalForest"].includes(m)).map(m=>{const active=(cfg.cfTuneModels||[]).includes(m);return(<label key={m} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"6px 14px",borderRadius:8,border:active?"1.5px solid #2d6a4f":"1.5px solid "+C.border,background:active?"#e8f5ec":"#fff",cursor:"pointer",fontSize:12.5,fontWeight:active?600:400,transition:"all 0.15s"}}><input type="checkbox" checked={active} style={{accentColor:"#2d6a4f"}} onChange={e=>{const s=new Set(cfg.cfTuneModels||[]);e.target.checked?s.add(m):s.delete(m);set({...cfg,cfTuneModels:[...s]})}}/>{m}</label>)})}{!(cfg.models||[]).some(m=>["CausalForestDML","CausalForest"].includes(m))&&<Info type="warn">Keine Forest-Modelle (CausalForestDML/CausalForest) in der Modellauswahl.</Info>}{(cfg.models||[]).some(m=>["CausalForestDML","CausalForest"].includes(m)) && (cfg.cfTuneModels||[]).length===0 && <Info type="warn">Kein Modell für GRF-Tuning ausgewählt. Alle verwenden EconML-Defaults.</Info>}</div><Divider/><div style={{marginBottom:10}}><Toggle label="Intensives Grid (48 statt 12 Kombis)" checked={cfg.cfTuneIntensive||false} onChange={v=>set({...cfg,cfTuneIntensive:v})} help="Normal: 12 Kombis (= EconML Default: min_weight_fraction_leaf × max_depth × min_var_fraction_leaf). Intensiv: 48 Kombis (+ criterion, erweiterte Wertebereiche)."/></div></>}{cfg.cfTune && <><Divider/><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
  <span style={{fontSize:13.5,fontWeight:700,color:"#2d6a4f"}}>Search-Grid</span>
  <span style={{fontSize:10.5,color:"#888",background:"#f0fdf4",padding:"2px 10px",borderRadius:10,border:"1px solid #d1fae5"}}>{cfg.cfTuneIntensive?"48 Kombis (intensiv)":"12 Kombis"}</span>
</div>
{(()=>{
  const intensive = cfg.cfTuneIntensive||false;
  const grids = intensive ? [
    {param:"min_weight_fraction_leaf",values:["0.0001","0.01"]},
    {param:"max_depth",values:["3","5","8","None"]},
    {param:"min_var_fraction_leaf",values:["None","0.001","0.01"]},
    {param:"criterion",values:["mse","het"]},
  ] : [
    {param:"min_weight_fraction_leaf",values:["0.0001","0.01"]},
    {param:"max_depth",values:["3","5","None"]},
    {param:"min_var_fraction_leaf",values:["0.001","0.01"]},
  ];
  return (<div style={{background:"#faf8f8",borderRadius:10,border:`1px solid ${C.borderLight}`,padding:"16px 20px"}}>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px 12px"}}>
      {grids.map(g => (
        <div key={g.param} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 12px",background:"#fff",borderRadius:8,border:`1px solid ${C.borderLight}`}}>
          <code style={{fontSize:11,fontWeight:600,color:"#2d6a4f",minWidth:130,fontFamily:"'JetBrains Mono',monospace"}}>{g.param}</code>
          <div style={{flex:1,display:"flex",gap:4,justifyContent:"flex-end",flexWrap:"wrap"}}>
            {g.values.map(v => (
              <span key={v} style={{fontSize:11,padding:"1px 8px",borderRadius:6,background:"#f0fdf4",border:"1px solid #d1fae5",color:"#2d6a4f",fontFamily:"'JetBrains Mono',monospace",fontWeight:500}}>{v}</span>
            ))}
          </div>
        </div>
      ))}
    ...
    <div style={{fontSize:11,color:"#6b7280",marginTop:10,lineHeight:1.5}}>
      Nicht-getunte Parameter verwenden EconML-Defaults (n_estimators=200, honest=true, min_samples_leaf=5, max_samples=0.45).
    </div>
  </div>);
})()}
</>}
...
const buildYaml = (cfg, sp, spFmt) => {
  ...
  if((cfg.models||[]).includes("CausalForestDML") || (cfg.models||[]).includes("CausalForest")) {
    ...
    if(cfg.cfTuneIntensive) a("  tune_intensive: true");
...
const PPreview = ({cfg,sp,spFmt}) => {const y=buildYaml(cfg,sp,spFmt),issues=validate(cfg);const mt=cfg.treatmentType==="multi";return(<>
  <Row gap={12}>
    ...
    <MC value={cfg.tuningEnabled?(cfg.tuningTrials||100)+"T":"Aus"} label="Tuning"/>
      ...
      <Sec title={done ? "Analyse abgeschlossen" : running ? "Analyse läuft …" : "Analyse starten"} accent={done ? "#1a7f37" : running ? "#D4A853" : undefined}>
        ...
        {(running || done || error) && (
          <>
            ...
            {disconnected && running && <div style={{fontSize:11.5,color:"#d97706",background:"#fffbeb",padding:"8px 14px",borderRadius:8,border:"1px solid #fbbf24",marginTop:6,display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:14}}>⟳</span>Verbindung unterbrochen — versuche Reconnect. Die Analyse läuft im Hintergrund weiter.</div>}
            ...
            {(running || (done && logText)) && logText && (
              <div style={{marginTop:14}}>
                ...
                {showLogs && (
                  <div style={{background:"#1e1e2e",borderRadius:10,padding:"16px 20px",fontFamily:"'JetBrains Mono',monospace",fontSize:11,whiteSpace:"pre-wrap",wordBreak:"break-all",height:280,overflowY:"auto",lineHeight:1.65,color:"#cdd6f4",boxShadow:"inset 0 2px 8px rgba(0,0,0,0.15)",border:"1px solid #313244"}} ref={el=>{if(el&&running){const atBottom=el.scrollHeight-el.scrollTop-el.clientHeight<40;if(atBottom)el.scrollTop=el.scrollHeight}}}>{logText}</div>
                )}
              </div>
            )}
          </>
...
function App() {
  ...
  const pageStatus = (() => {
    ...
    else s.datafiles = {st:"done",detail:[cfg.x_file?"X":"",cfg.t_file?"T":"",cfg.y_file?"Y":""].filter(Boolean).join(", ")+" Dateien"};
    ...
    else s.template = {st:"done",detail:(cfg.models||[]).length+" Modelle, "+cfg.treatmentType.toUpperCase()};
    ...
    if(!effectiveModels.length) s.models = {st:"required",detail:"Keine Modelle"};
      ...
      const parts = [effectiveModels.length+"M"];
  ...
  const estimateFits = () => {
    ...
    // 1. Feature-Selektion
    if (cfg.fsEnabled) (cfg.fsMethods||[]).forEach(x => { if (x==="lgbm_importance"||x==="lgbm_permutation") f++; if (x==="causal_forest") f++; });
    ...
    if (cfg.tuningEnabled) {
      ...
      // Getrennte Tasks (nach Signatur-Trennung):
      if (hasDml) f += tr * tc;                          // outcome (clf, all): DML model_y
    ...
    // 4. CF tune() — Nuisance-Vorab + Grid-Search
    if (cfg.cfTune) {
      const _grk = cfg.cfTuneIntensive ? 48 : 12;
      if ((cfg.cfTuneModels||[]).includes("CausalForestDML") && m.includes("CausalForestDML")) f += innerCv*2*mc + _grk;
      if ((cfg.cfTuneModels||[]).includes("CausalForest") && m.includes("CausalForest")) f += innerCv*2 + _grk;
    }
    ...
    m.forEach(x => {
      ...
      if (isExternal) {
        ...
        if (shap) f += pf;                                // Keep-last-fold-model für SHAP
      }
    });
    ...
    <div style={{display:"flex",minHeight:"100vh",fontFamily:"'DM Sans',system-ui,-apple-system,sans-serif",fontSize:15,color:"#24292f",background:"#faf7f8"}}>
      <nav style={{width:280,minWidth:280,background:"linear-gradient(180deg,#4a080f 0%,#6B0D15 30%,#8a0f1a 100%)",color:"#fff",position:"sticky",top:0,height:"100vh",overflowY:"auto",flexShrink:0,display:"flex",flexDirection:"column",boxShadow:"4px 0 24px rgba(107,13,21,0.15)"}}>
          ...
          {pages.map((p,idx) => {
            ...
            return (
              <div key={p.key}>
                ...
                {st && st.st==="required" && (
                  <div style={{fontSize:9.5,color:"#ff8a8a",padding:"0 18px 3px 44px",lineHeight:1.3}}>{st.detail}</div>
                )}
                {st && st.st==="done" && !isOverview && (
                  <div style={{fontSize:9.5,color:"rgba(255,255,255,0.3)",padding:"0 18px 2px 44px",lineHeight:1.3}}>{st.detail}</div>
                )}
              </div>
            );
          })}
        ...
        {totalFits > 0 && <div style={{padding:"4px 18px 0",display:"flex",alignItems:"center",justifyContent:"space-between",fontSize:10,color:"rgba(255,255,255,0.3)"}}>
          ...
          <span style={{fontFamily:"'JetBrains Mono',monospace",fontWeight:600,color:"rgba(255,255,255,0.5)"}}>{totalFits.toLocaleString("de-DE")}</span>
        </div>}

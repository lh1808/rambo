...
def _native_uplift_by_percentile(
    ...
    Repliziert ``sklift.viz.plot_uplift_by_percentile(kind='bar')`` vollständig:
    sortiert nach pred. Uplift absteigend, teilt in n_bins, zeigt pro Bin
    Uplift (oben), Treatment/Control Response Rates (unten) — inkl. Fehlerbalken
    und gewichtetem Durchschnitts-Uplift im Titel.

    Layout identisch zu sklift: axes[0]=Uplift, axes[1]=Response Rates.

    Hintergrund: sklift 0.5.1 hat einen bekannten numpy-Kompatibilitäts-Bug
    (GitHub Issue #213: ValueError 'inhomogeneous shape' mit numpy >=1.24).
    Diese native Implementierung liefert den gleichen Informationsgehalt.
    ...
    try:
        ...
        for i in range(n_bins):
            ...
            st = float(y_bin[treat_mask].std()) / max(np.sqrt(nt), 1) if nt > 1 else 0.0
            sc = float(y_bin[ctrl_mask].std()) / max(np.sqrt(nc), 1) if nc > 1 else 0.0
            rr_treat.append(rt)
            rr_ctrl.append(rc)
            std_treat.append(st)
            std_ctrl.append(sc)
            bin_uplifts.append(rt - rc)
            std_uplifts.append(np.sqrt(st**2 + sc**2))
            n_treat_bins.append(nt)
            pct_lo = int(round(i * 100 / n_bins))
            pct_hi = int(round((i + 1) * 100 / n_bins))
            bin_labels.append(f"{pct_lo}-{pct_hi}")

        # Weighted average uplift (gewichtet auf Treatment-Gruppengröße, wie sklift)
        ...
        # Oberer Plot: Uplift (wie sklift axes[0])
        ax0 = axes[0]
        ax0.bar(x, bin_uplifts, width=w * 2, yerr=std_uplifts, color=COLOR_MODEL,
                edgecolor="black", linewidth=0.5, alpha=0.7, capsize=2, label="uplift")
        ax0.axhline(0, color="black", linewidth=1)
        ax0.legend(loc="upper right")
        ax0.set_title(f"Uplift by percentile\nweighted average uplift = {weighted_avg:.4f}")
        ax0.set_ylabel("Uplift")

        # Unterer Plot: Response Rates (wie sklift axes[1])
        ax1 = axes[1]
        ax1.bar(x - w/2, rr_treat, width=w, yerr=std_treat, color=RUBIN_COLORS["ruby_dark"],
                edgecolor="black", linewidth=0.5, alpha=0.7, capsize=2,
                label="treatment\nresponse rate")
        ax1.bar(x + w/2, rr_ctrl, width=w, yerr=std_ctrl, color=COLOR_REFERENCE,
                edgecolor="black", linewidth=0.5, alpha=0.7, capsize=2,
                label="control\nresponse rate")
        ...
        ax1.set_title("Response rate by percentile")
...
def _native_treatment_balance(
    ...
    Sliding Window der Breite ``winsize * n``. Direkt in rubin-Farben."""
    try:
        ...
        ax.plot(x_axis, treatment_rates, linewidth=2, color=COLOR_MODEL,
                label="Treatment Rate")
        ax.fill_between(x_axis, treatment_rates, overall_rate, alpha=0.12, color=COLOR_MODEL)
        ax.axhline(overall_rate, color=COLOR_BASELINE, linewidth=1.0,
                   linestyle="--", label=f"Random ({overall_rate:.3f})")
...
def _native_qini_curve(
    ...
    Berechnet die Qini-Kurve: Q(k) = Y_t(k) − Y_c(k) · N_t(k) / N_c(k)
    (identische Formel wie sklift.metrics.qini_curve).
    Nutzt rubins eigene ``uplift_curve`` für die kumulativen Zähler."""
    try:
        ...
        # Qini-Formel: Q(k) = y_treat(k) - y_control(k) * n_treat(k) / n_control(k)
        ...
        # Random baseline: gerade Linie von (0,0) bis (1, qini_total)
        y_random = x * y_qini[-1]

        fig, ax = plt.subplots(figsize=(10, 6))
        ax.plot(x, y_qini, linewidth=2, color=COLOR_MODEL, label=f"Model (Qini = {qini:.4f})")
        ax.plot(x, y_random, linewidth=1.2, color=COLOR_BASELINE, linestyle="--", label="Random")
        ...
        ax.set_title(f"Qini Curve (Qini = {qini:.4f})")

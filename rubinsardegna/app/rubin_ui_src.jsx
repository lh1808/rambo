...
const LGBM={n_estimators:{min:50,max:1000,step:50,type:"int",dLow:200,dHigh:600},learning_rate:{min:0.001,max:0.3,step:0.005,type:"float",log:true,dLow:0.01,dHigh:0.15},num_leaves:{min:7,max:255,step:1,type:"int",dLow:15,dHigh:127},max_depth:{min:2,max:12,step:1,type:"int",dLow:3,dHigh:8},min_child_samples:{min:5,max:500,step:5,type:"int",dLow:10,dHigh:200},min_child_weight:{min:0.001,max:100,step:0.5,type:"float",dLow:0.01,dHigh:50},subsample:{min:0.3,max:1,step:0.05,type:"float",dLow:0.5,dHigh:1},colsample_bytree:{min:0.1,max:1,step:0.05,type:"float",dLow:0.3,dHigh:0.9},min_split_gain:{min:0,max:3,step:0.05,type:"float",dLow:0,dHigh:1},reg_alpha:{min:0,max:50,step:0.1,type:"float",dLow:0,dHigh:20},reg_lambda:{min:0,max:50,step:0.1,type:"float",dLow:0,dHigh:20},max_bin:{min:7,max:255,step:1,type:"int",dLow:15,dHigh:127},path_smooth:{min:0,max:20,step:0.5,type:"float",dLow:0,dHigh:10}};
...
const LGBM_FMT={n_estimators:{min:50,max:800,step:10,type:"int",dLow:100,dHigh:400},learning_rate:{min:0.001,max:0.2,step:0.005,type:"float",log:true,dLow:0.005,dHigh:0.12},num_leaves:{min:3,max:127,step:1,type:"int",dLow:7,dHigh:63},max_depth:{min:1,max:10,step:1,type:"int",dLow:2,dHigh:6},min_child_samples:{min:5,max:500,step:5,type:"int",dLow:20,dHigh:200},min_child_weight:{min:0.1,max:50,step:0.5,type:"float",dLow:0.5,dHigh:20},subsample:{min:0.2,max:1,step:0.05,type:"float",dLow:0.4,dHigh:0.85},colsample_bytree:{min:0.1,max:1,step:0.05,type:"float",dLow:0.3,dHigh:0.7},max_bin:{min:7,max:255,step:1,type:"int",dLow:15,dHigh:127},min_split_gain:{min:0,max:2,step:0.05,type:"float",dLow:0,dHigh:0.5},reg_alpha:{min:0,max:30,step:0.5,type:"float",dLow:0,dHigh:10},reg_lambda:{min:0,max:30,step:0.5,type:"float",dLow:0,dHigh:10},path_smooth:{min:0,max:15,step:0.5,type:"float",dLow:0,dHigh:5}};
const CB_FMT={iterations:{min:50,max:800,step:10,type:"int",dLow:100,dHigh:400},learning_rate:{min:0.001,max:0.2,step:0.005,type:"float",log:true,dLow:0.005,dHigh:0.12},depth:{min:1,max:10,step:1,type:"int",dLow:2,dHigh:6},l2_leaf_reg:{min:0.5,max:60,step:1,type:"float",dLow:3,dHigh:30},random_strength:{min:0.1,max:20,step:0.5,type:"float",dLow:0.5,dHigh:10},subsample:{min:0.2,max:1,step:0.05,type:"float",dLow:0.4,dHigh:0.85},rsm:{min:0.1,max:1,step:0.05,type:"float",dLow:0.3,dHigh:0.7},min_data_in_leaf:{min:5,max:500,step:5,type:"int",dLow:20,dHigh:200},model_size_reg:{min:0,max:30,step:0.5,type:"float",dLow:0.1,dHigh:10},leaf_estimation_iterations:{min:1,max:10,step:1,type:"int",dLow:1,dHigh:5}};
...
const YAML_TO_CFG = {
  ...
  "causal_forest.use_econml_tune":"cfTune","causal_forest.search_space":"_cfSS","causal_forest.depth_choices":"_cfDepthChoices","causal_forest.criterion_choices":"_cfCriterionChoices","causal_forest.n_trials":"cfTrials","causal_forest.single_fold":"cfSingleFold","causal_forest.scorer":"cfScorer","causal_forest.tune_models":"cfTuneModels",
  ...
  "final_model_tuning.enabled":"fmtEnabled","final_model_tuning.models":"fmtModels","final_model_tuning.cv_splits":"dmlCrossfitFolds","final_model_tuning.single_fold":"fmtSingleFold","final_model_tuning.overfit_penalty":"fmtOverfitPenalty","final_model_tuning.overfit_tolerance":"fmtOverfitTolerance","final_model_tuning.scorer":"fmtScorer","final_model_tuning.n_trials":"fmtTrials",
...
const PConfig = ({cfg,set}) => {
  ...
  <Sec title="Feature-Selektion" accent="#0d9488">
    ...
    {cfg.fsEnabled&&<>
      ...
      <Inp label="Max. Features" type="number" value={cfg.fsMaxFeatures||77} onChange={v=>set({...cfg,fsMaxFeatures:Number(v)})} help={`Maximale Anzahl Features nach Selektion. Bei ${(cfg.fsMethods||["catboost_importance"]).length} Methode(n): je ${Math.ceil((cfg.fsMaxFeatures||77)/Math.max(1,(cfg.fsMethods||["catboost_importance"]).length))} Features pro Methode, dann Union auf ${cfg.fsMaxFeatures||77} gekappt.`}/>
      {(()=>{
...
</>) : (<FixedParamsEditor params={cfg.fmtFixed||{}} defaults={(cfg.baseLearner||"catboost")==="catboost"?CB_FINAL_DEFAULTS:LGBM_FINAL_DEFAULTS} onChange={v=>set({...cfg,fmtFixed:v})} label="CATE-Parameter (Fixed)" accent="#D4A853"/>)}</>}</Sec>{((cfg.models||[]).includes("CausalForestDML")||(cfg.models||[]).includes("CausalForest"))&&<Sec title="CausalForest-Tuning" accent="#2d6a4f"><Info>Optimiert 4 kausale Parameter (max_depth, min_weight_fraction_leaf, min_var_fraction_leaf, criterion) via Optuna. Restliche Wald-Parameter auf EconML-Defaults fixiert (n_estimators=100 (Tuning) / 500 (Production), min_samples_leaf=5, max_samples=0.45). Bewertung per QiniScorer (RCT) oder RScorer (obs.), konfigurierbar über Scorer-Auswahl. n_estimators=100 beim Tuning, 500 bei Production.</Info><Row><Col><Toggle label="CausalForest-Tuning aktivieren" checked={cfg.cfTune||false} onChange={v=>{const u={cfTune:v};if(v&&(cfg.cfTuneModels||[]).length===0){u.cfTuneModels=(cfg.models||[]).filter(m=>["CausalForestDML","CausalForest"].includes(m));}set({...cfg,...u});}}/></Col><Col><Toggle label="Single-Fold-Tuning" checked={cfg.cfSingleFold||false} onChange={v=>set({...cfg,cfSingleFold:v})} help="Bewertet jeden Trial auf 1 statt K Folds — schneller, aber verrauschtere Score-Schätzung."/></Col></Row>{cfg.cfTune&&<><Row><Col><div style={{fontSize:12,fontWeight:500,color:C.text,marginBottom:4}}>Scorer</div><select value={cfg.cfScorer||"auto"} onChange={e=>{const v=e.target.value;set({...cfg,cfScorer:v,fmtScorer:v});}} style={{width:"100%",padding:"7px 10px",borderRadius:8,border:"1px solid "+C.border,fontSize:12.5,background:"#fff"}}><option value="auto">auto (Qini bei RCT, R-Score bei Beobachtungsdaten)</option><option value="qini">Qini — optimiert Ranking-Qualität (OOF-aggregiert)</option><option value="rscore">R-Score — optimiert CATE-Genauigkeit (EconML RScorer)</option></select><div style={{fontSize:10.5,color:C.textSec,marginTop:3}}>Qini: Direkt auf der Uplift-Metrik tunen (kein Pruning, robust bei schwachem Signal). R-Score: Pointwise CATE-MSE (Pruning möglich, EconML-Standard). Wird synchron mit FMT gesetzt.</div></Col></Row><Divider/><div style={{fontSize:13,fontWeight:600,color:C.dark,marginBottom:8}}>Tuning-Plan</div>{(() => {
...
function App() {
  ...
  const estimateFits = () => {
    ...
    const fmtScorerFitsPerFold = 4;                  // RScorer: cv=2 × (model_y + model_t)
    if (cfg.fmtEnabled) {
    ...
    if (cfg.cfTune) {
      ...
      const cfScorerFitsPerFold = 4; // RScorer: cv=2 × (model_y + model_t)

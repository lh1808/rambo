...
Nuisance-Residuals (cache_values + refit_final).
"""
...
in allen Cross-Prediction-Folds wiederverwendet."""
    ...
    def _apply_overfit_penalty(self, val_score: float, train_score: float,
        ...
        # Minimum-Scale 0.1: Analog zu BLT — verhindert disproportionale Penalty
        # bei R-Score ≈ 0 (schwaches CATE-Signal, aber moderater Overfit-Gap).
        scale = max(abs(val_score), 0.1)
            ...
            if _scorer_type == "qini":
                ...
                def objective(trial):
                    ...
                    for fold_i, (est, tr, va) in enumerate(_cached_folds):
                        ...
                        est.refit_final()
                        cate_val = est.effect(X_tune.iloc[va])
                        # Squeeze trailing dims für BT-Konsistenz (n,) statt (n,1)
                        cate_val = np.asarray(cate_val).squeeze()
                        all_cate.append(cate_val)
                        all_y.append(Y_tune[va])
                        all_t.append(T_tune[va])

                    cate = np.concatenate(all_cate)
                    y_agg = np.concatenate(all_y)
                    t_agg = np.concatenate(all_t)
                    curve = uplift_curve(y_agg, t_agg, cate)
                    return float(qini_coefficient(curve))

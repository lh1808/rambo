...
Wenn eine NVIDIA GPU erkannt wird (`catboost.utils.get_gpu_device_count() > 0`), nutzt CatBoost automatisch `task_type='GPU'` für alle Trainings- und Tuning-Operationen. LightGBM und EconML-Modelle (GRF, CausalForestDML) laufen weiterhin auf CPU.

### GPU-Parameter-Einschränkungen
- **rsm** (`colsample_bylevel`): Auf GPU nur für Pairwise-Ranking-Modes unterstützt. Wird automatisch aus dem Suchraum und allen CatBoost-Parametern entfernt.
- **Parallele Trials**: CatBoost GPU erlaubt keine parallelen Fits auf derselben GPU (`n_jobs=1` für CatBoost-Tasks). LightGBM-Tasks behalten volle CPU-Parallelisierung.
- **thread_count**: Steuert auf GPU nur die CPU-seitige Datenvorverarbeitung, nicht das Training. Wird bei GPU automatisch auf `-1` (alle Kerne) gesetzt.

### Logging
```
HH:MM:SS INFO [rubin.gpu] CatBoost GPU erkannt: 1 NVIDIA GPU(s) verfügbar → task_type='GPU'.
HH:MM:SS INFO [rubin.gpu] GPU-Modus: 'rsm' aus CatBoost-Suchraum entfernt (nicht unterstützt).
```

### Installation (CatBoost + GPU)

**Wichtig:** CatBoost muss über **PyPI** installiert werden, nicht über conda-forge.
Das conda-forge-Paket bringt eine eigene `cuda-cudart`-Version mit (z.B. 12.9), die
mit älteren NVIDIA-Treibern inkompatibel ist (CUDA error 222: "PTX compiled with
unsupported toolchain"). Das PyPI-Paket linkt alle CUDA-Libraries **statisch** und
benötigt nur einen NVIDIA-Treiber ≥ 450.80.02.

In `pixi.toml` ist CatBoost deshalb unter `[pypi-dependencies]` statt `[dependencies]` definiert:

```toml
# pixi.toml
[dependencies]
lightgbm = ">=4.0"    # conda (kein GPU-Conflict)
# catboost NICHT hier — siehe [pypi-dependencies]

[pypi-dependencies]
catboost = ">=1.2"     # PyPI: statisch gelinkte CUDA-Libraries
```

**GPU-Test nach Installation:**
```bash
pixi run python -c "

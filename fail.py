...
def plot_kumulierter_cate(df, cate_col, sort_col, ascending=False, step=50_000,
    ...
    # Stützpunkte: Vielfache von step + Endpunkt (Gesamtwert).
    # Liegt der Endpunkt zu dicht am letzten Vielfachen, ersetzt er es,
    # damit sich die Wert-Labels oben nicht überlagern.
    marks = list(range(step, n + 1, step))
    if not marks:
        marks = [n]
    elif marks[-1] != n:
        if n - marks[-1] >= 0.35 * step:
            marks.append(n)
        else:
            marks[-1] = n
